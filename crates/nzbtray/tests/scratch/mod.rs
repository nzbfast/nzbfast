...
//! `crates/nzbkit/tests/scratch/mod.rs` (§149 taught the copies to keep
//! a panicking test's tree).
//!
...
//! `#[path]`, exactly as the other two crates do, so the three files
//! stay one file in three places rather than becoming three designs.
//! `probe_body`'s key-source cases wrote 6,313 of the 66,095 leaked
//! `$TMPDIR` entries measured on the dev Mac on 31 Aug 2026 - the second
//! largest family - because this crate had no guard to reach for.
//!
