...
//! the `crates/nzbkit`, `crates/nzbtray`, `crates/nzbfast-core` and
//! `crates/nzbfast-unpack` copies.
//!
...
//! `#[path]`, exactly as the other five crates do, so the six files stay
//! one file in six places rather than becoming six designs. This copy
//! arrived with the `nzbfast-meta` cut (crate-split step 3):
//! `scan_spot_tests` attaches one for the spot-ingest fixtures, and it
//! moved up here with `scan`.
//!
