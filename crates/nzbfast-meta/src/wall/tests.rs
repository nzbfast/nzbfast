...
#[test]
fn tmdb_discover_parse_takes_title_and_year() {
    // A digital-release discover page: title and the year off
    // release_date; a result with no title falls back to
    // original_title, and one with neither is dropped.
    let v: serde_json::Value = serde_json::from_str(
        r#"{"results":[
          {"title":"Some Film","release_date":"2026-08-30"},
          {"original_title":"Auslandsfilm","release_date":"2026-08-29"},
          {"release_date":"2026-08-28"},
          {"title":"No Date"}
        ]}"#,
    )
    .unwrap();
    let ms = super::parse_tmdb_discover(&v);
    assert_eq!(ms.len(), 3, "{ms:?}");
    assert_eq!(ms[0].title, "Some Film");
    assert_eq!(ms[0].year, 2026);
    assert_eq!(ms[1].title, "Auslandsfilm");
    assert_eq!(ms[2].title, "No Date");
    assert_eq!(ms[2].year, 0, "no release_date -> year 0");
}

#[test]
fn tvmaze_schedule_parse_reads_both_feed_shapes() {
    // Broadcast feed nests the show at `show`; the web feed nests it
    // at `_embedded.show`. A special (null number) is dropped, and an
    // entry with no show name at all is dropped rather than queued as
    // an empty search.
    let v: serde_json::Value = serde_json::from_str(
        r#"[
          {"season":3,"number":5,"airdate":"2026-09-01",
           "show":{"name":"Broadcast Show"}},
          {"season":1,"number":12,"airdate":"2026-09-01",
           "_embedded":{"show":{"name":"Web Show"}}},
          {"season":2026,"number":null,
           "show":{"name":"Daily Special"}},
          {"season":1,"number":1,"show":{"name":"  "}}
        ]"#,
    )
    .unwrap();
    let eps = super::parse_tvmaze_schedule(&v);
    assert_eq!(eps.len(), 2, "{eps:?}");
    assert_eq!(eps[0].show, "Broadcast Show");
    assert_eq!((eps[0].season, eps[0].episode), (3, 5));
    assert_eq!(eps[1].show, "Web Show");
    assert_eq!((eps[1].season, eps[1].episode), (1, 12));
}
...
///
/// The prefix is not optional and is the same for all five live rigs
/// here: a unit-test build refuses every non-loopback destination by
/// construction (`netfetch::deny_test_callout`), so that a test which
/// dials the open internet by accident cannot pass quietly. These five
/// dial it ON PURPOSE, so they say so.
///
