...
pub fn omdb_signup(email: &str) -> Result<(), String> {
    ...
    // the button on any daemon started with NZBFAST_NO_ENRICH set - the
    // one thing that switch is not for. It therefore takes its own
    // agent instead of `shared_enrich_agent`, whose `EnrichResolver`
    // refuses everything off loopback when enrichment is off.
    //
    // ONE agent for all three round trips, so the WebForms dance still
    // reuses a single connection - which is the whole reason this
