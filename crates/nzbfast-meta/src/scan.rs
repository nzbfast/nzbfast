...
pub fn parse_age(s: &str) -> Result<u64> {
    let s = s.trim();
    if s.is_empty() || s == "0" {
        return Ok(0);
    }
...
/// The Date of the first article at or after `n` on this connection, 0
/// when the window is empty or the server refuses it. One OVER of up to
/// 1,000 lines; shared by the age bisection and the history-cursor
/// alignment below.
pub(crate) async fn date_at(conn: &mut Connection, n: u64, hi: u64) -> i64 {
...
/// How far back a server REALLY carries a group, as the Unix date of
/// the oldest article an OVER ladder can still find. `None` = nothing
/// was measurable.
///
/// `GROUP`'s `count` and `low` are an ESTIMATE, and at least one
/// provider on a live install returns `count == high - low` EXACTLY - a
/// 100.00% fill claim over 7.37 billion articles it does not have.
/// Electing the history leg's server on that number sent six of seven
/// live groups to a carrier holding 6-18 days over two holding 437-948,
/// and 18 of 18 deepen slices came back with ZERO headers (measured
/// 1 Sep 2026, `research/DEEPEN-LEG-IS-ON-THE-WRONG-PROVIDER-2026-09-01.md`,
/// TODO 321).
///
/// A DATE and not an article offset, because article numbers are
/// per-spool: "40M below the tip" is a different amount of history on
/// every server, while a date is directly the quantity the deepen leg
/// exists to extend.
///
/// **Stated limit: this is an ELECTION input, not a retention
/// measurement, and must not be published as one.** The article-number
/// space is full of holes, so a ladder that gives up after two empty
/// rungs can UNDERSTATE a server that has a wide gap above deeper
/// content. That is why the rungs DOUBLE rather than step: the question
/// here is "16 days or 650" between candidates probed the same way, not
/// what either one's floor is. A bisection would be the wrong tool
/// twice over - it assumes the space is monotonic, which it is not, and
/// it costs a round trip per bit for precision this decision cannot use.
async fn carriage_depth(conn: &mut Connection, low: u64, high: u64) -> Option<i64> {
    /// Articles per rung. Wide enough that one sparse patch inside an
    /// otherwise-carried region does not read as the end of history.
    const WINDOW: u64 = 1_000;
    /// Where the ladder starts. Below this every candidate answers the
    /// same and the probe tells the election nothing.
    const FIRST: u64 = 10_000_000;
    /// Bounds the whole probe at 12 OVERs of WINDOW articles per server
    /// per group, run at most once a week (see `cached_carriage_depth`).
    ...
    for _ in 0..MAX_RUNGS {
        let Some(lo) = high.checked_sub(off).filter(|lo| *lo > low) else {
            break;
        };
        match conn.over(lo, (lo + WINDOW - 1).min(high)).await {
            // The deepest rung that answered wins, so this overwrites
            // on the way down and the last write is the oldest.
            Ok(es) => match es.iter().map(|e| e.date).find(|d| *d > 0) {
                Some(d) => {
                    deepest = Some(d);
                    misses = 0;
                }
            ...
            // A refusal reads as a miss, not as a failure of the pass:
            // `423 No articles in that range` is the ANSWER here, and a
            // transport error two rungs running ends the ladder anyway.
            Err(_) => misses += 1,
        }
        if misses >= 2 {
            break;
...
/// How long a measured carriage depth is trusted before it is probed
/// again. Retention is a provider POLICY: it moves on the scale of a
/// contract, not of a scan lap, and a lap runs every ~18 minutes.
const CARRIAGE_TTL_SECS: i64 = 7 * 86_400;

/// `carriage_depth` behind a `kv` cache, so the ladder costs one probe
/// per (group, server) per week rather than one per pass.
async fn cached_carriage_depth(
    ix: &mut nzbkit::index::Index,
    group: &str,
    skey: &str,
    conn: &mut Connection,
    ...
    let k = format!("scan_depth:{group}:{skey}");
    if let Some(v) = ix.kv_get(&k)
        && let Some((date, at)) = v.split_once(',')
        && let (Ok(date), Ok(at)) = (date.parse::<i64>(), at.parse::<i64>())
        && now.saturating_sub(at) < CARRIAGE_TTL_SECS
    {
        return (date > 0).then_some(date);
    }
    let d = carriage_depth(conn, low, high).await;
    // A failure is cached too, as `0`. Otherwise a server that cannot
    // answer is re-laddered by every pass forever, which is the one
    // shape this cache exists to prevent.
    let _ = ix.kv_set(&k, &format!("{},{now}", d.unwrap_or(0)));
    d
}

/// Rank one primary candidate. Bigger wins.
///
/// Split out because it is the whole of TODO 321 and the only part of
/// the election that can be tested without a spool: measured depth
/// first (OLDEST date wins, hence the `Reverse`), then the claimed
/// span, then config order. An UNMEASURED candidate ranks below every
/// measured one (`None < Some(_)`), and when nothing could be measured
/// every candidate is `None` and the claimed span decides exactly as it
/// did before this existed.
pub(crate) fn primary_rank(
    depth: Option<i64>,
    span: u64,
    idx: usize,
) -> (
    ...
    (depth.map(std::cmp::Reverse), span, std::cmp::Reverse(idx))
}

/// One scan-eligible backbone that carries the group this pass: its
/// config, marks key, open connection and GROUP answer.
struct Probe {
    server: ServerConfig,
    key: String,
    conn: Connection,
...
/// A history cursor must not sit ABOVE ground another backbone has
/// already deepened through. Article numbers are per server, so the
/// comparison is by DATE: `cur_date` is when the primary's own low-water
/// article was posted, `frontier_date` the oldest low-water any other
/// server holds for the group. True when the primary should bisect down
/// to the frontier instead of re-walking the band between.
///
/// Why this exists (research/LIVE-INDEX-CENSUS-2026-09-02.md, section
/// 4c): marks are per (group, server), so a NEWLY ELECTED primary starts
/// its deepen leg from a fresh mark near its tip. On one install the
/// election moved a group from a backbone that had deepened it to
/// 17 Feb 2026 to one whose mark seeded at 31 Aug - and the leg then
/// spent 21 slices walking ground the index already held, inserting
/// nothing (rows are UNIQUE on stem/poster/grp). 315M articles of that
/// remained: ~21 days of laps before one new row. The same shape
/// appeared on a second install the hour its election changed.
///
/// One day of slack, so two servers whose marks differ by propagation
/// jitter or an OVER window's width never ping-pong; a zero date on
/// either side means "could not read it" and is never a reason to move.
pub(crate) fn history_cursor_should_jump(cur_date: i64, frontier_date: i64) -> bool {
    const SLACK: i64 = 86_400;
    cur_date > 0 && frontier_date > 0 && frontier_date + SLACK < cur_date
}

/// The oldest dated low-water any OTHER server holds for the group:
/// `(date, host, article)`. Undated entries (0) are skipped - a server
/// whose mark could not be read is not evidence of depth.
pub(crate) fn deepest_history_frontier(marks: &[(i64, String, u64)]) -> Option<(i64, String, u64)> {
    marks
        .iter()
        .filter(|(d, _, _)| *d > 0)
        .min_by_key(|(d, _, _)| *d)
...
/// Move the primary's history cursor down to the deepest frontier any
/// other backbone already covers, when it sits above one. Runs once per
/// pass, before the deepen slice, and costs one OVER per other server
/// with a mark plus one for the primary's own cursor; the bisection
/// (~30 OVERs) only when a jump is actually due, which is once per
/// election change. The band skipped is ground the previous primary
/// ingested; message-ids are portable and ingest merges, so nothing the
/// index holds is lost, and what the new spool carries that the old one
/// did not is the gap-fill pass's job, exactly as for every other
/// history band. Returns the new low-water when it moved.
async fn align_history_cursor(
    ix: &mut nzbkit::index::Index,
    group: &str,
    skey: &str,
    conn: &mut Connection,
    ...
    others: &mut [Probe],
    // The `index_max_age` cutoff as a date (0 = no cutoff). A frontier
    // older than it is not followed: the deepen leg never walks past the
    // cutoff, so a cursor parked below it would simply stop deepening.
    age_floor: i64,
) -> Option<u64> {
    ...
    let mut marks = Vec::with_capacity(others.len());
    for p in others.iter_mut() {
        let olow = ix.low_water(group, &p.key);
        if olow == 0 || olow > p.info.high {
            continue;
        }
        let d = date_at(&mut p.conn, olow, p.info.high).await;
        marks.push((d, p.server.host.clone(), olow));
    }
    let (fdate, fhost, flow) = deepest_history_frontier(&marks)?;
    if fdate < age_floor {
        return None;
    }
    let cur_date = date_at(conn, cur, g.high).await;
    if !history_cursor_should_jump(cur_date, fdate) {
        return None;
    }
    let target = bisect_cutoff(conn, g.low, g.high, fdate).await;
    if target >= cur {
        return None;
    }
    if ix.set_low_water(group, skey, target).is_err() {
        return None;
    }
    let msg = format!(
        "{group}: history cursor moved from article {cur} down to {target}: {fhost} already \
         deepened to article {flow} ({} days older), so the band between is covered ground",
        (cur_date - fdate).max(0) / 86_400
    );
    println!("{msg}");
    info!(target: "scan", "{msg}");
    Some(target)
}

/// Elect the server this pass will run its history leg from, and hand
/// back the probes that were not chosen.
///
/// Returns `(primary, others, now)`. `now` is minted INSIDE, after the
/// probe round trips, because that is where it was read before this
/// split and every age cutoff downstream is measured from it - lifting
/// it to the caller would move the clock by the probe time.
///
/// Moved verbatim out of [`index_scan_into`] for the 500-line function
/// ceiling: the probe loop, the two refusals, the depth measurement,
/// the rank, the candidate log line and the persisted choice are the
/// original lines and run in the original order.
async fn elect_scan_primary(
    cfg: &Config,
    group: &str,
    ix: &mut nzbkit::index::Index,
) -> anyhow::Result<(Probe, Vec<Probe>, i64)> {
    ...
    // Primary = the probe that can take the history leg FURTHEST BACK,
    // measured rather than claimed (TODO 321). This used to be the
    // largest `high - low`, on the reading that one number captures
    // both carriage and retention depth. It does not: `GROUP` returns
    // an ESTIMATE, and a provider that reports `count == high - low`
    // over billions of articles it does not hold wins the election
    // outright and then answers the deepen leg with nothing. See
    // `carriage_depth`, which is the probe, and `primary_rank`, which
    // is the order. The claimed span is still the tiebreak and still
    // the whole rule when nothing can be measured, so an install whose
    // servers all refuse the ladder behaves exactly as it did.
    //
    // The primary runs the full forward + deepen legs; every other
    // backbone contributes a cheap tip leg below.
    let mut depths: Vec<Option<i64>> = Vec::with_capacity(probes.len());
    for i in 0..probes.len() {
        ...
        let d = cached_carriage_depth(ix, group, &skey, &mut probes[i].conn, low, high, now).await;
        depths.push(d);
    }
    let pi = probes
        .iter()
        .enumerate()
        .max_by_key(|(i, p)| primary_rank(depths[*i], p.info.high.saturating_sub(p.info.low), *i))
        .map(|(i, _)| i)
        .expect("probes is non-empty");
    // Say what was elected and on what evidence: a group deepening
    // against the wrong spool is invisible until somebody counts
    // headers, which is how this went unnoticed.
    if probes.len() > 1 {
        let age = |d: Option<i64>| match d {
            Some(d) => format!("{}d", (now - d).max(0) / 86_400),
            None => "unmeasured".to_string(),
        };
        info!(
            target: "scan",
            "{group}: history leg -> {} ({} of carriage); candidates: {}",
            probes[pi].server.host,
            age(depths[pi]),
            probes
                .iter()
                .enumerate()
                .map(|(i, p)| format!("{} {}", p.server.host, age(depths[i])))
                .collect::<Vec<_>>()
                .join(", "),
        );
    }
...
pub async fn index_scan_into(
    ...
    // index-deepen-lap-timing (1 Sep 2026): every leg of this pass
    // carries its own wall clock, reported in one line at the end. The
    // pass total was already timed (`done:` below) but its SPLIT was
    // not, so nothing could say what share of a lap the deepen leg is -
    // which is exactly what raising `index_deepen` needs to be a
    // decision rather than a guess (research/
    // INDEX-DEPTH-AND-DEEPEN-RATE-2026-09-01.md). Same lesson as the
    // spot resolver's own clock: a duration derived from the gap
    // between two log lines measures the whole lap, not the leg.
    ...
    let (chosen, mut probes, now) = elect_scan_primary(&cfg, group, ix).await?;
    ...
    let t0 = Instant::now();
    // Everything before t0: the per-server connect + GROUP probes, the
    // primary choice, and the Date bisection when a max-age is set. It
    // is per PASS, not per article, so it is the part of a group's cost
    // that raising `index_deepen` does not move.
    let t_setup = t_pass.elapsed();
    let mut t_forward = std::time::Duration::ZERO;
    let mut n_forward = 0u64;
    let mut e_forward = 0u64;
    let mut t_deepen = std::time::Duration::ZERO;
    let mut n_deepen = 0u64;
    // Headers the deepen slice actually returned. This, not the slice
    // width, is what the leg's time is spent on - see `ScanPass`.
    let mut e_deepen = 0u64;
    // Articles ASKED for by the deepen leg (the `index_deepen` slice,
    // clamped by the group floor) as against headers the server
    // actually returned - a slice over expired numbers returns fewer.
    // The rate that prices the dial is articles per second, since that
    // is what moves the low-water mark.
    let mut a_deepen = 0u64;
    let mut t_coverage = std::time::Duration::ZERO;
    let mut n_coverage = 0u64;
    let mut e_coverage = 0u64;
    let mut legs_coverage = 0usize;
    ...
    if low > g.high {
        ...
        .await?;
        t_forward = t_leg.elapsed();
        n_forward = pass.scanned;
        e_forward = pass.entries;
    ...
    // A newly elected primary must not re-walk the old primary's ground;
    // see `align_history_cursor`. The other probes still hold their
    // connections here, which is what makes the date comparison cheap.
    if deepen > 0 && healthy {
        let age_floor = if max_age_secs > 0 {
            now - max_age_secs as i64
        } else {
            0
        };
        align_history_cursor(ix, group, &skey, &mut conn, &g, &mut probes, age_floor).await;
    }
    ...
    if deepen > 0 && healthy {
        ...
        if cur > floor {
            ...
            .await?;
            t_deepen = t_leg.elapsed();
            n_deepen = pass.scanned;
            e_deepen = pass.entries;
            a_deepen = hi2 - lo2 + 1;
            scanned += pass.scanned;
            completed += pass.completed;
    ...
    if coverage {
        for p in probes {
            ...
            .await;
            arm_arrival_watch(ix, arrival_watch.as_ref(), false);
            t_coverage += t_leg.elapsed();
            legs_coverage += 1;
            match leg {
                Ok(pass) => {
                    n_coverage += pass.scanned;
                    e_coverage += pass.entries;
                    scanned += pass.scanned;
                    completed += pass.completed;
                }
    ...
    // The leg breakdown, on the `index` target so it carries a
    // timestamp (the `indexing` / `deepening` / `done:` lines above are
    // `println!`s with none, which is why the 1 Sep depth memo had to
    // place them against neighbouring lines to time anything at all).
    // One line per group per pass, in the order the legs ran.
    //
    // Deliberately NOT emitted from a pass that returned early or was
    // dropped mid-leg: a stand-down already logs itself, and a partial
    // breakdown read as a whole one is worse than no breakdown. So
    // these lines describe COMPLETED group passes only.
    //
    // "articles" and not "headers": `scanned` counts the article SPAN a
    // leg covered (`scanned += hi - lo + 1`), not the OVER entries that
    // came back - a range over expired numbers covers its whole span
    // and returns almost nothing. The `done:` line above has always
    // called the same quantity headers; this line does not repeat the
    // mistake, and the gap between the deepen leg's asked-for slice and
    // what it covered is what says a leg was abandoned.
    info!(
        target: "index",
        "{group} pass in {:.1?}: setup {:.1?}, forward {n_forward} articles \
         ({e_forward} headers) in {:.1?}, deepen {n_deepen}/{a_deepen} articles \
         ({e_deepen} headers) in {:.1?} ({}), coverage {n_coverage} articles \
         ({e_coverage} headers) in {:.1?} over {legs_coverage} server(s), tail {:.1?}",
        t_pass.elapsed(),
        t_setup,
        t_forward,
        t_deepen,
        // Articles per second is the rate that prices `index_deepen`:
        // the dial is an article count, and it is articles that move
        // the low-water mark. Headers per second is the same number
        // scaled by the group's density and is already on the `done:`
        // line.
        if a_deepen > 0 && t_deepen > std::time::Duration::ZERO {
            format!(
                "{:.0} articles/s, {:.0} headers/s, {:.1}% dense",
                a_deepen as f64 / t_deepen.as_secs_f64(),
                e_deepen as f64 / t_deepen.as_secs_f64(),
                100.0 * e_deepen as f64 / a_deepen as f64,
            )
        } else {
            "no deepen leg".to_string()
        },
        t_coverage,
        t_tail.elapsed(),
    );
...
pub async fn index_gapfill_pass(
    config: &Path,
    ix: &mut nzbkit::index::Index,
    count: u32,
    stop: impl Fn() -> bool,
...
pub(crate) struct ScanPass {
    ...
    /// OVER entries the server actually returned, as against `scanned`,
    /// which is the article SPAN the leg covered. The two differ by the
    /// density of the range, and on this data that difference is the
    /// whole story: a 200,000-article deepen slice through a sparse
    /// stretch of history returns almost nothing and lands in ~1.5 s,
    /// while a dense one of the same width takes ~40 s (measured 1 Sep
    /// 2026 on a clone of the live index). Nothing could see that
    /// before, because every count on the scan's log lines is a span.
...
pub(crate) async fn collect_scan_pass(
    ...
    loop {
        ...
        match msg {
            Ok((lo, hi, entries)) => {
                // The fetch size and the ingest size are separate
                // decisions that want opposite answers, so this splits
                // them: the OVER above ran at `chunk`, which is up to
                // 100,000 on a single connection because that is what
                // the wire wants, and the ingest runs at
                // `INGEST_BATCH` because a 100,000-article transaction
                // holds the database's write lock for ~2.7 s at p50
                // (~4.1 s worst) against a 10 s `busy_timeout` that
                // every OTHER connection - the daemon's writer, the API,
                // the wall enricher - is queued behind. See
                // `INGEST_BATCH` for the table and for what this does to
                // generation outcomes, which is measured rather than
                // assumed.
                //
                // One `blocking_db` hop PER sub-batch, not one around
                // the loop: the point is that the write lock is dropped
                // between them, and a single hop holding the whole loop
                // would keep exactly the transaction the split exists to
                // shorten.
                for sub in entries.chunks(nzbkit::index::INGEST_BATCH) {
                    // blocking_db: same reasoning as the gapfill leg -
...
pub fn search_index(query: &str, db: &Path, nzb_out: Option<&std::path::Path>) -> Result<()> {
...
pub async fn spots_scan(config: &Path, group: &str, backfill: u64, db: &Path) -> Result<()> {
...
/// Spot resolution - the fetch budget, the miss report and `spot_search`.
/// Out of this file for the size gate's 4,000-line ceiling; re-exported
/// so callers keep their `scan::` paths.
mod spots;
pub use spots::{
    SpotResolveSummary, nzb_import, search_misses, spot_get, spot_resolve_pass, spot_search,
};
// The budget's own cases stay in this file's `mod scan_pass_tests`.
#[cfg(test)]
use spots::{SPOT_FETCH_CONNS_MAX, spot_fetch_conns};
...
mod scan_pass_tests {
    ...
    ///
    /// Lived in `repair/repair_tests.rs` and reached `trunc` through
    /// main.rs's `use scan::*;` root glob, which is why it was
    /// `#[cfg(feature = "indexer")]` in a file that has nothing else
    /// gated in it. Crate-split step 3 put `repair` in `nzbfast-unpack`,
    /// a sibling layer of `scan`'s, so the test moved to the subject
    /// rather than the subject being exported to the test.
    #[test]
    ...
    /// The cursor jumps only when another server's frontier is OLDER
    /// than the primary's own low-water by more than a day, and never
    /// on a date that could not be read. The shape that motivated it: the
    /// new primary at 31 Aug, the old one at 17 Feb.
    #[test]
    fn history_cursor_jumps_only_over_covered_ground() {
        let day = 86_400;
        let aug31 = 1_788_134_400;
        let feb17 = aug31 - 195 * day;
        assert!(history_cursor_should_jump(aug31, feb17));
        // The other way round: the primary is already deeper. Stay.
        assert!(!history_cursor_should_jump(feb17, aug31));
        // Inside the day of slack: propagation jitter, not depth.
        assert!(!history_cursor_should_jump(aug31, aug31 - day));
        assert!(!history_cursor_should_jump(aug31, aug31 - day + 1));
        assert!(history_cursor_should_jump(aug31, aug31 - day - 1));
        // An unreadable date on either side is never evidence.
        assert!(!history_cursor_should_jump(0, feb17));
        assert!(!history_cursor_should_jump(aug31, 0));
    }

    /// The frontier is the OLDEST dated mark among the other servers;
    /// an undated one is skipped even if it is the only one.
    #[test]
    fn deepest_frontier_is_the_oldest_dated_mark() {
        let marks = vec![
            (0, "news.c.example".to_string(), 10),
            (1_780_000_000, "news.b.example".to_string(), 20),
            (1_770_000_000, "news.a.example".to_string(), 30),
        ];
        let (d, host, low) = deepest_history_frontier(&marks).unwrap();
        assert_eq!(
            (d, host.as_str(), low),
            (1_770_000_000, "news.a.example", 30)
        );
        assert!(deepest_history_frontier(&[(0, "x".to_string(), 1)]).is_none());
        assert!(deepest_history_frontier(&[]).is_none());
    }

    /// The spot resolver's fan-out has to stay inside the account's
    /// budget on every shape of config, including the two that have no
    /// room for a fan-out at all. It takes half the sessions because a
    /// download is entitled to the rest, and one MORE session than this
    /// is opened for the corroborating STATs - so the arithmetic below
    /// is what keeps `fetchers + 1` off the account's ceiling for every
    /// configuration a user can plausibly have.
    #[test]
    fn the_spot_fan_out_stays_inside_the_account() {
        // Half, until the cap bites.
        assert_eq!(spot_fetch_conns(2, 500), 1);
        assert_eq!(spot_fetch_conns(4, 500), 2);
        assert_eq!(spot_fetch_conns(8, 500), 4);
        assert_eq!(spot_fetch_conns(16, 500), 8);
        // Capped: a 60-connection account does not get 30 background
        // sessions, and the resolver is latency-bound anyway.
        assert_eq!(spot_fetch_conns(60, 500), SPOT_FETCH_CONNS_MAX);
        assert_eq!(spot_fetch_conns(u32::MAX, 500), SPOT_FETCH_CONNS_MAX);
        // The degenerate accounts. Never zero: a pass with no fetcher
        // fetches nothing, which is a silently dead lane rather than a
        // narrow one. At `connections = 1` the extra STAT session is
        // the one the server refuses, and the pass still resolves.
        assert_eq!(spot_fetch_conns(1, 500), 1);
        assert_eq!(spot_fetch_conns(0, 500), 1);
    }

    /// A short pending list must not open sessions it will leave idle -
    /// the last pass of a drained backlog is one or two spots.
    #[test]
    fn the_spot_fan_out_never_exceeds_the_work() {
        assert_eq!(spot_fetch_conns(60, 1), 1);
        assert_eq!(spot_fetch_conns(60, 3), 3);
        assert_eq!(spot_fetch_conns(60, 40), SPOT_FETCH_CONNS_MAX);
        // Belt and braces: the caller returns early on an empty list,
        // so this shape never reaches the fan-out - but a zero here
        // would mean zero workers and a pass that hangs on a receive
        // that can never fire.
        assert_eq!(spot_fetch_conns(60, 0), 1);
    }
...
#[cfg(test)]
#[path = "scan_spot_tests.rs"]
mod scan_spot_tests;

#[cfg(test)]
mod primary_election_tests {
    use super::primary_rank;

    /// One candidate per (measured depth, claimed span), elected the
    /// way `index_scan_into` elects.
    fn elect(cands: &[(Option<i64>, u64)]) -> usize {
        cands
            .iter()
            .enumerate()
            .max_by_key(|(i, (d, span))| primary_rank(*d, *span, *i))
            .map(|(i, _)| i)
            .expect("no candidates")
    }

    /// TODO 321, the defect this exists for, with the live numbers.
    ///
    /// `usenet.farm` answered `GROUP alt.binaries.movies` with a
    /// claimed span of 7,372,647,585 while carrying ~10 days, against
    /// giganews at 5,906,081,615 claimed and 656 days real and
    /// vipernews at 6,297,024,382 and 642. Under the old rule the
    /// largest span won outright, so six of seven live groups ran their
    /// history leg on the shallowest carrier and 18 of 18 deepen slices
    /// came back with zero headers.
    #[test]
    fn the_deepest_carrier_wins_even_with_the_smallest_claimed_span() {
        let day = 86_400;
        let farm = (Some(-10 * day), 7_372_647_585u64);
        let giga = (Some(-656 * day), 5_906_081_615u64);
        let viper = (Some(-642 * day), 6_297_024_382u64);
        assert_eq!(elect(&[farm, giga, viper]), 1, "giganews carries the most");
        // Order must not decide it: the same three in any arrangement.
        assert_eq!(elect(&[giga, farm, viper]), 0);
        assert_eq!(elect(&[viper, farm, giga]), 2);
    }

    /// The fallback is the OLD rule exactly, so an install whose
    /// servers all refuse the ladder is unaffected by TODO 321.
    #[test]
    fn with_nothing_measurable_the_claimed_span_still_decides() {
        assert_eq!(elect(&[(None, 10), (None, 900), (None, 20)]), 1);
    }

    /// A measured candidate beats an unmeasured one whatever the spans
    /// say. Refusing the ladder is not evidence of depth, and the
    /// alternative - letting an unmeasured server keep winning on a
    /// claimed span - is the defect.
    #[test]
    fn a_measured_carrier_outranks_an_unmeasured_one() {
        let day = 86_400;
        assert_eq!(elect(&[(None, u64::MAX), (Some(-3 * day), 1)]), 1);
    }

    /// Ties fall to config order, first entry first, which is what the
    /// old rule did through `Reverse(i)` and what the level ranking
    /// upstream of it expects.
    #[test]
    fn an_exact_tie_keeps_config_order() {
        let d = Some(-100 * 86_400);
        assert_eq!(elect(&[(d, 5), (d, 5), (d, 5)]), 0);
    }

    /// Depth outranks the span, but the span is still consulted when
    /// two carriers measured identically - the case a single provider
    /// with two hostnames actually produces.
    #[test]
    fn equal_depth_falls_through_to_the_claimed_span() {
        let d = Some(-500 * 86_400);
