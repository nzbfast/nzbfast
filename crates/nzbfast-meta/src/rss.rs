...
// The two date helpers below and `urlenc` in `netfetch` were `newznab`'s
// until the crate-split prep (step 1 of
// research/PLAN-NZBFAST-CRATE-SPLIT-2026-09-01.md). `newznab` already
// reads this module's `attr` / `tag_text` / `unescape` - the wire-text
// half both feed parsers share - so a date parser living up in newznab
// made the two need each other, and `urlenc` reached even further, from
// `xrel` in the lowest layer up into the metadata one. Both moved DOWN,
// to the module that was already the shared util.

/// Days from civil date (Howard Hinnant's algorithm), the inverse of
/// `civil_from_days` in crates/nzbfast-daemon/src/disk.rs.
