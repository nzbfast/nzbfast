...
/// differently, and once `1dbcca3c2` moved its pick to the front of the
/// confirm lane three unit tests started dialling api.tvmaze.com.
fn get_json(p: Provider, url: &str) -> Option<serde_json::Value> {
    parse_answer(&get_body(p, url)?)
}

/// The fetch half of [`get_json`], for the two providers that answer in
/// HTML rather than JSON (the keyless at-home movie calendars in
/// `digital`). Same call-out gate, same cooldown, same bucket, same
/// 429/503 handling - the body is simply handed back unparsed.
fn get_body(p: Provider, url: &str) -> Option<String> {
...
/// One row of a TVmaze day schedule: which episode of which show airs
/// on a date. The expectation oracle's input - the TARGETING use of
/// the calendar (research/AIRDATE-CORRELATION-2026-09-01.md measured
/// the correlation use of the same calendar and found its blocker).
...
/// Pure parse of a TVmaze `/schedule` or `/schedule/web` response
/// (tested). The two endpoints nest the show differently - `show` on
/// the broadcast feed, `_embedded.show` on the web feed - so one
/// parser reads both. Specials (null season or number) are dropped,
/// exactly as [`parse_tvmaze_episodes`] drops them.
fn parse_tvmaze_schedule(v: &serde_json::Value) -> Vec<SchedEp> {
    v.as_array()
        .map(|list| {
            list.iter()
                .filter_map(|e| {
                    let show = e["show"]["name"]
                        .as_str()
                        .or_else(|| e["_embedded"]["show"]["name"].as_str())?
                        .trim()
                        .to_string();
                    if show.is_empty() {
                        return None;
...
/// The TVmaze air schedule for one date - broadcast (US) or the web
/// feed (worldwide, where the day-and-date releases live). Keyless,
/// same pacing and cooldown as every TVmaze call here.
pub fn tvmaze_schedule(date: &str, web: bool) -> Vec<SchedEp> {
    let url = if web {
        format!("https://api.tvmaze.com/schedule/web?date={date}")
    } else {
        format!("https://api.tvmaze.com/schedule?country=US&date={date}")
    };
    get_json(Provider::Tvmaze, &url)
        .map(|v| parse_tvmaze_schedule(&v))
        .unwrap_or_default()
}

/// One movie the expectation oracle wants confirmed: a title newly
/// released for home viewing. The USEFUL date for naming is the
/// DIGITAL (at-home) release, not theatrical - a theatrical premiere
/// is months before anything is posted, while a digital/web release is
/// the day-and-date event that a scene post follows.
#[derive(Debug, Clone, PartialEq)]
pub struct MovieRelease {
    pub title: String,
    /// Release year, from the primary `release_date`; 0 when unknown.
    pub year: u32,
}

/// Pure parse of a TMDB `/discover/movie` response (tested).
fn parse_tmdb_discover(v: &serde_json::Value) -> Vec<MovieRelease> {
    v["results"]
        .as_array()
        .map(|list| {
            list.iter()
                .filter_map(|m| {
                    ...
                    let year = m["release_date"]
                        .as_str()
                        .and_then(|d| d.get(0..4))
                        .and_then(|y| y.parse::<u32>().ok())
                        .unwrap_or(0);
                    Some(MovieRelease { title, year })
                })
                .collect()
        })
        .unwrap_or_default()
...
/// Movies whose DIGITAL release (`with_release_type=4`) falls in a
/// date window - the at-home calendar the expectation oracle targets.
/// TMDB, keyed; the same data MovieInsider / BoxOfficeMojo /
/// RottenTomatoes render as HTML, fetched from the API this codebase
/// already holds a key for rather than scraped. Empty on no key.
pub fn tmdb_digital_releases(api_key: &str, gte: &str, lte: &str) -> Vec<MovieRelease> {
    if api_key.is_empty() {
        return Vec::new();
    }
    // `with_release_type=4` is Digital; `release_date.gte/lte` then
    // bound it to the window. `region` is deliberately omitted - a
    // release is worth targeting wherever it went out.
    let url = format!(
        "https://api.themoviedb.org/3/discover/movie?api_key={api_key}\
         &with_release_type=4&sort_by=primary_release_date.desc\
         &release_date.gte={gte}&release_date.lte={lte}"
    );
    get_json(Provider::Tmdb, &url)
        .map(|v| parse_tmdb_discover(&v))
        .unwrap_or_default()
}
...
fn get_json_ua(p: Provider, url: &str) -> Option<serde_json::Value> {
    ...
    // the dashboard's metadata search box with NZBFAST_NO_ENRICH set.
    if !crate::identity::may_call_out() {
        return None;
    }
    if cooling_off(p) {
...
fn get_json_paced(p: Provider, url: &str) -> Option<serde_json::Value> {
    ...
    // differently, and MusicBrainz, OpenLibrary and cover-art are the
    // same kind of provider call as Wikidata.
    if !crate::identity::may_call_out() {
        return None;
    }
    if cooling_off(p) {
...
pub fn fetch_image_res(url: &str) -> Result<Vec<u8>, ArtMiss> {
    ...
    // measured reaching image.tmdb.org from a daemon started with
    // NZBFAST_NO_ENRICH=1. Transient, not NoImage: "we could not ask" is
    // what happened, and NoImage is the verdict that stamps a card's
    // hole permanent.
    if !crate::identity::may_call_out() {
        return Err(ArtMiss::Transient);
    }
...
/// and pins both.
pub const ART_DECORATION: &str = "..bd.jpg.new-4294967295-999999999";
