...
#[cfg(feature = "indexer")]
pub const OPENED_COALESCE_SECS: i64 = 3_600;
...
/// them in anyway.
#[cfg(feature = "indexer")]
pub const OPENED_MAX_ENTRIES: usize = 5_000;
...
#[cfg(feature = "indexer")]
pub const EVICT_MAX_PASSES: usize = 8;
...
#[cfg(feature = "indexer")]
pub(crate) const EVICT_PREVIEW_MAX_EXAMINE: usize = 200_000;
...
impl OpenedLog {
    ...
    pub fn touch_title(&mut self, key: &str, now: i64) -> bool {
        if key.is_empty() {
            return false;
        }
        let fresh = self
    ...
    pub fn touch_release(&mut self, id: i64, now: i64) -> bool {
        if id < 0 {
            return false;
        }
        let fresh = self
    ...
    pub(crate) fn trim<K: Clone + std::hash::Hash + Eq>(m: &mut std::collections::HashMap<K, i64>) {
    ...
    pub fn expire(&mut self, now: i64, window_secs: i64) {
