...
/// **The one place a user script becomes a `Command`.** TODO 314 stage 1.
///
/// Post-processing chains, the pre-queue hook and the queue-finished
/// script all come through here, so the confinement policy is stated
/// once. `extra` names directories this particular invocation has told
/// the script about (its job directory, a final directory a previous
/// chain link set) on top of the ones the daemon knows about anyway.
///
/// `script_confined` off returns a plain `Command::new`, unchanged from
/// what every one of these callers built before this existed. So does a
/// box with no confinement mechanism - see [`crate::sandbox`], which
/// says which in one log line at startup and in the settings page's
/// `sandbox` row.
pub(super) fn confined_command(
    ...
    if !d.script_confined.load(Ordering::Relaxed) {
        // sandbox-seam-gate: THE escape hatch, and the one place the seam's
        // rule is deliberately not applied. `script_confined` off is a setting
        // the user turned off on purpose, for a script that genuinely needs
        // more than `Policy::script` allows; the doc comment above and the
        // settings page both say so, and confining here would make the
        // setting a lie.
        return std::process::Command::new(script);
    }
    crate::sandbox::command(
        script,
        &crate::sandbox::Policy::script(script_writable(d, extra)),
    )
}

/// Where a confined script may write: everywhere the daemon itself
/// files things, plus whatever this invocation named.
///
/// Generous on purpose. The policy that matters is that a script may not
/// write ANYWHERE ELSE - not `~/.zshrc`, not a LaunchAgent, not the
/// daemon's own settings file - and a set narrow enough to break a
/// library filer would simply get the whole setting turned off, taking
/// that with it.
fn script_writable(d: &Daemon, extra: &[&std::path::Path]) -> Vec<PathBuf> {
    let mut out: Vec<PathBuf> = vec![crate::naming::out_dir(d), std::env::temp_dir()];
    out.extend(extra.iter().map(|p| p.to_path_buf()));
    if let Some(dest) = d.move_completed.read_ok().clone() {
        out.push(dest);
    }
    out.extend(
    ...
    if let Some(w) = d.watch_dir.lock_ok().clone() {
        out.push(w);
    }
...
fn run_capped_inner(
    ...
    let sieve = Arc::new(Mutex::new(crate::nzbget_script::LineSieve::default()));
...
pub const SCRIPT_ERR_TAIL: usize = 8 << 10;
...
pub static DRAIN_THREADS: AtomicUsize = AtomicUsize::new(0);
...
#[cfg(unix)]
pub trait PipeRead: std::io::Read + std::os::unix::io::AsRawFd {}
#[cfg(unix)]
impl<T: std::io::Read + std::os::unix::io::AsRawFd> PipeRead for T {}
#[cfg(not(unix))]
pub trait PipeRead: std::io::Read {}
#[cfg(not(unix))]
...
/// `sink`.
#[cfg(unix)]
pub fn drain(mut r: impl PipeRead, stop: &AtomicBool, mut sink: impl FnMut(&[u8])) {
...
pub(super) fn drain_into_sieve(
    r: impl PipeRead,
    stop: &AtomicBool,
    sieve: &Mutex<crate::nzbget_script::LineSieve>,
) {
...
impl Daemon {
    ...
    fn script_facts(
        ...
        Some(ScriptFacts {
            ...
            filed_tail: super::job::delete_tail(&j, || {
                crate::naming::job_suffix(self, super::job::filed_stem(&j))
            }),
        })
    }
    ...
    fn script_command(
        ...
        // TODO 314 stage 1: the two directories this link is being TOLD
        // about ride along with the daemon's own - a chain link that
        // moved the payload to `final_dir` has to be able to write there.
        let mut extra: Vec<&std::path::Path> = vec![std::path::Path::new(directory)];
        if !final_dir.is_empty() {
            extra.push(std::path::Path::new(final_dir));
        }
        let mut cmd = confined_command(self, script, &extra);
