...
pub use daemon_speed::AUTO_SPEED_FLOOR;
pub use daemon_speed::AUTO_SPEED_MAX;
pub use daemon_speed::AUTO_SPEED_START;
...
#[path = "daemon_cats.rs"]
mod daemon_cats;
pub(crate) use daemon_cats::CatMeta;
pub use daemon_cats::DEFAULT_CATS;
...
// The §74 instant watchlist path - whether the watchlist may spend the
// user's indexer accounts, the matcher compiled from the live
// watchlist, and staging an arrival so the pass wakes for it (TODO
// 106). Same child-module shape as daemon_index. No cfg on the module:
// `watchlist_external_on` is read in every build, so the `indexer`
// gates stay per-item.
#[path = "daemon_watchlist.rs"]
mod daemon_watchlist;

// The pre feed's connection - the two-switch admission, the stored
// settings turned into a connection description, and what the settings
// card says it is doing (TODO 106). Same child-module shape. Distinct
// from daemon_watchlist above: that one is what the daemon does with a
// name once it has heard it, this one is one of the places it hears.
#[path = "daemon_predb.rs"]
mod daemon_predb;

// What this daemon already has - the dupe-key and title-key sets a wall
// verdict joins browse rows against, over Completed history plus the
// live queue (TODO 106). Same child-module shape as daemon_index.
#[path = "daemon_dupekeys.rs"]
mod daemon_dupekeys;
...
pub struct LadderPermit(std::sync::Arc<Daemon>);

impl LadderPermit {
    /// `None` when another ladder already holds it.
    pub fn try_take(d: &std::sync::Arc<Daemon>) -> Option<Self> {
        d.ladder_busy
            .compare_exchange(
                false,
                true,
...
pub struct Daemon {
    ...
    /// §292's same-post arm, memoised: the message-id set of a queued
    /// row's spool NZB, keyed by nzo_id.
    ///
    /// The remedy `the_same_post_arm_stays_cheap_against_a_wide_queue`
    /// named in as many words ("a PostIds cache keyed by nzo_id, evicted
    /// to queue membership - build that, do not widen the prefilter"),
    /// built 3 Sep 2026 because the shape it was insurance against
    /// turned out to be the ordinary one: `dupe_post_collision` re-read
    /// and re-parsed EVERY queued row's spool file on EVERY add, so
    /// building a queue through `mode=addfile` was quadratic - 1,162.6 s
    /// for 5,000 jobs, 44.6 ms/add over the first 500 rising to 232.5
    /// ms/add as the mean (audit round 21).
    ///
    /// A pure memo of an immutable file, never a statement about
    /// membership: which rows are collision targets is still read live
    /// off the queue under its own lock, so a stale entry can only cost
    /// a wasted lookup and never a missed - or invented - duplicate. The
    /// spool copy a key names is written once, atomically, before the
    /// row is published and is only ever renamed aside when the row
    /// LEAVES the queue, which is what makes the memo sound.
    ///
    /// Primed at the publish that admits a row (the add has just parsed
    /// that NZB, so the entry is free) and filled lazily otherwise, so
    /// the first add after a restart pays one parse per restored row and
    /// nothing after it does. Evicted to live queue membership when it
    /// outgrows the queue it describes.
    pub post_ids: Mutex<std::collections::HashMap<String, Arc<spare::PostIds>>>,
    ...
    /// §7a: what this process has published to `.spool/queue.jsonl`, so
    /// `save_queue` appends the lines that CHANGED instead of rewriting
    /// the whole queue. Only ever touched under
    /// [`Daemon::hold_queue_writes`]; see `queuestore.rs`.
    pub queue_pub: Mutex<crate::queuestore::QueuePub>,
    ...
    pub hist_inflight: Mutex<std::collections::HashSet<String>>,
    ...
    pub pause_announced: AtomicBool,
    ...
    /// cannot strand it.
    pub postproc_backlog: Arc<AtomicUsize>,
    pub finish: finish_action::FinishState,
    ...
    pub save_soon: AtomicBool,
    /// Wakes `spawn_queue_saver` when `save_soon` is set.
    pub save_wake: tokio::sync::Notify,
    /// Wakes the local seed harvester after an accepted NZB becomes
    /// durable in the queue or history. A notification is only a hint:
    /// the worker scans the retained records, so coalescing cannot lose
    /// an add and a restart can recover work whose wake never ran.
    #[cfg(feature = "indexer")]
    pub seed_harvest_wake: tokio::sync::Notify,
    ...
    pub hooks_tx: Mutex<Option<std::sync::mpsc::SyncSender<Value>>>,
    ...
    /// lanes (mover.rs) - same destination stays serial, two
    /// different destinations never queue behind each other.
    pub mover_q: Mutex<VecDeque<Arc<Mutex<Job>>>>,
    ...
    pub mover_inflight: std::sync::atomic::AtomicUsize,
    ...
    /// the `.par2` files are cleaned up. Costs serialization only - the
    /// data is already in memory at settle.
    ///
    /// **ON by default since 2 Sep 2026 (§310), and the reason it was
    /// off before is worth keeping.** Stage 1 shipped it off because a
    /// new file in every finished folder is a default to choose
    /// deliberately rather than inherit - but the consequence, once the
    /// heal road landed on top of it, was that the whole of §310 was
    /// unreachable on every install: no manifest is written, so
    /// `nzbfast verify` has nothing to fall back on and `heal_offer`
    /// refuses every folder. A feature nobody can reach is not a
    /// conservative default, it is an absent one. It ships ON together
    /// with the dashboard tick that turns it back off (Settings ->
    /// Downloading -> Checking downloads later), never before it: a
    /// user who finds a new hidden file in every finished folder needs
    ...
    /// in `metrics.rs`, which carry no job names and no paths, but
    ...
    /// applies to the NEXT enqueue without a restart. Read via `crate::naming::out_dir()`.
    ...
    /// §310's scheduled library heal. OFF, and the ceilings a road
    /// nobody clicked needs are on the type.
    pub heal_auto: super::healauto::HealAutoSettings,
    ...
    ///
    /// ENFORCED: a manifest whose serial is lower than this, or which has
    /// no usable serial once this is non-zero, is refused outright. It
    /// shipped read-only from 28 Jul to 2 Sep so the enforcing build would
    ...
    /// server-side reset: a release that shipped a wrong serial would wedge
    /// the update channel on every install that recorded it, and no later
    /// release could unwedge it. What unwedges it is deleting the
    /// `update_serial_seen` line from settings.json and restarting, which
    /// every refusal message spells out with the actual path.
    ///
    /// Deliberately NOT a settable setting, for the same reason it is a
    /// usable escape hatch: the API is reachable over the LAN and, on a
    /// keyless install, by anyone at all, so a settable reset would hand
    /// the network the ability to lower this install's rollback floor -
    /// which is precisely the attack the ratchet exists to stop. Needing a
    /// shell on the box and a stopped daemon is the point, not friction to
    /// be smoothed away later.
    ...
    /// Seconds of index write-lock time ONE fold slice may spend (live
    /// setting index_fold_secs), shared by the shatter fold and the
    /// session fold - one dial, both `Index::*_fold` calls. Each scan
    /// lap runs up to `FOLD_SLICES_PER_LAP` slices of each fold and
    /// releases the write mutex between slices, so this is the longest
    /// hold a queued HTTP worker waits behind, and slices times this is
    /// the fold's work per lap. Default and range are measured in
    /// research/FOLD-BUDGET-DIAL-2026-09-02.md.
    pub index_fold_secs: AtomicU64,
    ...
    pub indexer_rt: Mutex<IndexerRuntime>,
    ...
    /// applies over.
    #[cfg(feature = "indexer")]
    pub instant_kicks: Mutex<std::collections::VecDeque<i64>>,
    ...
    #[cfg(feature = "indexer")]
    pub instant_pending: Mutex<std::collections::HashMap<i64, i64>>,
    ...
    pub instant_hint: Mutex<Vec<String>>,
    ...
    /// TODO 313 items 2-5 and 10: lend a struggling head's unused
    /// sockets to the queue (settings.json `queue_spill`).
    ///
    /// **OFF, and it stays off until somebody measures it on their own
    /// link.** The study that specified it measured 29-45% of total
    /// wall where the jobs behind are SMALL and, in the same document,
    /// proves by identity that between two jobs of comparable size
    /// overlap cannot buy total wall at all. Which of those two a given
    /// queue is cannot be known from here, and the head pays 17-44% of
    /// its own wall either way - a product judgement about whose
    /// download is waiting, not a default this lane may take.
    ///
    /// The memory precondition IS met, and it was not when this was
    /// written: TODO 313 item 1 landed the same day (`e7c393951`), so
    /// `inflight_cap` is now one shared `WireCharge` ledger rather than
    /// a per-pool counter, and two concurrent download phases admit one
    /// wire-side body ceiling between them instead of two. Without that
    /// this switch would have been a sustained doubling on exactly the
    /// small-RAM boxes the B4 clamp exists for; with it, what is left
    /// to decide is the product question above and nothing else.
    pub queue_spill: AtomicBool,
    /// The governor behind that switch: the trigger's window, the
    /// stand-downs and the live episode. See `serve::spill`.
    pub spill: super::spill::Governor,
    ...
    /// `serve::slowstore`.
    pub slow_storage: super::slowstore::Governor,
    ...
    pub pause_cost: super::requeue::PauseCostCache,
    ...
    /// The auto-rename sub-settings, as one value.
    ///
    /// Grouped 2 Sep 2026 to keep `build_daemon` under the 500-line
    /// function ceiling: it is one `field: value` line per `Daemon`
    /// field and had five lines of headroom left. Thirteen switches
    /// that are read together, set together and saved together are
    /// the natural first group. The SETTINGS API keys are unchanged
    /// (`rename_junk` and friends are string literals in
    /// `serve::settings`), so nothing a user or an *arr sees moves.
    ...
    pub preview_cache: Mutex<Vec<(u64, i64, Value)>>,
    ...
    pub preview_busy: std::sync::atomic::AtomicBool,
    ...
    pub index_pause_on_download: std::sync::atomic::AtomicBool,
    ...
    pub index_paused: std::sync::atomic::AtomicBool,
    ...
    pub enrich_paused: std::sync::atomic::AtomicBool,
    ...
    pub index_enabled: std::sync::atomic::AtomicBool,
    /// The `predb_*` settings, as one value - see [`PredbSettings`].
    ///
    /// Grouped for `build_daemon`'s 500-line function ceiling, which
    /// is one `field: value` line per `Daemon` field and nothing else.
    /// The settings API keys are string literals in `serve::settings`
    /// and did not move.
    pub predb: PredbSettings,
    /// The `scoreboard_*` settings, as one value - see [`ScoreboardSettings`].
    ///
    /// Grouped for `build_daemon`'s 500-line function ceiling, which
    /// is one `field: value` line per `Daemon` field and nothing else.
    /// The settings API keys are string literals in `serve::settings`
    /// and did not move.
    pub scoreboard: ScoreboardSettings,
    ...
    pub corr_confirm_enabled: std::sync::atomic::AtomicBool,
    ...
    pub corr_confirm_source: Mutex<String>,
    ...
    // The `#[cfg_attr(not(feature = "dashboard"), allow(dead_code))]`
    // this carried until lane 2 of the serve split is GONE, and both
    // halves of why are worth keeping: `dashboard` is not a feature of
    // THIS crate (assets, webasset and devweb are api-layer and stayed
    // in the bin), so the predicate named a cfg that does not exist
    // here; and the field is `pub` on a library type now, which
    // `dead_code` cannot judge in any configuration. Step 3 of the
    // crate-split plan met the identical pair on
    // `nzbfast_core::interests::Interest::cats`.
    pub spot_enabled: std::sync::atomic::AtomicBool,
    /// Spot groups to scan. free.pt is the one live Spotnet group.
    pub spot_groups: Mutex<Vec<String>>,
    /// Articles to walk back on the first pass; later passes resume from
    /// the stored `spots:<group>` high-water mark.
    pub spot_backfill: AtomicU64,
    ...
    pub index_jobs_active: Arc<AtomicUsize>,
    ...
    pub whyslow: super::whyslow::WhySlow,
    ...
    pub cpu_sample: Mutex<Option<(Instant, f64, f64)>>,
    ...
    pub usage: Mutex<serde_json::Map<String, Value>>,
    ...
    pub provquality: super::provquality::ProvQuality,
    ...
    pub run_usage_flushed: Mutex<std::collections::HashMap<String, u64>>,
    ...
    pub block_band: Mutex<std::collections::HashMap<String, u8>>,
    ...
    pub pause_wake: std::sync::Condvar,
    ...
    pub pause_timer_live: std::sync::atomic::AtomicBool,
    ...
    pub queue_hold: Mutex<Option<(String, f64, f64)>>,
    ...
    pub pause_source: Mutex<&'static str>,
    ...
    pub limit_source: Mutex<&'static str>,
    ...
    /// job completion.
    pub cat_meta: Mutex<std::collections::HashMap<String, CatMeta>>,
    /// Watch folder for dropped .nzb files (None = off).
    pub watch_dir: Mutex<Option<PathBuf>>,
    ...
    /// paused. Off; caps and reasoning in `refeed.rs`. Live.
    pub refeed_nzb: AtomicBool,
    /// TODO 314 stage 1: run post-processing, pre-queue and
    /// queue-finished scripts through the confinement wrapper
    /// (`crate::sandbox`). ON by default, and what it buys is WRITE
    /// confinement: a confined script keeps its network and its reads -
    /// a notifier needs one and a library filer needs the other - but
    /// may only write under the directories the daemon already knows
    /// about (the job, the download root, the move-completed
    /// destinations, the watch folder and the system temp directory).
    /// That still removes traversal writes and every persistence trick.
    /// Turn it off for a script that genuinely needs to write somewhere
    /// else. Live - read per spawn, so a change applies to the next
    /// script without a restart. No-op where no confinement mechanism
    /// is available; the settings page's `sandbox` row says which.
    pub script_confined: AtomicBool,
    ...
    pub watch_failed: Mutex<std::collections::HashMap<PathBuf, (u64, u64, String, String)>>,
    ...
    pub delete_kept: Mutex<std::collections::VecDeque<KeptNote>>,
    ...
    pub auth_fails: Mutex<std::collections::HashMap<std::net::IpAddr, (u32, Instant)>>,
    ...
    #[cfg(feature = "indexer")]
    pub enrich_hot: Mutex<std::collections::VecDeque<String>>,
    ...
    #[cfg(feature = "indexer")]
    pub group_catalog: Mutex<Option<Arc<crate::groups::Catalog>>>,
    /// True while a catalogue fetch is in flight (single-flight guard).
    #[cfg(feature = "indexer")]
    pub group_fetching: std::sync::atomic::AtomicBool,
    /// Last catalogue-fetch failure, surfaced in the browser UI.
    #[cfg(feature = "indexer")]
    pub group_fetch_err: Mutex<Option<String>>,
    ...
    #[cfg(feature = "indexer")]
    pub group_stats: Mutex<Arc<crate::groupstats::StatsCache>>,
    ...
    #[cfg(feature = "indexer")]
    pub group_sampling: Mutex<std::collections::HashSet<String>>,
    ...
    pub group_desc_isc: std::sync::atomic::AtomicBool,
    ...
    pub pre_queue_script: Mutex<Option<PathBuf>>,
    pub pre_queue_timeout: AtomicU64,
    /// Media servers / webhooks told about every finished job. Empty =
    /// off, which is the default. See [`crate::notify`].
    pub notify_targets: Mutex<Vec<crate::notify::Target>>,
    ...
    pub quality_prefs: Mutex<nzbfast_meta::watchlist::QualityPrefs>,
    ...
    pub omdb_key: Mutex<Option<String>>,
    ...
    pub tmdb_key: Mutex<Option<String>>,
    /// Re-verify interval for parked library jobs.
    pub library_recheck_secs: AtomicU64,
    /// Index scanner inputs, read each cycle.
    pub index_groups: Mutex<Vec<String>>,
    ...
    /// they did not choose.
    pub index_interests: Mutex<String>,
    ...
    pub index_interests_applied: Mutex<String>,
    ...
    pub index_interest_groups: Mutex<Vec<String>>,
    pub index_interval_secs: AtomicU64,
    pub index_backfill: AtomicU64,
    ...
    pub watch_state: Mutex<nzbfast_meta::watchlist::WatchState>,
    /// mode=watchlist_check_now: wakes the watcher out of its sleep.
    pub watch_now: tokio::sync::Notify,
    /// §151 external list sources. Declared in `crates/nzbfast-daemon/src/listsrc.rs`, which
    /// owns it; `Daemon::watch_items` is how it is read.
    pub lists: super::listsrc::ListState,
    ...
    pub giveup: Arc<Mutex<super::giveup::GiveupState>>,
    /// §282 section C: the replacement hunt's work queue. Its ceilings
    /// are item 13's and live on `alt` above (see `hunt.rs`).
    pub hunt: super::hunt::HuntState,
    ...
    #[cfg(feature = "indexer")]
    pub taste_cache: Mutex<Option<(std::time::Instant, TasteProfile)>>,
...
mod daemon_evict;
#[cfg(feature = "indexer")]
pub use daemon_evict::EVICT_MAX_PASSES;
#[cfg(feature = "indexer")]
pub(crate) use daemon_evict::EVICT_PREVIEW_MAX_EXAMINE;
#[cfg(feature = "indexer")]
...
#[cfg(all(any(test, feature = "test-support"), feature = "indexer"))]
pub use daemon_evict::OPENED_COALESCE_SECS;
#[cfg(all(any(test, feature = "test-support"), feature = "indexer"))]
pub use daemon_evict::OPENED_MAX_ENTRIES;
...
/// tick.
#[cfg(feature = "indexer")]
pub fn wall_tip_body(tip: Option<nzbkit::index::TipInfo>, initialized: bool) -> serde_json::Value {
...
mod daemon_maint;
#[cfg(feature = "indexer")]
pub use daemon_maint::{COMPACT_CHUNK_PAGES, MaintenanceArm, abort_compact_when_job_starts};
#[cfg(feature = "indexer")]
...
#[cfg(all(any(test, feature = "test-support"), feature = "indexer"))]
pub use daemon_maint::COMPACT_ABORT_POLL_MS;
...
pub struct BenchRun<'a>(&'a std::sync::atomic::AtomicBool);
...
impl Daemon {
    /// `index_fold_secs` default and range, in one place: the setter,
    /// the restore clamp and the two daemon constructors all read
    /// these. The default is the value
    /// research/FOLD-BUDGET-DIAL-2026-09-02.md measured against 1 s
    /// on a clone of the live index; the floor is 1 because 0 is not
    /// "off" for a fold whose cursor must follow the top; the ceiling
    /// is where a slice's hold would outlast the bounded HTTP waiter
    /// (`HTTP_INDEX_WAIT`, 5 s) by so much that a dashboard edit is
    /// refused behind every slice.
    pub const FOLD_SECS_DEFAULT: u64 = 4;
    pub(crate) const FOLD_SECS_MIN: u64 = 1;
    pub(crate) const FOLD_SECS_MAX: u64 = 30;
    ...
    pub fn watch_failed_remove(&self, p: &std::path::Path) {
    ...
    pub fn watch_failed_prune_missing(&self) {
        let changed = {
            let mut wf = self.watch_failed.lock_ok();
            let before = wf.len();
    ...
    pub fn note_event(&self, kind: &'static str, detail: impl Into<String>) {
    ...
    /// the pool ring.
    pub fn recent_events(&self, limit: usize) -> Vec<DaemonEvent> {
        self.events
            .lock()
...
pub const FAILURE_REGRAB_MAX: u8 = 3;
...
impl Daemon {
    ...
    /// not scan data).
    #[cfg(feature = "indexer")]
    pub fn groups_cache_path(&self) -> PathBuf {
    ...
    /// lifecycle.
    #[cfg(feature = "indexer")]
    pub fn groupstats_cache_path(&self) -> PathBuf {
    ...
    pub fn pick_job(&self, queue_paused: bool) -> Option<Arc<Mutex<Job>>> {
        let q = self.queue.lock_ok();
    ...
    pub fn bench_history(&self) -> Vec<Value> {
    ...
    /// included.
    pub fn bench_begin(&self) -> Option<BenchRun<'_>> {
        self.bench_running
            .compare_exchange(
                false,
                true,
    ...
    pub fn queue_job(&self, id: &str) -> Option<Arc<Mutex<Job>>> {
        find_job(self.queue.lock_ok().iter(), id)
    }

    /// Same, for history.
    pub fn history_job(&self, id: &str) -> Option<Arc<Mutex<Job>>> {
    ...
    pub fn owns_hub(&self, want: impl Fn(&str) -> bool) -> bool {
    ...
    /// other way.
    pub fn tail_phase(&self, nzo_id: &str) -> Option<&'static str> {
...
/// The `rename_*` half of the auto-rename settings, lifted out of
/// [`Daemon`] so its construction is one line rather than thirteen (see
/// the field's own doc). Every switch keeps its documentation, its type
/// and its shipped default; only the `rename_` prefix moves from the
/// field name into the path.
#[derive(Debug)]
pub struct RenameSettings {
    pub resolution: std::sync::atomic::AtomicBool,
    pub vcodec: std::sync::atomic::AtomicBool,
    pub acodec: std::sync::atomic::AtomicBool,
    pub source: std::sync::atomic::AtomicBool,
    pub group: std::sync::atomic::AtomicBool,
    ...
    pub year_parens: std::sync::atomic::AtomicBool,
    /// Wrap the quality facts in square brackets. Off by default.
    pub quality_brackets: std::sync::atomic::AtomicBool,
    ...
    pub extra_words: std::sync::atomic::AtomicBool,
...
impl RenameSettings {
    /// The shipped defaults: on, with resolution in the name; codecs /
    /// source / group off; junk sweep on; keep-media-only off. Saved
    /// settings replay over these at startup.
    ///
    /// `legacy_punctuation` is the install-age flag `build_daemon` reads
    /// for the two punctuation switches - it decides them and nothing
    /// else, which is why it is the only parameter.
    pub fn new(legacy_punctuation: bool) -> Self {
        use std::sync::atomic::AtomicBool;
        Self {
            resolution: AtomicBool::new(true),
            vcodec: AtomicBool::new(false),
            acodec: AtomicBool::new(false),
            source: AtomicBool::new(false),
            group: AtomicBool::new(false),
            year_parens: AtomicBool::new(legacy_punctuation),
            quality_brackets: AtomicBool::new(legacy_punctuation),
            extra_words: AtomicBool::new(true),
            identify: AtomicBool::new(true),
            ...
            episode_titles: AtomicBool::new(false),
            junk: AtomicBool::new(true),
            media_only: AtomicBool::new(false),
            from_nzb: AtomicBool::new(false),
        }
    }
}

/// The `scoreboard_*` fields, lifted out of [`Daemon`] so their construction
/// is one line rather than 8 (see the field's own doc). Every one keeps
/// its documentation, its type, its `#[cfg]` and its shipped default;
/// only the `scoreboard_` prefix moves from the field name into the path.
#[derive(Debug)]
pub struct ScoreboardSettings {
    ...
    pub calibrate: std::sync::atomic::AtomicBool,
...
/// The `predb_*` fields, lifted out of [`Daemon`] so their construction
/// is one line rather than 12 (see the field's own doc). Every one keeps
/// its documentation, its type, its `#[cfg]` and its shipped default;
/// only the `predb_` prefix moves from the field name into the path.
#[derive(Debug)]
pub struct PredbSettings {
    ...
    #[cfg(feature = "indexer")]
    pub pending: Mutex<Vec<nzbkit::predb::PreLine>>,
    ...
    pub corr_enabled: std::sync::atomic::AtomicBool,
    ...
    pub corr_auto: std::sync::atomic::AtomicBool,
    ...
    #[cfg(feature = "indexer")]
    pub max_rows: std::sync::atomic::AtomicU64,
