...
//! `pub(crate)` for exactly that reason: `super` is `daemon`
...
pub(crate) const WIND_DOWN_BUDGET: std::time::Duration = std::time::Duration::from_secs(4);
...
pub(crate) const OFFLINE_PARK_BUDGET: std::time::Duration = std::time::Duration::from_secs(60);
...
pub fn wind_down(d: &Arc<Daemon>, rt: &tokio::runtime::Handle, reason: &str) {
    ...
    // TODO 313 item 8: and stop the standing reserve refilling what the
    // clear below is about to empty. Unlike going offline, a wind-down
    // does not latch `set_accepting(false)` - `clear` deliberately does
    // not touch that flag, because config reloads call it - so pointing
    // the reserve at an empty server list is what tells it to want
    // nothing and hand its lease slots back. Sync, and `.get()` for the
    // same reason as the pool below: asking for the reserve in order to
    // stop it must not CREATE one on a thread with no reactor.
    if let Some(reserve) = d.hub.warm_reserve.get() {
        reserve.set_servers(&[]);
    }
    let warms: Vec<_> = d
        .hub
...
/// blocks forever.
pub fn wind_down_and_exit(d: &Arc<Daemon>, rt: &tokio::runtime::Handle, reason: &str) -> ! {
    {
...
pub fn install_shutdown_signals(daemon: &Arc<Daemon>) {
...
pub fn timed_pause(d: &Arc<Daemon>, mins: u64, graceful: bool) {
...
pub fn arm_pause_timer(d: &Arc<Daemon>, dur: std::time::Duration) {
    ...
    if spawn {
        ...
        if !crate::spawn_aux("pause-timer", move || pause_timer(&weak, stop)) {
...
fn pause_timer(weak: &std::sync::Weak<Daemon>, stop: crate::RunStop) {
...
pub fn set_paused_cancel_timer(d: &Daemon, paused: bool) -> bool {
    let was = {
        let mut until = d.pause_until.lock_ok();
...
pub fn pause_source_now(d: &Daemon) -> &'static str {
...
pub fn announce_pause(d: &Daemon) {
...
pub fn restore_pause(d: &Arc<Daemon>, saved: &serde_json::Map<String, Value>) {
