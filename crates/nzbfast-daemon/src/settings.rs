...
/// The floor under a non-zero `index_tip_secs`, in seconds. Production is
/// 5: the tick costs one GROUP command per group when nothing has arrived,
/// and there is no reason to spin faster than posts appear. Test hook:
/// `NZBFAST_TEST_TIP_FLOOR_SECS` lowers it for the watchlist-instant legs,
/// whose assertions are "N tip ticks of silence" and paid 13 s of sleep
/// per tick pair at the production floor. Unset (the only production
/// state) reads exactly as the literal did.
pub(crate) fn index_tip_floor_secs() -> u64 {
    std::env::var("NZBFAST_TEST_TIP_FLOOR_SECS")
        .ok()
...
pub const fn assembled(name: &'static str) -> Setting {
    Setting {
        name,
...
pub const fn ro(name: &'static str, read: fn(&ConfigCtx) -> Value) -> Setting {
    Setting {
        name,
        expose: Expose::Config(read),
        write: Write::No,
...
pub(super) const PATHS: &[Setting] = &[
    ...
    rw("out_dir", |c| {
        json!(crate::naming::out_dir(c.d).to_string_lossy())
    }),
    ...
    // TODO 314 stage 1.
    rw("script_confined", |c| {
        json!(c.d.script_confined.load(Ordering::Relaxed))
    }),
    // Which confinement mechanism this box actually has, and one
    // sentence saying what it does. Read-only: nothing here is a
    // choice, it is what the machine offers.
    ro("sandbox", |_| crate::sandbox::health()),
...
pub(super) const DOWNLOAD: &[Setting] = &[
    ...
    // The final check, which is a DIFFERENT pass from `fast_verify`
    // above: that one is what gets checked as articles arrive, this one
    // is the full read-back of the finished files at settle and what
    // `nzbfast verify` does on the command line.
    //
    // NO DAEMON FIELD BEHIND IT, deliberately. The value lives in
    // nzbkit's par2 module because the CLI, this setting and
    // NZBFAST_VERIFY_IFSC_ONLY must resolve to ONE value; a mirror here
...
pub(super) const SPEED: &[Setting] = &[
    ...
    // §310's scheduled library heal. Four rows, all defaulting to "do
    // not sweep, and if you do, spend almost nothing" - the argument for
    // every default, and why an automatic heal cannot inherit the
    // clicked road's ceilings, is on `healauto`'s module header.
    rw("heal_auto", |c| {
        json!(c.d.heal_auto.enabled.load(Ordering::Relaxed))
    }),
    rw("heal_auto_interval_h", |c| {
        json!(c.d.heal_auto.interval_h.load(Ordering::Relaxed))
    }),
    rw("heal_auto_max_jobs", |c| {
        json!(c.d.heal_auto.max_jobs.load(Ordering::Relaxed))
    }),
    rw("heal_auto_max_bytes", |c| {
        json!(c.d.heal_auto.max_bytes.load(Ordering::Relaxed))
    }),
...
pub(super) const RENAME: &[Setting] = &[
    ...
    rw("rename_resolution", |c| {
        json!(c.d.rename.resolution.load(Ordering::Relaxed))
    }),
    rw("rename_vcodec", |c| {
        json!(c.d.rename.vcodec.load(Ordering::Relaxed))
    }),
    rw("rename_acodec", |c| {
        json!(c.d.rename.acodec.load(Ordering::Relaxed))
    }),
    rw("rename_source", |c| {
        json!(c.d.rename.source.load(Ordering::Relaxed))
    }),
    rw("rename_group", |c| {
        json!(c.d.rename.group.load(Ordering::Relaxed))
    }),
    rw("rename_year_parens", |c| {
        json!(c.d.rename.year_parens.load(Ordering::Relaxed))
    }),
    rw("rename_quality_brackets", |c| {
        json!(c.d.rename.quality_brackets.load(Ordering::Relaxed))
    }),
    rw("rename_extra_words", |c| {
        json!(c.d.rename.extra_words.load(Ordering::Relaxed))
    }),
    rw("rename_identify", |c| {
        json!(c.d.rename.identify.load(Ordering::Relaxed))
    }),
    rw("rename_episode_titles", |c| {
        json!(c.d.rename.episode_titles.load(Ordering::Relaxed))
    }),
    rw("rename_junk", |c| {
        json!(c.d.rename.junk.load(Ordering::Relaxed))
    }),
    rw("rename_media_only", |c| {
        json!(c.d.rename.media_only.load(Ordering::Relaxed))
    }),
    rw("skip_samples", |c| {
...
pub(super) const INDEXING: &[Setting] = &[
    ...
    // The per-slice write-lock budget of the shatter and session folds
    // (seconds); see `Daemon::index_fold_secs`.
    rw("index_fold_secs", |c| {
        json!(c.d.index_fold_secs.load(Ordering::Relaxed))
    }),
    ...
    rw("predb_enabled", |c| {
        json!(c.d.predb.enabled.load(Ordering::Relaxed))
    }),
    rw("predb_server", |c| {
        json!(c.d.predb.server.lock_ok().clone())
    }),
    rw("predb_channels", |c| {
        json!(c.d.predb.channels.lock_ok().clone())
    }),
    rw("predb_nick", |c| json!(c.d.predb.nick.lock_ok().clone())),
    ...
    rw("predb_corr_enabled", |c| {
        json!(c.d.predb.corr_enabled.load(Ordering::Relaxed))
    }),
    rw("predb_corr_auto", |c| {
        json!(c.d.predb.corr_auto.load(Ordering::Relaxed))
    }),
    // Capacity, not policy: how many pre rows to keep, and how far back
    // a seed import reaches when it is not told.
    #[cfg(feature = "indexer")]
    rw("predb_max_rows", |c| {
        json!(c.d.predb.max_rows.load(Ordering::Relaxed))
    }),
    #[cfg(feature = "indexer")]
    rw("predb_seed_days", |c| {
        json!(c.d.predb.seed_days.load(Ordering::Relaxed))
    }),
    ...
    rw("scoreboard_enabled", |c| {
        json!(c.d.scoreboard.enabled.load(Ordering::Relaxed))
    }),
    rw("scoreboard_url", |c| {
        json!(c.d.scoreboard.url.lock_ok().clone())
    }),
    ...
    rw("scoreboard_source", |c| {
        json!(c.d.scoreboard.source.lock_ok().clone())
    }),
    ...
    rw("scoreboard_cats", |c| {
        json!(c.d.scoreboard.cats.lock_ok().clone())
    }),
    ...
    rw("scoreboard_calibrate", |c| {
        json!(c.d.scoreboard.calibrate.load(Ordering::Relaxed))
    }),
];
...
pub(super) const AUTOMATION: &[Setting] = &[
    ...
    // the session. See finish_action.rs for why sleep and shutdown
...
pub fn path_str(p: &Option<PathBuf>) -> String {
    p.as_ref()
...
pub fn key_charset_ok(k: &str) -> bool {
    k.chars()
...
pub fn apply_setting(
    ...
    Ok(match name {
        ...
        // §310. Clamped rather than refused, the same reading the two
        // counts above take: these arrive from number inputs and a value
        // out of range is a typo, not an instruction to stop honouring
        // the setting. The interval's floor is an hour and its ceiling a
        // year; the job count's ceiling is the clicked road's own
        // `heal::MAX_HEAL_JOBS`, which is the most a PERSON who is
        // looking at the damage report may start at once - an unattended
        // sweep has no business being allowed more than that.
        "heal_auto" => {
            let on = flag();
            d.heal_auto.enabled.store(on, Ordering::Relaxed);
            (true, json!(on))
        }
        "heal_auto_interval_h" => {
            let n = uint()?.clamp(1, 8760);
            d.heal_auto.interval_h.store(n, Ordering::Relaxed);
            (true, json!(n))
        }
        "heal_auto_max_jobs" => {
            let n = (uint()? as u32).min(super::heal::MAX_HEAL_JOBS as u32);
            d.heal_auto.max_jobs.store(n, Ordering::Relaxed);
            (true, json!(n))
        }
        // `size()` takes the same vocabulary every other byte setting on
        // this page takes, and 0 here is NOT unlimited: it means the
        // sweep starts nothing. That is the opposite of
        // `alt_max_extra_bytes` next door, deliberately - an automatic
        // road with no byte ceiling is the one shape §310 must not have,
        // so there is no value of this setting that grants one.
        "heal_auto_max_bytes" => {
            let n = size()?;
            d.heal_auto.max_bytes.store(n, Ordering::Relaxed);
            (true, json!(n))
        }
        "wall_hide_adult" => {
            let on = flag();
        ...
        "fast_final_check" => {
            ...
            // the global, so a change applies without a restart. Written
            // straight to nzbkit rather than to a daemon field - see the
            // table entry for why there is no second copy.
            let on = flag();
            nzbkit::par2::set_fast_check(on);
            (true, json!(on))
        }
        "script_confined" => {
            // Live: read per spawn, so a user who has just been refused
            // by the policy can turn it off and re-run the job without
            // restarting the daemon.
            let on = flag();
            d.script_confined.store(on, Ordering::Relaxed);
            (true, json!(on))
        }
        "script_timeout_secs" => {
