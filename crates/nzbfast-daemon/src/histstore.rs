...
impl Daemon {
    ...
    pub fn history_upsert(&self, jobs: &[Arc<Mutex<Job>>]) -> bool {
    ...
    pub fn history_publish_change(&self, job: &Arc<Mutex<Job>>, what: &str) -> HistWrite {
    ...
    #[must_use]
    pub fn delete_prewrite(&self, jobs: &[Arc<Mutex<Job>>], status: &str) -> bool {
    ...
    pub fn delete_prewrite_filed(&self, ids: &[String]) {
        let mut inflight = self.hist_inflight.lock_ok();
        for id in ids {
            inflight.remove(id);
        }
    ...
    /// not merely the file.
    #[must_use]
    pub fn history_tombstone(&self, ids: &[String]) -> bool {
        if ids.is_empty() {
            return true;
        }
    ...
    pub fn history_restore(&self, removed: Vec<(usize, Arc<Mutex<Job>>)>) {
        if removed.is_empty() {
            return;
        }
    ...
    /// newest-last.
    pub fn history_replay(&self) -> (Vec<Job>, bool) {
        let path = self.history_store_path();
...
impl Daemon {
    ...
    pub fn life_emit(&self, kind: &str, mut payload: Value) {
        ...
        crate::hooks::hooks_offer(self, &published);
    }
    ...
    pub fn life_since(&self, since: u64) -> (Vec<Value>, bool, u64) {
    ...
    /// no locks held.
    pub fn life_emit_parked(&self, job: &Arc<Mutex<Job>>) {
        let g = job.lock_ok();
        match g.state {
            ...
            _ => {
                ...
                self.life_emit(
                    ...
                    json!({
                        ...
                        "locked": g.password_required,
                        "moved_to": if g.out_dir.starts_with(crate::naming::out_dir(self)) {
                            String::new()
                        } else {
    ...
    pub fn history_enforce_retention(&self) {
...
/// queue walk.
pub fn spawn_retention_sweep(daemon: &Arc<Daemon>) {
    let d = daemon.clone();
