...
pub(super) fn apply_setting_tail(
    ...
    Ok(match name {
        ...
        "queue_finished_action" => {
            let a = crate::finish_action::FinishAction::parse(v).ok_or_else(|| {
                format!(
        ...
        "predb_seed_days" => {
            ...
            d.predb.seed_days.store(n, Ordering::Relaxed);
            (true, json!(n))
        }
        ...
        "scoreboard_calibrate" => {
            let on = v == "1" || v.eq_ignore_ascii_case("true");
            d.scoreboard.calibrate.store(on, Ordering::Relaxed);
            (true, json!(on))
        }
        ...
        "scoreboard_key" => {
            ...
            *d.scoreboard.key.lock_ok() = (!k.is_empty()).then(|| k.clone());
