...
fn pending_job_of(d: &Arc<Daemon>, name: &str, cat: &str, bytes: usize) -> Arc<Mutex<Job>> {
    let job_dir = crate::naming::out_dir(d).join(name);
    ...
    let job = Arc::new(Mutex::new(
        job_from_json(&json!({
            ...
            "name": name,
            "nzb_path": crate::naming::out_dir(d).join(format!("{name}.nzb")).to_string_lossy(),
...
pub const NO_PROGRESS: Duration = crate::testutil::NO_PROGRESS;
