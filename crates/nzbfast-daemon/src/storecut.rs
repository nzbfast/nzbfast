...
pub enum Store {
    ...
    HistoryRewrite = 4,
    /// `Daemon::queue_rewrite_locked`'s atomic replacement of the whole
    /// `queue.jsonl` - the queue's half of the same asymmetry, and the
    /// rescue [`Store::Queue`]'s refused append falls back to. Outside
    /// the budget for [`Store::HistoryRewrite`]'s reason.
    QueueRewrite = 8,
}
...
pub fn arm_store_cut(stores: &[Store]) {
...
///
/// [`Store::HistoryRewrite`] and [`Store::QueueRewrite`] are deliberately
/// OUTSIDE the budget and reachable only through [`arm_store_cut`]. The budget models a kill,
...
pub fn cut_here(store: Store) -> bool {
    if STORES.with(|s| s.get()) & store as u8 != 0 {
        return true;
    }
    if matches!(store, Store::HistoryRewrite | Store::QueueRewrite) {
        return false;
    }
...
pub fn on_park_gap(f: impl FnOnce(&Daemon) + 'static) {
...
/// in `daemon_tests/spare_tests.rs`, which is a §282 spare test
