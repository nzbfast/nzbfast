...
pub(super) fn set_predb_enabled(
    ...
    Ok({
        ...
        let was = d.predb.enabled.swap(on, Ordering::Relaxed);
        if on != was {
            ...
            *d.predb.status.lock_ok() = String::new();
            if on {
                info!(
                    target: "predb",
                    "pre feed ON - connecting to {} and listening on {}",
                    d.predb.server.lock_ok(),
                    d.predb.channels.lock_ok()
                );
            } else {
...
pub(super) fn set_predb_corr_enabled(
    ...
    Ok({
        let on = v == "1" || v.eq_ignore_ascii_case("true");
        let was = d.predb.corr_enabled.swap(on, Ordering::Relaxed);
        if on != was {
            info!(
                target: "predb",
...
pub(super) fn set_predb_corr_auto(
    ...
    Ok({
        let on = v == "1" || v.eq_ignore_ascii_case("true");
        let was = d.predb.corr_auto.swap(on, Ordering::Relaxed);
        if on != was {
            info!(
                target: "predb",
...
pub(super) fn set_predb_max_rows(
    ...
    Ok({
        ...
        d.predb.max_rows.store(n, Ordering::Relaxed);
...
pub(super) fn set_scoreboard_enabled(
    ...
    Ok({
        let on = v == "1" || v.eq_ignore_ascii_case("true");
        let was = d.scoreboard.enabled.swap(on, Ordering::Relaxed);
        if on != was {
            ...
            if on {
                ...
                let from = {
                    let src = d.scoreboard.source.lock_ok().clone();
                    if src.is_empty() {
                        d.scoreboard.url.lock_ok().clone()
                    } else {
                        src
                    }
                };
...
pub(super) fn set_scoreboard_url(
    ...
    Ok({
        ...
        *d.scoreboard.url.lock_ok() = s.clone();
        (true, json!(s))
    })
}
...
pub(super) fn set_scoreboard_source(
    ...
    Ok({
        ...
        *d.scoreboard.source.lock_ok() = s.clone();
        (true, json!(s))
    })
}
    ...
    Ok({
        ...
        if on != was {
            ...
            if on {
                ...
                // The budget derives from the named account's own
                // quotas, so resolve it for the announcement - the
                // floor when the account is not found (or none named),
                // which is also what the worker would spend then.
                let budget = d
                    .indexers
                    .lock_ok()
                    .iter()
                    .find(|c| c.name == src)
                    .map(crate::indexers::confirm_budget)
                    .unwrap_or(crate::indexers::CONFIRM_PER_DAY);
                ...
                info!(
                    target: "confirm",
                    "indexer-confirm ON - up to {budget} suggestion lookup(s)/day from {from}",
                );
            } else {
...
pub(super) fn set_scoreboard_cats(
    ...
    Ok({
        ...
        *d.scoreboard.cats.lock_ok() = cats.clone();
        info!(
            target: "scoreboard",
...
pub(super) fn set_predb_server(
    ...
    Ok({
        ...
        *d.predb.server.lock_ok() = if s.is_empty() {
            nzbkit::predb::DEFAULT_HOST.to_string()
        } else {
            s.clone()
        };
        (true, json!(d.predb.server.lock_ok().clone()))
    })
}
...
pub(super) fn set_predb_channels(
    ...
    Ok({
        ...
        *d.predb.channels.lock_ok() = joined.clone();
        (true, json!(joined))
    })
}
...
pub(super) fn set_index_enabled(
    ...
    Ok({
        ...
        if on {
            ...
            apply_interests(d);
            // ...and if that could not resolve them because no group
            // list has been fetched yet (a new install switches on
            // with none), start the fetch it is waiting on.
            fetch_catalogue_for_interests(d);
            d.scan_now.notify_one();
