...
pub struct BodyBudget {
    ...
    pub cur: std::sync::Mutex<Reserved>,
...
pub const BODY_BUDGET_WAIT: std::time::Duration = std::time::Duration::from_secs(5);
...
impl BodyBudget {
    pub fn new(cap: u64) -> BodyBudget {
        BodyBudget::with_wait(cap, BODY_BUDGET_WAIT)
    }

    pub fn with_wait(cap: u64, wait: std::time::Duration) -> BodyBudget {
        BodyBudget {
            cap,
            wait,
    ...
    /// old rule.
    pub fn grow(&self, h: &mut Hold, more: u64) {
...
pub struct BodyHold(Option<Hold>);
...
pub const API_BODY_MAX: u64 = 256 << 20;
...
/// the ceiling.
pub const API_BODY_DEFAULT: u64 = 1 << 20;
...
pub fn api_body_cap(mode: &str) -> u64 {
    match mode {
...
pub fn read_body_capped_hold(r: impl std::io::Read, cap: u64) -> (Vec<u8>, BodyHold) {
