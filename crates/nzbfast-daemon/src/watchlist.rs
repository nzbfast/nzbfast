...
#[cfg(feature = "indexer")]
pub fn watch_calendar_refresh(d: &Arc<Daemon>) {
...
pub const INSTANT_MAX_DEFAULT: u32 = 6;
...
/// restart.
pub const MIN_FREE_DEFAULT: u64 = 2_000_000_000;
...
#[cfg(feature = "indexer")]
pub const INSTANT_PENDING_SECS: i64 = 10 * 60;

/// §74: cadence of that re-check.
#[cfg(feature = "indexer")]
pub const INSTANT_RECHECK_SECS: u64 = 30;
...
#[cfg(test)]
pub(crate) static SETTLE_CUSTODY_BARRIER: Mutex<
...
fn settle_may_remove_files(d: &Arc<Daemon>, nzo_id: &str) -> bool {
    ...
    let queue_dirs: Vec<PathBuf> = d
        ...
        .collect();
    // `None` for the §96 published-Failed map, and unconditionally so:
    // this is a single-id delete, and `plan_history_delete` reads
    // `published_failed` only under the `failed` and `completed` bulk
    // words. Answering it here stat'ed every Completed row's output
    // folder - a share that has gone away blocks for the mount timeout
    // on each - inside a `d.history` hold that the park path, the
    // enqueue dupe check and the queue saver all queue behind (bug sweep
    // 1 Sep 2026, F20).
    let records: Vec<DeleteRecord> = {
        let h = d.history.lock_ok();
        crate::job::delete_records(h.as_slice(), None)
    };
...
fn settle_superseded_record(
    d: &Arc<Daemon>,
    p: nzbfast_meta::watchlist::PendingDelete,
    job: &Arc<Mutex<Job>>,
    sidecar: Option<&(String, Arc<AtomicBool>)>,
) -> Option<nzbfast_meta::watchlist::PendingDelete> {
    let (dir, nzb, name, filed, tail) = {
        ...
        let t = delete_tail(&g, || crate::naming::job_suffix(d, filed_stem(&g)));
        (
            g.out_dir.clone(),
            g.nzb_path.clone(),
...
fn settle_pending_upgrades(
    d: &Arc<Daemon>,
    state: &mut nzbfast_meta::watchlist::WatchState,
) -> bool {
    let mut dirty = false;
    ...
    for p in pending {
        ...
        match hist.map(|j| j.lock_ok().state) {
            Some(JobState::Completed) => {
                ...
                crate::postproc::cancel_tail_fetches(d, |id| id == p.old_nzo);
                #[cfg(test)]
                {
...
fn reconcile_slots(d: &Arc<Daemon>, state: &mut nzbfast_meta::watchlist::WatchState) -> bool {
    let mut dirty = false;
...
/// What both candidate collectors need from the pass around them: the
/// item being matched, the give-up breaker's snapshot and its threshold,
/// the pass clock, and the categories `classify` reads.
///
/// A bundle because there are two collectors and five values, and
/// `clippy::too_many_arguments` is fatal here.
struct CandCtx<'a> {
    item: &'a nzbfast_meta::watchlist::WatchItem,
    giveup: &'a super::giveup::GiveupState,
    giveup_threshold: u32,
    now_unix: i64,
    cats: &'a [nzbkit::categories::CustomCategory],
}

/// Fold this item's INDEX hits into the best candidate per slot.
///
/// Moved verbatim out of [`watchlist_pass`] for the 500-line function
/// ceiling. Every skip test, the order they run in and the same-rank
/// bigger-post tiebreak are the original lines.
#[cfg(feature = "indexer")]
fn fold_index_hits(
    best: &mut std::collections::HashMap<String, Cand>,
    hits: &[nzbkit::index::Release],
    state: &mut nzbfast_meta::watchlist::WatchState,
    ctx: &CandCtx<'_>,
) {
    use nzbfast_meta::watchlist as wl;
    let classify = |stem: &str| nzbkit::categories::classify(stem, ctx.cats);
    for r in hits.iter() {
        ...
        if !wl::age_ok(ctx.item, r.first_posted, ctx.now_unix) {
            wl::note_skip(&mut state.skips, ctx.item.id, "age");
            continue;
        }
        // §96.3: the give-up breaker has concluded this target is
        // not obtainable - stop pursuing it (any release of it).
        if ctx.giveup.tripped(&p, ctx.giveup_threshold) {
            wl::note_skip(&mut state.skips, ctx.item.id, "ctx.giveup");
            continue;
        }
        let Some(slot) = wl::slot_of(ctx.item, &p) else {
            continue;
        };
        ...
        let skey = wl::state_key(ctx.item.id, &slot);
        if state
            .slots
            .get(&skey)
...
/// Ask the user's indexer accounts for this item on their own slow
/// cadence and let anything they offer compete in the same `best` map.
///
/// Moved verbatim out of [`watchlist_pass`] for the 500-line function
/// ceiling. `dirty` is `&mut` for the one reason it was before: recording
/// the check against `ext_checked` is state the pass must persist.
fn fold_external_hits(
    best: &mut std::collections::HashMap<String, Cand>,
    d: &Arc<Daemon>,
    state: &mut nzbfast_meta::watchlist::WatchState,
    dirty: &mut bool,
    ...
    if d.watchlist_external_on() {
        let due = state
            .ext_checked
            .get(&ctx.item.id)
            .is_none_or(|last| ctx.now_unix - *last >= WATCH_EXT_INTERVAL_SECS);
        if due {
            state.ext_checked.insert(ctx.item.id, ctx.now_unix);
            *dirty = true;
            let (ext, gated) = watchlist_external_candidates(d, ctx.item);
            ...
            if let Some(reason) = gated {
                wl::note_skip(&mut state.skips, ctx.item.id, &reason);
            }
            for r in ext {
                let p = classify(&r.title);
                if !wl::matches(ctx.item, &r.title, &p) {
                    continue;
                }
                ...
                if !wl::age_ok(ctx.item, r.posted, ctx.now_unix) {
                    wl::note_skip(&mut state.skips, ctx.item.id, "age");
                    continue;
                }
                let Some(slot) = wl::slot_of(ctx.item, &p) else {
...
pub fn watchlist_pass(d: &Arc<Daemon>) {
    use nzbfast_meta::watchlist as wl;
    ...
    for item in items.iter().filter(|i| i.enabled) {
        ...
        #[cfg(feature = "indexer")]
        fold_index_hits(&mut best, &hits, &mut state, &ctx);
        fold_external_hits(&mut best, d, &mut state, &mut dirty, &ctx);
        ...
        for (slot, c) in ordered {
            ...
            match wl::decide(cur_rank, c.rank, min, target, item.upgrade) {
                ...
                wl::Decision::Grab => {
                    ...
                    let origin = crate::origin::watchlist_origin(&slot, &c.quality, &item.title);
...
pub(super) fn note_instant(
    ...
    state.instant.insert(
        item_id.to_string(),
        nzbfast_meta::watchlist::InstantGrab {
            stem: stem.to_string(),
            at: now,
            lag,
        },
...
pub(super) fn aired_episodes(
    d: &Arc<Daemon>,
    item: &nzbfast_meta::watchlist::WatchItem,
    season: u32,
) -> Vec<u32> {
    if item.kind != "tv" {
        return Vec::new();
...
pub(super) fn covered_slots(
    ...
    let Some(primary) = nzbfast_meta::watchlist::slot_of(item, p) else {
        return Vec::new();
    };
    std::iter::once(primary)
        .chain(nzbfast_meta::watchlist::extra_slots(item, p))
        .map(|s| nzbfast_meta::watchlist::state_key(item.id, &s))
        .collect()
}
...
pub fn upgrade_supersedes_all(
    item: &nzbfast_meta::watchlist::WatchItem,
    state: &nzbfast_meta::watchlist::WatchState,
    prev: &nzbfast_meta::watchlist::Slot,
...
fn remember_refused_grab(
    ...
    let s = state
        .slots
        .entry(key.to_string())
        .or_insert_with(|| nzbfast_meta::watchlist::Slot {
            rank: 0,
            stem: String::new(),
            quality: String::new(),
            nzo_id: String::new(),
...
mod settle_tests {
    ...
    fn pending(old: &str, new: &str) -> nzbfast_meta::watchlist::PendingDelete {
        nzbfast_meta::watchlist::PendingDelete {
    ...
    fn a_settled_upgrade_reaches_the_dashboard_as_a_lifecycle_event() {
        ...
        state.pending.push(nzbfast_meta::watchlist::PendingDelete {
            slot: "7:s01e01".to_string(),
