...
/// file we cannot read.
pub fn settings_beyond_setup_answers(path: &std::path::Path) -> bool {
...
pub fn settings_file(config: &std::path::Path) -> PathBuf {
...
pub fn legacy_rename_punctuation(
...
pub fn new_spool_state(new: &std::path::Path) -> NewSpool {
...
pub fn spool_dir(config: &std::path::Path, out_root: &std::path::Path) -> PathBuf {
...
pub fn load_settings(path: &std::path::Path) -> serde_json::Map<String, Value> {
...
pub fn save_settings(path: &std::path::Path, values: &[(&str, Value)]) -> bool {
...
pub fn merge_cat_list(on_disk: &str, mine: &str) -> String {
...
pub fn random_apikey() -> Option<String> {
    let mut buf = [0u8; 24];
...
pub fn write_runtime_file(settings_path: &std::path::Path, port: u16, tls: bool, token: &str) {
...
pub fn launcher_proof(token: &str, nonce: Option<&str>) -> Option<String> {
...
pub fn keyless_help(keyfile: &std::path::Path, what: &str) -> String {
    format!(
        "{KEYLESS_MARKER} {what}.\n\n\
...
#[cfg(feature = "indexer")]
pub fn install_live_ingest_policy(
...
#[cfg(feature = "indexer")]
pub fn is_kind_slug(k: &str) -> bool {
    !k.is_empty()
        && k.chars()
...
pub fn arm_handoff_in(slot: &Mutex<Option<Handoff>>, key: &str) -> Option<String> {
...
pub fn disarm_handoff_in(slot: &Mutex<Option<Handoff>>) {
...
#[cfg(any(test, feature = "test-support"))]
pub fn redeem_handoff_in(slot: &Mutex<Option<Handoff>>, token: &str) -> Redeem {
...
pub fn dashboard_argv_in(
    ...
    let q = key
        ...
        .map(|t| format!("?handoff={}", query_escape(&t)))
        .unwrap_or_default();
...
/// new one is.
pub fn route_handoff(mut req: tiny_http::Request, d: &Arc<Daemon>) {
...
/// is tested.
pub fn open_dashboard(port: u16, tls: bool, key: Option<String>) {
    std::thread::spawn(move || {
        ...
        // sandbox-seam-gate: `prog` is a &'static str picked by
        // #[cfg(target_os)] inside `dashboard_argv_in` (open, `cmd /C start`,
        // xdg-open) and the URL is built from our own port plus a token we
        // minted. The child is the user's BROWSER, and a tool policy would
        // deny it $HOME and the network, which is most of what a browser is.
