...
/// half-there.
pub(crate) const INSURANCE_MAX_ATTEMPTS: u32 = 3;
...
pub fn insure_refusal(j: &Job) -> Option<&'static str> {
    if !j.paused {
...
pub fn slot_payload(j: &Job) -> Value {
    if !j.insurance {
        return Value::Null;
    }
    json!({
...
pub fn insure_arm(d: &Arc<Daemon>, params: &std::collections::HashMap<String, String>) -> Value {
...
impl Daemon {
    ...
    pub fn pick_insurance_job(&self) -> Option<Arc<Mutex<Job>>> {
...
/// runner state here.
pub fn insurance_yields_to_arrivals(d: &Arc<Daemon>) {
...
mod tests {
    ...
    pub fn tmp(tag: &str) -> crate::testscratch::ScratchDir {
        let d =
    ...
    fn row(id: &str, insurance: bool, total: u64) -> Arc<Mutex<Job>> {
        let j = crate::job_from_json(&serde_json::json!({
            "nzo_id": id,
            "name": id,
            "out_dir": "/tmp/o",
            "nzb_path": "/tmp/n.nzb",
