...
pub(super) fn set_index_fold_secs(
    d: &Arc<Daemon>,
    name: &str,
    v: &str,
) -> std::result::Result<(bool, Value), String> {
    ...
    Ok({
        // Seconds one fold slice may hold the index write lock. The
        // floor is 1 because 0 is not "off" here (the fold's cursor
        // must keep following the top or the dark band never merges);
        // the ceiling is the bounded HTTP waiter's own limit
        // (`Daemon::HTTP_INDEX_WAIT`, 5 s) times a few - past it a
        // dashboard edit is refused as IndexBusy behind every slice.
        let n = uint()?;
        if !(Daemon::FOLD_SECS_MIN..=Daemon::FOLD_SECS_MAX).contains(&n) {
            return Err(format!(
                "index_fold_secs: {}-{} seconds per fold slice",
                Daemon::FOLD_SECS_MIN,
                Daemon::FOLD_SECS_MAX
            ));
        }
        d.index_fold_secs.store(n, Ordering::Relaxed);
        (true, json!(n))
    })
}
...
pub(super) fn set_index_interests(
    ...
    Ok({
        ...
        *d.index_interests.lock_ok() = norm.clone();
        // Resolve now if the catalogue is already here; otherwise the
        // fetch applies it when it lands - and that fetch has to be
        // STARTED, because the scheduled one stood down while the
        // indexer was off (fetch_catalogue_for_interests says why).
        apply_interests(d);
...
pub(super) fn set_index_tip_secs(
    ...
    Ok({
        // 0 = off. Otherwise floored - see index_tip_floor_secs.
        let n = uint()?;
        let n = if n == 0 {
            0
        } else {
            n.max(super::index_tip_floor_secs())
        };
...
pub(super) fn set_watchlist(
    ...
    Ok({
        ...
        let text = v.trim();
        let list: Vec<nzbfast_meta::watchlist::WatchItem> = if text.is_empty() {
            Vec::new()
        } else {
...
pub(super) fn set_password_file(
    ...
    Ok({
        ...
        *d.hub.unpack_password_file.lock_ok() = Some(path.clone());
        crate::pwfile::set_operator_password_file(Some(path.clone()));
...
pub(super) fn set_prefer_quality(
    ...
    Ok({
        ...
        let p = nzbfast_meta::watchlist::QualityPrefs::from_json(v)
