...
pub fn add(d: &Arc<Daemon>, seg: &str, name: &str) -> Result<Enqueued> {
    d.enqueue(
...
#[test]
fn a_successful_enqueue_is_reported_durable_and_lands_in_the_queue_store() {
...
fn a_category_sidecar_lives_and_dies_with_its_spool_copy() {
    with_daemon("a12-sidecar-life", |d| {
        ...
        let side = crate::job::spool_cat_path(&nzb);
