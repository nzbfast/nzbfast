...
fn a_hold_is_not_published_against_an_original_deleted_during_admission() {
    with_daemon("dupe-admit-window", |d| {
        ...
        *crate::daemon::daemon_enqueue::DUPE_ADMIT_BARRIER.lock_ok() = Some((
        ...
        *crate::daemon::daemon_enqueue::DUPE_ADMIT_BARRIER.lock_ok() = None;
...
/// and the remedy that comment named for the day a real queue shape
/// made it matter (a PostIds cache keyed by nzo_id, evicted to queue
/// membership; never a wider prefilter) IS BUILT, 3 Sep 2026. That day
/// arrived as audit round 21's quadratic add path, where these per-add
/// spool parses were 115.7 s of a 1,500-job build and 193x the memoised
/// cost. `Daemon::post_ids` carries the design; the prefilter is still
/// not widened.
///
/// So both legs below now read the memo, and the figures above are the
/// PRE-MEMO ones - kept because they are still what the arm costs the
/// first time it meets a row. A queue restored from disk pays them once,
/// on its first add, and nothing after it does.
#[test]
fn the_same_post_arm_stays_cheap_against_a_wide_queue() {
    // 210 enqueues x 2 state saves x 2 fsyncs was ~29 s of this test's
    // 30 s; the subject is the parse cost, so the fsyncs are skipped on
    // this thread (see persist::SKIP_FSYNC).
...
/// The shape a RESTART leaves: rows in the queue, nothing in the §292
/// memo. The arm has to fall back to the spool for every row it has
/// never seen, because the alternative - treating an unmemoised row as
/// carrying no ids - is a MISSED duplicate, which is the direction this
/// whole ladder exists to avoid.
///
/// Written with the memo (3 Sep 2026), because a cache on a duplicate
/// check has exactly one interesting failure and this is it: the fast
/// path is exercised by every other test in this file, and the cold
/// path only ever runs on the first add after a start.
#[test]
fn a_cold_same_post_memo_still_finds_the_duplicate() {
    with_daemon("dupe-memocold", |d| {
        let add = |xml: &str, name: &str| {
            d.enqueue(xml.as_bytes(), name, "", -100, None, None, "test", false)
                .map(|e| e.nzo_id)
                .unwrap()
        };
        let post = samepost_nzb(&["cm1@x", "cm2@x", "cm3@x"]);
        let first = add(&post, "998877665544.nzb");
        assert!(
            d.post_ids.lock_ok().contains_key(&first),
            "the publish primes the memo from the NZB it already parsed"
        );
        d.post_ids.lock_ok().clear();
        let second = add(&post, "887766554433.nzb");
        assert!(
            d.held_as_duplicate(&second),
            "a cold memo must read the spool back, not miss the same post"
        );
        assert_eq!(d.queue_job(&second).unwrap().lock_ok().held_for, first);
        assert!(
            d.post_ids.lock_ok().contains_key(&first),
            "and what it read back is memoised, so the next add does not"
        );
    });
}

/// The §292 memo is bounded by QUEUE MEMBERSHIP, which is the only
/// thing it ever describes. Rows that have left take their entries with
/// them - eventually: the retain is a walk of both collections, so it
/// runs only once the map has outgrown the queue by more than its
/// slack, and a survivor in between costs nothing but a map slot (the
/// arm reads membership off the live queue, never off the memo).
#[test]
fn the_same_post_memo_is_evicted_to_queue_membership() {
    with_daemon("dupe-memoevict", |d| {
        let ids = || Arc::new(crate::spare::PostIds::default());
        d.queue
            .lock_ok()
            .push_back(jv("live", "live", serde_json::json!({})));
        {
            let mut m = d.post_ids.lock_ok();
            m.insert("live".into(), ids());
            for i in 0..40 {
                m.insert(format!("gone{i}"), ids());
            }
        }
        d.evict_post_ids();
        assert_eq!(
            d.post_ids.lock_ok().len(),
            41,
            "inside the slack, the retain is not worth its own walk"
        );
        {
            let mut m = d.post_ids.lock_ok();
            for i in 40..200 {
        ...
        d.evict_post_ids();
        let m = d.post_ids.lock_ok();
        assert_eq!(m.len(), 1, "past the slack, only queued rows survive");
