...
impl Daemon {
    ...
    pub(crate) fn alt_gate(&self) -> std::sync::MutexGuard<'_, ()> {
    ...
    pub(crate) fn alt_ctx(&self, id: &str, name: &str, keys: Vec<String>) -> AltCtx {
        let found = self
            .queue
            .lock_ok()
            .iter()
    ...
    pub(crate) fn alt_admit(
        &self,
        ctx: &AltCtx,
        want: u64,
        trigger: Trigger,
    ) -> std::result::Result<(), NoHunt> {
        let policy = crate::hunt::hunt_policy(self);
        ...
        if policy.max_extra_bytes == 0 {
            return if trigger == Trigger::Auto && crate::hunt::hunt_metered(self) {
