...
pub const NO_PROGRESS: std::time::Duration = std::time::Duration::from_secs(60);
...
pub fn test_daemon(dir: &Path) -> Arc<Daemon> {
    ...
    Arc::new(Daemon {
        ...
        // Mirrors `serve()`, which flipped this ON in Sep 2026 (§310).
        // A unit-test daemon that settles a job therefore writes a
        // manifest, exactly as a real one does; a test that wants the
        // other arm stores `false` itself, per this file's own rule.
        ...
        spill: crate::spill::Governor::default(),
        ...
        // Grouped into `RenameSettings` - see the field on `Daemon`.
        rename: crate::daemon::RenameSettings::new(false),
        ...
        predb: super::settings_restore::seeded_predb(&settings_path),
        scoreboard: super::settings_restore::seeded_scoreboard(&settings_path),
...
/// is a real defect.
pub fn flat_rate_config(d: &Daemon) {
    std::fs::write(
        &d.cfg_path,
        r#"{"servers":[{"host":"flat.example","enabled":true}]}"#,
    )
...
// The two Job fixtures, moved DOWN from the bin's
// `tests_jobs.rs` with the daemon layer. `Job`,
// `job_from_json` and `LossCauses` all live below this crate's
// boundary, and five daemon-layer test files build a Job this way;
// `tests_jobs.rs` imports them back under their old bare names.
/// A LossCauses with nothing known, for messages under test.
pub fn no_causes() -> crate::LossCauses<'static> {
    crate::LossCauses {
        missing_430: 0,
        takedown_430: 0,
        unasked_430: 0,
...
pub fn job(v: serde_json::Value) -> Job {
    crate::job::job_from_json(&v).expect("job_from_json")
}

// The five shared Daemon fixtures, moved here from
// `daemon_tests.rs` by lane 2 of the serve split: that module is
// `#[cfg(test)]` and therefore invisible across a crate boundary,
// and the fifteen daemon-layer tests lane 2 had to lift into the bin
// build their Daemons with these. `daemon_tests.rs` imports them
// back under their old bare names.
...
pub fn jv(id: &str, name: &str, extra: serde_json::Value) -> Arc<Mutex<Job>> {
...
pub fn restart(d: &Arc<Daemon>) -> Arc<Daemon> {
...
pub fn one_file_nzb(seg: &str) -> String {
    format!(
...
/// The queue store's records, as they are ON DISK, in queue order.
///
/// Reads the bytes rather than asking the daemon, and through
/// `queuestore::replay_bytes` rather than a second copy of the tolerance
/// rules - a test whose subject is "what survived the kill" must not be
/// answered from the memory the kill was supposed to leave behind.
pub fn stored_queue(d: &Arc<Daemon>) -> Vec<Value> {
    let raw = std::fs::read(d.queue_store_path()).unwrap_or_default();
    crate::queuestore::replay_bytes(&raw)
        .rows
        .into_iter()
        .map(|(_, v, _)| v)
        .collect()
...
pub fn stored_next_id(d: &Arc<Daemon>) -> u64 {
    let raw = std::fs::read(d.queue_store_path()).unwrap_or_default();
    crate::queuestore::replay_bytes(&raw)
        .next_id
        .expect("the queue store carries next_id")
}

/// A COMPLETED job record, the sibling of [`jv`]: same shape with
/// `state: Completed` and `fetched` set, which is what the tail and
/// mover fixtures want. Moved here from `stream.rs`'s own test
/// module by lane 2 of the serve split, for the reason the five
/// fixtures above it moved.
pub fn rec(id: &str, extra: serde_json::Value) -> Arc<Mutex<Job>> {
    let mut v = serde_json::json!({
...
/// A scratch directory under the OS temp dir, named for the caller and
/// this process, wiped first.
///
/// The two `tmp` helpers `sidecar.rs` and `histstore.rs` each declared
/// are this function with their own prefix. They were `#[cfg(test)]`
/// items of a `#[cfg(test)] mod`, so the two tests lane 2 lifted into
/// the bin could not reach either; one helper with the prefix as an
/// argument is what both now call.
pub fn scratch_dir(prefix: &str, tag: &str) -> std::path::PathBuf {
    let d = std::env::temp_dir().join(format!("nzbfast-{prefix}-{tag}-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&d);
    std::fs::create_dir_all(&d).expect("create scratch dir");
    d
}

/// The smallest NZB that parses and enqueues, so "the retry can still
/// use it" is a claim about real bytes rather than a stat(). Moved out
