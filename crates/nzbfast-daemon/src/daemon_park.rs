...
//! including `pub(super)` becoming `pub(crate)`, which is what
...
pub static IDLE_CAS_BARRIER: Mutex<
...
#[cfg(any(test, feature = "test-support"))]
pub(crate) static PARK_GEN_BARRIER: Mutex<
    Option<(String, Arc<std::sync::Barrier>, Arc<std::sync::Barrier>)>,
> = Mutex::new(None);
...
pub(crate) struct SidecarTailGuard {
    d: Arc<Daemon>,
...
impl Daemon {
    ...
    /// [`fail_kind`]).
    pub(crate) fn will_auto_retry(&self, job: &Arc<Mutex<Job>>) -> bool {
    ...
    pub(crate) fn report_failure(&self, job: &Arc<Mutex<Job>>) {
    ...
    pub fn sidecar_owner(&self) -> Option<(String, Arc<AtomicBool>)> {
        self.sidecar
            .lock_ok()
            .as_ref()
    ...
    pub(crate) fn remove_after_sidecar_drain(
        self: &Arc<Self>,
        target: Arc<AtomicBool>,
        name: String,
    ...
    pub(crate) fn remove_files_in_custody(
        self: &Arc<Self>,
    ...
    /// dropped at shutdown.
    pub(crate) fn sidecar_tail_begin(
        self: &Arc<Self>,
        target: Arc<AtomicBool>,
    ) -> SidecarTailGuard {
    ...
    fn signal_sidecar(sc: &crate::sidecar::Sidecar) {
    ...
    pub fn record_generation(g: &Job) -> (u32, u64) {
    ...
    pub(crate) fn same_generation(g: &Job, gen0: Option<(u32, u64)>) -> bool {
    ...
    fn spend_deferred_delete(&self, job: &Arc<Mutex<Job>>) {
        ...
        let (del, gone_nzb) = {
            ...
            let del = g.del_on_drop.then(|| {
                (
                    delete_tail(&g, || crate::naming::job_suffix(self, filed_stem(&g))),
                    g.out_dir.clone(),
    ...
    pub fn park_gen(&self, job: Arc<Mutex<Job>>, gen0: Option<(u32, u64)>) {
        ...
        if failed {
            ...
            crate::earlyfile::early_unlink(&taken);
        }
    ...
    /// emit: §282 section C landed the same day and `hunt.rs`
    ...
    fn promote_held_alternative(
        ...
        let failed_why = crate::altcand::why_from_fail(&fail_message);
        ...
        let ctx = self.alt_ctx(
            id,
            &failed_name,
            crate::giveup::target_keys(&crate::wall::parse_release(&failed_name)),
        );
...
impl CustodyBatch {
    ...
    pub fn plan(
        ...
        let tail = delete_tail(g, || crate::naming::job_suffix(d, &stem));
...
mod park_custody_tests {
    ...
    fn early_job(dir: &std::path::Path, state: JobState) -> Arc<Mutex<Job>> {
        ...
        ));
        {
            let mut g = job.lock_ok();
            g.state = state;
            g.early_published = vec![crate::earlyfile::EarlyFile {
                name: "ep1.mkv".into(),
                len: 1 << 20,
                mtime_ns: 0,
                nzf_id: String::new(),
    ...
    pub const NZB: &[u8] = br#"<?xml version="1.0"?>
