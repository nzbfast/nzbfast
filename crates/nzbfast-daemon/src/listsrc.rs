...
use crate::plex;
use nzbfast_meta::listsrc::{ListSource, SourceHealth, SyncOutcome, SyncedItem, merge};
use nzbfast_meta::watchlist::WatchItem;
...
impl Daemon {
    ...
    /// array.
    pub fn watch_items(&self) -> Vec<nzbfast_meta::watchlist::WatchItem> {
        nzbfast_meta::listsrc::union_watchlist(
            &self.watchlist.lock_ok(),
            &self.lists.items.lock_ok(),
        )
    }
}
...
pub(super) fn set_list_sources(
    ...
    for s in &list {
        ...
        if s.series_scope != nzbfast_meta::listsrc::SCOPE_NEW
            && s.series_scope != nzbfast_meta::listsrc::SCOPE_ALL
        {
...
fn fetch_account(
    token: &str,
    client_id: &str,
) -> std::result::Result<Vec<nzbfast_meta::listsrc::ListEntry>, String> {
...
pub fn seed_lists(settings_path: &std::path::Path, spool: &std::path::Path) -> ListState {
...
/// than an accident.
pub fn spawn_list_sync(daemon: &Arc<Daemon>) {
    let d = daemon.clone();
...
pub fn plex_link_start(d: &Arc<Daemon>, src_id: u64) -> std::result::Result<Value, String> {
    {
...
pub fn plex_link_poll(d: &Arc<Daemon>, pin_id: &str) -> std::result::Result<Value, String> {
...
pub fn plex_forget(d: &Arc<Daemon>, src_id: u64) -> std::result::Result<Value, String> {
    let (sources, prev) = {
        let mut sources = d.lists.sources.lock_ok();
...
mod tests {
    ...
    use super::*;
    use nzbfast_meta::listsrc::ListEntry;
    ...
    fn the_list_the_watcher_walks_is_the_union() {
        ...
        d.watchlist.lock_ok().push(WatchItem {
            ...
            kind: "movie".into(),
            title: "Arrival".into(),
            ..nzbfast_meta::listsrc::to_watch_item(&src, &entry("movie", "Arrival"))
        });
    ...
    fn a_source_we_cannot_read_is_refused() {
        ...
        assert_eq!(
            ...
            nzbfast_meta::listsrc::MIN_INTERVAL_SECS
        );
    }
