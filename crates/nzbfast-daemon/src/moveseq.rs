...
/// meaning.
pub(crate) fn move_winner(queue_seq: u64, hist_seq: u64) -> MoveWinner {
    #[cfg(test)]
...
pub(crate) fn stamp_move(job: &Arc<Mutex<Job>>) {
...
impl Daemon {
    ...
    pub(crate) fn commit_to_queue(&self, nzo_id: &str, seq: u64, why: Requeue) -> bool {
    ...
    pub(crate) fn activate_parked(&self, job: &Arc<Mutex<Job>>) {
...
mod harness {
    ...
    pub fn job(d: &Arc<Daemon>, id: &str, state: JobState) -> Arc<Mutex<Job>> {
        let out = crate::naming::out_dir(d).join(format!("Release.{id}"));
        let v = json!({
            "nzo_id": id,
    ...
    fn on_disk(dir: &Path, id: &str) -> (Option<(JobState, u64)>, Option<(JobState, u64)>) {
        ...
        let queue = crate::testutil::stored_queue(&d)
            .iter()
    ...
    fn snapshot(d: &Arc<Daemon>) -> (Vec<u8>, Vec<u8>) {
        (
            std::fs::read(d.queue_store_path()).unwrap_or_default(),
            std::fs::read(d.history_store_path()).unwrap_or_default(),
        )
    }

    fn rewind(d: &Arc<Daemon>, snap: &(Vec<u8>, Vec<u8>)) {
        std::fs::write(d.queue_store_path(), &snap.0).unwrap();
        std::fs::write(d.history_store_path(), &snap.1).unwrap();
        // §7a: the queue store's publish cache says what THIS daemon
        // last wrote, and rewinding the bytes under it would leave a
        // later `save_queue` appending only what changed since a file
        // that is no longer there. Re-priming from the rewound bytes is
