...
impl Daemon {
    ...
    pub fn run_due_auto_retries(self: &Arc<Self>) {
    ...
    pub fn settle_move_attempt(&self, j: &mut Job) {
    ...
    /// a re-download.
    pub fn redrive_move(self: &Arc<Self>, nzo_id: &str) -> bool {
    ...
    pub fn retry_move_now(self: &Arc<Self>, nzo_id: &str) -> bool {
    ...
    pub fn retry(&self, nzo_id: &str) -> bool {
        ...
        let stale = !cur.starts_with(crate::naming::out_dir(self))
            && !self
        ...
        if filed || taken || stale {
            let (dir, replaces) = match &wt_root {
                ...
                Some(root) => {
                    ...
                    crate::job::choose_out_dir(&root.join(&stem), &stem, &|p| self.dir_claim(p))
                }
                None => refile_out_dir(&crate::naming::out_dir(self), &category, &name, &|p| {
                    self.dir_claim(p)
                }),
            };
        ...
        let had_del_on_drop;
        {
            ...
            j.tombstone = false;
            // One field further again, for the NZBGet pair: a row an
            // *arr hid with `HistoryDelete` is re-queued by
            // `HistoryRedownload` off the very same facade, and a
            // re-queued job the user asked to RUN must not park back
            // into a history the display still refuses to show. See
            // `Job::hidden`.
            j.hidden = false;
