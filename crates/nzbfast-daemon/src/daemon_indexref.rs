...
//! as they were inline. `pub(super)` becomes `pub(crate)`
...
impl Daemon {
    ...
    /// reference, as before.
    #[cfg(feature = "indexer")]
    pub fn scoreboard_reference(&self) -> Result<(String, String), String> {
        let source = self.scoreboard.source.lock_ok().trim().to_string();
        if !source.is_empty() {
        ...
        let url = self.scoreboard.url.lock_ok().trim().to_string();
        if url.is_empty() {
            return Err(
        ...
        let key = self.scoreboard.key.lock_ok().clone().unwrap_or_default();
        Ok((url, key))
    }
    ...
    #[cfg(feature = "indexer")]
    pub fn corr_confirm_reference(&self) -> Result<crate::newznab::IndexerConfig, String> {
    ...
    /// tick is refused.
    #[cfg(feature = "indexer")]
    pub fn corr_confirm_source_state(&self) -> &'static str {
        let source = self.corr_confirm_source.lock_ok().trim().to_string();
        if source.is_empty() {
            return "none";
        }
    ...
    #[cfg(feature = "indexer")]
    pub fn scoreboard_categories(&self) -> Vec<(u32, &'static str)> {
        let picked = self.scoreboard.cats.lock_ok().clone();
    ...
    /// indexer quota (up to `confirm_budget` lookups a day) on a lane
    ...
    /// off/on cycle.
    #[cfg(feature = "indexer")]
    pub fn corr_confirm_on(&self) -> bool {
        self.predb.corr_enabled.load(Ordering::Relaxed)
