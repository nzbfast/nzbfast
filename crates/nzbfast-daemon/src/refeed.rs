...
pub(super) fn refeed_completed(d: &Arc<Daemon>, job: &Arc<Mutex<Job>>, gen0: Option<(u32, u64)>) {
    ...
    for p in found.into_iter().take(REFEED_MAX_PER_JOB) {
        refeed_one(d, &p, &parent_id, &parent_name, &category, depth);
    }
}
...
fn refeed_one(
    ...
    if d.queue.lock_ok().iter().any(|j| j.lock_ok().nzb_sha == sha) {
    ...
    match d.enqueue(&bytes, &name, category, -2, None, None, "refeed", false) {
        Ok(e) => {
...
<segments><segment bytes="100" number="1">{subject}@example</segment></segments>
    ...
    fn parent(d: &Arc<Daemon>, id: &str, depth: u8) -> Arc<Mutex<Job>> {
        let out = crate::naming::out_dir(d).join(format!("Container.{id}"));
    ...
    async fn the_blocking_pool_call_site_the_tail_uses_does_not_panic() {
        ...
        tokio::task::spawn_blocking(move || refeed_completed(&d5, &j5, None))
            .await
