...
pub fn read_import_config(path: &std::path::Path) -> std::io::Result<String> {
...
pub fn os_open(path: &std::path::Path) -> bool {
    // sandbox-seam-gate: `open` is a literal program and only the path
    // argument varies. The child is whatever handler the USER registered - a
    // file manager, a media player - so a tool policy would deny it $HOME and
    // the network and break the action outright.
    #[cfg(target_os = "macos")]
    let mut cmd = std::process::Command::new("open");
    // sandbox-seam-gate: `xdg-open`, the same shape as the macOS arm above -
    // a literal program, only the path argument varying, and a child that is
    // the user's own desktop handler.
    ...
    // sandbox-seam-gate: `explorer`, the same shape as the two arms above;
    // the paragraph above is why it is not `cmd /C start`.
    #[cfg(windows)]
...
pub fn path_writable(p: &std::path::Path) -> bool {
    #[cfg(unix)]
    {
...
pub fn parse_cat_dests(v: &str) -> Result<Vec<(String, PathBuf)>, String> {
...
pub fn fmt_cat_dests(list: &[(String, PathBuf)]) -> String {
    list.iter()
...
pub fn fs_roots(cur_download: &std::path::Path) -> Value {
