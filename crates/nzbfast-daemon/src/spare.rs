...
pub(crate) const SPARE_HOLD_COUNT: usize = 2;
...
impl PostIds {
    ...
    /// Paired with [`PostIds::len`] because clippy asks for it on a
    /// `pub len`, and the pairing became reachable at lane 2's cut: the
    /// method was `pub(crate)` while serve was one crate with the bin,
    /// and `len_without_is_empty` only judges a PUBLIC surface.
...
pub fn post_ids(nzb: &nzbkit::nzb::Nzb) -> PostIds {
    PostIds {
        ids: nzb
            .files
            .iter()
...
pub(crate) fn nzb_at(path: &std::path::Path) -> Option<nzbkit::nzb::Nzb> {
...
pub(crate) fn admits(primary: &PostIds, cand: &PostIds, unknown: bool) -> bool {
...
pub(crate) fn post_covers(row: &PostIds, add: &PostIds) -> bool {
    if add.is_empty() || row.is_empty() {
        return false;
    }
...
pub(crate) fn post_origin(nzb: &nzbkit::nzb::Nzb) -> PostOrigin {
...
pub(crate) fn looks_independent(a: &PostOrigin, b: &PostOrigin) -> bool {
...
pub(crate) fn best_alternative(
    ...
    for (i, (name, path, band)) in cands.iter().enumerate() {
        let rank = nzbfast_meta::watchlist::quality_rank(&crate::wall::parse_release(name));
...
impl Daemon {
    ...
    fn spare_rank(&self, p: &crate::wall::Parsed) -> i64 {
        nzbfast_meta::watchlist::preference_score(p, &self.quality_prefs.lock_ok())
    }
...
impl Daemon {
    ...
    pub(crate) fn drop_spares_for(&self, owner: &str) {
        let dropped = {
    ...
    pub(crate) fn drop_spares_if_still_stranded(&self, owner: &str) {
        let dropped = {
            let mut q = self.queue.lock_ok();
    ...
    pub(crate) fn repoint_spares(&self, from: &str, to: &str) {
    ...
    pub(crate) fn drop_stranded_spares(&self) {
...
pub(crate) fn repoint_spares_in<'a>(
