//! `Job` <-> JSON wire form: the pair the queue store round-trips through
...
pub(crate) const JOB_SCHEMA_VERSION: u64 = 1;
...
pub fn job_json(j: &Job) -> Value {
    json!({
        ...
        // §310 stage 2. ADDITIVE, so it does NOT bump
        // `JOB_SCHEMA_VERSION`: a record written before this existed
        // reads back with an empty path, which is exactly what it can
        // truthfully say - that job was not healing anything.
        "heal_dir": j.heal_dir.to_string_lossy(),
        ...
        "fetched": j.fetched,
        // NZBGet's `HistoryDelete` hid this row. ADDITIVE, so it does
        // NOT bump `JOB_SCHEMA_VERSION`: a record written before this
        // existed reads back visible, which is what it truthfully was.
        // Persisted at all because the whole point of hiding rather
        // than erasing is that the row - and the dupe key on it -
        // outlives the *arr's import, and so has to outlive a restart.
        "hidden": j.hidden,
...
pub fn job_from_json(v: &Value) -> Option<Job> {
    ...
    match v.get("schema_version") {
        ...
        Some(other) => {
            ...
            // did not load. The QUEUE store answers the same way and
            // for the same reason since §7a made it append-only: a
            // record this build could not read is not in the live
            // queue, so the next full `save_queue` appends its
            // TOMBSTONE - the shape a rewrite-from-memory had anyway.
            // What changed is that there is no `queue.json.bak` behind
            // it any more; `queue.json.premigrate`, if the move to the
            // store happened on this machine, is the last copy.
            warn!(
                target: "queue",
                "not loading a persisted job record stamped schema_version \
                 ...
                 queue save drops it from queue.jsonl, and a history compaction \
    ...
    Some(Job {
        ...
        heal_dir: PathBuf::from(s("heal_dir").unwrap_or_default()),
        ...
        tombstone: false,
        // ...but `hidden` DOES survive: see `Job::hidden`. Absent in
        // every store written before 4 Sep 2026, and `false` is the
        // right reading of those rows.
        ...
        early_published: v
            ...
            .map(|a| {
                a.iter()
                    .filter_map(|e| {
                        Some(crate::earlyfile::EarlyFile {
