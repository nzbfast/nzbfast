...
#[cfg(feature = "indexer")]
pub const BURST_TICK_SECS: u64 = 60;
...
#[cfg(feature = "indexer")]
pub fn should_burst_profiles(profiled: usize, burst_samples: usize, since_start_secs: u64) -> bool {
...
mod interests_need_catalogue_tests {
    use super::interests_need_catalogue as need;

    /// The shape measured 2 Sep 2026: indexer on, interests chosen, no
    /// catalogue yet - the one case a fetch must start.
    #[test]
    fn fires_only_for_unresolved_interests_on_a_live_indexer() {
        assert!(need(false, "books,music,anime", true, false, false));
        // Already have a list to resolve against.
        assert!(!need(true, "books", true, false, false));
        // Nothing chosen, or only keys this build does not know.
        assert!(!need(false, "", true, false, false));
        assert!(!need(false, "zzz", true, false, false));
        // The indexer is off: the loop stands down and so does this.
        assert!(!need(false, "books", false, false, false));
        // Offline means no provider traffic, same as the loop.
        assert!(!need(false, "books", true, true, false));
        // A failed fetch waits for the explicit refresh, not a save.
        assert!(!need(false, "books", true, false, true));
    }
}
...
/// concurrency tier.
#[cfg(feature = "indexer")]
pub async fn sample_top_groups(d: &Arc<Daemon>, config: &Path, burst: bool) -> usize {
...
///    answer.
#[cfg(feature = "indexer")]
pub fn apply_interests(d: &Arc<Daemon>) {
...
/// Whether a mid-run interest change needs a catalogue fetch started
/// for it. Pure so the decision can be pinned without a daemon.
///
/// The scheduled fetch loop (`tasks.rs`) ticks once at startup and then
/// hourly, and it stands down while the indexer is off - which is how
/// every NEW install starts. Measured 2 Sep 2026 on a scratch daemon: the
/// indexer was switched on and `books,music,anime` chosen at 03:53Z, and
/// with no catalogue to resolve against `index_groups` stayed EMPTY until
/// a hand-triggered `groups_refresh` at 04:00Z; the loop's next tick was
/// 04:53Z. A user who opens Settings, flips the switch and ticks three
/// interests would have waited up to an hour for an empty wall with
/// nothing saying why. `apply_interests` already leaves the marker for
/// "the fetch will finish this"; this is what makes sure a fetch is
/// coming.
#[cfg(feature = "indexer")]
pub(super) fn interests_need_catalogue(
    has_catalogue: bool,
    interests: &str,
    indexer_on: bool,
    offline: bool,
    ...
    // A dead server must not turn every settings save into a reconnect;
    // the explicit refresh button clears the error and retries.
    !has_catalogue
        && !crate::interests::parse(interests).is_empty()
        && indexer_on
        && !offline
        && !fetch_failed
}

/// Start the catalogue fetch a mid-run interest change is waiting on
/// (see [`interests_need_catalogue`]). Single-flight through
/// [`kick_group_fetch`], so a second save while one is running is free.
#[cfg(feature = "indexer")]
pub(super) fn fetch_catalogue_for_interests(d: &Arc<Daemon>) {
    let want = d.index_interests.lock_ok().clone();
    let need = interests_need_catalogue(
        d.group_catalog.lock_ok().is_some(),
        &want,
        !d.indexer_off(),
        d.offline.load(Ordering::Relaxed),
        d.group_fetch_err.lock_ok().is_some(),
    );
    if need && kick_group_fetch(d, d.cfg_path.clone()) {
        info!(
            target: "groups",
            "interests ({want}) chosen before the group list arrived - fetching it now"
        );
    }
}
...
#[cfg(feature = "indexer")]
pub fn kick_group_fetch(d: &Arc<Daemon>, config: PathBuf) -> bool {
...
pub const PROBE_GROUP: &str = "alt.binaries.boneless";
...
pub fn measure_system(
    ...
    let disk = nzbkit::sysbench::disk_write(&crate::naming::out_dir(d), 512).unwrap_or(0.0);
