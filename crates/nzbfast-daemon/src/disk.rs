...
pub use crate::diskfree::disk_stat_walk;
pub use crate::diskfree::free_bytes;
...
impl QuotaLedger {
    ...
    pub fn period_start_on(period: char, (y, m, d): (i64, u32, u32)) -> u64 {
    ...
    pub fn open(spool: &std::path::Path, period: char) -> Self {
...
pub(crate) async fn park_on_full_disk(
    ...
    let disk_full_hold = err.is_some_and(|e| crate::disk_full_mid_download(&e.to_string()))
        ...
        && {
            ...
            min > 0 && {
                ...
                // parking. Identical to `crate::naming::out_dir(d)` for every job
