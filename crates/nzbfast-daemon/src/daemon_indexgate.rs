...
//! `pub(super)` becomes `pub(crate)` here, because `super` is
...
impl Daemon {
    ...
    #[cfg(feature = "indexer")]
    pub fn indexing_pause_reason(&self) -> Option<&'static str> {
    ...
    #[cfg(feature = "indexer")]
    pub fn spot_pause_reason(&self) -> Option<&'static str> {
        if self.offline.load(Ordering::Relaxed) {
            return Some("offline");
        }
    ...
    /// that names none.
    #[cfg(feature = "indexer")]
    pub fn pause_phrase(reason: &str) -> &'static str {
        match reason {
    ...
    ///
    /// NO `#[cfg(feature = "indexer")]`, unlike both its callers above.
    /// It carried one until 2 Sep 2026, when `serve::healauto` became a
    /// third caller: the scheduled library heal stands down for a
    /// download by exactly this test, and it is not indexer work - it
    /// reads the disk and re-fetches a post this daemon already has on
    /// record. The alternative was a second copy of the same two-line
    ...
    #[cfg(feature = "indexer")]
    pub fn index_db_wanted(&self) -> bool {
    ...
    pub fn begin_index_job(self: &Arc<Self>) -> IndexJobGuard {
