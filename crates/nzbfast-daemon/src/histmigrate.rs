...
fn hist_migrate_path(d: &Daemon) -> PathBuf {
    d.spool.join("hist-media.json")
}

fn hist_notice_path(d: &Daemon) -> PathBuf {
    d.spool.join("hist-notice.json")
}

pub(super) fn hist_migrate_state(d: &Daemon) -> MigrateState {
    std::fs::read(hist_migrate_path(d))
        .ok()
        .and_then(|b| serde_json::from_slice(&b).ok())
        .unwrap_or_default()
}

fn save_hist_migrate(d: &Daemon, state: &MigrateState) {
    if let Ok(text) = serde_json::to_string_pretty(state) {
        let _ = crate::persist::write_atomic(&hist_migrate_path(d), text.as_bytes());
    }
}

/// The notice owed to the user, or `None`. Read on the queue-payload
/// path, so it stays one cheap read - see [`MigrateNotice`].
pub fn hist_notice(d: &Daemon) -> Option<MigrateNotice> {
    std::fs::read(hist_notice_path(d))
        .ok()
        .and_then(|b| serde_json::from_slice(&b).ok())
}
...
fn raise_hist_notice(d: &Daemon, notice: &MigrateNotice) {
    let notice = match hist_notice(d) {
    ...
    if let Ok(text) = serde_json::to_string_pretty(&notice)
        && let Err(e) = crate::persist::write_atomic(&hist_notice_path(d), text.as_bytes())
    {
...
pub fn dismiss_hist_migrate(d: &Daemon) -> bool {
    if hist_notice(d).is_none() {
        return false;
    }
    let path = hist_notice_path(d);
...
pub(super) fn rederive_row(d: &Daemon, job: &Arc<Mutex<Job>>) -> RowOutcome {
    ...
    let fresh = match super::mediadisk::probe_disk_facts_checked(d, job) {
        Ok(Some(f)) => f,
...
impl PassOutcome {
    ...
    pub fn notice(&self) -> MigrateNotice {
        MigrateNotice {
...
pub(super) fn migrate_once(d: &Arc<Daemon>) {
    ...
    if pass.corrected > 0 {
        raise_hist_notice(d, &pass.notice());
    }
    ...
    let mut state = hist_migrate_state(d);
    if !pass.complete() {
...
pub fn spawn_history_media_migrate(daemon: &Arc<Daemon>) {
    if !migrate_owed(&hist_migrate_state(daemon)) {
        return;
    }
    let d = daemon.clone();
    ...
    crate::spawn_aux("hist-media", move || migrate_once(&d));
}
