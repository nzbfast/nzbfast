...
fn the_notice_persists_until_it_is_dismissed() {
    let dir = tdir("notice");
    let d = super::super::testutil::test_daemon(&dir);
    crate::histmigrate::raise_hist_notice(
        &d,
        &MigrateNotice {
            corrected: 3,
            kept: 1,
    ...
    assert_eq!(
        crate::histmigrate::hist_notice(&d).map(|n| n.corrected),
        Some(3),
        "the notice must survive the restart that raised it"
    );

    let rev = d.queue_rev.load(Ordering::Relaxed);
    assert!(crate::histmigrate::dismiss_hist_migrate(&d));
    assert!(crate::histmigrate::hist_notice(&d).is_none());
    assert!(
    ...
    assert!(
        !crate::histmigrate::dismiss_hist_migrate(&d),
...
fn a_pass_with_nothing_to_report_raises_no_notice() {
    ...
    assert!(
        crate::histmigrate::hist_notice(&d).is_none(),
...
fn a_correction_that_could_not_be_written_does_not_stamp_the_version() {
    ...
    assert_ne!(
        crate::histmigrate::hist_migrate_state(&d).version,
...
fn a_volume_that_never_comes_back_stops_being_walked() {
    ...
    for _ in 1..MAX_ATTEMPTS {
        migrate_once(&d);
        assert!(
            migrate_owed(&crate::histmigrate::hist_migrate_state(&d)),
...
fn a_dismissal_that_could_not_remove_the_notice_reports_failure() {
    ...
    let rev = d.queue_rev.load(Ordering::Relaxed);
    let said = crate::histmigrate::dismiss_hist_migrate(&d);
    ...
    assert!(
        crate::histmigrate::hist_notice(&d).is_some(),
...
fn a_second_notice_adds_to_one_the_user_has_not_read() {
    ...
    let n = crate::histmigrate::hist_notice(&d).expect("a notice is still owed");
    assert_eq!(
        n.corrected, 5,
