...
#[cfg(test)]
pub(crate) static TAIL_GEN_BARRIER: Mutex<
...
#[cfg(test)]
pub(crate) static SUBMIT_GEN_BARRIER: Mutex<
    Option<(String, Arc<std::sync::Barrier>, Arc<std::sync::Barrier>)>,
> = Mutex::new(None);
...
pub struct PostprocTicket {
    ...
    pub fetch: tokio::task::JoinHandle<Result<()>>,
    /// Snapshotted pre-handoff: still THIS job's, even after the next
    /// job swaps the hub's slot.
    pub verifier: Option<Arc<nzbkit::live::LiveVerifier>>,
    pub shaper: Option<Arc<nzbkit::extract::Extractor>>,
...
impl PostprocLane {
    ...
    pub fn reserve(&self) -> BacklogTicket {
    ...
    pub async fn submit(&self, t: PostprocTicket) {
        ...
        let supervisor = tokio::spawn(async move {
            ...
            if let Err(e) = tail.await {
                ...
                if file_crashed_tail(&job, &e, Some(gen0)) {
                    crate::hooks::run_post_job_hooks_gen(&d, &job, Some(gen0));
                }
    ...
    pub async fn submit_hooks_only(&self, job: Arc<Mutex<Job>>, ticket: BacklogTicket) {
        ...
        let supervisor = tokio::spawn(async move {
            ...
            let hooks = tokio::spawn({
                ...
                async move { crate::hooks::run_post_job_hooks_before_park(&d, &job, gen0).await }
            });
...
pub fn cancel_tail_fetches(d: &Daemon, hit: impl Fn(&str) -> bool) {
    for (id, c) in d.hub.tail_cancel.lock_ok().iter() {
        if hit(id) {
...
pub(super) fn clear_postproc_hold(d: &Daemon) {
...
///
/// **CALLED BY EVERY TAIL THAT AWAITS `finalize_completed_gen`, and
/// that is a correctness requirement rather than tidiness.** The
/// deferral is a HOLD, so whoever arms it owes the release: with
/// `write_manifest` on, `finalize_cleanup_exts` drops `par2` from the
/// finalize sweep and only this function ever puts it back. The
/// prefetch's own tail (`sidecar::completion_tail`) did not call it,
/// which was invisible while the flag was opt-in and became "every
/// prefetch-completed folder keeps its recovery files forever" the day
/// §310 turned the flag on. A `verifier` of `None` is a normal caller
/// and not a degenerate one - it means no manifest is owed here, so the
/// sweep simply happens, exactly as it did before the deferral existed.
/// (There are two production tails and no more: [`run_tail`] and
/// `sidecar::completion_tail`. `job::finalize_completed`, the third
/// caller a grep finds, is the `cfg(test)` wrapper the finalize-marker
/// tests drive and is not a tail.)
///
/// `gen0` is an `Option` for the same reason `Daemon::same_generation`
/// takes one: a caller with no round to fence against passes `None` and
/// every check below reads as "this record has not moved".
pub(super) async fn settle_manifest_and_deferred_par2_sweep(
    d2: &Arc<Daemon>,
    job2: &Arc<Mutex<Job>>,
    ...
    // poorer for it.
    //
    // It is not free any more, and this is where the time goes: since
    // 2 Sep 2026 `write_reconciled` READS every file the recovery set
    // never covered - the extracted film, which is the half of an
    // archive post a library actually keeps - and records a CRC32 grid
    // for it, so it can be convicted and healed rather than recorded as
    // present and unverifiable. Measured at 0.11 s/GiB, read included
    // (`manifest::grid_from_disk`), against 1.49 s/GiB for the
    // whole-file MD5 that was the other option, which is why there is
    // no setting to turn it off separately from the manifest itself. It
    // is on the blocking pool with every other filesystem step on this
    // tail.
    ...
    if par2_may_sweep {
        ...
        let dir = {
            let j = job2.lock_ok();
            (Daemon::same_generation(&j, gen0) && j.state == JobState::Completed)
...
fn fold_provider_quality(
    d: &Arc<Daemon>,
    job: &Arc<Mutex<Job>>,
    facts: Vec<crate::provquality::HostFacts>,
    post_unix: i64,
    demoted: bool,
    gen0: (u32, u64),
) {
...
/// [`crate::provquality::Outcome`]'s own doc: a job that arrived
...
fn provquality_outcome(j: &Job) -> crate::provquality::Outcome {
    use crate::provquality::Outcome;
...
pub(super) async fn run_tail(d2: Arc<Daemon>, t: PostprocTicket, gen0: (u32, u64)) {
    ...
    settle_manifest_and_deferred_par2_sweep(
        &d2,
        &job2,
        verifier.as_ref(),
        shaper.as_ref(),
    ...
    // exist to express.
    if !demoted {
        crate::hooks::run_post_job_hooks_before_park(&d2, &job2, Some(gen0)).await;
    }
...
mod tests {
    ...
    pub fn with_daemon(name: &str, f: impl FnOnce(&Arc<Daemon>)) {
        ...
        // the queue store after `f` returns - `submit` only awaits its
    ...
    /// inside `finalize_completed_gen`; `write_manifest` was OFF by
    /// default when this was written (ON since 2 Sep 2026) and runs in
    /// [`run_tail`], AFTER that function returns. A
    ...
    ///
    /// The manifest-OFF configuration is asserted FIRST and is what
    /// made this a safe fix: with no manifest owed nothing is deferred
    /// and the finalize sweep list is byte-for-byte what it always was.
    /// It is no longer the default - §310 turned `write_manifest` on -
    /// but it is still the state a user who unticks the box is in, and
    /// it is still the arm that proves the deferral is scoped rather
    /// than unconditional, so every arm below stores the flag it means
    /// rather than inheriting one.
    #[test]
    fn the_par2_sweep_is_deferred_only_while_the_settle_manifest_is_owed() {
        with_daemon("par2defer", |d| {
            use crate::job::{finalize_cleanup_exts, par2_sweep_deferred};
            // Manifest OFF, cleanup on: nothing deferred, and the
            // sweep list is what it was before §310 existed.
    ...
    fn cancel_tail_fetches_hits_the_named_owner_only() {
        with_daemon("cancel", |d| {
            ...
            cancel_tail_fetches(d, |id| id == "SABnzbd_nzo_mine");
            ...
            cancel_tail_fetches(d, |id| id == "SABnzbd_nzo_ghost");

            // "all" reaches everything, which is what a delete-all asks
            // for.
            cancel_tail_fetches(d, |_| true);
    ...
    async fn a_lane_tail_never_writes_over_a_record_that_was_retried_out_from_under_it() {
        ...
        // --- the stale direction ---------------------------------------
        let out = crate::naming::out_dir(&d).join("Laned.Release");
        ...
        *TAIL_GEN_BARRIER.lock_ok() = None;
        ...
        let out2 = crate::naming::out_dir(&d).join("Ordinary.Release");
    ...
    async fn the_lane_fences_on_the_generation_it_took_custody_of() {
        ...
        let out = crate::naming::out_dir(&d).join("Waiting.Release");
        std::fs::create_dir_all(&out).expect("payload dir");
        let job = Arc::new(Mutex::new(
            job_from_json(&json!({
