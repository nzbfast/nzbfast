...
pub fn post_age_days(fail_message: &str, nzb_path: &Path, now: i64) -> Option<u32> {
...
/// they are atomics.
pub(crate) fn hunt_policy(d: &Daemon) -> HuntPolicy {
    HuntPolicy {
        enabled: d.alt.auto_search.load(Ordering::Relaxed),
        ...
        max_copies: match d.alt.max_copies.load(Ordering::Relaxed).min(64) {
            0 => HuntPolicy::default().max_copies,
            n => n,
        },
        max_extra_bytes: d.alt.max_extra_bytes.load(Ordering::Relaxed),
    }
}
...
pub(crate) fn hunt_request(d: &Daemon, job: &Arc<Mutex<Job>>) {
    ...
    q.push_back(req);
    drop(q);
    d.hunt.wake.notify_one();
}

/// Drain everything queued. Runs on a blocking thread: the search,
/// the NZB fetch and the enqueue are all synchronous.
pub fn hunt_tick(d: &Daemon) {
    loop {
        let Some(req) = d.hunt.q.lock_ok().pop_front() else {
            return;
        };
        if let Err(no) = hunt_one(d, &req) {
            info!(
                target: "queue",
...
fn hunt_one(d: &Daemon, req: &HuntRequest) -> Result<(), NoHunt> {
    let policy = hunt_policy(d);
    let now = unix_now();
    ...
    // forever.
    if d.giveup.lock_ok().tripped(&p, threshold) {
    ...
    let budget = hunt_budget(d, &keys, policy, Trigger::Auto)?;

    let mut cands = hunt_candidates(d, &req.name, &p, &keys);
    ...
    let ctx = d.alt_ctx(&req.nzo_id, &req.name, keys.clone());
    ...
    for cand in cands.into_iter().take(MAX_CANDIDATE_FETCHES) {
        let Some(bytes) = hunt_fetch(d, &cand) else {
            continue;
        };
        ...
        let gate = d.alt_gate();
        match d.alt_admit(&ctx, want, Trigger::Auto) {
            Ok(()) => {
                if hunt_enqueue(d, req, &cand, bytes, &keys) {
                    return Ok(());
                }
            }
...
fn hunt_budget(
    ...
    if policy.max_extra_bytes == 0 {
        ...
        return if trigger == Trigger::Auto && hunt_metered(d) {
            Err(NoHunt::MeteredNoBudget)
        } else {
            Ok(None)
        };
    }
    let spent = hunt_spent(d, keys);
...
pub(super) fn hunt_metered(d: &Daemon) -> bool {
    nzbkit::config::Config::load(&d.cfg_path)
        .map(|c| {
            c.servers
                .iter()
...
fn hunt_spent(d: &Daemon, keys: &[String]) -> u64 {
    let mine = |g: &Job| -> u64 {
    ...
    let q: u64 = d.queue.lock_ok().iter().map(|j| mine(&j.lock_ok())).sum();
    let h: u64 = d.history.lock_ok().iter().map(|j| mine(&j.lock_ok())).sum();
    q.saturating_add(h)
}
...
/// `name` is the RELEASE the search is aimed at, not a job: §310's
/// heal road reaches this with a name recovered from a settle
/// manifest and no failed row anywhere. It was `&HuntRequest` until
/// then and read exactly one field of it, so taking the field is
/// what stops the heal having to build a request whose other five
/// fields would be fiction.
fn hunt_candidates(d: &Daemon, name: &str, p: &Parsed, keys: &[String]) -> Vec<Cand> {
    let want_dupe = dupe_key(name).unwrap_or_default();
    ...
    #[cfg(feature = "indexer")]
    {
        ...
        for r in hits.iter() {
            ...
            out.push(Cand {
                rank: nzbfast_meta::watchlist::quality_rank(&crate::wall::parse_release(&stem)),
    ...
    for c in hunt_search_external(d, p) {
        if !keep(&c.title) {
            continue;
        }
        out.push(Cand {
            rank: nzbfast_meta::watchlist::quality_rank(&crate::wall::parse_release(&c.title)),
            posted: c.posted,
            size: c.size,
            stem: c.title,
...
/// §310 stage 2: the §282 search reached by a RELEASE NAME, for a caller
/// that has no failed job to hand.
///
/// The heal road's fallback. A library file whose damage the settle
/// manifest found is re-fetched from the post the manifest recorded, but
/// that post's spooled `.nzb` lives exactly as long as the history
/// record does - and the manifest is built to outlive it. When it is
/// gone the only way back to the bytes is a search for the same release,
/// which is what this is.
///
/// **What it deliberately does NOT run, and why each one is a heal
/// question rather than a hunt question.**
///
/// * [`hunt_gates`] in full. Every gate in it is about a job that FAILED
///   - the *arr that owns its retry, the age that says it is still
///   propagating, the verdict that says the post is unavailable. A heal
///   has no failed job: it has a file that was complete once and is not
///   any more. Asking those questions here would answer about a job that
///   does not exist. The identity half IS run, below, because a search
///   that cannot be aimed is the one refusal that still applies.
/// * The same-post admission test, and this is the inversion worth
///   stating plainly. §282 refuses a candidate that is the same post
///   because there the post is DEAD, so an identical copy fails
///   identically. Here the post was fine and the damage is on our own
///   disk, so the same post is the BEST source there is, not the worst.
/// * The copy cap and the byte ceiling. Both bound what this daemon may
///   spend ON ITS OWN INITIATIVE chasing a download that cannot finish.
///   A heal is neither automatic nor speculative: a person named a
///   directory and asked for their own file back. `heal.rs` states
///   the bound that does apply.
///
/// `None` when the name carries no identity a search can be aimed with,
/// when nothing matched, or when no candidate's `.nzb` could be fetched.
/// Returns the candidate's stem alongside its bytes, because the caller
/// enqueues under that name and not under the damaged file's.
pub(crate) fn hunt_by_name(d: &Daemon, name: &str) -> Option<(String, Vec<u8>, Option<Fetched>)> {
    if !searchable_by_name(name) {
        return None;
    }
    let p = crate::wall::parse_release(name);
    let keys = target_keys(&p);
    let mut cands = hunt_candidates(d, name, &p, &keys);
    sort_best_first(&mut cands);
    for cand in cands.into_iter().take(MAX_CANDIDATE_FETCHES) {
        if let Some((bytes, headers)) = hunt_fetch(d, &cand) {
            return Some((cand.stem, bytes, headers));
        }
    }
    None
}

/// Can a search be AIMED with this release name alone?
///
/// [`hunt_gates`]'s identity half, as a predicate a caller can ask
/// BEFORE deciding to spend anything. Both tests, never one, for the
/// reason stated there: `target_keys` names the episode or the film and
/// `dupe_key` is the release identity a candidate has to meet, and a
/// name that answers only one is a name a search wanders off with. A
/// fully obfuscated release answers neither.
///
/// Its own function because §310's heal has to say so on the READ-ONLY
/// door - "there is nothing a search could be aimed at here" is an
/// answer a user should get from the report rather than discover when
/// the second click refuses.
pub(crate) fn searchable_by_name(name: &str) -> bool {
    !target_keys(&crate::wall::parse_release(name)).is_empty() && dupe_key(name).is_some()
}
...
fn hunt_search_external(d: &Daemon, p: &Parsed) -> Vec<HuntExt> {
        ...
        match r {
            ...
            Err(e) => {
                if matches!(e, crate::newznab::NewznabError::Limit(..)) {
                    d.indexer_rt.lock_ok().penalty_until.insert(
                        name.clone(),
...
fn hunt_fetch(d: &Daemon, cand: &Cand) -> Option<(Vec<u8>, Option<Fetched>)> {
    match &cand.src {
        #[cfg(feature = "indexer")]
        CandSrc::Local(id) => {
            let xml = d.with_index_read(|ix| ix.make_nzb(*id).ok())?;
...
fn hunt_enqueue(
    ...
    match out {
        Ok(e) => {
            ...
            // next candidate.
            if !stamp_hunt_switch(d, req, &e.nzo_id, &cand.stem) {
                info!(
                    target: "queue",
            ...
            let placed = if d.held_as_duplicate(&e.nzo_id) {
...
fn stamp_hunt_switch(d: &Daemon, req: &HuntRequest, new_id: &str, new_name: &str) -> bool {
    ...
    let why = crate::altcand::why_from_fail(&req.fail_message);
    let stamped = {
...
fn hunt_click_request(d: &Daemon, nzo_id: &str) -> std::result::Result<HuntRequest, String> {
    let job = d
        .queue
        .lock_ok()
        .iter()
    ...
    let Some(job) = job else {
        ...
        return hunt_parked_request(d, nzo_id);
    };
    let g = job.lock_ok();
    ...
    if matches!(g.state, JobState::Downloading | JobState::Finishing) {
        return Err("this download has already started - pause it first, or let it finish".into());
    }
    let Some((_, why, lead)) = crate::altcand::terminal_reason(&g) else {
...
fn hunt_parked_request(d: &Daemon, nzo_id: &str) -> std::result::Result<HuntRequest, String> {
    let Some(job) = d.history_job(nzo_id) else {
        return Err("that download is no longer in the queue".into());
    };
    let g = job.lock_ok();
    if !crate::altcand::parked_replaceable(&g) {
...
fn hunt_copies_spent(d: &Daemon, keys: &[String]) -> u32 {
    let st = d.giveup.lock_ok();
    keys.iter()
        .filter_map(|k| st.targets.get(k))
...
fn hunt_click_gates(
    ...
    let policy = hunt_policy(d);
    let (p, keys) = hunt_gates(req, policy, unix_now(), Trigger::Clicked).map_err(|e| e.why())?;
    ...
    let copies = hunt_copies_spent(d, &keys);
    if copies >= policy.max_copies {
        return Err(NoHunt::CopyCap(copies).why());
    }
    let budget = hunt_budget(d, &keys, policy, Trigger::Clicked).map_err(|e| e.why())?;
    Ok((p, keys, budget))
}
...
pub fn hunt_offer(d: &Daemon, nzo_id: &str) -> std::result::Result<Value, String> {
    let req = hunt_click_request(d, nzo_id)?;
    let (p, keys, budget) = hunt_click_gates(d, &req)?;
    let mut cands = hunt_candidates(d, &req.name, &p, &keys);
    sort_best_first(&mut cands);
    ...
    let rows: Vec<Value> = cands
        ...
        .collect();
    hunt_remember(d, nzo_id, cands);
    Ok(json!({"candidates": rows}))
}

/// Keep this row's pick list for the click that follows it.
fn hunt_remember(d: &Daemon, nzo_id: &str, cands: Vec<Cand>) {
    let now = unix_now();
    let mut m = d.hunt.offers.lock_ok();
...
fn hunt_remembered(d: &Daemon, nzo_id: &str, key: &str) -> Option<Cand> {
    let now = unix_now();
    let m = d.hunt.offers.lock_ok();
...
pub fn hunt_pick(d: &Daemon, nzo_id: &str, key: &str) -> std::result::Result<String, String> {
    let req = hunt_click_request(d, nzo_id)?;
    let (_, keys, budget) = hunt_click_gates(d, &req)?;
    let cand = hunt_remembered(d, nzo_id, key)
        .ok_or("that list of copies is out of date - search again")?;
    if !affordable(cand.size, budget) {
        return Err(NoHunt::ByteCap.why());
    }
    let Some((bytes, headers)) = hunt_fetch(d, &cand) else {
        return Err(
    ...
    let origin = hunt_origin(&keys[0]);
    let e = hunt_hold(d, &req, &cand, &bytes, headers.as_ref(), &origin)
        .map_err(|e| format!("that copy could not be queued: {e}"))?;
    if let Some(err) = d.alt_switch(&req.nzo_id, &e) {
    ...
    d.hunt.offers.lock_ok().remove(nzo_id);
    info!(
        target: "queue",
...
/// `crates/nzbfast-daemon/src/daemon.rs`, which sat on its size-gate ceiling when this
/// was written (recalibrated 31 Aug 2026). The one
...
/// turn, remembering that `save_queue` is durable-or-previous
/// whichever way it writes - since §7a it appends one line to
/// `queue.jsonl` and replay skips a torn tail, before that it wrote
/// a whole `queue.json` through a temp file and renamed - so a crash
/// during it leaves the store as it was, still holding this row:
///
...
///   name), but its FIRST is not: the queue store does not learn of
///   the change until the save, so a crash between the rename and it
///   restores the
...
fn hunt_unhold(d: &Daemon, nzo_id: &str) {
    let nzb = {
