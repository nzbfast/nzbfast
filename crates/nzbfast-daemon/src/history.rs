...
pub fn retry_space_needed(j: &Job) -> u64 {
    let to_fetch = if crate::disk_full_mid_download(&j.fail_message) {
    ...
    let moved = if !filed && !source_missing {
        let base = if state == JobState::Completed {
            ...
            match (cat_root, d.move_completed.read_ok().clone()) {
                ...
                (None, Some(root)) => root,
                (None, None) if !cat.is_empty() => crate::naming::out_dir(d).join(cat),
                (None, None) => crate::naming::out_dir(d),
            }
            ...
            crate::naming::out_dir(d).join(cat)
        };
...
pub const HISTORY_DEFAULT_LIMIT: usize = 500;

/// How many filesystem probes ONE `history_page` call will spend on the
/// §96 storage-deleted question while computing its facets.
///
/// The facets are counted over the whole matched set - that is what a
/// chip count means - but `storage_deleted` is the only classifier in
/// that loop that touches the disk, and history is UNLIMITED by default
/// (histstore.rs:6, a product ruling), so an unbudgeted probe made the
/// facet scan cost one `exists()` per Completed row of the entire store
/// on every request. On a completed folder that lives on an SMB share
/// that is one round trip each, and on a share that is unmounted or
/// asleep each one blocks for the mount timeout while the row's own
/// mutex is held. `mode=history` is the ungated path every *arr polls
/// (histstore.rs:2289-2295), so the cost scaled with client count.
///
/// The bound this restores is the one written down when the default
/// cap landed: the stats are paid "for the rows on
/// the requested page", 2N bounded by the same cap. `5b30d28b2` moved
/// the counters, `failed_only`, the bucket chips and `clearable` onto
/// `publishes_as_failed` (read-only sweep finding 13) and took the bound
/// with them; the budget puts it back without putting finding 13 back,
/// because every row a client can render is inside the budget and the
/// render probes independently anyway.
///
/// This is the FLAT allowance only. `history_page` spends a second one
/// of at most `limit` probes on rows still inside the render window,
/// because the flat allowance is counted over CANDIDATES and a query
...
pub fn history_page(d: &Daemon, q: &HistQuery, summary: bool) -> (Vec<Value>, usize, Value) {
    ...
    let held: Vec<crate::altcand::HeldSpare> = if summary {
        Vec::new()
    } else {
        d.alt_held_spares()
    };
    ...
    // The facet loop's filesystem allowance - see `HISTORY_STAT_BUDGET`.
    // An explicit `limit` bigger than the budget is honoured as asked
    // (the same escape hatch `HISTORY_DEFAULT_LIMIT` documents), and
    // `limit == 0` is the unbounded shape, which renders every matched
    // row and so probes each of them at render time anyway.
    //
    // `start` IS DELIBERATELY NOT IN THIS SUM. It was for a few hours on
    // 1 Sep 2026, on the reasoning that a deep page needs a deep prefix
    // probed - but `start` is a client-controlled integer parsed
    // unclamped from the query string, so `?start=100000000&limit=1`
    // bought a hundred million stats and the facets moved as the client
    // paged. A budget an unauthenticated caller can raise is not a
    // budget. What the deep page loses is answered by the second term of
    // the probe test below instead, which is bounded by `limit`.
    let probe_cap = if q.limit == 0 {
        usize::MAX
    } else {
        q.limit.max(HISTORY_STAT_BUDGET)
    };
    let mut probes = 0usize;
    // The render-window allowance, spent only once `probes` is out - see
    // the probe test in the loop.
    let mut window_probes = 0usize;
    let mut slots: Vec<Value> = Vec::new();
    for j in arcs.iter().rev() {
        let j = j.lock_ok();
        // NZBGet's `HistoryDelete` hid this row (see `Job::hidden`).
        // THE ONE PLACE display is filtered, and that is why the filter
        // is here rather than at each caller: the SAB `mode=history`
        // body, the dashboard's own history payload and the playback
        // listing are all this function, so a row hidden for one is
        // hidden for all three and a fourth render added tomorrow
        // inherits it. Ahead of every other filter, so a hidden row is
        // absent from the facet counts and the bucket chips too - a
        // count that includes rows the page cannot show is the
        // finding-13 disagreement in a new spelling.
        //
        // NOTHING ELSE that walks `self.history` filters on it. The
        // duplicate check, `owned_dupe_keys`, retention and the retry
        // lookup must all keep seeing hidden rows: keeping the row's
        // identity alive after an *arr's post-import cleanup is the
        ...
        //
        // ...AND THE PROBE IT NEEDS IS SPENT OUT OF A BUDGET, because
        // this loop runs over the WHOLE store while the facade's own
        // page is a window over it. `storage_deleted` is the one
        // classifier here that touches the filesystem, and moving the
        // counters onto it in `5b30d28b2` moved its stats with them:
        // every Completed row in a 14,500-row history was stat'ed on
        // every ungated `mode=history` poll, where the render had only
        // ever paid for the rows on the page (see `HISTORY_STAT_BUDGET`
        // and the note that landed with the default cap
        // for the bound this restores). The cheap prefix is asked for
        // free; only a genuine flip candidate spends a probe.
        //
        // TWO ALLOWANCES, and the second one is not decoration.
        // `probes` counts CANDIDATE rows, and a candidate is tested here,
        // BEFORE `failed_only`, the bucket chips and `status=` narrow
        // anything; `matched` counts only rows that survive all of them.
        // So a query whose filters reject most candidates - `bucket=
        // failed` on a mostly-Completed store is the everyday one - burns
        // the flat allowance on rows it will never render and then
        // classifies the rows it DOES render on the cheap word. A
        // storage-deleted row past the prefix is then counted under
        // `done` AND dropped by `bucket=failed`, so "Show failed" hides a
        // row that renders as Failed: the finding-13 disagreement this
        // loop exists to end, back through a different door.
        //
        // `window_probes` is a SECOND allowance, its own counter and its
        // own ceiling of `limit`, spent only while this row is still
        // inside the render window (`matched` at this point is this
        // row's index among the survivors). Its own counter is what
        // makes it safe: `matched < q.limit` alone never stops being true
        ...
        // Past both allowances the row is classified on the cheap word,
        // which is what this trade costs: a storage-deleted row lying
        // beyond both prefixes is counted under `done` and missed by
        // `failed_only`/`bucket=failed`. Two shapes still reach it - a
        // deep `start=` page (the budget no longer grows for `start`, so
        // those facets are cheap-word past the allowances) and a filter
        // that rejects more than `limit` candidates before the row. The
        // RENDER still probes per row (`history_row`/`history_summary`
        // call `sab_history_status_published` themselves), so a row that
        // does get rendered never shows the wrong word, whatever the
        // chips above it say.
        let pub_failed = if probe {
            publishes_as_failed(&j)
        } else {
            sab_history_status(&j) == "Failed"
        };
        // The published word, derived ONCE from the decision just made.
        // `status=` below used to call `sab_history_status_published`
        // itself, which walks straight past the budget and back into two
        // `exists()` per candidate row of the whole store - so
        // `mode=history&status=Completed` still stat'ed everything. Same
        // answer, no second probe: `publishes_as_failed` is true exactly
        // when that call's word is "Failed".
        let pub_word = if pub_failed {
            "Failed"
        } else {
        ...
        // the row publishes - `pub_word`, which is
        // `sab_history_status_published`'s answer carried down from the
        // budgeted decision above rather than a second unbudgeted call
        // to it, and never the bare `sab_history_status`, or a
        // `status=Completed` request could return a row whose own
        // `"status"` key says `"Failed"` (§96's storage-deleted flip).
        // `None` (no param, or `failed_only`'s own arm above already
        // narrowed to Failed) matches everything.
        if let Some(want) = &q.status
            && !want.contains(pub_word)
        {
            continue;
        }
        let idx = matched;
...
/// restored folder restores the row. That "requested page only" is a
/// COST bound as much as a semantic one, and it was silently lost for a
/// day when `5b30d28b2` moved the facet counters onto this call: the
/// facets are counted over the whole store, so every Completed row in it
/// was stat'ed per request. `HISTORY_STAT_BUDGET` is how the bound is
/// held now - the facet loop asks
/// [`storage_deleted_candidate`] for free and spends a probe only
/// inside the budget, while every rendered row still asks in full.
///
/// AND SINCE 1 Sep 2026 THE RENDER IS NOT THE ONLY ASKER: the `failed`
/// and `completed` bulk delete words classify on the published word too
/// (finding 13), so they answer this over the whole store as well. That
/// pass is `published_failed_map`, which is deliberately taken with no
/// global lock held - see its own note, and F20 for what holding one
/// across these stats cost. Every other delete shape (a single id, a
/// comma list, `value=all`) never reads the answer and never asks it.
fn storage_deleted(j: &Job) -> bool {
    if !storage_deleted_candidate(j) {
        return false;
    }
    // The seam's OTHER half. `STORAGE_PROBES` is behind `test-support`
    // so the bin's `tests_jobs` can read it; leaving the INCREMENT at
    // plain `cfg(test)` left that test reading a counter nothing moves,
    // which is the shape that also wedged the idle-CAS barrier.
    #[cfg(any(test, feature = "test-support"))]
    STORAGE_PROBES.with(|c| c.set(c.get() + 1));
    !j.out_dir.exists() && j.out_dir.parent().is_some_and(|p| p.exists())
}

/// The free half of [`storage_deleted`]: everything the verdict needs
/// that costs no syscall. A row this rejects can never be
/// storage-deleted, so the facet loop can classify it on the cheap word
/// without touching the disk; a row it accepts is a flip CANDIDATE and
/// nothing more - only the two `exists()` stats in `storage_deleted`
/// decide it. One rule, one copy: `storage_deleted` is the only caller
/// that completes it.
fn storage_deleted_candidate(j: &Job) -> bool {
...
#[cfg(any(test, feature = "test-support"))]
thread_local! {
    /// Test seam for the budget above: filesystem probes
    /// `storage_deleted` has spent on THIS thread. Per-thread rather
    /// than global so a test reading it cannot be perturbed by whatever
    /// else `cargo test` is running in the same process beside it.
    pub static STORAGE_PROBES: std::cell::Cell<usize> = const { std::cell::Cell::new(0) };
}
...
///
/// The bulk delete words (`failed`, `completed`) are the only readers of
/// `DeleteRecord::published_failed`, and answering the question costs
/// `storage_deleted` up to two `exists()` calls per Completed row on a
/// folder that can be an SMB share that has gone away. The delete arm
/// holds `d.history` across its whole plan-and-retain transaction, and
/// the park path takes that same lock to file a finished job, so paying
/// for the stats inside the hold stalled a finishing download behind a
/// mount timeout (bug sweep 1 Sep 2026, F20). The global lock is
/// therefore taken here only to clone the Arcs, and the probes are spent
/// under each row's own mutex with nothing global held.
///
/// No budget, unlike the facet loop's `HISTORY_STAT_BUDGET`: this runs
/// once per user-initiated bulk clear rather than on the ungated poll
/// every *arr makes, and a bulk DELETE that classified some of its rows
/// on the cheap word would remove the wrong ones.
pub fn published_failed_map(d: &Daemon) -> std::collections::HashMap<String, bool> {
    let arcs: Vec<Arc<Mutex<Job>>> = d.history.lock_ok().clone();
    arcs.iter()
        .map(|j| {
            let g = j.lock_ok();
            (g.nzo_id.clone(), publishes_as_failed(&g))
        })
        .collect()
}
...
fn history_summary(d: &Daemon, j: &Job) -> Value {
    ...
    json!({
        ...
        "moved_to": if j.out_dir.starts_with(crate::naming::out_dir(d)) {
            String::new()
        } else {
...
pub fn history_json(d: &Daemon, params: &std::collections::HashMap<String, String>) -> Value {
...
fn history_row(d: &Daemon, j: &Job, held: &[crate::altcand::HeldSpare]) -> Value {
    {
        ...
        let moved_to = if j.out_dir.starts_with(crate::naming::out_dir(d)) {
            String::new()
        } else {
            j.out_dir.to_string_lossy().into_owned()
        };
        ...
        json!({
            ...
            "alt_offer": crate::altcand::parked_offer_json(j, held),
            "fail_message": if deleted { STORAGE_DELETED_MSG } else { j.fail_message.as_str() },
