...
pub(crate) const REFETCH_FLOOR_BYTES: u64 = 1_000_000_000;
...
pub(crate) const REFETCH_SHIELD_FACTOR: u64 = 8;
...
#[derive(Default)]
pub struct PauseCostCache(Mutex<PauseCostState>);
...
struct WireOwner {
    ...
    pub answer: Option<(Instant, Option<RequeueCost>)>,
}
...
impl Daemon {
    ...
    pub fn note_wire_owner(&self, id: &str, out_dir: &std::path::Path) {
    ...
    pub fn pause_cost(&self) -> Vec<(String, RequeueCost)> {
    ...
    pub fn pause_cost_under(&self, budget: nzbkit::mem::MemBudget) -> Vec<(String, RequeueCost)> {
        let cap = budget.holds_cap() as u64;
...
mod pause_cost_tests {
    ...
    fn a_stale_owner_reports_no_cost_once_the_job_leaves_the_wire() {
        ...
        *d.active_dl.lock_ok() = Some("SABnzbd_nzo_nzbfast2".to_string());
        ...
        let fresh = crate::testutil::test_daemon(&scratch("stale2"));
