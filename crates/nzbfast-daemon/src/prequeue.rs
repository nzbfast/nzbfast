...
pub(crate) fn parse_verdict(stdout: &str) -> Option<PreVerdict> {
...
impl Daemon {
    ...
    pub(crate) fn run_pre_queue(
        ...
        // TODO 314 stage 1. Nothing has been downloaded yet, so a
        // pre-queue hook has no job directory to be told about.
        let mut cmd = script::confined_command(self, &script, &[]);
    ...
    pub(crate) fn restore_script_knobs(&self, saved: &serde_json::Map<String, Value>) {
        ...
        // TODO 314 stage 1. Absent = the default (on), which is what an
        // install that predates the setting gets.
        if let Some(on) = saved.get("script_confined").and_then(Value::as_bool) {
            self.script_confined.store(on, Ordering::Relaxed);
        }
    }
}
