...
///   there is none.
///
/// The route is read through [`crate::lanaddr::route_src`], at
...
/// not pre-empt it.
pub fn pointer_authority(bind: &str, port: u16) -> String {
    pointer_authority_from(bind, port, || {
        crate::lanaddr::route_src("8.8.8.8:53")
...
pub fn warn_docker_pointer_once(authority: &str) {
...
pub fn payload_in_flight(d: &Daemon, job: &Arc<Mutex<Job>>) -> bool {
    let (owed, id) = {
        let j = job.lock_ok();
...
fn serve_finished_from_disk(
    ...
    let (done, dir, filed, stem, tail) = {
        let j = job.lock_ok();
        let sfx = delete_tail(&j, || crate::naming::job_suffix(d, filed_stem(&j)));
        (
...
pub const PREVIEW_DEFAULT: &str = "metadata-only";
...
pub fn preview_mode(d: &Daemon) -> String {
...
/// out of it.
pub fn finished_media_path(d: &Daemon, job: &Arc<Mutex<Job>>) -> Option<PathBuf> {
...
pub fn preview_probe_request(d: Arc<Daemon>, req: tiny_http::Request, id: String) {
...
pub fn playback_readiness(d: &Daemon, id: &str) -> serde_json::Value {
...
pub fn stream_runway_wait_ms() -> u64 {
...
mod strm_tests {
    ...
    fn the_live_lookup_yields_a_usable_authority() {
        ...
        match crate::lanaddr::route_src("8.8.8.8:53") {
...
mod stream_move_window_tests {
    use super::*;
    // `rec` moved into `testutil` with lane 2 of the serve split - the
    // mover test it built records for had to be lifted into the bin,
    // and a `#[cfg(test)] mod` of this crate is invisible there. It
    // builds that history record, from the same wire shape the
    // stores replay.
    ...
    fn a_real_move_reads_owed_then_in_flight_then_settled() {
        ...
        *d.move_completed.write_ok() = Some(dest.clone());
        ...
        let job_dir = crate::naming::out_dir(&d).join("Some.Show.S01E01");
    ...
    fn readiness_says_moving_rather_than_no_media_while_the_payload_is_in_flight() {
        ...
        let job_dir = crate::naming::out_dir(&d).join("Some.Show.S01E02");
        std::fs::create_dir_all(&job_dir).unwrap();
        std::fs::write(job_dir.join("ep.mkv"), vec![b'x'; 4096]).unwrap();
        let job = rec(
            ID,
