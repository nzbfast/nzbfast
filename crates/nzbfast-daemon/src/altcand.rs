...
/// the thing it bounds lives somewhere else. That argument is the whole
/// reason and it does not expire.
///
/// A SECOND reason applied when this was written and NO LONGER DOES.
/// `crates/nzbfast-daemon/src/daemon.rs` was ONE line under its size-gate ceiling, which is
/// why the field over there is a single line carrying a trailing
/// comment instead of a doc comment: two lines refused the build. That
/// has gone twice over - the 31 Aug 2026 recalibration raised the
/// ceiling to 4,000, and the 1 Sep 2026 split left the file at 2,748,
/// 1,252 clear, measured that day - so nothing forces that field to
/// stay one line any more, and a
/// lane that wants to give it an ordinary doc comment should. Kept as
/// history rather than deleted, because the FIRST reason above is
/// still why the struct exists at all.
///
...
pub struct AltSettings {
    ...
    /// which is the one door that ships ON. `altspend.rs` is where
    /// both roads now ask.
...
/// between this file and `hunt.rs`. Item 12 said that with nothing
...
pub fn offer_json(j: &Job, held: &[HeldSpare], auto_search: bool) -> Option<Value> {
...
impl Daemon {
    ...
    pub fn alt_held_spares(&self) -> Vec<HeldSpare> {
        self.queue
            .lock_ok()
            .iter()
            .filter_map(|j| {
    ...
    pub fn parked_spare_owner(&self, target: &str) -> bool {
...
pub fn switch_lines(j: &Job) -> Vec<(&'static str, String)> {
    let mut out = Vec::new();
