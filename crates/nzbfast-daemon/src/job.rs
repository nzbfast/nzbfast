...
pub struct Job {
    ...
    pub tombstone: bool,
    /// NZBGet `HistoryDelete`: this history row is HIDDEN, not deleted.
    ///
    /// The two NZBGet verbs are not synonyms and this facade treated
    /// them as one until 4 Sep 2026. `HistoryDelete` is documented as
    /// "hide history items (mark as hidden)" - the row stays, its dupe
    /// key stays, and only `history(true)` lists it; `HistoryFinalDelete`
    /// is the erase. Sonarr's `NzbgetProxy.RemoveItem` sends the FORMER,
    /// and "Remove completed downloads" fires it after every import, so
    /// under the shared arm every release an *arr imported left our
    /// history outright: `dupe_collision` and `owned_dupe_keys` read
    /// Completed history rows, so a later re-grab of the identical
    /// release was not held and the wall lost its "you have this" badge.
    ///
    /// A hidden row is therefore INVISIBLE TO DISPLAY and VISIBLE TO
    /// IDENTITY, deliberately: `history_page` (the one funnel the SAB
    /// history body and the dashboard both render through) skips it,
    /// and every consumer that walks `self.history` for what the user
    /// already has - the duplicate check, the owned-key sets, retention,
    /// the retry lookup - keeps seeing it. That asymmetry IS the fix;
    /// filtering it out of the identity side would restore the defect.
    ///
    /// Not a `tombstone`, which means the opposite (this record is on
    /// its way out and its Arc must find nothing to do). Persisted, and
    /// additively: a store written before this field existed reads back
    /// as `false`, which is the truthful answer for every row in it.
    ...
    /// §310 stage 2: the LIBRARY FOLDER this job was started to repair.
    ///
    /// Set only by the heal road (`heal.rs`), empty on every other
    /// job. `tasks::worker` adds it to `donor_dirs`, which is what makes
    /// a heal cheap: the intact bytes already on disk are adopted (§293
    /// Shape A) and only the damaged blocks come off the wire.
    ///
    /// A path of its own rather than a reuse of `alt_from`, and the
    /// difference is not cosmetic. `alt_from` names a ROW, and the
    /// worker resolves it to that row's `out_dir` through history - so
    /// it can only ever point at a directory history still remembers.
    /// The whole point of the settle manifest is to outlive that record:
    /// a library folder whose job aged out of history has no row to name
    /// and would silently donate nothing. Nor is a heal an alternate -
    /// it replaces no queue row, so `altspend`'s ledger must not see it.
    pub heal_dir: PathBuf,
    ...
    /// `strip_prefix(crate::naming::out_dir())` - which does not match - so it would
    ...
    /// for. See `moveseq.rs` for the whole protocol.
    ///
...
impl Job {
    ...
    pub(crate) fn clear_attempt_verdicts(&mut self) {
        self.whyslow = None;
...
async fn settle_locked_failure(
    ...
    let locked =
        tokio::task::spawn_blocking(move || crate::unlockpw::encrypted_archive(&probe_dir))
            .await
            .unwrap_or(None);
...
/// files inside [`finalize_completed_gen`]; `write_manifest` was OFF by
/// default when this was written and has been ON since 2 Sep 2026 (so
/// the deferral below is now the ordinary path rather than the opted-in
/// one) and the manifest is written in
...
pub(super) async fn finalize_completed_gen(
    ...
    if done_ok {
        ...
        if d.refeed_nzb.load(Ordering::Relaxed) {
            ...
            if let Err(e) =
                tokio::task::spawn_blocking(move || crate::refeed::refeed_completed(&d5, &j5, gen0))
                    .await
            {
...
impl DupeExempt<'_> {
    ...
    pub fn asked(allow_dupe: bool) -> Self {
        if allow_dupe {
            Self::Anybody
        } else {
            Self::Nobody
    ...
    pub fn or_anybody(self, yes: bool) -> Self {
...
/// looks like.
pub fn is_held_alternative(g: &Job) -> bool {
...
pub fn mask_spool_path(nzb: &Path, suffix: &str) -> Option<PathBuf> {
    let adoptable = nzb
        .file_name()
        .and_then(|f| f.to_str())
...
pub fn priority_name(p: i32) -> &'static str {
    match p {
        2 => "Force",
        1 => "High",
        -1 => "Low",
...
pub const SAB_DEFAULT_PRIORITY: i32 = -100;
...
pub fn sab_pp_param(pp: Option<&str>) -> Option<i64> {
...
pub fn enqueue_priority(requested: i32, duplicate: bool) -> i32 {
    if duplicate {
        DUPE_PRIORITY
...
pub fn demote_requeues(demote: bool, tombstone: bool, failed: bool) -> bool {
...
pub fn nzbget_status(j: &Job) -> (&'static str, &'static str, &'static str) {
...
pub fn shape_unpacks_on_disk(shape: &str) -> bool {
    shape
        .split_whitespace()
...
pub fn unpack_space_needed(to_fetch: u64, total_bytes: u64, shape: &str) -> u64 {
...
pub fn nzbget_priority(p: i64) -> i32 {
    match p {
        900.. => 2,
        50..=899 => 1,
        i64::MIN..=-50 => -1,
...
pub const MOVE_RETRY_MAX_SECS: u64 = 6 * 3600;
...
/// through.
pub const MOVE_RETRY_GIVE_UP: u32 = 8;
...
pub fn move_retry_delay(base: u64, attempts: u32) -> u64 {
...
pub fn is_season_dir(p: &std::path::Path) -> bool {
    p.file_name()
        .and_then(|n| n.to_str())
...
/// the fact.
pub fn delete_tail(j: &Job, legacy: impl FnOnce() -> String) -> crate::smart::FiledTail {
...
pub struct DeleteRecord {
    ...
    /// NZBGet's `HistoryDelete` hid this row (see [`crate::job::Job::hidden`]).
    ///
    /// Read by the BULK WORDS only. `history_page` filters a hidden row
    /// out of the rows it renders AND out of the `clearable` /
    /// `clearable_failed` counts the dashboard's two one-click sweeps
    /// are labelled with, so a sweep that took hidden rows anyway would
    /// be a bulk delete removing more rows than the button it sits under
    /// says it will - the finding-13 disagreement, in the direction that
    /// destroys. An explicit id (or a comma list) still takes it: a row
    /// the caller names by id is one it can see, and `HistoryFinalDelete`
    /// is the NZBGet spelling of the same instruction.
    pub hidden: bool,
}

/// Snapshot every history row as [`DeleteRecord`], for
/// [`plan_history_delete`].
///
/// `published` is the §96 storage-deleted verdict per nzo_id, taken
/// BEFORE the global history lock by
/// [`crate::history::published_failed_map`]; `None` means this
/// request will never read the field, and then it is `false` for every
/// row rather than answered. NOTHING IN HERE MAY TOUCH THE FILESYSTEM,
/// which is the whole point of the argument (bug sweep 1 Sep 2026, F20):
/// this runs with `d.history` held, and `publishes_as_failed` costs two
/// `exists()` calls on a completed folder that can be an SMB share which
/// has gone away. Every park, every enqueue dupe check, `history_publish`
/// and the queue saver queue behind that lock, so a finishing download
/// stalls at the park for as long as the mount takes to time out.
///
/// Only the `failed` and `completed` bulk words read `published_failed`
/// (see `plan_history_delete`), so a single id, a comma list and
/// `value=all` all pass `None` and pay nothing. A row that arrives in
/// the gap between the map and the lock defaults to `false`, which is
/// the right answer rather than merely a safe one: a row parked inside
/// that window is a just-finished job whose folder is there, so it
/// publishes Completed - "Clear failed" leaves it and "Clear completed"
/// takes it, which is what the user asked for either way.
///
/// One door for both callers (the REST `mode=history&name=delete` arm
/// and `settle_may_remove_files`), so the stat cannot creep back into
/// one of the two copies this used to be.
pub fn delete_records(
    rows: &[Arc<Mutex<Job>>],
    published: Option<&std::collections::HashMap<String, bool>>,
) -> Vec<DeleteRecord> {
    rows.iter()
        .map(|j| {
            let g = j.lock_ok();
            DeleteRecord {
                published_failed: published
                    .and_then(|m| m.get(g.nzo_id.as_str()).copied())
                    .unwrap_or(false),
...
pub fn plan_history_delete(
    ...
    let doomed: Vec<bool> = records
        ...
        .map(|r| {
            ...
            // The bulk words never reach a hidden row - see
            // `DeleteRecord::hidden`. Ahead of the three tests rather
            // than added to each, so a fourth bulk word added later
            ...
            let swept = !r.hidden
                && (value == "all"
                    ...
                    || (value == "completed"
                        ...
                        && !r.published_failed
                        && !r.locked))
                && sabvocab::sab_search_matches(&r.name, search);
            swept
...
pub fn restore_records(queue_arr: &[Value], hist_arr: &[Value]) -> (Vec<Job>, Vec<Job>) {
...
pub(super) fn claim_extra_slot(
    slots: &mut std::collections::HashMap<String, nzbfast_meta::watchlist::Slot>,
    key: String,
    val: &nzbfast_meta::watchlist::Slot,
) {
...
pub fn nzb_sha(bytes: &[u8]) -> String {
...
pub const REPLACED_SUFFIX: &str = ".nzbfast-replaced-";
...
pub const API_MB: f64 = 1024.0 * 1024.0;

/// Integer form of [`API_MB`], for the NZBGet fields that are whole MB.
pub const API_MB_U: u64 = 1024 * 1024;
...
#[path = "job_wire.rs"]
mod job_wire;
pub use job_wire::job_from_json;
pub use job_wire::job_json;
...
#[path = "job_fail.rs"]
mod job_fail;
// `FailKind` is the only one of these the layers above name (four sites
// in tasks/ and sabcompat/), so it is the only one that crosses.
pub use crate::failkind::FailKind;
// serve's `use job::*;` is how the bin's own tests reach the failure
// vocabulary, so the whole group crosses rather than only `FailKind`.
pub use crate::failkind::{
    RETRY_WHY_PROPAGATION, RETRY_WHY_TRANSPORT, another_copy_can_help, disk_full_failure,
...
pub use crate::failkind::fail_kind;
pub use job_fail::auto_retry_eligible;
pub use job_fail::merge_notify_tokens;
pub use job_fail::post_job_plan;
...
#[cfg(any(test, feature = "test-support"))]
pub use job_fail::post_job_duties;
...
#[path = "job_dupe.rs"]
mod job_dupe;
pub use job_dupe::dupe_key;
pub(crate) use job_dupe::exact_dupe_key;
pub use job_dupe::is_proper;
...
#[path = "job_publish.rs"]
mod job_publish;
pub use job_publish::DirClaim;
pub use job_publish::choose_out_dir;
pub(crate) use job_publish::publish_over_previous;
pub use job_publish::recover_interrupted_publishes;
pub use job_publish::refile_out_dir;
pub(crate) use job_publish::same_dir;
...
/// in memory either way; `durable: false` means the write of queue.jsonl
