...
#[test]
fn set_index_fold_secs_validates_and_stores() {
    let (_t, d) = daemon_in("fold_secs");
    d.index_fold_secs
        .store(Daemon::FOLD_SECS_DEFAULT, Ordering::Relaxed);
    let e = set_index_fold_secs(&d, "index_fold_secs", "x").unwrap_err();
    assert!(e.contains("not a number"), "{e}");
    // 0 is refused: the fold has no "off", its cursor must follow the top.
    let e = set_index_fold_secs(&d, "index_fold_secs", "0").unwrap_err();
    assert!(e.contains("seconds per fold slice"), "{e}");
    let e = set_index_fold_secs(&d, "index_fold_secs", "31").unwrap_err();
    assert!(e.contains("seconds per fold slice"), "{e}");
    assert_eq!(
        d.index_fold_secs.load(Ordering::Relaxed),
        Daemon::FOLD_SECS_DEFAULT
    );
    assert_eq!(
        set_index_fold_secs(&d, "index_fold_secs", " 10 ").unwrap(),
        (true, json!(10))
    );
    assert_eq!(d.index_fold_secs.load(Ordering::Relaxed), 10);
}
...
fn set_predb_server_shape_checks_and_default_fallback() {
    ...
    assert_eq!(*d.predb.server.lock_ok(), "irc.example.net:7000");
    ...
    assert_eq!(*d.predb.server.lock_ok(), nzbkit::predb::DEFAULT_HOST);
}
...
fn set_arr_giveup_threshold_caps_at_the_evidence_cap() {
    ...
    assert!(e.contains("not a number"), "{e}");
    let cap = crate::giveup::MAX_STEMS as u64;
    assert_eq!(
