...
fn nothing_writes_through_until_somebody_asks_for_it() {
    ...
    *d.move_completed_cats.write_ok() = vec![("tv".into(), nas.clone())];
    ...
    assert_eq!(
        crate::naming::out_dir(&d).join("tv").join("Some.Release"),
...
/// a mover whose relative path is `strip_prefix(crate::naming::out_dir())` - which a
...
fn a_destination_side_path_is_not_a_stale_download_folder() {
    ...
    let stale = |cur: &Path| {
        !cur.starts_with(crate::naming::out_dir(&d))
            && !d
                .move_dest_root("tv")
    ...
    assert!(
        !stale(&crate::naming::out_dir(&d).join("tv").join("Some.Release")),
...
fn the_write_through_record_survives_a_restart() {
    ...
    let back = crate::job::job_from_json(&crate::job::job_json(&j)).unwrap();
    assert!(
        back.write_through,
        "the whole point is that a restart still knows"
    );

    let old = crate::testutil::job(serde_json::json!({
...
fn the_category_rule_list_is_spelled_the_way_a_category_is() {
    use crate::fsutil::parse_cat_names;
    assert_eq!(
...
fn enqueue_creates_a_write_through_job_in_the_destination() {
    ...
    assert!(!tv.starts_with(crate::naming::out_dir(&d)));
...
/// BUG (HIGH, data loss): a completed payload whose history row the
/// user cleared was handed straight back to a re-add of the same
/// release, which truncated it in place at the first decoded article.
///
/// The trigger is a routine housekeeping button, not an exotic API
/// call: the dashboard's "clear the finished downloads" posts
/// `mode=history&name=delete` with NO `del_files`, under a confirm that
/// promises no files are deleted - so the row goes and the payload
/// stays. `dir_claim` answers from the queue and history alone, so with
/// no row naming it the folder read as Free, `choose_out_dir` took it
/// as-is, and nothing downstream disambiguates: no `read_dir` seeds the
/// writer's name set on a job with no journal, so the inner writer
/// opens the same-named file with `LeafOpen::Truncate`. Under TODO 317
/// that folder is not the daemon's own download root, it is the user's
/// library.
///
/// The fix is `dir_claim_for_add`: an occupied directory no record
/// names is `Occupied` for a FRESH add, so the re-add downloads beside
/// it and NEVER takes the canonical name. Not `Payload` - that answer
/// records a `replaces`, and `publish_over_previous` cashes a
/// `replaces` on success by renaming the canonical directory aside and
/// deleting it, which on a folder the daemon has no record of is the
/// same data loss from the other end. The kept payload here has to
/// survive this job FINISHING, not just starting.
#[test]
fn a_reblasted_release_never_downloads_into_a_kept_payload() {
    let s = scratch("kept-payload");
    let nas = s.join("nas");
    let d = armed(&s.join("d"), "tv", &nas, false);

    // The previous, verified result, still in the user's library after
    // they cleared the history row that named it. NOTHING in the queue
    // or history names this directory.
    let canon = nas.join("Write.Through.Test");
    std::fs::create_dir_all(&canon).unwrap();
    std::fs::write(canon.join("payload.mkv"), b"the verified payload").unwrap();
    assert!(d.queue.lock_ok().is_empty() && d.history.lock_ok().is_empty());

    let nzb = "<?xml version=\"1.0\"?>\n<nzb xmlns=\"http://www.newzbin.com/DTD/2003/nzb\">\
               <file poster=\"x\" date=\"0\" subject=\"&quot;a.bin&quot; yEnc (1/1)\">\
               <groups><group>g</group></groups><segments>\
               <segment bytes=\"1000\" number=\"1\">wt2@x</segment>\
               </segments></file></nzb>";
    d.enqueue(
        nzb.as_bytes(),
        "Write.Through.Test.nzb",
    ...
    let job = d.queue.lock_ok().front().cloned().expect("a queued job");
    let g = job.lock_ok();
    assert_eq!(
        nas.join("Write.Through.Test.2"),
        g.out_dir,
        "the re-add downloads BESIDE the kept payload, not into it"
    );
    assert_eq!(
        None, g.replaces,
        "and records NO replacement: nothing in the daemon names that \
         folder, so it is not ours to publish over. A `replaces` here \
         would have `publish_over_previous` rename it aside and \
         `remove_dir_all` it the moment this job verified"
    );
    drop(g);
    ...
    assert_eq!(
        ...
        std::fs::read(canon.join("payload.mkv")).expect("the previous payload"),
    );
}
