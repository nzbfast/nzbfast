...
use super::*;
// The parent's copy of this import moved to par2repair/fastpar.rs with
// `ntt_stripe_geometry`, so name it here rather than through
// `super::*`. Same cfg as its one call site: a Windows core census,
// and off Windows nothing asks the question.
...
impl Reconstructor {
    ...
    pub fn new_with_path<D: AsRef<[u8]>>(
        ...
        let structured = if consecutive {
            if forney::backsub_gate(missing.len()) {
                ForneyPlan::prepare(&ks, recovery[0].0).map(BackSub::Forney)
            } else {
                invert_vandermonde(&ks, recovery[0].0).map(BackSub::Dense)
            }
        } else {
            None
        };
        let (label, solve) = match structured {
            ...
            None => {
                ...
                let a: Vec<Vec<u16>> = recovery
                    ...
                    .collect();
                ("gauss-jordan", BackSub::Dense(invert(a)?))
            }
        };
        ...
        let worker = std::thread::spawn(move || {
            ...
            (syndromes, retained, ntt_windows, ntt_window_used)
        });
        Ok(Reconstructor {
            block_size,
    ...
    pub fn finish_reported(mut self) -> (Vec<Vec<u8>>, SyndromeReport) {
        self.flush();
        drop(self.tx.take());
        let (mut syndromes, retained, ntt_windows, ntt_window_used) = self
            .worker
            .take()
...
fn ntt_syndromes_into(
...
) -> (bool, usize) {
    {
        ...
        let syn = SynPtrs(
            ...
            exponents.iter().map(|&e| e as usize).collect(),
        );
