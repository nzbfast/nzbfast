...
/// the review's read-only sweep (M7, 10 Aug 2026).
...
/// The ROT13 and reversed-stem rescue passes, out of this file for the
/// size gate's 4,000-line ceiling.
mod rescue;
use rescue::{reversed_rescue, rot13, rot13_rescue, volume_tail_cut};

/// Formatting a [`Parsed`] back into text - badges, suffixes, file base
/// names. Same ceiling. Re-exported so callers keep their paths.
mod render;
pub use render::{NameStyle, movie_name, quality_label, quality_suffix, sanitize_name};

/// Reading and writing dates. Same ceiling; same re-export.
mod dates;
pub use dates::air_date_parts;
use dates::{date_at, month_issue};
