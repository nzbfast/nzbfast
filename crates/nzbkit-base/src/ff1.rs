...
//!
//! STATED LIMIT: the UPPER length bound is the caller's too. FF1 has no
//! maximum in the standard, and this implementation has none either, but
//! the digit conversions are quadratic in `n` (num-bigint's
//! `from_radix_digits_be` is a per-chunk multiply-add over a growing
//! limb vector, twice per round for ten rounds) and the minlen check
//! plus the `radix^u` / `radix^v` moduli allocate BigUints of that same
//! order before the first round. A caller that feeds attacker-chosen
//! lengths in here owes its own cap; `yencrypt`'s is `MAX_CONTROL_LINE`.
