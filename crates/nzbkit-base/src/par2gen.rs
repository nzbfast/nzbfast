...
/// and the batched path's a-priori rule - split out on 17 Sep 2026 for
/// the 4,000-line file ceiling.
mod fold_pace;
...
mod scan;
/// The create request validated and the set geometry it implies -
/// everything before the admission gate - split out on 17 Sep 2026 for
/// the 500-line ceiling on `create_body`.
mod shape;
mod stripe_first;
...
pub enum Par2GenError {
    ...
    /// helper, and most of its 33 sites are WRITES - the create of a
    /// volume (`control::CreateTrail::create`, which `volwrite` and
    /// `stripe_first` both open through) and the open-for-write of
...
/// Everything about a create that is the CALLER's choice rather than a
/// property of the payload: how the recovery slices are split into
/// volumes, how many copies of the critical block each volume carries,
/// and whether a file already sitting under one of the set's names may
/// be written over.
///
/// One value threaded through one parameter, so a fourth knob lands
/// here rather than growing a fourth argument. Every engine caller
/// wants [`CreatePlan::ENGINE`]; the drop-in CLI is the only thing that
/// asks for anything else.
///
/// The first three are LAYOUT and `no_clobber` is not, which is why
/// this doc no longer says the word: it is here because it is `Copy`,
/// fieldless and set at the one place the CLI already translates its
/// switches into a plan, and the alternative was a ninth positional
/// argument on the one door that would carry it. The two knobs that
/// really could not live here say so at their own doors
/// ([`create_into_exact_with_comment`], [`create_into_exact_controlled`]).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct CreatePlan {
    ...
    /// Refuse to write over a file that is already under one of this
    /// set's names, instead of truncating it.
    ///
    /// `false` - truncate, as `File::create` always has - is what every
    /// engine caller wants and what par2cmdline itself does, so it is
    /// the default and a create that says nothing keeps the behaviour
    /// it has always had. `true` opens every member with `O_EXCL`, so
    /// an existing file unwinds the create with
    /// [`std::io::ErrorKind::AlreadyExists`] and is neither written nor
    /// removed by a later cancel (`control::CreateTrail`).
    ///
    /// **This is the only no-overwrite guard that is not a preflight.**
    /// A caller that stats the directory first answers for the instant
    /// it looked; two creates started together on one base, or a create
    /// racing anything else writing there, walk straight through that
    /// check and both truncate. The engine's own open is the only place
    /// the question can be asked and answered in one step, which is why
    /// it is asked here.
    ///
    /// It decides nothing about NAMES, so [`plan_files`] ignores it -
    /// the planned set is the same set either way, and the difference
    /// is only whether writing it is allowed to destroy anything.
    pub no_clobber: bool,
}
...
impl CreatePlan {
    ...
    /// This plan refusing, or allowing, an overwrite of a file already
    /// under one of the set's names. See [`CreatePlan::no_clobber`].
    pub const fn with_no_clobber(self, no_clobber: bool) -> CreatePlan {
        CreatePlan { no_clobber, ..self }
    }
}
...
/// Write one whole file of the set through the trail's one door, so it
/// is created, noted and filled by a single call.
///
/// The index is the only set member small enough to hold in memory and
/// write in one go; the volumes stream through `BufWriter`s of their
/// own and open the same door directly.
fn write_member(
    trail: &CreateTrail,
    dir: &Path,
    name: &str,
...
fn create_into_inner(
    ...
    // The plan's no-clobber decision reaches every file of the set
    // through the trail, which is the one door they are all created by.
    let trail = CreateTrail::new(plan.no_clobber);
    let r = create_body(
        dir,
        members,
        base,
...
fn create_body(
    ...
    let shape::CreateShape {
        ...
        n_slices,
        n_recovery,
    } = shape::resolve(members, spec, exact_recovery, control)?;
    ...
    if n_recovery == 0 {
        ...
        control.check()?;
        // Created and noted in ONE call: a cancel that lands after the
        // open must still find the file, and a no-clobber create that
        // was REFUSED the open must never have named somebody else's
        // index as its own (`CreateTrail::create`).
        write_member(trail, dir, &index, &critical)?;
    ...
    // As above, and this is the site the no-clobber door most often
    // answers on: the index is the FIRST file of the set to be created,
    // so two creates started together on one base meet here and exactly
    // one of them gets past it.
    write_member(trail, dir, &index, &critical_shape)?;
    ...
    let batches: Result<(), Par2GenError> = std::thread::scope(|sc| {
        ...
        let mut body = || -> Result<(), Par2GenError> {
            ...
            //
            // Grouped ONCE, up front, rather than greedily inside the
            // loop that runs them, because the COUNT is a fact a bar
            // needs before the first batch: `recovery_slices` re-sizes
            // `CreatePhase::Fold` per batch, so a caller that is not
            // told how many there will be either draws 0 to 100 once
            // per batch or reserves no headroom for batches 2..N and
            // has to take the bar backwards to make room. That is the
            // same defect the repair's slabbed solve had until 16 Sep
            // 2026 (`ProgressSink::slab`), and this is the same fix
            // through the same channel.
            let groups = fold_batches(&layout, per_batch);
            for (bn, &(vi, vj)) in groups.iter().enumerate() {
                ...
                control.gate()?;
                let held: usize = layout[vi..vj].iter().map(|v| v.1).sum();
                let first = layout[vi].0;
                // Before this batch's `begin(Fold, ..)`, which is the
                // order `ProgressSink::slab` documents.
                control.batch(bn, groups.len());
...
/// Group a volume layout into the fold batches that will run over it,
/// as `(first volume, one past the last)` index pairs.
///
/// One volume already fits the accumulator budget by construction, so
/// the first volume of a batch is taken whatever it costs and the rest
/// are taken while they still fit `per_batch` recovery rows between
/// them. The rule is the one the batch loop ran inline until 17 Sep
/// 2026; it moved here so the COUNT can be read before the first batch
/// starts - see the call site for why a bar needs that, and
/// [`crate::par2repair::ProgressSink::slab`] for the channel it is
/// announced on.
///
/// `layout` is `(first exponent, rows)` per volume. An empty layout
/// gives no batches, which is the `n_recovery == 0` create that never
/// reaches here.
fn fold_batches(layout: &[(usize, usize)], per_batch: usize) -> Vec<(usize, usize)> {
    let mut out = Vec::new();
    let mut vi = 0usize;
    while vi < layout.len() {
        let mut vj = vi;
...
mod fold_batch_tests {
    use super::fold_batches;

    /// The grouping rule, which ran inline in the batch loop until
    /// 17 Sep 2026: the first volume of a batch is taken whatever it
    /// costs (one volume fits the accumulator by construction), and the
    /// rest are taken while the rows between them still fit.
    #[test]
    fn a_batch_takes_its_first_volume_whatever_it_costs() {
        // One volume of 50 rows against a 10-row budget: it goes alone
        // rather than refusing, and the next batch starts after it.
        let layout = [(0usize, 50usize), (50, 4), (54, 4)];
        assert_eq!(fold_batches(&layout, 10), vec![(0, 1), (1, 3)]);
    }

    /// The count is the thing the call site needs BEFORE the first
    /// batch, and it is the number of passes the fold will make.
    #[test]
    fn the_count_is_the_number_of_passes() {
        let layout: Vec<(usize, usize)> = (0..8).map(|i| (i * 4, 4)).collect();
        assert_eq!(fold_batches(&layout, 8).len(), 4);
        assert_eq!(fold_batches(&layout, 32).len(), 1);
        // Every volume is covered exactly once, in order, whatever the
        // budget - a batch that dropped one would lose its recovery
        // rows silently.
        for budget in [1usize, 3, 8, 9, 32, 1000] {
            let groups = fold_batches(&layout, budget);
            let mut at = 0usize;
            for (vi, vj) in groups {
                assert_eq!(vi, at);
                assert!(vj > vi);
            ...
            assert_eq!(at, layout.len());
        }
    }

    /// The `-r0` create never reaches the batch loop, and an empty
    /// layout answers with no passes rather than one empty one.
    #[test]
    fn an_empty_layout_has_no_batches() {
        assert!(fold_batches(&[], 8).is_empty());
    }
}
