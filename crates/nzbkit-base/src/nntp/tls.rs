...
///
/// WHAT THE PIN IS WORTH, END TO END (3 Sep 2026, round 15: the same
/// 24 GB loopback download run pinned and with `NZBFAST_TLS_AES256=1`,
/// median of 3). M3 Ultra: **+12% of the client's user CPU** for
/// AES-256 (0.538 -> 0.605 s/GB) and +6% instructions. EPYC 9354P:
/// nothing measurable - +1% instructions, user CPU inside the spread.
/// The split is VAES: aws-lc-rs runs four blocks per instruction on
/// Zen 4, which absorbs 14 rounds as easily as 10, while the ARMv8
/// crypto extensions take one block at a time and pay for every extra
/// round. So this pin is a real per-byte win on Apple silicon and the
/// phone targets, and free insurance everywhere else. `aes_accelerated`
/// below is not the same question and does not see this: it asks
/// whether AES exists at all, which is what the ChaCha20 fallback
/// needs.
