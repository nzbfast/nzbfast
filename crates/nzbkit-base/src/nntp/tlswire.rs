...
///
/// RE-LADDERED 3 Sep 2026 at the fleet size that ships (16 connections,
/// 24 GB set, three reps, round 15), because the 22 Aug figure was an
/// 8-connection reading and a per-connection buffer is exactly the knob
/// whose knee could move with the fleet. It did not. Instructions per
/// GB on the M3: raw socket 7.691, 16 KB 7.879, **64 KB 7.629**, 256 KB
/// 7.587 - 256 KB buys 0.6% for 4x the memory, and 16 KB is worse than
/// no buffer at all (below one record's plaintext it amortises
/// nothing). On the EPYC VPS every rung sat inside the run-to-run
/// spread. Unchanged, now on two kernels.
///
/// AND THIS BUFFER'S COPY IS THE BIGGEST SINGLE `memcpy` ON THE TLS
/// PATH - 38-40% of it, more than either plaintext copy (round 15's
/// caller split, both M3 arms agreeing over 236 and 414 samples). Not
/// an argument against it: that is exactly what the ladder above
/// measures, since `0` removes this copy outright and costs +0.8%
/// instructions, because copying the ciphertext is cheaper than the
/// syscalls it buys. Stated here so the next reader who profiles a TLS
/// download, finds `memmove` at the top and reaches for the note below
/// knows this copy is the larger half and rustls' plaintext copy the
/// smaller one.
///
