...
///
/// The plain side's buffer is our own [`wirebuf::WireBuf`], not
...
enum Wire {
    ...
    Buffered(wirebuf::WireBuf<Box<dyn Transport>>),
}

impl Wire {
    fn buffered(t: Box<dyn Transport>) -> Wire {
        Wire::Buffered(wirebuf::WireBuf::with_capacity(256 * 1024, t))
    }
...
/// The plain side reads direct when `NZBFAST_WIRE_DIRECT` asks (up to
/// its buffer's size per read, so the overrun always fits back); the
/// TLS side never does - see the note on [`Wire`].
impl MultilineSource for Wire {
    fn direct_cap(&self) -> usize {
        match self {
            Wire::Buffered(b) => wirebuf::direct_read_cap().min(b.capacity()),
            Wire::Tls(_) => 0,
        }
    }
    fn buffered(&self) -> &[u8] {
        match self {
            Wire::Buffered(b) => b.buffer(),
            Wire::Tls(_) => &[],
        }
    }
    fn unread(&mut self, tail: &[u8]) {
        match self {
            Wire::Buffered(b) => b.unread(tail),
            Wire::Tls(_) => unreachable!("TLS never reads direct"),
        }
    }
}
...
#[doc(hidden)]
pub async fn dial_in_order<T, F, Fut>(addrs: &[std::net::SocketAddr], one: F) -> std::io::Result<T>
where
...
impl Connection {
    ...
    /// memmove. Whole chunks since, scanned in place against the two
    /// bytes before them (`read_multiline_paced_noting`); the plain
    /// path CAN read the socket straight into `out`, but that is off
    /// by default (`wirebuf::direct_read_cap`).
...
// The direct (into-the-Vec) body read and its in-place terminator
// scan across read boundaries, same pattern.
#[cfg(test)]
mod direct_read_tests;
...
// DNS fault families (§129 3a) were `mod resolve_tests;` here until the
// nzbkit-base cut on 3 Sep 2026. Four of their tests drive a fleet
// through `pool::fetch_all_multi`, and `pool` is a SIBLING of this
// crate, not something under it - so the file went up to the facade
// whole, as `nzbkit`'s own `nntp_resolve_tests`. The three items it
// reaches here (`dial_in_order`, `CandidateWalk`, `dial_candidate_cap`)
// are `#[doc(hidden)] pub` for it and for nothing else.
