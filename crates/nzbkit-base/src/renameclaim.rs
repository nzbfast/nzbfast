...
//!
//! Two doors in this crate and seven in `nzbfast` rename a file onto a
...
//! COPIED, not shared, from `nzbfast-unpack/src/renameclaim.rs`, which is
//! where the two doors' siblings are pinned and where the sweep
//! decision above was measured. A crate cannot see another crate's
//! `#[cfg(test)]` items and making this `pub` in a release build would
//! be a test harness on a shipped surface; the two are BYTE-IDENTICAL below this module
//! doc, so `diff <(tail -n +N a) <(tail -n +M b)` settles whether they
//! still are. `testscratch.rs` beside this
//! one is duplicated per crate for the same reason and says so. Keep
...
pub fn never_renames_over_a_neighbour(
    target: &Path,
    trials: usize,
