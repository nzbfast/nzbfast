...
pub struct ServerConfig {
    ...
    /// TODO 313 item 8: connections kept AUTHENTICATED AND PARKED on
    /// this server with nothing to do, as a standing surge source.
    ///
    /// `None` or 0 is OFF, which is the default and changes nothing:
    /// with no reserve asked for, the pool below behaves exactly as it
    /// did before this field existed.
    ///
    /// **Not the same quantity as [`Self::idle_keep`], which is two
    /// fields above and reads almost identically.** `idle_keep` is a
    /// floor on what a RELEASE keeps - how much of an already-parked set
    /// survives the idle trim - so it can never cause a dial and can
    /// never exceed what some job already parked. This is a floor a
    /// background dialler MAINTAINS, so it can put a socket on the wire
    /// that no job asked for, which is why it is bounded by the
    /// account's own lease and `idle_keep` is not.
    ///
    /// A REQUEST and not a guarantee. The effective number is bounded by
    /// what the account's [`connections`](Self::connections) leave
    /// spare after the live fleet, so on a server whose fleet runs at
    /// max it is zero - see `nzbkit::warmreserve` for the whole rule and
    /// for where the effective figure is published.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub warm_reserve: Option<u32>,
}
...
// THE FIVE FLEET LIMITS BELOW moved down here from `pool` and
// `warmpool` on 3 Sep 2026 (nzbkit split lane 1), each re-exported from
// its old home so every `MAX_SERVERS` and
// `nzbkit::warmpool::MAX_PER_SERVER` path is unchanged. They are
// constants this file VALIDATES and CLAMPS against - `Config::load`
// refuses a server list longer than `MAX_SERVERS`, and
// `idle_release_policy` picks between the two timeouts and clamps `keep`
// to `MAX_PER_SERVER` - so declaring them upstairs made the
// configuration layer depend on the connection pool, which was five of
// the six edges standing between nzbkit and a partition. Every number's
// rationale travelled with it, unchanged.
...
pub const DEFAULT_IDLE_RELEASE: std::time::Duration = std::time::Duration::from_secs(300);
...
pub const CAPPED_IDLE_RELEASE: std::time::Duration = std::time::Duration::from_secs(120);
...
pub const DEFAULT_MAX_IDLE: std::time::Duration = std::time::Duration::from_secs(600);
...
impl ServerConfig {
    ...
    /// `crates/nzbkit-base/src/nntp.rs` builds that `ServerName` at one site
        ...
        ReleasePolicy {
            after: match self.idle_release_secs {
                ...
                None => Some(match capped {
                    true => CAPPED_IDLE_RELEASE,
                    false => DEFAULT_IDLE_RELEASE,
                }),
                Some(0) => None,
            ...
            keep: match self.idle_keep {
                ...
                Some(n) => (n as usize).min(MAX_PER_SERVER),
            },
        }
    }
}
...
impl Config {
    ...
    /// the same reason, as `storage_override` in `crates/nzbkit-base/src/disk.rs`:
    ...
    fn load_or_sab(
        ...
        match std::fs::read(path) {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                ...
                // `crates/nzbfast-core/src/logging.rs` gives stderr WARN and
...
mod warm_pool_default_tests {
    ...
    fn one_strict_provider_does_not_drag_down_a_lax_one() {
        ...
        assert_eq!(lax.after, Some(DEFAULT_IDLE_RELEASE));
        assert_eq!(lax.keep, 1, "the lax provider keeps a warm connection");
        assert_eq!(strict.after, Some(CAPPED_IDLE_RELEASE));
        assert_eq!(
            strict.keep, 0,
    ...
    fn explicit_settings_win_and_do_not_drag_each_other() {
        ...
        assert_eq!(p.after, Some(CAPPED_IDLE_RELEASE));
    ...
    fn a_keep_floor_above_the_pool_cap_is_clamped_not_ignored() {
        ...
        assert_eq!(s.idle_release_policy().keep, MAX_PER_SERVER);
    ...
    fn neither_default_releases_fast_enough_to_defeat_the_pool() {
        for d in [DEFAULT_IDLE_RELEASE, CAPPED_IDLE_RELEASE] {
            assert!(
...
"#;
    ...
    /// It must be stripped (sanitize_filename does not map `*` on unix -
    /// only the Windows arm added for F15 does - so it would otherwise
    /// become a real folder with a star in its name) and the behaviour
    /// must be reported as not carried over.
    #[test]
