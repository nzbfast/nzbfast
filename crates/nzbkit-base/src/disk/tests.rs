...
/// `read_covered_at` serves written bytes and refuses a hole - the
/// distinction a plain pread cannot make, since a sparse hole and a run
/// of written zeros read identically. The mapped container path reads
/// through this so a racing reader gets an error it can retry instead of
/// zeros it will hash (research/MEASURED-2026-09-03-zip-mapped-damaged-container-races.md,
/// "Race 2").
#[test]
fn read_covered_at_refuses_a_hole_a_plain_pread_would_serve_as_zeros() {
    let dir = std::env::temp_dir().join(format!("nzbfast-covread-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("holey.bin");
    let w = FileWriter::create(&path, 12).unwrap();
    // A written run of ZEROS either side of a real hole: the bytes and
    // the hole are byte-identical on read, so only coverage separates
    // them and this test would pass vacuously without them.
    w.write_at(0, &[0u8; 4]).unwrap();
    w.write_at(8, &[0u8; 4]).unwrap();

    let mut buf = [0xAAu8; 4];
    w.read_covered_at(&mut buf, 0).unwrap();
    assert_eq!(buf, [0u8; 4], "written zeros are data and must be served");

    let mut buf = [0xAAu8; 4];
    let e = w.read_covered_at(&mut buf, 4).unwrap_err();
    assert_eq!(e.kind(), io::ErrorKind::NotFound, "{e}");
    // The unguarded read is what shipped, and it cannot tell the two
    // apart - which is the defect, recorded here so a future reader does
    // not "simplify" the guard away.
    let mut buf = [0xAAu8; 4];
    w.read_at(&mut buf, 4).unwrap();
    assert_eq!(buf, [0u8; 4], "a plain pread serves the hole as zeros");

    // A range STRADDLING the hole fails too - partial coverage is not
    // coverage.
    let mut buf = [0xAAu8; 8];
    assert_eq!(
        w.read_covered_at(&mut buf, 0).unwrap_err().kind(),
...
fn storage_probe_never_guesses_rotational() {
    ...
    // A checkout on an SMB/NFS share is a real configuration (and the
    // one this probe gained a variant for), so the non-Linux arm allows
    // `Network` beside `Unknown` - what it still refuses is a guessed
    // `Rotational`.
    #[cfg(not(target_os = "linux"))]
    assert!(
        matches!(here, Storage::Unknown | Storage::Network),
        "only Linux exposes the rotational flag, got {here:?}"
    );
    #[cfg(target_os = "linux")]
    assert!(
        matches!(
            here,
            Storage::Solid | Storage::Unknown | Storage::Rotational | Storage::Network
        ),
        "{here:?}"
    );
}
...
fn rotational_clamp_only_bites_nas_class_boxes() {
    let d = |class| StorageProbe {
        class,
        direct_dev: true,
    };
    assert_eq!(decoders_for_storage(d(Storage::Rotational), 4, 4), 1);
    assert_eq!(decoders_for_storage(d(Storage::Rotational), 2, 8), 1);
    // Big box: a rotational device here is usually a wide array.
    assert_eq!(decoders_for_storage(d(Storage::Rotational), 8, 4), 4);
    assert_eq!(decoders_for_storage(d(Storage::Rotational), 32, 4), 4);
    // Never clamp on anything we did not positively identify as spinning.
    assert_eq!(decoders_for_storage(d(Storage::Unknown), 2, 4), 4);
    // A network mount reported `Unknown` here until `Network` was split
    // out on 3 Sep 2026 for the read-side cache policy. The clamp rule
    // must not have changed under those mounts: a NAS-class box writing
    // to an SMB share keeps all four decoders, exactly as before.
    assert_eq!(decoders_for_storage(d(Storage::Network), 2, 4), 4);
    assert_eq!(decoders_for_storage(d(Storage::Network), 32, 8), 8);
    assert_eq!(decoders_for_storage(d(Storage::Solid), 2, 4), 4);
    // Already serial, or explicitly asked for one: nothing to say.
    assert_eq!(decoders_for_storage(d(Storage::Rotational), 2, 1), 1);
}

/// A `Rotational` the probe reached only through the MOUNT TABLE does
/// not clamp, and that is the whole of TODO 325's stand-down: closing
/// the anonymous-device hole made this class visible on btrfs and ZFS,
/// where the copy-on-write allocator and not our `pwrite` offset decides
/// where a block lands, and where no box on this fleet can measure the
/// clamp anyway. Everything else about the rule is unchanged, so the
/// same NAS-class box on ext4 still clamps.
#[test]
fn the_decoder_clamp_stays_on_the_directly_probed_device() {
    let via_mount = StorageProbe {
        class: Storage::Rotational,
        direct_dev: false,
    };
    assert_eq!(decoders_for_storage(via_mount, 2, 4), 4);
    assert_eq!(decoders_for_storage(via_mount, 4, 8), 8);
    // The same box, same class, read off the device itself.
    assert_eq!(
    ...
    // The operator's own way in stays open: `probe_storage` marks an
    // `NZBFAST_STORAGE` answer `direct_dev`, so `=rotational` still
    // reaches this clamp. Asserted at `storage_override`'s own test
    // rather than here - reading the variable would race every other
    // test in this process.
}

/// The two readings of the probe are one probe: `detect_storage` is
/// exactly `probe_storage(..).class` for whatever this box is, so a
/// caller that wants only the class cannot get a different answer from
/// one that wants both.
#[test]
fn the_thin_and_wide_readings_of_the_probe_agree() {
    for p in [Path::new("."), Path::new("/nonexistent-nzbfast-probe")] {
        assert_eq!(detect_storage(p), probe_storage(p).class, "{p:?}");
    }
    assert!(
        probe_storage(Path::new("/nonexistent-nzbfast-probe")).direct_dev,
        "a path with nothing to probe took no fallback"
    );
}
...
fn sanitize() {
    ...
    // F15: the rest of the Win32/NTFS illegal set. These carry no path
    // meaning - they simply make the create FAIL, because NTFS validates
    // the leaf name and NtCreateFile answers STATUS_OBJECT_NAME_INVALID
    // (ERROR_INVALID_NAME, 123), which `winbind::create_at` propagates
    // out of `FileWriter::create_under`. A poster-declared "Who Are
    // You?.flac" is an ordinary music/TV name, so before this the
    // DOWNLOAD of that slot failed on Windows with an OS error. Asserted
    // through the `_for` seam so it holds on the Mac and Linux boxes.
    for s in [
        "Who Are You?.flac",
        "What*.mkv",
        "\"quoted\".mkv",
    ...
    assert_eq!(
        sanitize_filename_for("Who Are You?.flac", true),
        "Who Are You_.flac"
    );
    // ...and the counter-arm: all six are legal and ordinary on unix, so
    // widening the map to every platform would corrupt a legitimate name.
    for s in [
        "Who Are You?.flac",
        "What*.mkv",
        "\"quoted\".mkv",
        ...
        assert_eq!(sanitize_filename_for(s, false), s, "{s:?} was altered");
    }
...
// ---------------------------------------------------------------- stage
//
// The write-coalescing window (round 41). Every test here drives
// [`FileWriter`] directly and pins one half of the contract the window
// has to keep: FEWER CALLS, and NOTHING ELSE MOVES.

/// One scratch directory per staging test, swept by the shared guard -
/// see `crate::testscratch`.
fn stage_dir(tag: &str) -> crate::testscratch::ScratchDir {
    crate::testscratch::ScratchDir::attach(
        &std::env::temp_dir().join(format!("nzbfast-{tag}-{}", std::process::id())),
    )
}

/// The point of the window, stated as a number: a run of contiguous
/// articles becomes ONE positioned write, and the bytes on disk are the
/// same bytes the unwindowed writer would have left.
///
/// Both arms run in this one process against the same input, so the
/// comparison cannot be an artifact of a latched environment variable
/// (`without_coalescing` is the control - see it).
#[test]
fn coalescing_turns_a_run_of_articles_into_one_write() {
    ...
    let dir = stage_dir("coalesce-one-write");
    let art = vec![7u8; 4096];
    let n = 16u64;

    let w = FileWriter::create(&dir.join("on.bin"), n * art.len() as u64)
        .unwrap()
        .coalescing(true);
    let _busy = w.hold_busy();
    for i in 0..n {
        w.write_article_at(i * art.len() as u64, &art).unwrap();
    }
    w.sync().unwrap();
    let on = w.writes();

    let c = FileWriter::create(&dir.join("off.bin"), n * art.len() as u64)
        .unwrap()
        .coalescing(false);
    let _busyc = c.hold_busy();
    for i in 0..n {
        c.write_article_at(i * art.len() as u64, &art).unwrap();
    }
    c.sync().unwrap();
    let off = c.writes();

    assert_eq!(off, n, "the control still issues one write per article");
    assert_eq!(
        on, 1,
        "sixteen contiguous articles inside one window are one write"
    );
    assert_eq!(
        std::fs::read(dir.join("on.bin")).unwrap(),
        std::fs::read(dir.join("off.bin")).unwrap(),
        "coalescing may change WHEN bytes land, never WHICH"
    );
}

/// Out-of-order arrival is the NORMAL case across sixteen connections,
/// and the window's answer to it is a second run rather than a write -
/// the whole difference between this design and the single-run one that
/// measured as coalescing nothing. Whatever it holds still lands, in the
/// right places.
#[test]
fn a_gap_opens_a_second_run_rather_than_forcing_a_write() {
    ...
    let dir = stage_dir("coalesce-gap");
    // Declared larger than what is written, so the completion rule (an
    // article that finishes the file flushes at once) stays out of it.
    let w = FileWriter::create(&dir.join("gap.bin"), 1 << 20)
        .unwrap()
        .coalescing(true);
    let _busy = w.hold_busy();
    w.write_article_at(0, b"aaaa").unwrap();
    // Not the run's next byte: a SECOND run opens beside the first.
    w.write_article_at(8, b"cccc").unwrap();
    assert_eq!(w.writes(), 0, "a gap costs a run, not a write");
    // And the article that fills it makes the three one run, so the
    // whole span is a single write when it goes out.
    w.write_article_at(4, b"bbbb").unwrap();
    w.sync().unwrap();
    assert_eq!(
        w.writes(),
        1,
        "twelve bytes delivered out of order by three articles, one write"
    );
    let mut got = [0u8; 12];
    w.read_at(&mut got, 0).unwrap();
...
/// The rule every reader in the tree depends on: a STAGED byte is not a
/// covered byte. Publishing coverage early would tell the live verifier,
/// the streaming frontier and §296's early publish that a hole is
/// payload - which is the one failure mode this design must not have.
#[test]
fn a_staged_span_is_never_published_as_covered() {
    ...
    let dir = stage_dir("coalesce-cover");
    let w = FileWriter::create(&dir.join("cov.bin"), 8)
        .unwrap()
        .coalescing(true);
    let _busy = w.hold_busy();
    w.write_article_at(0, b"abcd").unwrap();
    assert_eq!(
        w.written(),
        0,
        "nothing has been written through the handle yet"
    );
    // `covered` flushes an OVERLAPPING run rather than answering false
    // about bytes it is holding, so ask about a range that does not
    // touch it.
    assert!(!w.covered(4, 4), "the untouched half is still a hole");
    // And the answer for the staged range itself is true only because
    // asking WROTE it.
    assert!(w.covered(0, 4));
    assert_eq!(w.written(), 4, "asking about the range wrote the run out");
}

/// Every door that hands back BYTES flushes first: the writer's own
/// pread, and the raw descriptor `open_read` gives a live reader (which
/// reads the inode, so nothing downstream could flush for it).
#[test]
fn every_read_back_door_sees_staged_bytes() {
    ...
    let dir = stage_dir("coalesce-read");
    let w = FileWriter::create(&dir.join("r.bin"), 8)
        .unwrap()
        .coalescing(true);
    let _busy = w.hold_busy();
    w.write_article_at(0, b"abcdefgh").unwrap();
    ...
    let mut got = [0u8; 4];
    w.read_at(&mut got, 2).unwrap();
    assert_eq!(&got, b"cdef", "read_at must not read under a staged run");

    let w2 = FileWriter::create(&dir.join("r2.bin"), 8)
        .unwrap()
        .coalescing(true);
    let _busy2 = w2.hold_busy();
    w2.write_article_at(0, b"12345678").unwrap();
    let (f, _lease) = w2.open_read().unwrap();
    let mut got = [0u8; 8];
    read_exact_at(&f, &mut got, 0).unwrap();
    assert_eq!(
        &got, b"12345678",
        "a live reader's own handle sees them too"
    );
}

/// A writer that is simply DROPPED - a slot torn down on an error path -
/// still lands its run; a writer that was ABANDONED throws it away,
/// because the file it belongs to has been unlinked.
///
/// Both files are declared far larger than what is written, so the
/// COMPLETION rule (an article that finishes the file writes at once)
/// stays out of it and the close doors are what is being tested.
#[test]
fn drop_lands_the_run_and_abandon_close_throws_it_away() {
    ...
    let dir = stage_dir("coalesce-drop");
    let kept = dir.join("kept.bin");
    {
        let w = FileWriter::create(&kept, 1 << 20).unwrap().coalescing(true);
        let busy = w.hold_busy();
        w.write_article_at(0, b"keep").unwrap();
        // The guard releases without flushing, so the run is still open
        // when the writer goes out of scope - which is the whole point.
        drop(busy);
    }
    assert_eq!(
    ...
    let w = FileWriter::create(&gone, 1 << 20).unwrap().coalescing(true);
    let busy = w.hold_busy();
    w.write_article_at(0, b"gone").unwrap();
    drop(busy);
    ...
    assert_eq!(
        w.writes(),
        0,
        "an unlinked file's run is the one run this module discards"
    );
}

/// The per-run cap is what closes a run, an article at least as big as
/// a whole run never enters the window at all (coalescing it would buy a
/// copy and no call), and a gap FILLED LATE makes two runs one write -
/// the property the single-run first cut lacked and the ladder measured
/// as coalescing nothing at all.
#[test]
fn the_window_merges_across_a_late_gap_and_is_bounded() {
    ...
    // enough for a run to fill; a test driving `offer` directly says so.
    st.arm_for_test();

    // Two runs opened out of order by two connections, with a hole
    // between them - the shape sixteen connections actually produce.
    let (out, held) = st.offer(
        0,
        &[1u8; 200],
        super::stage::Caps {
            file: cap,
    ...
    assert!(out.is_empty() && held);
    assert_eq!(st.spans(), vec![(0, 200), (400, 600)]);

    // The article that fills the hole joins them into ONE run.
    let (out, held) = st.offer(
        200,
        &[2u8; 200],
        super::stage::Caps {
    ...
    assert_eq!(
        st.spans(),
        vec![(0, 600)],
        "a gap filled late is what makes two writes into one"
    );

    // Growing past the per-run cap closes it.
    let (out, held) = st.offer(
        600,
        &[4u8; 500],
        super::stage::Caps {
    ...
    assert!(held);
    assert_eq!(out.len(), 1);
    assert_eq!((out[0].start, out[0].end()), (0, 1100));
    assert!(st.spans().is_empty());

    // An article at least as big as a run is never staged.
    let (out, held) = st.offer(
        2000,
        &[5u8; 1024],
        super::stage::Caps {
...
#[test]
fn staged_bytes_are_charged_to_the_gauge_and_released() {
    // Same guard every staging test above takes, for the reason given
    // there: this one reads the gauge ABSOLUTELY.
    let _g = crate::memgauge::one_gauge_test_at_a_time();
    let dir = stage_dir("coalesce-gauge");
    // Declared LARGER than what is written: an article that completes
    // the file is flushed at once by the completion rule, and this test
    // is about the run that is still open.
    let w = FileWriter::create(&dir.join("g.bin"), 8192)
        .unwrap()
        .coalescing(true);
    let _busy = w.hold_busy();
    let before = crate::memgauge::cur(crate::memgauge::Sub::WriteStage);
    w.write_article_at(0, &[9u8; 4096]).unwrap();
    assert_eq!(w.staged_bytes(), 4096, "the run is open and holding");
    // A DELTA, never the absolute gauge: this is a process counter and
    // `cargo test --lib` runs the whole crate in one process, so a
    // test that reads it absolutely is reading every other test's
    // charges too - which is exactly how this one flaked there while
    // passing under nextest, where each test owns a process.
    assert_eq!(
        crate::memgauge::cur(crate::memgauge::Sub::WriteStage) - before,
        4096,
        "an open run is held memory and says so"
    );
    w.sync().unwrap();
    assert_eq!(w.staged_bytes(), 0);
    assert_eq!(
        crate::memgauge::cur(crate::memgauge::Sub::WriteStage),
        before,
        "and the charge follows the bytes out"
    );
}

/// The window, driven the way sixteen connections drive it: a file's
/// articles offered in a SHUFFLED order, with the runs written out
/// wherever the window says so, reassembled at the end. Every byte must
/// come back exactly once and at its own offset.
///
/// Broad coverage of the window under disorder. It is NOT what pins the
/// index bug below - measured: with that bug restored, all six shuffles
/// still pass, because the ordering it needs does not arise here.
#[test]
fn the_window_reassembles_every_byte_under_shuffled_arrival() {
    ...
    let art = 1500usize;
    let n = 400usize;
    // A cheap deterministic shuffle: a stride coprime with `n` visits
    // every article exactly once in an order that is nothing like
    // ascending, and a different stride per round is a different order.
    for stride in [7usize, 13, 31, 97, 173, 199] {
        let mut st = WriteStage::default();
        // The shipped window arms only once a file has proved it is fed
        // fast enough for a run to fill; a test driving `offer` directly
        // says so.
        st.arm_for_test();
        let mut disk = vec![0u8; n * art];
        let wrote = |disk: &mut Vec<u8>, r: &super::stage::StagedRun| {
            let at = r.start as usize;
            disk[at..at + r.buf.len()].copy_from_slice(&r.buf);
        };
        for k in 0..n {
            let i = (k * stride) % n;
            let data = vec![(i % 251) as u8; art];
            let (runs, held) = st.offer(
                (i * art) as u64,
                &data,
                super::stage::Caps {
                    file: cap,
            ...
            if !held {
                // The caller's own write, exactly as `write_article_at`
                // issues it when the window has no room.
                let at = i * art;
                disk[at..at + art].copy_from_slice(&data);
            }
        }
        ...
        for i in 0..n {
            assert_eq!(
                disk[i * art..(i + 1) * art],
                vec![(i % 251) as u8; art][..],
                "stride {stride}: article {i} did not come back at its own offset"
            );
        }
    }
}

/// The merge that shifts the Vec UNDER the index doing the merging.
///
/// A run is joined to the one that continues it by `Vec::remove`, which
/// shifts every LATER index down by one - so when the removed run sat
/// BEFORE the growing one, the growing one's index is now stale. The
/// first multi-run cut shadowed that index instead of correcting it,
/// took the wrong run out, and off the end of the Vec panicked the
/// decode worker holding the window. What that looks like from outside
/// is not a crash: it is a pool that wedges with articles outstanding
/// and reports "0 decode/write errors" while the wire sits idle -
/// measured on the round 23 ladder as 166.9 s blocked on the write side
/// and two files left incomplete.
///
/// The ordering it needs is specific, which is why the shuffled test
/// above does not find it: a run must be opened AHEAD of another, the
/// earlier run opened second, and the article between them must then
/// close the gap from below.
#[test]
fn a_merge_that_removes_an_earlier_run_keeps_the_growing_one() {
    ...
    assert!(out.is_empty() && held);
    // Index 1: the run BEHIND it, opened second.
    let (out, held) = st.offer(
        100,
        &[1u8; 100],
        super::stage::Caps {
    ...
    assert!(out.is_empty() && held);
    // Extends index 1 up to 300, so the run at index 0 is what
    // continues it - the `j < i` case.
    let (out, held) = st.offer(
        200,
        &[2u8; 100],
        super::stage::Caps {
    ...
    assert_eq!(
        st.spans(),
        vec![(100, 400)],
        "three articles delivered back-to-front are one run, and one write"
    );
    let runs = st.take_all();
    assert_eq!(runs.len(), 1);
    assert_eq!((runs[0].start, runs[0].end()), (100, 400));
    assert_eq!(&runs[0].buf[..100], &[1u8; 100]);
    assert_eq!(&runs[0].buf[100..200], &[2u8; 100]);
    assert_eq!(&runs[0].buf[200..], &[3u8; 100]);
}

/// THE POSTCONDITION, stated at the writer where the three `extract/`
/// tests reach it: with the window ON and no feeding signal raised, an
/// article delivered and then read back BY PATH is there.
///
/// This is the property round 41 built `articles_in_flight` for and the
/// one round 44's feeding signal had to preserve rather than trade
/// away. It preserves it by CONSTRUCTION and not by argument: the signal
/// is an `Arc<AtomicBool>` a caller has to hand the writer, so a caller
/// that never mentions it - every library user, every test, and the
/// three `extract/` tests that write one article and `std::fs::read` the
/// output - keeps exactly the rule it had, and this test is that
/// sentence executed.
#[test]
fn an_unfed_writer_still_answers_a_by_path_read_after_one_article() {
    let _gauge = crate::memgauge::one_gauge_test_at_a_time();
    let dir = stage_dir("unfed-postcondition");
    let art = vec![3u8; 4096];
    let p = dir.join("one.bin");
    let w = FileWriter::create(&p, 1 << 20).unwrap().coalescing(true);
    w.write_article_at(0, &art).unwrap();
    assert_eq!(
        w.staged_bytes(),
        0,
        "no signal was raised, so the article that left the file idle wrote the run"
    );
    assert_eq!(
        std::fs::read(&p).unwrap()[..art.len()],
        art[..],
        "the bytes are readable by path the instant write_article_at returns"
    );
}

/// THE SIGNAL, and the size of what it recovers. Round 41 measured the
/// gap: `articles_in_flight` is an INFERENCE, and a fast decoder drives
/// it to zero between arrivals, so a file that is being fed hard looks
/// idle between every pair of articles and the run goes out for nothing.
///
/// Both arms run here in one process over the same articles, delivered
/// one at a time exactly as a decode thread delivers them - no
/// `hold_busy`, because pinning a virtual article in flight is precisely
/// the thing the signal replaces.
#[test]
fn the_feeding_signal_holds_a_run_across_the_idle_the_counter_infers() {
    let _gauge = crate::memgauge::one_gauge_test_at_a_time();
    let dir = stage_dir("feeding-signal");
    let art = vec![9u8; 4096];
    let n = 16u64;
    let size = n * art.len() as u64;

    // FED: the engine's promise is up, so a gap in the in-flight count
    // is not evidence of anything and the run survives it.
    let (w, sig) = FileWriter::create(&dir.join("fed.bin"), size)
        .unwrap()
        .coalescing(true)
        .fed();
    for i in 0..n {
    ...
    // UNFED: the same articles, the same window, the rule round 41
    // shipped - and this is the number that made it not worth turning on.
    let c = FileWriter::create(&dir.join("unfed.bin"), size)
        .unwrap()
        .coalescing(true);
    for i in 0..n {
        c.write_article_at(i * art.len() as u64, &art).unwrap();
    ...
    assert_eq!(
        unfed, n,
        "without the signal every article leaves an idle file behind it and writes"
    );
    assert!(
        fed < unfed,
        "the signal is the whole lever: {fed} writes fed against {unfed} unfed"
    );

    // Whatever it held still lands, and lands the same. The fed writer's
    // last article completes the file, so the completion rule has
    // already written it - `sync` is the belt, not the mechanism.
    sig.store(false, std::sync::atomic::Ordering::Relaxed);
    w.sync().unwrap();
    c.sync().unwrap();
    assert_eq!(
        std::fs::read(dir.join("fed.bin")).unwrap(),
        std::fs::read(dir.join("unfed.bin")).unwrap(),
        "the signal may change WHEN bytes land, never WHICH"
    );
}

/// A FED file whose last article has arrived is written AT ONCE, without
/// waiting for anyone to lower the signal. That is the completion rule
/// (`covered + staged >= size`), and it is what makes the signal safe
/// for every by-path reader that matters: settle's read-back, the native
/// repair's PAR2 scan, the unpack step and §296's early publish all read
/// FINISHED files, and a finished file is never holding bytes.
#[test]
fn a_fed_file_that_completes_is_written_without_lowering_the_signal() {
    let _gauge = crate::memgauge::one_gauge_test_at_a_time();
    let dir = stage_dir("fed-completion");
    let art = vec![5u8; 4096];
    let n = 8u64;
    let size = n * art.len() as u64;
    let p = dir.join("done.bin");
    let (w, _sig) = FileWriter::create(&p, size).unwrap().coalescing(true).fed();
    for i in 0..n {
        w.write_article_at(i * art.len() as u64, &art).unwrap();
    }
    assert_eq!(
        w.staged_bytes(),
        0,
        "the article that completed the file wrote the rest of it"
    );
    assert_eq!(
        std::fs::read(&p).unwrap().len() as u64,
        size,
        "and a by-path reader sees the whole file with the signal still raised"
    );
}

/// The doors do not care about the signal. A FED writer still writes its
/// runs out before answering a read, promising durability, or being torn
/// down - which is the rule `stage`'s header states and the signal must
/// not weaken.
#[test]
fn every_door_still_flushes_a_fed_writer() {
    let _gauge = crate::memgauge::one_gauge_test_at_a_time();
    let dir = stage_dir("fed-doors");
    let art = vec![6u8; 4096];
    // A file DECLARED far larger than what arrives, so the completion
    // rule cannot be what does the flushing here.
    let big = 1 << 20;

    let (w, _s1) = FileWriter::create(&dir.join("read.bin"), big)
        .unwrap()
        .coalescing(true)
        .fed();
    w.write_article_at(0, &art).unwrap();
    assert!(w.staged_bytes() > 0, "the signal is holding the run");
    let mut got = vec![0u8; art.len()];
    w.read_at(&mut got, 0).unwrap();
    assert_eq!(got, art, "read_at answers with the staged bytes");
    assert_eq!(w.staged_bytes(), 0, "and it answered by writing them");

    let (w2, _s2) = FileWriter::create(&dir.join("path.bin"), big)
        .unwrap()
        .coalescing(true)
        .fed();
    w2.write_article_at(0, &art).unwrap();
    assert!(w2.staged_bytes() > 0);
    w2.flush_staged().unwrap();
    assert_eq!(
        std::fs::read(dir.join("path.bin")).unwrap()[..art.len()],
        art[..],
        "flush_staged is the door the engine opens for every by-path reader"
    );

    let dropped = dir.join("drop.bin");
    {
        let (w3, _s3) = FileWriter::create(&dropped, big)
            .unwrap()
            .coalescing(true)
            .fed();
        w3.write_article_at(0, &art).unwrap();
        assert!(w3.staged_bytes() > 0);
    }
    assert_eq!(
        std::fs::read(&dropped).unwrap()[..art.len()],
        art[..],
        "a fed writer that is simply dropped still writes what it held"
    );
}

/// THE AGE BOUND, and the invariant it is there for. The journal states
/// that "a kill loses at most `BATCH_AGE` of placements, refetched on
/// resume, never corrupting anything". A placement record is queued when
/// the article completes; if the article's BYTES were still in a run
/// when the record landed, a kill leaves a landed record naming bytes
/// that are not on disk and a resume replays a hole as payload.
///
/// `stage::STAGE_MAX_AGE_DEFAULT` is that same `BATCH_AGE`, so a run
/// cannot outlive the record for it. Checked on ARRIVAL at the file
/// rather than by a timer thread, because a file that stops receiving
/// articles is already covered by the completion rule, by every door, by
/// `flush_staged` and by `Drop`.
#[test]
fn a_run_older_than_the_age_bound_is_written_on_the_next_arrival() {
    let _gauge = crate::memgauge::one_gauge_test_at_a_time();
    let dir = stage_dir("stage-age");
    let art = vec![4u8; 4096];
    let p = dir.join("aged.bin");
    let (w, _sig) = FileWriter::create(&p, 1 << 20)
        .unwrap()
        .coalescing(true)
        .fed()
        .0
        .staging_age(std::time::Duration::from_millis(15))
        .fed();
    w.write_article_at(0, &art).unwrap();
    assert!(w.staged_bytes() > 0, "the signal is holding the run");
    std::thread::sleep(std::time::Duration::from_millis(35));
    // A SECOND, DISJOINT article: it cannot extend the first run, so
    // nothing but the age rule can be what writes it out.
    w.write_article_at(1 << 16, &art).unwrap();
    assert_eq!(
        std::fs::read(&p).unwrap()[..art.len()],
        art[..],
        "the expired run went to disk when the next article arrived"
    );
}

/// THE §217 RESUME MARK SURVIVES THE WINDOW, and the two things that
/// make it survive are both round 44's.
///
/// `PrefixHash` advances only on a write landing exactly at its hashed
/// end and FREEZES on one landing ahead of it. That puts a coalescing
/// window on a collision course with it twice over: a run's bytes are
/// not on disk when the mark is read, and a flush that wrote a file's
/// open runs oldest-first would hand the hash a hole and stop it at the
/// first run out of sequence. Round 41 could not see either - with the
/// window's default at 0 there was never a run to flush - and turning it
/// on in round 44 took
/// `e2e_chaseresume::a_forfeited_7z_chase_resumes_its_member_on_disk`
/// red with "the 7z arm wrote no ledger, so the member re-extracted from
/// byte zero".
///
/// So [`FileWriter::prefix_hash`] is a DOOR like every other one on this
/// writer - it reads a property OF the bytes, so it writes them out
/// first - and [`stage::WriteStage::take_all`] takes its runs in
/// ASCENDING OFFSET order, which is the order the hash can advance over.
/// The arrival order here is IN ORDER, which is exactly the shape §217's
/// mark is for: the chase's member sinks decode sequentially, and a
/// sequential writer is the only one whose hash ever advances at all
/// (an out-of-order arrival freezes it with or without the window, which
/// is the pre-existing rule and not something this round moved).
#[test]
fn the_resume_mark_survives_bytes_held_in_the_window() {
    let _gauge = crate::memgauge::one_gauge_test_at_a_time();
    let dir = stage_dir("hash-through-window");
    let art = vec![2u8; 4096];
    let n = 8usize;
    let size = (n * art.len()) as u64;
    let p = dir.join("hashed.bin");
    // DECLARED far larger than what arrives, so the writer's completion
    // rule cannot be what flushes the run - a forfeit takes its mark
    // over a PARTIAL member by definition, and that is the only case in
    // which the mark is worth anything.
    let (w, _sig) = FileWriter::create(&p, 1 << 20)
        .unwrap()
        .coalescing(true)
        .with_prefix_hash()
    ...
    let (len, _crc) = w
        .prefix_hash()
        .expect("the hash is armed and must not be poisoned by the window");
    assert_eq!(
        len, size,
        "the mark must cover the whole contiguous prefix - a short one is the ledger \
         entry that made a forfeited member re-extract from byte zero"
    );
    assert_eq!(
