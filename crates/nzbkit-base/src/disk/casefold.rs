...
mod tests {
    ...
    /// half has been closed IN THE FOLD - delete it and say so.
    ///
    /// It being open in the fold is NOT the same as it being open at a
    /// call site. A site that must not merge two spellings the volume
    /// merges asks the VOLUME instead of asking this function, through
    /// `super::file_object_id` - which is what the zip extractor's
    /// duplicate-target guard does since 4 Sep 2026
    /// (`nzbfast_unpack::rarfix`), and is why no fold swap was the fix
    /// there. This pin says what THIS function does not do; it does not
    /// say the tree still has the gap.
    #[test]
