...
//! `sabcompat.rs`'s, which sat on the size gate's then 3,000-line
//! ceiling when this file was made (TODO 106; the 31 Aug 2026
//! recalibration moved it to 4,000). Same module either way.
//!
...
//! `job_finalize_marker_tests.rs`, the tree's other two, and
...
/// `HistoryFinalDelete` whose spool unlink is REFUSED must not have the
...
#[cfg(unix)]
#[test]
fn a_history_final_delete_whose_spool_unlink_is_refused_is_not_re_adopted() {
    ...
    let answer = jr_editqueue(
        &d,
        &[json!("HistoryFinalDelete"), json!(0), json!([7311])],
        &mut rpc_error,
    );
