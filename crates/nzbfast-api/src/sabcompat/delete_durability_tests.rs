...
// EVERY delete below drives `HistoryFinalDelete`, and that is the point
// of the spelling rather than a preference. The two NZBGet verbs shared
// one arm until 4 Sep 2026, so these fixtures were written against
// `HistoryDelete` and were really testing the erase; `HistoryDelete` is
// now the HIDE, which destroys nothing and so cannot answer any question
// in this file. What it does instead is pinned in
// `history_hidden_tests.rs`.
...
/// §296 (sweep S9): `HistoryFinalDelete` on a record whose move never
...
fn a_history_final_delete_takes_back_the_early_copies() {
    ...
    let answer = jr_editqueue(
        &d,
        &[json!("HistoryFinalDelete"), json!(0), json!([9296])],
        &mut rpc_error,
    );
...
fn an_active_delete_never_publishes_absence_before_its_history_row() {
    ...
    let job = Arc::new(Mutex::new(
        job_from_json(&json!({
            ...
            "out_dir": crate::naming::out_dir(&d).join("Cancelled.Release").to_string_lossy(),
...
///
/// `HistoryFinalDelete` unlinked the spooled `.nzb` inside its own `retain`
...
fn a_refused_tombstone_destroys_nothing_and_says_so() {
    ...
    let out = crate::naming::out_dir(&d).join("Refused.Release");
    std::fs::create_dir_all(&out).expect("out dir");
    ...
    let other = Arc::new(Mutex::new(
        job_from_json(&json!({
            ...
            "out_dir": crate::naming::out_dir(&d).join("Kept.Release").to_string_lossy(),
    ...
    let answer = jr_editqueue(
        &d,
        &[json!("HistoryFinalDelete"), json!(0), json!([9401])],
        &mut rpc_error,
    );
    disarm();
...
fn a_refused_append_is_rescued_by_the_rewrite_and_the_record_stays_gone() {
    ...
    let job = Arc::new(Mutex::new(
        job_from_json(&json!({
            ...
            "out_dir": crate::naming::out_dir(&d).join("Rescued.Release").to_string_lossy(),
            "nzb_path": nzb.to_string_lossy(), "state": "Completed",
        }))
        .expect("job"),
    ));
    ...
    let answer = jr_editqueue(
        &d,
        &[json!("HistoryFinalDelete"), json!(0), json!([9403])],
        &mut rpc_error,
    );
    disarm();
...
fn a_refused_placeholder_cancels_nothing() {
    ...
    let job = Arc::new(Mutex::new(
        job_from_json(&json!({
            ...
            "out_dir": crate::naming::out_dir(&d).join("Live.Release").to_string_lossy(),
