...
pub fn respond_page(req: tiny_http::Request, body: String, ctype: &str) {
...
#[derive(Clone, Copy)]
// `pub` because `assets::MANUAL_EN`, `i18n_catalog` and `manual_i18n`
// are - `private_interfaces` refuses a `pub` item whose signature names
// a crate-private type, and http.rs in the bin is what names those
// three. The fields stay `pub(crate)`: only the TYPE crosses.
...
pub fn respond_static(req: tiny_http::Request, asset: Precompressed, ctype: &str) {
...
impl ShellKey {
    ...
    fn render(&self) -> String {
        let (rel, embedded) = match self.page {
        ...
        // it borrows the compiled-in `&'static str` and costs nothing.
        let src = devweb::text_or(rel, embedded);
...
///
/// DEV-ONLY caveat: the cache is keyed on daemon SETTINGS, and saving
/// `web/dashboard.html` moves none of them - so under
/// NZBFAST_DEV_WEB_DIR a cached page would be served forever and the
/// override would appear to do nothing after the first load. Dev mode
/// therefore neither reads nor writes the cache. The ETag is hashed
/// over the rendered bytes either way, so a disk edit changes the
/// validator and no browser holds a stale copy.
fn shell_page(key: ShellKey) -> Arc<ShellPage> {
    let dev = devweb::dev_web_dir().is_some();
    if !dev && let Some(hit) = promote(&key) {
        return hit;
    }
