...
#[cfg(feature = "indexer")]
use nzbfast_daemon::daemon::wind_down;
use nzbfast_daemon::job::JobState;
use nzbfast_daemon::job::job_json;
use nzbfast_daemon::testutil::{jv, one_file_nzb, restart, stored_next_id, with_daemon};
...
fn a_lane_tail_never_parks_a_record_that_was_retried_out_from_under_it() {
    with_daemon("park-generation", |d| {
        let out = crate::naming::out_dir(d).join("Finishing.Release");
...
fn a_retried_delete_does_not_carry_its_removal_into_the_next_park() {
    with_daemon("delondrop-retry", |d| {
        let out = crate::naming::out_dir(d).join("Deleted.Release");
        std::fs::create_dir_all(&out).expect("payload dir");
...
fn save_queue_lock_hold_at_15k_jobs() {
    with_daemon("15k-bench", |d| {
        ...
        let (all_took, all_worst) = contend(&d.clone(), || {
            for _ in 0..4 {
                let v = crate::sabcompat::queue_json(d, &qp(&[]));
                assert_eq!(v["queue"]["slots"].as_array().map(Vec::len), Some(N));
            }
        });
        let (win_took, win_worst) = contend(&d.clone(), || {
            for _ in 0..4 {
                let v = crate::sabcompat::queue_json(d, &qp(&[("start", "0"), ("limit", "60")]));
...
fn an_enqueue_cannot_interleave_into_the_idle_scan_cas_window() {
    with_daemon("idle-aba", |d| {
        ...
        *nzbfast_daemon::daemon::daemon_park::IDLE_CAS_BARRIER.lock_ok() = Some((
        ...
        entered.wait();
        *nzbfast_daemon::daemon::daemon_park::IDLE_CAS_BARRIER.lock_ok() = None;
