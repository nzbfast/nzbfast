...
fn a_recategorize_whose_record_cannot_persist_is_reported() {
    use crate::job::{JobState, job_from_json};
...
fn no_call_site_builds_a_throwaway_http_agent() {
    ...
    assert!(
        ...
        "these build a fresh agent (and so a fresh TLS handshake) per \
         request - use crate::shared_enrich_agent():\n  {}",
        offenders.join("\n  ")
    );
}
...
fn apply_arms_match_the_table() {
    ...
    let arms: std::collections::BTreeSet<String> = {
        const INDEXER_ARMS: &[&str] = &[
            ...
            "index_evict_order",
            "index_evict_kinds",
            // THREE JOINED ON 3 Sep 2026 and they had been missing since
            // they were written. This arm had never RUN: while these
            // tests lived in the bin, whose default set turns `indexer`
            // ON, `slim-check` only CHECKED them. `nzbfast-api`'s own
            // default set is empty (every layer crate's is - the bin
            // forwards `indexer` to it), so `cargo test -p nzbfast-api
            // --lib` is the first thing ever to execute the slim branch,
            // and it named all three at once.
            "index_keep_kinds",
            "index_evict_scope",
            "index_evict_headroom",
            "predb_max_rows",
            "predb_seed_days",
        ];
        arms.into_iter()
...
/// TODO 313 item 8: `warm_reserve` is a HAND-EDITED setting in this lane
/// (no UI, so no `web/i18n/extract.js`, no 27 catalogues and no 16
/// manuals - the `postproc_jobs` precedent, where the knob waits for
/// demand). What that costs is a promise the save path has to keep: a
/// number somebody typed into config.local.json must survive the next
/// press of Save in the server editor, which knows nothing about it.
///
/// Asserted rather than trusted, because the failure is silent and the
/// same shape as the `pin_connections` one below it - the value is gone,
/// the daemon is running the default, and nothing logs a word.
#[test]
fn a_hand_edited_warm_reserve_survives_the_server_editor() {
    let stored = json!({
        "host": "news.example.com", "port": 563, "tls": true,
        "connections": 8, "warm_pool": true, "warm_reserve": 2,
    });
    // Everything the editor form actually sends. No mention of the
    // reserve, because there is no control for it.
    let from_the_form = json!({
        "host": "news.example.com", "port": 563, "tls": true, "connections": 8,
        "block_account": false, "pin_connections": false, "warm_pool": true,
    });
    let out = normalized_server(Some(&stored), &from_the_form).expect("host is set");
    assert_eq!(
        out["warm_reserve"], 2,
        "a setting with no control was eaten by the control panel"
    );
    let parsed: nzbkit::config::ServerConfig = serde_json::from_value(out).unwrap();
    assert_eq!(parsed.warm_reserve, Some(2));
    assert_eq!(
        nzbkit::warmreserve::request_for(&parsed),
        (2, nzbkit::warmreserve::ReserveNote::Held)
    );
}
...
fn relocate_attempts_the_move_for_an_out_of_root_job() {
    ...
    *d.move_completed.write_ok() = Some(dir.join("nas"));
    // The job's folder is NOT under crate::naming::out_dir(d) - strip_prefix fails and
...
fn a_move_that_keeps_failing_backs_off_and_finally_gives_up() {
    use crate::job::{MOVE_RETRY_GIVE_UP, MOVE_RETRY_MAX_SECS, move_retry_delay};
...
async fn wait_for_move_quiesce(d: &Arc<Daemon>, nzo_id: &str) {
    let deadline = std::time::Instant::now() + crate::testutil::NO_PROGRESS;
...
async fn the_move_ladder_stops_and_a_manual_retry_restarts_it() {
    use crate::job::{JobState, MOVE_RETRY_GIVE_UP, job_from_json};
    use crate::testutil::test_daemon;
...
fn a_move_to_an_unmounted_volume_explains_itself_and_arms_one_retry() {
    ...
    *d.move_completed.write_ok() = Some(absent.clone());
    ...
    let job_dir = crate::naming::out_dir(&d).join("Some.Release");
    std::fs::create_dir_all(&job_dir).unwrap();
    std::fs::write(job_dir.join("payload.bin"), b"bytes").unwrap();
...
async fn redrive_move_retries_the_move_and_clears_the_marker() {
    use crate::job::{JobState, job_from_json};
    use crate::testutil::{NO_PROGRESS, test_daemon};
...
fn custom_keys_are_charset_checked_and_links_encode_them() {
    ...
    assert_eq!(query_escape("0123abcdef"), "0123abcdef");
    assert_eq!(query_escape("a&b c+d"), "a%26b%20c%2Bd");
...
fn concurrent_bench_appends_lose_no_rows() {
    ...
    let threads: Vec<_> = (0..8)
        .map(|t| {
            let d = d.clone();
            std::thread::spawn(move || {
                // The subject is that no row is lost across writers, not
                // that each append is durable; 200 x 2 fsyncs was ~14 s.
                crate::persist::SKIP_FSYNC.with(|c| c.set(true));
                for i in 0..25 {
...
/// Bug sweep 1 Sep 2026, F19: the facet loop's §96 storage-deleted probe
/// is spent out of `HISTORY_STAT_BUDGET`, so ONE `mode=history` poll
/// cannot stat every Completed row of an unlimited history.
///
/// `5b30d28b2` fixed finding 13 by moving the counters, `failed_only`,
/// the bucket chips and `clearable` onto `publishes_as_failed` - and
/// took `storage_deleted`'s two `exists()` stats with them, out of the
/// windowed page rows and into a loop over the whole store. A 14,500-row
/// history (issue #38 scale) on an SMB completed folder then paid 14,500
/// round trips per Sonarr poll, each one under the row's own mutex, and
/// a share that was unmounted or asleep blocked for the mount timeout on
/// every one of them. The bound restored here is the one written down
/// with the default cap: the probes are paid for the rows the request
/// can render, not for the store.
///
/// The rendered rows still probe individually (`history_summary` asks
/// `sab_history_status_published` itself), which is why the ceiling
/// below is two per rendered row rather than one.
#[test]
fn history_facets_spend_a_bounded_number_of_folder_probes() {
    use super::history::{HISTORY_DEFAULT_LIMIT, STORAGE_PROBES};
    use crate::job::job_from_json;
    use crate::testutil::test_daemon;
    let dir = std::env::temp_dir().join(format!("nzbfast-histprobe-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let d = test_daemon(&dir);
    let vol = dir.join("vol");
    std::fs::create_dir_all(&vol).unwrap();
    // Six times the budget, all Completed and none *arr-grabbed, so
    // every one of them is a flip candidate: the unbudgeted loop probes
    // all of them, the budgeted one probes the first 500 it walks. Six
    // rather than three so the widest ceiling asserted below (three
    // budgets: the flat allowance, the render-window allowance and the
    // render itself) is still only half the store, and so says
    // something.
    let rows_n = 6 * HISTORY_DEFAULT_LIMIT;
    for i in 0..rows_n {
        let out = vol.join(format!("Job.{i}"));
        std::fs::create_dir_all(&out).unwrap();
        let job = Arc::new(Mutex::new(
            job_from_json(&json!({
                "nzo_id": format!("SABnzbd_nzo_probe{i}"),
                "name": format!("Job.{i}"),
                "nzb_path": dir.join("some.nzb").to_string_lossy(),
                "out_dir": out.to_string_lossy(),
                "state": "Completed",
    ...
    // The newest row is the one the page renders first (the loop walks
    // the store in reverse), and its folder is genuinely gone: the
    // budget must not cost finding 13's fix for a row on the page.
    std::fs::remove_dir_all(vol.join(format!("Job.{}", rows_n - 1))).unwrap();
    // A SECOND one, 600 rows in - past the flat allowance (500) and
    // still inside the render window. It is what the second allowance
    // exists for: with `bucket=failed` the flat allowance is burnt on
    // the Completed rows the chip rejects, so without a window
    // allowance this row is classified on the cheap word, counted under
    // `done`, dropped by the chip, and "Show failed" hides a row that
    // renders as Failed.
    const DEEP_GONE: usize = 600;
    std::fs::remove_dir_all(vol.join(format!("Job.{}", rows_n - 1 - DEEP_GONE))).unwrap();
    ...
    let (rows, matched, facets) = history_page(&d, &q, true);
    let probes = STORAGE_PROBES.with(|c| c.get());

    assert_eq!(matched, rows_n, "every row is still counted");
    assert_eq!(rows.len(), HISTORY_DEFAULT_LIMIT, "the window is unchanged");
    // Facet budget (500) plus one probe per rendered row (500). Without
    // the budget this is rows_n + 500 = 2000.
    assert!(
        probes <= 2 * HISTORY_DEFAULT_LIMIT,
        "one mode=history poll must not stat the whole store: \
         {probes} folder probes over {rows_n} rows, ceiling {}",
        2 * HISTORY_DEFAULT_LIMIT
    );
    // ...and the row that IS on the page still publishes, counts and
    // filters as Failed - the budget buys the bound, not finding 13.
    assert_eq!(rows[0]["status"], "Failed", "{}", rows[0]);
    assert_eq!(facets["failed"], 1, "{facets}");

    // `status=` IS INSIDE THE BUDGET TOO. The filter used to call
    // `sab_history_status_published` on every candidate that reached it,
    // which walks straight past the budget and back into two `exists()`
    // per row of the whole store - so the one shape an *arr actually
    // sends, `mode=history&status=Completed`, kept stat'ing everything
    // while the facets above were bounded.
    let q_status = HistQuery {
        status: Some(std::collections::HashSet::from(["Completed".to_string()])),
        ..q
    };
    STORAGE_PROBES.with(|c| c.set(0));
    let (rows_s, matched_s, _) = history_page(&d, &q_status, true);
    let probes_s = STORAGE_PROBES.with(|c| c.get());
    assert_eq!(
        matched_s,
        rows_n - 1,
        "the storage-deleted row publishes Failed and is filtered out"
    );
    // Three budgets is the honest worst case for a filtered query: the
    // flat allowance, plus one probe for each of the first `limit` rows
    // that survive the filter (the render-window term), plus the
    // render's own probe per rendered row. Unbudgeted it is rows_n +
    // 500.
    assert!(
        probes_s <= 3 * HISTORY_DEFAULT_LIMIT,
        "a status= filter must not buy an unbudgeted probe per row: \
         {probes_s} folder probes over {rows_n} rows, ceiling {}",
        3 * HISTORY_DEFAULT_LIMIT
    );
    assert_eq!(rows_s.len(), HISTORY_DEFAULT_LIMIT, "window unchanged");

    // AND `start` CANNOT BUY PROBES. It is parsed unclamped from the
    // query string, so a cap that added it let `?start=100000000&limit=1`
    // ask for a hundred million stats.
    let q_deep = HistQuery {
        start: 100_000_000,
        limit: 1,
        status: None,
        ..q_status
    ...
    STORAGE_PROBES.with(|c| c.set(0));
    let (rows_d, matched_d, _) = history_page(&d, &q_deep, true);
    let probes_d = STORAGE_PROBES.with(|c| c.get());
    assert!(rows_d.is_empty(), "that page is past the end of the store");
    assert_eq!(matched_d, rows_n, "every row is still counted");
    assert!(
        probes_d <= 2 * HISTORY_DEFAULT_LIMIT,
        "a start= far past the store must not raise the budget: \
         {probes_d} folder probes over {rows_n} rows, ceiling {}",
        2 * HISTORY_DEFAULT_LIMIT
    );

    // THE RENDER-WINDOW ALLOWANCE, which the flat one cannot stand in
    // for: `bucket=failed` rejects every healthy Completed row, so the
    // flat allowance is spent long before the window is full.
    let q_bucket = HistQuery {
        bucket: Some("failed".to_string()),
        start: 0,
        limit: HISTORY_DEFAULT_LIMIT,
        status: None,
        ..q_deep
    ...
    STORAGE_PROBES.with(|c| c.set(0));
    let (rows_b, matched_b, facets_b) = history_page(&d, &q_bucket, true);
    let probes_b = STORAGE_PROBES.with(|c| c.get());
    assert_eq!(
        matched_b, 2,
        "both storage-deleted rows publish Failed, so the chip keeps \
         both - including the one past the flat allowance: {facets_b}"
    );
    assert_eq!(rows_b.len(), 2, "and both render");
    assert!(
        probes_b <= 3 * HISTORY_DEFAULT_LIMIT,
        "the window allowance has its own ceiling - `matched < limit` \
         alone never stops being true for a chip that matches almost \
         nothing, which would probe the whole store: {probes_b} folder \
         probes over {rows_n} rows, ceiling {}",
        3 * HISTORY_DEFAULT_LIMIT
    );
...
fn a_delete_that_beats_the_pipelines_publish_still_stops_it() {
    ...
    *d.active_stream.lock_ok() = Some("SABnzbd_nzo_gone".into());
    ...
    crate::api::queue::stop_deleted_transfer(&d, vec!["SABnzbd_nzo_gone".into()]);
