...
// The two Job fixtures went down with the daemon layer (see
// `tests_jobs.rs`'s own note); `tests_jobs` is a private sibling module
// of `serve`, so they are taken from where they now live.
use super::*;
use nzbfast_daemon::testutil::{job, no_causes};
...
fn watch_folder_states_are_told_apart() {
    use super::watchfail;
    use super::watchfail::{watch_fail_ingested, watch_fail_kind};
...
/// The hermeticity backstop (`netfetch::deny_test_callout`), pinned
/// here beside the SSRF rules it rides on top of - two of which it took
/// red on its first cut, which is why the literal-address carve-out
/// exists and why it is worth a test of its own.
///
/// A NAME resolving outside loopback is what a live provider is
/// (api.tvmaze.com is the one three tests were dialling on 1 Sep 2026);
/// a literal is the resolver tests' own vocabulary and connects to
/// nothing. `NZBFAST_TEST_ALLOW_CALLOUT` is not exercised here - it is
/// process-wide, and a test that sets it would change the answer for
/// whatever runs beside it in the same process.
#[test]
fn a_unit_test_may_not_reach_a_named_host_outside_loopback() {
    let at = |s: &str| vec![s.parse::<std::net::SocketAddr>().unwrap()];
    let deny = super::deny_test_callout;

    // The defect this exists for: a provider name, resolved publicly.
    let e = deny("api.tvmaze.com:443", &at("52.1.2.3:443")).expect_err("a named provider");
    assert!(
        e.to_string().contains("must be hermetic"),
        "the refusal has to say what to do: {e}"
    );

    // A name that resolves to loopback IS a fixture - `localhost`, and
    // any name a test points at its own listener.
    assert!(deny("localhost:9696", &at("127.0.0.1:9696")).is_ok());
    assert!(deny("fixture.test:80", &at("[::1]:80")).is_ok());

    // Literals pass through, bracketed v6 included: the SSRF rules
    // above are unit-tested by resolving exactly these.
    for netloc in ["8.8.8.8:443", "192.168.1.9:5077", "[2606:4700::1111]:443"] {
        assert!(deny(netloc, &at("8.8.8.8:443")).is_ok(), "{netloc}");
    }

    // Mixed answers are refused on the strength of the public one -
    // `all`, never `any`, or a resolver returning both smuggles it in.
    assert!(
        deny(
            "split.test:443",
            &[at("127.0.0.1:443")[0], at("52.1.2.3:443")[0]]
        )
        .is_err()
    );
}

/// The PRODUCTION half of the same invariant: `EnrichResolver`, the
/// resolver `shared_enrich_agent` installs.
///
/// It is pinned with LITERALS on purpose, and that is what makes the
/// test discriminating rather than decorative. A named host is already
/// refused by `deny_test_callout` in this build, so a name would pass
/// whether or not the new arm exists; a literal walks straight through
/// that carve-out, so `8.8.8.8:443` can only be refused by the
/// enrichment gate itself. `may_call_out()` is false in every
/// unit-test build, which is the "enrichment off" state this measures,
/// and neither address is connected to.
///
/// The defect it pins: measured 1 Sep 2026 on a real daemon started
/// with NZBFAST_NO_ENRICH=1, the dashboard's metadata search still
/// reached www.wikidata.org and its art fetch still reached
/// image.tmdb.org, because the gate lived on one of three
...
fn the_enrich_pool_refuses_the_wire_when_enrichment_is_off() {
    ...
    // otherwise red this test rather than the rig it was aimed at. Stand
    // down instead of asserting a state the runner deliberately lifted -
    // the same reason the test above does not exercise the variable.
...
// ---- anti-rollback ratchet (ENFORCING) ----------------------------
//
// These pin the decision `check_manifest_serial` acts on. Two shapes are
// accepted and two are refused, and the pair that must not drift is
// "absent with no serial ever seen" (accepted: nothing to roll back
// below) against "absent once a serial HAS been seen" (refused: a
// channel that has proved it emits serials cannot drop one, or stripping
// the field would be the way around the whole ratchet). No variant ever
// lowers the stored value.
...
#[test]
fn manifest_serial_junk_and_absence_never_lower_the_ratchet() {
    use super::SerialStep::*;
    // Every unusable shape lands in ONE arm, so junk cannot coerce into
    // a huge serial (which would pin the install above every real
    // release it will ever be offered) and cannot be waved through as
    // "fine" either.
    let unusable = [
        serde_json::json!({ "version": "1.0.0" }), // absent
        serde_json::json!({ "serial": "999999" }), // string
        serde_json::json!({ "serial": -5 }),       // negative
        serde_json::json!({ "serial": 1.5 }),      // float
        serde_json::json!({ "serial": null }),
    ];
    for m in &unusable {
        // With a serial already on record, the channel has proved it
        // emits them: dropping one is a refusal, because it is
        // indistinguishable from replaying a manifest that predates
        // them. It is NOT a clear - a manifest must never be able to
        // lower this install's floor, or "serve junk" would be the way
        // to disarm the defence rather than merely fail it.
        assert_eq!(super::serial_ratchet(140, m), Missing { seen: 140 }, "{m}");
        // With no serial ever seen there is no floor to duck under, so
        // the same manifest is accepted. This is the arm that keeps a
        // fresh install on a serial-less custom channel working, and
        // the state the documented local reset returns an install to.
        assert_eq!(super::serial_ratchet(0, m), Hold, "{m}");
    }
}

#[test]
fn manifest_serial_is_enforced_in_this_build() {
    // The enforcement guarantee, stated as a test so that any future
    // softening has to come here and undo it deliberately rather than
    // inherit it. This replaced `manifest_serial_is_not_enforced_in_
    // this_build`, which pinned the opposite promise from 28 Jul until
    // the field evidence arrived (2 Sep 2026: serial present and
    // strictly increasing across all 26 manifests from v1.0.11 to
    // v1.3.1).
    use super::SerialStep::*;
    // A stale manifest advertising a version we WOULD otherwise take.
    // That is the whole attack: the signature is genuine, the version
    // comparison says yes, and only the serial says no.
    let stale = serde_json::json!({ "version": "99.0.0", "serial": 1 });
    assert!(
        super::version_newer("99.0.0", env!("CARGO_PKG_VERSION")),
        "this fixture must be one the version comparison would accept, \
         or the test proves nothing about what the serial adds"
    );
    assert_eq!(
        super::serial_ratchet(500, &stale),
        Regressed { got: 1, seen: 500 }
    );
    // And the same manifest with the serial simply removed, which is
    // the cheapest way around a ratchet that only looked at values.
    let stripped = serde_json::json!({ "version": "99.0.0" });
    assert_eq!(super::serial_ratchet(500, &stripped), Missing { seen: 500 });
    // Both refusing variants exist and are distinct from the accepting
    // ones, which is what lets a caller act on them at all - the
    // read-only build's guarantee was precisely that no such variant
    // existed. That `check_manifest_serial` turns these into an Err,
    // and that a refused manifest never moves the stored floor, is
    // pinned in update.rs's own test module, where a Daemon is
    // available. Nothing here reaches `check_update` itself, which
    // needs a network: what carries the refusal past the version
    // comparison and the latch there is one `?`.
    assert_ne!(super::serial_ratchet(500, &stale), Hold);
    assert_ne!(super::serial_ratchet(500, &stripped), Hold);
}
