...
// this file needs nothing from `super` with `dashboard` off. (The one
// item the settings boundary read on every build, `UI_LOCALES`, moved
// down to `uilocales.rs`.) Gating the IMPORT
...
use super::*;
// `web_icon` hands back either a compiled-in slice or, under the dev
// override, bytes it just read - one type covering both, with no copy
...
#[cfg(feature = "dashboard")]
pub fn user_css(cfg_path: &Path) -> String {
...
///
/// Each arm names the file's path under `web/` beside its own
/// `include_bytes!`, and `devweb::bytes_or` takes that literal - so the
/// dev override reads the SAME table this serves from and there is no
/// second name list to drift. Unset, `bytes_or` hands the embedded
/// slice straight back.
#[cfg(feature = "dashboard")]
pub fn web_icon(path: &str) -> Option<(Cow<'static, [u8]>, &'static str)> {
    let (rel, embedded, ctype) = match path {
    ...
    Some((devweb::bytes_or(rel, embedded), ctype))
}
...
#[cfg(feature = "dashboard")]
pub fn i18n_catalog(lang: &str) -> Option<Precompressed> {
    Some(match lang {
...
#[cfg(feature = "dashboard")]
pub const MANUAL_EN: Precompressed = precompressed!("manual-en");

/// Translated manuals.
#[cfg(feature = "dashboard")]
pub fn manual_i18n(lang: &str) -> Option<Precompressed> {
    Some(match lang {
...
pub(super) fn ui_themed(page: &str) -> String {
    page.replace(
        ...
        &devweb::text_or("ui-tokens.html", UI_TOKENS_HTML),
    )
    .replace(
