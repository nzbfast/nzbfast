...
// The two Job fixtures this file used to declare moved DOWN into
// `nzbfast_daemon::testutil` with the daemon layer: `Job`,
// `job_from_json` and `LossCauses` are all below this crate now, and
// five daemon-layer test files build a Job the same way. Imported
// under their old bare names, so nothing in this file moved.
use nzbfast_daemon::testutil::job;
...
/// Bug sweep 1 Sep 2026, F20: `delete_records` runs with the GLOBAL
/// history lock held, so it must not touch the filesystem.
///
/// Finding 13's fix put `publishes_as_failed` on the record snapshot,
/// and with it `storage_deleted`'s two `exists()` calls on every
/// Completed row's output folder - inside the `d.history` hold the
/// delete arm keeps across its whole plan-and-retain transaction. On a
/// completed folder that lives on an SMB share that has gone away each
/// of those blocks for the mount timeout, and the park path takes that
/// same lock to file a finished job (`daemon_park`), as do the enqueue
/// dupe check, `history_publish` and the queue saver: one click on ✕
/// stalled every download finishing in that window. Worse, the value was
/// dead work for that click - only the `failed` and `completed` bulk
/// words ever read `published_failed`.
///
/// So the verdict is asked BEFORE the lock, by `published_failed_map`,
/// and only when a bulk word will read it. The two assertions that
/// matter: the map is where the probes are spent, and `delete_records`
/// spends none whichever way it is called.
#[test]
fn building_the_delete_records_stats_no_folder_under_the_history_lock() {
    use super::history::{STORAGE_PROBES, published_failed_map};
    use crate::testutil::test_daemon;
    let dir = std::env::temp_dir().join(format!("nzbfast-f20-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let d = test_daemon(&dir);
    // The §96 shape: Completed, not *arr-grabbed, absolute `out_dir`
    // that is gone while its parent is still there. Nothing cheaper than
    // a stat can tell this row from a healthy one, which is why it is
    // the row that costs.
    let vol = dir.join("vol");
    std::fs::create_dir_all(&vol).unwrap();
    let gone = vol.join("Gone.2024");
    d.history.lock_ok().push(Arc::new(Mutex::new(job(json!({
        "nzo_id": "SABnzbd_nzo_gone",
        "name": "Gone.2024",
        "nzb_path": dir.join("gone.nzb").to_string_lossy(),
        "out_dir": gone.to_string_lossy(),
        "state": "Completed",
        "origin": "dashboard",
    })))));

    // The bulk-word path: the stats live here, with no global lock held.
    STORAGE_PROBES.with(|c| c.set(0));
    let map = published_failed_map(&d);
    assert!(
        STORAGE_PROBES.with(|c| c.get()) > 0,
        "the published-Failed map is where the folder probe is paid"
    );
    assert_eq!(map.get("SABnzbd_nzo_gone"), Some(&true), "{map:?}");

    let h = d.history.lock_ok();
    // Handed the answer, the snapshot carries it and still costs nothing.
    STORAGE_PROBES.with(|c| c.set(0));
    let recs = super::delete_records(h.as_slice(), Some(&map));
    assert_eq!(
        STORAGE_PROBES.with(|c| c.get()),
        0,
        "delete_records runs under the global history lock and must not stat"
    );
    assert!(recs[0].published_failed, "the verdict rode the map");

    // And a single-id or `value=all` delete never asks the question.
    STORAGE_PROBES.with(|c| c.set(0));
    let recs = super::delete_records(h.as_slice(), None);
    assert_eq!(
        STORAGE_PROBES.with(|c| c.get()),
        0,
        "a delete that never reads published_failed must not pay for it"
    );
    assert!(!recs[0].published_failed);
    drop(h);
    // ...and the gate is not a behaviour change: the row is still doomed
    // by its own id and by `all`, neither of which reads the field.
    for value in ["SABnzbd_nzo_gone", "all"] {
        let plan = super::plan_history_delete(&recs, value, None, &[]);
        assert!(plan[0].doomed, "value={value} must still take the row");
    }
...
/// The bulk words never take a row NZBGet's `HistoryDelete` hid, and an
/// explicit id always does.
///
/// `history_page` filters a hidden row out of the rows it renders AND
/// out of the `clearable` / `clearable_failed` counts that label the
/// dashboard's two one-click sweeps. A sweep that took hidden rows
/// anyway would remove more rows than its own button says it will -
/// the finding-13 disagreement, in the direction that destroys, and
/// over exactly the rows an *arr has been quietly accumulating since
/// its first import.
#[test]
fn the_bulk_clears_spare_a_hidden_record_and_an_explicit_id_takes_it() {
    let rec = |id: &str, state: JobState, hidden: bool| super::DeleteRecord {
        nzo_id: id.to_string(),
        name: id.to_string(),
        state,
        out_dir: PathBuf::from(format!("/dl/{id}")),
    ...
    let recs = vec![
        rec("seen", JobState::Completed, false),
        rec("hidden", JobState::Completed, true),
        rec("hidden-failed", JobState::Failed, true),
    ];
    ...
    for value in ["completed", "all"] {
        ...
        assert_eq!(
            ...
            "value={value} must take only what the count under the button included"
        );
    }
    let plan = super::plan_history_delete(&recs, "failed", None, &[]);
    assert!(
        !plan.iter().any(|p| p.doomed),
        "the only failure here is hidden, so `failed` sweeps nothing"
    );

    // ...but a caller that NAMES the row means it: an id is something
    // the caller can see, and `HistoryFinalDelete` is the NZBGet
    // spelling of the same instruction.
    let plan = super::plan_history_delete(&recs, "hidden,hidden-failed", None, &[]);
    assert_eq!(
        plan.iter().map(|p| p.doomed).collect::<Vec<_>>(),
        vec![false, true, true],
        "an explicit id erases a hidden row"
    );
}
...
fn a_refused_delete_keeps_the_files_and_says_why() {
    ...
    // `$TMPDIR` is a PERMANENT delete by default and this test's whole
    // subject is a refused RECOVERABLE one, so it asks for the exemption
    // `smart`'s own trash tests get from `cfg(test)`. It holds the write
    // side of the trash lock, and sets both flags back below.
    crate::smart::force_temp_trash(true);
    ...
    crate::smart::force_temp_trash(false);
...
/// The control arm for the crate-split step 3 cut.
///
/// `smart::under_temp_dir` forces every `$TMPDIR` path to a PERMANENT
/// delete, which is what keeps this crate's fixtures out of the
/// developer's real ~/.Trash now that `smart` is a dependency rather
/// than a module - its `cfg!(test)` exemption is false here, and the
/// `TRASH` flag consequently defaults ON. Nothing about that guard
/// failing would be loud: the tests would still pass and the fixtures
/// would quietly pile up in the Trash. So it is pinned from the side
/// that depends on it, at rest, with the seam that lifts it untouched.
#[test]
fn a_temp_path_is_never_binned_from_this_crates_tests() {
    let _steady = crate::smart::trash_globals_steady();
    let dir = crate::testscratch::ScratchDir::attach(
        &std::env::temp_dir().join(format!("nzbfast-tmptrash-{}", std::process::id())),
    );
    let f = dir.join("fixture.mkv");
    std::fs::write(&f, b"x").unwrap();
    let out = crate::smart::remove_user_file(&f, true).expect("the delete must succeed");
    assert!(!f.exists(), "the fixture must be gone");
    assert_eq!(
        out,
        crate::smart::Removed::Gone,
        "a $TMPDIR fixture took the RECOVERABLE route from this crate's \
         tests, so every cleanup test here is filling the developer's \
         real ~/.Trash. `smart::under_temp_dir` is what stops it: check \
         that its `cfg!(test)` arm has not grown a `test-support` term, \
         which reads true here AND in the binary the integration suites \
         spawn."
    );
}
...
/// ...and the dashboard's own two header sweeps have to SEND it.
///
/// The daemon half above landed 31 Aug 2026 and the page never used it:
/// `clearHistDone` / `clearHistFailed` posted a bare
/// `value=completed` / `value=failed` while their visibility, and the
/// confirmation they show ("out of this list"), were scoped to whatever
/// the user had typed in the History search box. History holding 40
/// failures, "Alpha" typed, one row on screen, one click - and all 40
/// records went, with the toast reporting 40 (bug sweep 1 Sep 2026,
/// F26). No files are destroyed by a plain clear, but the record is the
/// only handle on a kept output folder and the only Retry the user has.
///
/// Read off the SOURCE because that is where the defect lived: both
/// legs of the daemon half are already pinned by the two tests around
/// this one, and they passed throughout. `api()` adds no ambient
/// parameters, so if the string is not in the call it is not on the
/// wire.
///
/// Asserted per SWEEP rather than only on the shared helper: what a
/// future edit drops is one call site, and the helper would still be
/// there for the other.
#[test]
fn the_dashboards_header_sweeps_carry_the_history_search() {
    let dash = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../web/dashboard.html"),
    )
    .expect("web/dashboard.html");
    assert!(
        dash.contains("function histClearSearch()"),
        "histClearSearch moved or was renamed - the two sweeps below build their \
         search suffix with it"
    );
    for (func, value) in [
        (
            "async function clearHistDone(btn){",
            "&name=delete&value=completed",
        ),
        (
            "async function clearHistFailed(btn){",
            "&name=delete&value=failed",
        ),
    ] {
        let at = dash
            .find(func)
            .unwrap_or_else(|| panic!("{func} moved or was renamed"));
        // The body, to the close brace in column 1 - every top-level
        // function on this page ends that way, and nothing nested in
        // one is written out to column 1.
        let end = dash[at..].find("\n}").map(|e| at + e).unwrap_or(dash.len());
        let body = &dash[at..end];
        let call = body
            .find(value)
            .unwrap_or_else(|| panic!("{func} no longer sends {value}"));
        // That one statement, not the whole function: a search suffix
        // built somewhere else in the body and never concatenated on
        // is exactly the bug.
        let line_end = body[call..]
            .find('\n')
            .map(|e| call + e)
            .unwrap_or(body.len());
        let stmt = &body[call..line_end];
        assert!(
            stmt.contains("histClearSearch()"),
            "{func} sends an UNSCOPED class sweep while the button that fires it is \
             scoped to the history search: {stmt}"
        );
    }
}
...
fn the_sab_search_predicate_never_matches_more_than_sab_would() {
    use crate::sabvocab::sab_search_matches as m;
...
/// The download-root half of the same defect the write-through rig
/// pins, and the two arms that must NOT move with it.
///
/// A directory holding files that no queue row and no history row names
/// is somebody else's - a completed result whose history row the user
/// cleared with the files kept, or a folder the user built by hand that
/// happens to share the release stem - so a FRESH add must be refiled
/// beside it rather than handed it, or the first decoded article
/// truncates it in place.
///
/// It answers `Occupied` and NOT `Payload`, and that distinction is the
/// whole point of the variant. `Payload` is a claim by a record we hold,
/// and `choose_out_dir` answers it with `replaces = Some(base)`, which
/// `publish_over_previous` cashes on success by renaming the canonical
/// directory aside and `remove_dir_all`ing it. Aimed at a directory we
/// have no record of, that is a delete of files we never owned - the
/// data loss the fix would have introduced in exchange for the one it
/// removed. So an unrecorded occupant makes the add climb and records
/// NOTHING, and the folder is untouched before completion and after.
///
/// The bare `dir_claim` keeps answering Free for exactly that
/// directory, and that is not an oversight: `daemon_retry` removes a
/// job's own history record BEFORE asking, on purpose, so that an
/// ordinary failed retry finds its own folder unclaimed and reuses it
/// in place instead of climbing .2/.3/.4 and abandoning its journal. At
/// that instant the folder is non-empty with the failed run's partials
/// and no record names it - the shape below exactly. So all three
/// answers are pinned in one test: whichever way a later reader
/// collapses the entry points or the variants, one of these assertions
/// fails.
#[test]
fn an_occupied_unrecorded_directory_is_occupied_to_an_add_and_free_to_a_retry() {
    ...
    assert!(d.queue.lock_ok().is_empty() && d.history.lock_ok().is_empty());

    let kept = crate::naming::out_dir(&d).join("Some.Release.2024.1080p");
    std::fs::create_dir_all(&kept).unwrap();
    std::fs::write(kept.join("payload.mkv"), b"the verified payload").unwrap();

    assert!(
        matches!(d.dir_claim_for_add(&kept), DirClaim::Occupied),
        "a fresh add must not be handed a folder with somebody's files \
         in it - and must not read it as a payload of ours either, \
         which would make it a replace target"
    );
    assert!(
        matches!(d.dir_claim(&kept), DirClaim::Free),
        "the retry path's in-place reuse must keep reading its own \
         partials as free, or every failed retry loses its journal"
    );

    // A FAILED record's leftovers are junk and are still reused in
    // place - `Daemon::enqueue`'s own rule, so that a re-add of a flaky
    // post does not climb .2, .3, .4. The arm asks that NO record name
    // the directory, and a failed row names its own.
    let junk = crate::naming::out_dir(&d).join("Flaky.Post.2024.1080p");
    std::fs::create_dir_all(&junk).unwrap();
    std::fs::write(junk.join("part.bin"), b"half an article").unwrap();
    d.history.lock_ok().push(Arc::new(Mutex::new(job(json!({
        "nzo_id": "SABnzbd_nzo_flaky1",
        "name": "Flaky.Post.2024.1080p",
        "nzb_path": "/spool/flaky.nzb",
        "state": "Failed",
        "out_dir": junk.to_string_lossy(),
    })))));
    assert!(
        matches!(d.dir_claim_for_add(&junk), DirClaim::Free),
        "a failed job's own leftovers are junk and are reused in place"
    );

    // An EMPTY directory is not an occupant, so nothing climbs for
    // nothing: `choose_out_dir`'s candidates are directories the caller
    // is about to create, and a leftover empty one is not evidence.
    let empty = crate::naming::out_dir(&d).join("Empty.Release.2024.1080p");
    std::fs::create_dir_all(&empty).unwrap();
    assert!(
        matches!(d.dir_claim_for_add(&empty), DirClaim::Free),
        "an empty unrecorded directory is free ground"
    );
    // And neither is a path that is not there at all.
    assert!(matches!(
        d.dir_claim_for_add(&crate::naming::out_dir(&d).join("Never.Seen.2024")),
        DirClaim::Free
    ));

    // End to end through the production placement rule: the add is
    // refiled beside the occupied folder and records NO replacement, so
    // there is nothing for `publish_over_previous` to cash and the
    // folder survives this job's success as well as its failure.
    let stem = "Some.Release.2024.1080p";
    let (fresh, replaces) = choose_out_dir(&kept, stem, &|p| d.dir_claim_for_add(p));
    assert_eq!(
        crate::naming::out_dir(&d).join("Some.Release.2024.1080p.2"),
        fresh
    );
    assert_eq!(
        None, replaces,
        "an unrecorded occupant must NEVER be recorded as a replace \
         target - publish_over_previous deletes what it replaces"
    );

    // The contrast, through the same function: OUR OWN completed
    // result of the release does claim its directory as a payload, and
    // that one IS replaced once the re-download verifies. This is the
    // behaviour the Occupied arm had to be kept out of, not a
    // behaviour it changed.
    let ours = crate::naming::out_dir(&d).join("Ours.Release.2024.1080p");
    std::fs::create_dir_all(&ours).unwrap();
    std::fs::write(ours.join("payload.mkv"), b"our own verified result").unwrap();
    d.history.lock_ok().push(Arc::new(Mutex::new(job(json!({
        "nzo_id": "SABnzbd_nzo_ours1",
        "name": "Ours.Release.2024.1080p",
        "nzb_path": "/spool/ours.nzb",
        "state": "Completed",
        "out_dir": ours.to_string_lossy(),
    })))));
    let ours_stem = "Ours.Release.2024.1080p";
    let (fresh, replaces) = choose_out_dir(&ours, ours_stem, &|p| d.dir_claim_for_add(p));
    assert_eq!(
        crate::naming::out_dir(&d).join("Ours.Release.2024.1080p.2"),
        fresh
    );
    assert_eq!(
        Some(ours.as_path()),
        replaces.as_deref(),
        "a recorded Completed payload of ours is still taken over once \
         the re-download has verified"
    );
