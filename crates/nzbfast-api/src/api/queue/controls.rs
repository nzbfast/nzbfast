...
//!
//! Re-exported by the parent, so `crate::api::queue::apply_pause`
...
pub fn apply_pause(d: &Arc<Daemon>, g: &mut Job, pause: bool) -> bool {
...
pub(crate) fn stop_deleted_transfer(d: &Arc<Daemon>, stopped: Vec<String>) {
    if stopped.is_empty() {
        return;
    }
...
/// one meaning.
pub fn reposition_for_priority(
...
pub(crate) fn search_param(params: &std::collections::HashMap<String, String>) -> Option<String> {
    params
        .get("search")
