...
/// The `delete` and `purge` arm of [`m_queue`], as its own function
/// the way every other non-trivial arm in this match already is
/// (`promote_arm`, `insure_arm`, `rename_arm`, `sort_arm`). Split out
/// for the 500-line ceiling on `m_queue`; the body is verbatim, and
/// it shadowed the outer `hit_id`/`hit` with its own already, so only
/// `d`, `params`, `op` and `value` cross.
///
...
fn delete_arm(
    ...
    crate::postproc::cancel_tail_fetches(d, hit_id);
...
pub(super) fn m_queue(
    ...
    Some({
        ...
        match params.get("name").map(String::as_str) {
            ...
            Some("insure") => crate::insurance::insure_arm(d, params),
