...
fn out_dir_for(d: &Daemon, ctx: &ApiCtx<'_>) -> String {
    if ctx.via_add_only {
        ...
        crate::naming::out_dir(d).to_string_lossy().into_owned()
    }
}
...
fn m_open_dir(
    ...
    Some({
        ...
        let target: Option<PathBuf> = match key {
            "out_dir" => Some(crate::naming::out_dir(d).clone()),
...
fn fs_list(d: &Arc<Daemon>, params: &std::collections::HashMap<String, String>) -> Value {
    ...
    let start = if raw.is_empty() {
        crate::naming::out_dir(d)
            .canonicalize()
            .unwrap_or_else(|_| crate::naming::out_dir(d).clone())
    } else {
        PathBuf::from(raw)
    };
    ...
    match listed {
        Some((shown, entries, writable, note)) => {
            let mut v = json!({
                ...
                "writable": writable,
                "entries": entries,
                "roots": fs_roots(&crate::naming::out_dir(d)),
            });
...
fn m_remote_info(
    ...
    Some({
        ...
        if !ctx.host_hdr.is_empty() && !is_loopback {
            ...
            if let Some(a) = crate::lanaddr::route_src("8.8.8.8:53") {
        ...
        // sandbox-seam-gate: `hostname`, a literal program with no
        // arguments at all, read for this machine's own name. Nothing an NZB
        // or an HTTP request can influence reaches it.
        ...
        // never changes.
        let ts = crate::lanaddr::route_src("100.100.100.100:53").filter(|ip| match ip {
            std::net::IpAddr::V4(v) => {
                let o = v.octets();
...
/// for_servers` keyed the live rows from; `tasks/tuner.rs` mints
...
fn sab_servers(d: &Daemon, ctx: &ApiCtx<'_>) -> Vec<Value> {
    ...
    // the discipline `metrics.rs::server_metrics` records: these
...
fn sab_status_body(d: &Arc<Daemon>, ctx: &ApiCtx<'_>, unlimited_abs_as_empty: bool) -> Value {
    ...
    let (_, total_b) = disk_stat_walk(&crate::naming::out_dir(d)).unwrap_or((0, 0));
...
pub(crate) fn dispatch(
    ...
    Some(match mode {
        ...
        "report" => {
            ...
            match crate::report::job_report(d, id) {
        ...
        "usage" => {
            ...
            json!({"days": Value::Object(d.usage.lock_ok().clone()), "blocks": blocks,
                   "quality": d.provquality.report_json(crate::unix_now())})
        }
        ...
        "warnings" => json!({
            "warnings": sab_warnings(d, ctx.cfg_path, ctx.via_add_only, free_bytes(&crate::naming::out_dir(d)))
        }),
...
fn mem_floor_json(at_peak: Option<nzbkit::memgauge::PeakAttribution>) -> Value {
    let names = [
        ...
        nzbkit::memgauge::Sub::WriteStage,
    ];
