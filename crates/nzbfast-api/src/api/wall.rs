    ...
    Some({
        ...
        if target == "all" {
            ...
            let swept = match d.index_write_checked(|ix| {
                ...
                Ok(w) => w,
                Err(why) => return Some(why.refusal()),
            };
            match swept {
...
mod tests {
    ...
    fn the_art_staging_name_is_never_servable() {
        ...
        let staged = crate::maint::art_staging_name(&live);
        assert!(crate::apiutil::art_name_ok(&live), "{live}");
        assert!(
            !crate::apiutil::art_name_ok(&staged),
    ...
    fn an_art_name_leaves_room_for_its_staging_name() {
        ...
        for key in [long.as_str(), "m:the matrix:1999"] {
            for backdrop in [false, true] {
                ...
                assert!(!crate::maint::is_art_staging_name(&live), "{live}");
                assert!(crate::apiutil::art_name_ok(&live), "{live}");
                assert!(
                    crate::apiutil::art_name_ok(&format!("thumb_{live}")),
    ...
    fn staging_files(dir: &std::path::Path) -> usize {
        std::fs::read_dir(dir)
            ...
            .filter(|e| {
                e.file_name()
                    .to_str()
                    .is_some_and(crate::maint::is_art_staging_name)
            })
            .count()
    }
}
