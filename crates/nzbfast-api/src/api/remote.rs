...
pub(crate) fn move_by_offset(d: &Arc<Daemon>, ids: &[i64], offset: i64) -> bool {
    let nzo_int = crate::sabcompat::nzo_int;
    let mut ok = false;
...
///   loop holds.
pub(crate) fn pause_by_ids(d: &Arc<Daemon>, ids: &[i64], pause: bool) -> bool {
    let nzo_int = crate::sabcompat::nzo_int;
    if pause {
...
pub(crate) fn rename_by_ids(d: &Arc<Daemon>, ids: &[i64], newname: &str) -> bool {
    let nzo_int = crate::sabcompat::nzo_int;
    if newname.is_empty() {
        return false;
    }
...
pub(crate) fn set_parameter(d: &Arc<Daemon>, ids: &[i64], param: &str) -> bool {
    let nzo_int = crate::sabcompat::nzo_int;
...
pub(crate) fn complete_action_arm(params: &std::collections::HashMap<String, String>) -> Value {
...
pub(crate) fn sort_queue(d: &Arc<Daemon>, key: &str, asc: bool) -> bool {
    ...
    fn a_rename_moves_the_queue_revision() {
        ...
        let job = Arc::new(Mutex::new(
            job_from_json(&json!({
                ...
                "nzb_path": "/tmp/x.nzb", "out_dir": crate::naming::out_dir(&d).join("Before.Rename"),
                "state": "Queued",
            }))
