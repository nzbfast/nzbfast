...
// callers are in this file - which is reason enough on its own. It also
// had nowhere to go at the time: daemon.rs was on TODO 106's burn-down
// list with no room left. That half expired: the 31 Aug 2026
// recalibration and then the 1 Sep 2026 split left it 1,252 clear,
// measured that day. So a later lane moving this back beside
// `cat_list` is arguing about co-location, not about the gate.
...
pub fn client_cats(d: &Daemon) -> std::collections::BTreeSet<String> {
    ...
    let mut set = d.cats.lock_ok().clone();
    for c in d.custom_categories.read_ok().iter() {
        if !c.slug.is_empty() {
...
fn m_get_config(
    ...
    Some({
            ...
            .map(|c| {
                c.servers
                    ...
                    .map(|s| {
                        ...
                        // TODO 313 item 8: what the standing warm
                        // reserve is ACTUALLY holding on this account,
                        // which is a request bounded by the gap under
                        // the account's cap and not the configured
                        // number. Published beside the configured one
                        // for the same reason `idle_release_effective`
                        // is published beside the raw fields above: a
                        // setting quietly different from what somebody
                        // typed is how they lose an evening
                        // (`nzbfast-redeploy-resets-server-enables`).
                        //
                        // `None` until the reserve's first tick, or on
                        // an install that has never had a pool, and the
                        // echo says so rather than reporting a zero that
                        // reads like "your reserve is not working".
                        let res = d
                            .hub
                            .warm_reserve
                            .get()
                            .and_then(|r| r.status_for(s));
                        json!({
                            "host": s.host,
                            "port": s.port,
                            "tls": s.tls,
                            ...
                            // TODO 313 item 8. Config-file only in this
                            // lane, deliberately: a setting is three
                            // places plus 27 i18n catalogues and 16
                            // manuals, and the knob waits for demand
                            // (the `postproc_jobs` precedent). The echo
                            // is not a control - it is the diagnostic
                            // the item requires.
                            "warm_reserve": s.warm_reserve.unwrap_or(0),
                            "warm_reserve_effective": res.as_ref().map(|r| json!({
                                "held": r.effective,
                                "parked": r.parked,
                                "note": r.note.as_str(),
                            })),
        ...
        for (k, active) in [
            ...
            ("tls_cert", crate::settings::path_str(&d.tls_cert)),
            ("tls_key", crate::settings::path_str(&d.tls_key)),
        ] {
        ...
        json!({
            "config": {
                ...
                "misc": {"complete_dir": crate::naming::out_dir(d).to_string_lossy(),
    ...
    Some({
        ...
        match (name.as_deref(), value.as_deref()) {
            ...
            (Some("queue_finished_cancel"), _) => {
                let mut out = crate::finish_action::cancel(d);
            ...
            Ok(text) => {
                ...
                let cat_report = {
                    ...
                    if !names.is_empty() {
                        ...
                        let merged = crate::merge_cat_list(&before, &names.join(", "));
                        if merged != before {
                    ...
                    let before = crate::fmt_cat_dests(&d.move_completed_cats.read_ok());
...
pub(crate) fn dispatch(
    ...
    Some(match mode {
        ...
        "get_cats" => {
            json!({"categories": client_cats(d).into_iter().collect::<Vec<_>>()})
        }
