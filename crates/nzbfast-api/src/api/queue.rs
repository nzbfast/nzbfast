...
// controls are re-exported because `crate::api::queue::apply_pause`
...
// stayed in the bin (it composes an api item with a `tasks::` one, and
// those are sibling crates), and a `pub` item behind a `pub(crate)`
// re-export is still invisible across the boundary - step 3's own
// finding, met again at this cut.
...
fn m_alt_hunt(
    ...
    let id = params.get("value").cloned().unwrap_or_default();
    Some(match crate::hunt::hunt_offer(d, &id) {
        Ok(mut v) => {
...
fn m_alt_hunt_pick(
    ...
    let id = params.get("value").cloned().unwrap_or_default();
    let alt = params.get("alt").cloned().unwrap_or_default();
    Some(match crate::hunt::hunt_pick(d, &id, &alt) {
        Ok(nzo_id) => json!({"status": true, "nzo_id": nzo_id}),
        Err(e) => json!({"status": false, "error": e}),
    })
}

/// §310 stage 2, the READ-ONLY half. `value` is a library folder; the
/// answer is what its settle manifest says is damaged, grouped by the
/// post each piece would be re-fetched from.
///
/// The verify it runs is a full read of the folder's payload, which is
/// the price of knowing. Beyond that it spends NOTHING: no indexer is
/// asked, no grab is spent, nothing is enqueued and nothing downloads.
/// `mode=heal_start` is the one that spends, on a second call.
fn m_heal_offer(
    d: &Arc<Daemon>,
    _req: &mut tiny_http::Request,
    params: &std::collections::HashMap<String, String>,
    ...
    let dir = params.get("value").cloned().unwrap_or_default();
    Some(match crate::heal::heal_offer(d, &dir) {
        Ok(mut v) => {
            if let Some(o) = v.as_object_mut() {
                o.insert("status".into(), json!(true));
            }
...
/// §310 stage 2, the half that spends. `value` is the library folder,
/// and the optional `alt` is ONE post's `nzb_sha` from the offer's list;
/// without it every damaged post in the folder is healed, up to the
/// module's own cap.
///
/// This starts real downloads, which is why it is a separate call the
/// user makes deliberately rather than something `heal_offer` or
/// `nzbfast verify` does on their behalf. Each job it queues carries the
/// folder as a donor directory, so §293's adoption scan reads every
/// intact byte off the disk and only the damaged blocks come off the
/// wire.
fn m_heal_start(
    d: &Arc<Daemon>,
    _req: &mut tiny_http::Request,
    ...
    let dir = params.get("value").cloned().unwrap_or_default();
    let sha = params.get("alt").cloned().unwrap_or_default();
    Some(match crate::heal::heal_start(d, &dir, &sha) {
        Ok(mut v) => {
            if let Some(o) = v.as_object_mut() {
                o.insert("status".into(), json!(true));
            }
                ...
                Some(job) => {
                    ...
                    if locked {
                        ...
                        tokio::task::spawn_blocking(move || {
                            ...
                            // [`crate::unlockpw::unlock`].
                            let unlocked = crate::unlockpw::unlock(&out_dir, &pw);
                            if unlocked.is_ok() {
    ...
    Some({
        ...
        match nzbkit::nzblnk::parse(&raw) {
            ...
            Ok(l) => {
                ...
                let peer = crate::httputil::peer_ip(req);
...
fn m_watch_failed_delete(
    ...
    Some({
        ...
        let path = {
            ...
            wf.keys()
                .find(|p| crate::watchfail::watch_fail_id(p) == want)
...
fn m_hist_upgraded_dismiss(
    ...
    Some(json!({"status": crate::histmigrate::dismiss_hist_migrate(d)}))
}
...
///
/// `pub(crate)` for the daemon-side test that pins the bump -
/// the ring and its revision are serve-wide state, not this module's.
pub fn spend_kept_notice(d: &Arc<Daemon>, path: &str) -> bool {
    let gone = {
...
#[cfg(test)]
pub(crate) static KEPT_RETRY_STALL_MS: AtomicU64 = AtomicU64::new(0);
...
pub(crate) fn retry_kept_notice(d: &Arc<Daemon>, path: &str) -> Value {
...
pub(crate) fn dispatch(
    ...
    Some(match mode {
        ...
        // §310 stage 2: the settle manifest can already say a library
        // folder is damaged; these two turn that into a repair. The
        // offer reads and reports, the start queues one download per
        // damaged post with the folder itself as a donor. Two calls
        // rather than one because the second spends the user's bytes.
        "heal_offer" => return m_heal_offer(d, req, params, ctx, api_body),
        "heal_start" => return m_heal_start(d, req, params, ctx, api_body),
...
// `pub` for the reason `webasset::Precompressed` is: `requeue_category`
// is `pub` because `daemon_api_tests.rs` stayed in the bin, and
// `private_interfaces` refuses a `pub` fn returning a crate-private
// type.
...
pub(crate) fn persist_relocations(d: &Daemon, fences: Vec<Relocation>) -> bool {
    if d.save_queue() {
        return true;
    }
    for f in fences {
