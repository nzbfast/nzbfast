...
#[cfg(test)]
pub(crate) static DELETE_PREWRITE_BARRIER: Mutex<
...
pub fn public_base(req: &tiny_http::Request, d: &Daemon) -> String {
...
/// become Normal.
pub(crate) fn parse_priority_token(v: &str) -> Option<i32> {
    if let Ok(n) = v.parse() {
        return Some(n);
    }
...
fn watch_failed_json(d: &Daemon) -> Vec<Value> {
    ...
    v.into_iter()
        .map(|(name, error, nzo_id, wf_id)| {
            ...
            let kind = crate::watchfail::watch_fail_kind(&error);
            json!({"name": name, "error": error, "kind": kind,
                   "ingested": crate::watchfail::watch_fail_ingested(kind),
...
fn queue_notices(d: &Daemon) -> QueueNotices {
    ...
    let hist_upgraded = crate::histmigrate::hist_notice(d)
...
struct SlotCtx {
    ...
    alt_held: Vec<crate::altcand::HeldSpare>,
    ...
    pause_cost: Vec<(String, crate::requeue::RequeueCost)>,
}
...
fn slot_json(
    ...
    let labels = {
        ...
        if j.paused && j.priority == crate::job::DUPE_PRIORITY {
    ...
    json!({
        ...
        "alt_offer": crate::altcand::offer_json(j, alt_held, *alt_auto_search),
        ...
        "insurance": crate::insurance::slot_payload(j),
...
pub(super) use units::{pause_int, sab_age, sab_elapsed, sab_timeleft};
...
pub fn queue_json(d: &Daemon, params: &std::collections::HashMap<String, String>) -> Value {
    ...
    let pause_source = crate::pause_source_now(d);
    ...
    json!({"queue": {
        ...
        "finish_action": crate::finish_action::payload(d),
        ...
        "hold": hold, "storage_pause": crate::slowstore::payload(d),
        ...
        // unknown); source "measured" or "line" (see linkpeak.rs).
...
fn jr_status(d: &Arc<Daemon>) -> Value {
    ...
    };
    {
        ...
        let (disk_free, _) = disk_stat_walk(&crate::naming::out_dir(d)).unwrap_or((0, 0));
...
/// NZBGet's `history(bool hidden)`. `hidden = true` asks for the rows
/// `HistoryDelete` marked hidden IN ADDITION to the live ones, which is
/// how NZBGet's own client shows a "hidden" history view; the default
/// listing omits them.
///
/// Neither Sonarr nor Radarr ever passes the flag - their proxies call
/// `history` with no params - so nothing an *arr does depends on this
/// arm. It exists because the verb that produces a hidden row is
/// otherwise a row nothing in this dialect can ever list again, which
/// would make "hidden" indistinguishable from "erased" from the outside.
fn jr_history(d: &Arc<Daemon>, params: &[Value]) -> Value {
    // FIRST POSITIONAL, per the NZBGet signature, and absent means
    // false - a client that sends no params gets the live listing.
    let with_hidden = params.first().and_then(Value::as_bool).unwrap_or(false);
    let unix_now = || {
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map(|d| d.as_secs() as i64)
    ...
    };
    {
        let entries: Vec<Value> = d
            ...
            .iter()
            .rev()
            .filter(|j| with_hidden || !j.lock_ok().hidden)
            .map(|j| {
                let g = j.lock_ok();
...
fn jr_editqueue(d: &Arc<Daemon>, params: &[Value], rpc_error: &mut Option<String>) -> Value {
    {
        ...
        match cmd {
            ...
            "HistoryDelete" | "HistoryFinalDelete" => {
                ...
                // 4 Sep 2026. NZBGet documents `HistoryDelete` as "hide
                // history items (mark as hidden)" - the row survives for
                // the duplicate check and is listed by `history(true)` -
                // and `HistoryFinalDelete` as the erase. Sonarr's
                // `NzbgetProxy.RemoveItem` sends the FORMER, and its
                // "Remove completed downloads" fires it after every
                // import, so every release an *arr imported left our
                // history for good: `dupe_collision` and
                // `owned_dupe_keys` read Completed history rows, so a
                // re-grab of the identical release was not held and the
                // wall lost its "you have this" badge. See `Job::hidden`
                // for the asymmetry that fixes it. Neither verb has ever
                ...
                if busy {
                    // BOTH verbs, unchanged: a record whose files are
                    // mid-move is being worked on disk right now, and
                    // hiding it takes the row out of every view the user
                    // could watch that work from. The refusal is the
                    // same statement for either spelling.
                    *rpc_error = Some(
                    ...
                    // The HIDE. Nothing is destroyed and nothing leaves
                    // the list: the row keeps its place, its dupe key,
                    // its spooled `.nzb` (which the Redownload arm below
                    // needs) and its early-published copies. Only the
                    // display filter in `history_page` changes what it
                    // does with it.
                    let hidden: Vec<Arc<Mutex<Job>>> = h
                        .iter()
                        .filter(|j| {
                            let mut g = j.lock_ok();
                            // Already hidden is not a fault and not a
                            // write: an *arr that repeats its cleanup
                            // must not cost a store line per poll.
                            let hit = ids.contains(&nzo_int(&g.nzo_id)) && !g.hidden;
                            if hit {
                                g.hidden = true;
                            }
                            hit
                        ...
                        .cloned()
                        .collect();
                    // `true` for an id that was already hidden, exactly
                    // as NZBGet answers a second hide - the state the
                    // caller asked for is the state it has.
                    ok = h
                        .iter()
                        .any(|j| ids.contains(&nzo_int(&j.lock_ok().nzo_id)));
                    drop(h);
                    // Through the same seam a state change takes, so the
                    // hide survives a restart - a hide that did not would
                    // put every imported release back in the *arr's face
                    // at the next start.
                    for j in &hidden {
                        if d.history_publish_change(j, "hiding the history record")
                            == nzbfast_daemon::histstore::HistWrite::Refused
                        {
                            // Refused means the store cannot be written
                            // at all (the append AND the atomic rewrite),
                            // so the flag is true in memory and nowhere
                            // else. Put it back rather than answer `true`
                            // for a change a restart undoes - the same
                            // take the erase arm below makes about its
                            // own refusal.
                            for j in &hidden {
                                j.lock_ok().hidden = false;
                            }
                            *rpc_error = Some(
                    ...
                    // The releases this verb removes for good. Parity
                    // with the REST delete and with `editqueue_delete`,
                    // both of which stamp `note_releases_deleted`; this
                    // arm never did, so the "you deleted this recently,
                    // so the re-add is the download you asked for" mark
                    // (`dupe.rs`) was unreachable through the NZBGet
                    // door and an *arr's delete-then-re-grab was held as
                    // a duplicate of the copy the user had just thrown
                    // away.
                    ...
                    h.retain(|j| {
                        ...
                        if hit {
                            ...
                            // The REST delete's other statement, which
                            // this door never made - see below.
                    ...
                    if d.history_tombstone(&gone) {
                        ...
                        // After the removal rather than inside it, and
                        // only once it is durable - the same placement
                        // the REST arm gives it: this is the client's
                        // statement that it no longer has these
                        // releases, and it touches nothing the removal
                        // touches. Not stamped by the HIDE above, which
                        // is the opposite statement: a hidden row is one
                        // we still have.
...
fn jr_config(d: &Arc<Daemon>, _rpc_error: &mut Option<String>) -> Value {
    {
        ...
        let mut cfg = vec![
            ...
            json!({"Name": "DestDir", "Value": crate::naming::out_dir(d).to_string_lossy()}),
        ...
        for (i, c) in d.cats.lock_ok().iter().filter(|c| *c != "*").enumerate() {
            ...
            cfg.push(json!({
                ...
                "Value": crate::naming::out_dir(d).join(c).to_string_lossy(),
            }));
        }
        json!(cfg)
    }
...
// The other half of the NZBGet delete pair: `HistoryDelete` HIDES.
// Nothing in it needs a mode bit or a read-only directory, so it is not
// unix-gated like the two modules below.
#[cfg(test)]
mod history_hidden_tests;
...
// `slot_progress` moved DOWN into `nzbfast_daemon::sabvocab` when the
// daemon layer became its own crate. It is pure - a `JobState` and two
// byte counters in, a percentage and a remainder out - and what it
// decides is a DAEMON-layer question that this facade only prints, so
// five daemon-layer test files assert on it. Reached here as a bare
// name through serve's root `use sabvocab::*;` and NOT re-exported, for
// the reason `sabcompat/units.rs` gives about the two `sab_units`.
