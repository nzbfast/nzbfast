...
//! the `crates/nzbkit`, `crates/nzbtray` and `crates/nzbfast-core`
//! copies.
//!
...
//! `#[path]`, exactly as the other four crates do, so the five files
//! stay one file in five places rather than becoming five designs. This
//! copy arrived with the `nzbfast-unpack` cut (crate-split step 3):
//! `smart`'s filing tests and `unpack`'s publish-name tests both build
//! real directories under the OS temp dir, and they moved up here with
//! the modules.
//!
