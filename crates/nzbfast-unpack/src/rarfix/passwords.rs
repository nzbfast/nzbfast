...
pub const PW_PROBE_BUDGET: std::time::Duration = std::time::Duration::from_secs(20);
...
pub fn harvest_password_candidates(
    ...
    for pw in crate::pwfile::operator_passwords() {
...
pub(crate) fn sevenz_password_check_capped(
    ...
    let Ok(mut reader) = super::open_sevenz(parts, password) else {
    ...
    // An entry with NO checksum settles nothing either, and this is the
    // one arm where a wrong key reads as a right one.
    //
    // What rejects a wrong key on a data-encrypted entry is the entry's
    // CRC at its end - the doc above says so. `has_crc` false means the
    // library builds no verifying reader at all, and 7z AES-256-CBC has
    // no MAC and no padding a key error shows up in: `do_final` only
    // objects to a sub-block tail, which is a property of the LENGTH,
    // not of the key. So a stored (Copy) AES entry with the CRC omitted
    // - legal in the format - decodes to `entry.size` bytes of garbage
    // under ANY non-empty password, the copy succeeds, and the candidate
    // was ranked `Opens` and announced as an auto-unlock.
    //
    // `Unknown` is the state the 64 MB cap already uses for exactly this
    // reason: the read finished without ever reaching a checksum.
    let unchecked = std::cell::Cell::new(false);
...
pub(crate) fn resolve_level_password(
    ...
    if let Ok(jobs) = collect_sevenz_archives(dir)
        ...
        && super::sevenz_set_is_encrypted(z)
...
pub(crate) fn sevenz_password_candidates(
    ...
    if !super::sevenz_set_is_encrypted(z) {
