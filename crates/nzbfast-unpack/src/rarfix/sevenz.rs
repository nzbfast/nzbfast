...
pub use crate::archname::split_7z_part;
...
pub fn collect_sevenz_archives(dir: &std::path::Path) -> Result<Vec<Vec<PathBuf>>> {
...
pub fn extract_sevenz(
    ...
    for parts in jobs.iter() {
        ...
        let cands = crate::rarfix::passwords::sevenz_password_candidates(parts, dir, password);
                ...
                .and_then(|_| out.publish_into(dir))
            {
                Ok(()) => {
                    if pw.is_some() && source != "job password" {
                        crate::rarfix::passwords::log_auto_unlocked(first, source);
                    }
...
pub fn concat_files(parts: &[PathBuf], dest: &std::path::Path) -> Result<()> {
...
pub fn open_sevenz(
    parts: &[PathBuf],
    password: Option<&str>,
) -> Result<sevenz_rust2::ArchiveReader<std::io::BufReader<SplitParts>>> {
    ...
    // A wide read buffer in front of the raw part files: sevenz-rust2's
    // AES decoder pulls its input 512 bytes at a time and the Copy path
    // reads in the writer's chunk, so an unbuffered source cost an
    // encrypted 1 GiB member two million read syscalls (1.46 s of system
    // time on an M1 Ultra against 7-Zip's 0.23 for the whole job;
    // research/RAR-PERF-AUDIT-2026-09-02.md, round 7). The header walk
    // seeks through it, which just drops the buffer.
    let src = std::io::BufReader::with_capacity(1 << 20, src);
    let mut reader = ArchiveReader::new(src, pw).map_err(|e| anyhow::anyhow!("opening 7z: {e}"))?;
    // LZMA2 streams 7-Zip wrote with its default multithreading are a
    // sequence of independently decodable blocks (a dictionary reset every
    // block), and sevenz-rust2 decodes them in parallel when asked - its
    // default is ONE thread, which left a 1 GiB LZMA2 member at 9.2 s
    // against 7-Zip's 1.13 on an M1 Ultra. Each worker holds its own
    // dictionary, so the count is capped; Copy and AES folders ignore it
    // (research/RAR-PERF-AUDIT-2026-09-02.md, round 11).
    // Each worker holds a block of output plus its dictionary and the
    // reader buffers finished blocks out of order: eight workers peaked
    // at 2.7 GB on a 1 GiB LZMA2 member with a 32 MiB dictionary, so the
    // count is also bounded by the process budget's decoder allowance at
    // a conservative ~320 MiB per worker.
    let allowance = nzbkit::mem::process_budget()
        .rar_execution_policy()
        .working_memory_limit;
    let by_memory = (allowance / (320 << 20)).clamp(1, 8) as usize;
    let threads = nzbkit::mem::cpu_workers().clamp(1, 8).min(by_memory) as u32;
...
pub fn sevenz_set_is_encrypted(parts: &[PathBuf]) -> bool {
...
fn sevenz_pass(
    ...
    // Budget and preallocation ceiling together - see
    // `extract_budget_and_prealloc_cap` for why they cannot be derived
    // from one flattened `u64`.
    ...
    let result = reader.for_each_entries(|entry, rd| {
        ...
        let (file, skip, crc) = match resume.get(entry.name.as_str()) {
            ...
            None => {
                ...
                // Reservation bounded by the free space this pass may use.
                let _ = nzbkit::disk::preallocate_output(&f, entry.size, free_for_output);
                (f, 0, 0)
            }
        };
        let mut w = crate::resumeout::ResumeWriter::verified(
            ...
            mismatch.clone(),
            BombGuardWriter {
                // A wide buffer: `std::io::copy` moves data through the
                // writer's own buffer, so its capacity IS the syscall
                // size. At the 8 KiB default a 1 GiB Copy-method member
                // was 131,072 reads and writes (0.79 s of system time on
                // an M1 Ultra against 7-Zip's 0.20); at 1 MiB it is 1,024
        ...
        let copied = piped_copy(rd, &mut w)?;
        use std::io::Write as _;
        w.flush()?;
        // A SHORT member is not a successful extract.
        //
        // The library hands back `Ok` on a packed stream that ends
        // early: `Crc32VerifyingReader` compares the CRC only once
        // `remaining` reaches zero, and an early EOF returns `Ok(0)`
        // without ever entering that arm - so the one check that would
        // have caught it is the one that never runs. Nothing else here
        // looked at the length, so a 7z whose end header parsed but
        // whose packed stream cannot supply `entry.size` (a folder
        // declaring more than its pack area holds, a short joined split
        // whose last part is intact, a decoder stopping early) wrote a
        // truncated file and logged the unpack complete. That is the
        // path a demoted in-stream chase takes.
        //
        // Both siblings already draw this line - the in-stream chase at
        // `nzbkit::extract::sevenz` (`sink.pos != entry.size`, with a
        // comment naming this same library behaviour) and the zip disk
        // arm in `nzbkit_base::zip` ("entry shorter than its declared
        // size"). This arm is the one that never grew it.
        //
        // `copied` is the whole decoded stream from offset 0 in both
        // arms: a resumed member sends the bytes below its mark to a
        // counter rather than to the device, so the figure is comparable
        // to `entry.size` either way.
        if copied != entry.size {
            return Err(sevenz_rust2::Error::Other(std::borrow::Cow::Owned(
                format!(
                    "{}: extracted {copied} bytes of a declared {}",
...
/// `std::io::copy` with the writes on their own thread: the reader here
/// is the archive's decoder chain (Copy, AES, LZMA2 - whatever the folder
/// declares), and running it in series with the disk write left a 1 GiB
/// Copy member at 0.71 s of wall for 0.46 s of CPU on an M1 Ultra, 2x
/// 7-Zip, whose own loop overlaps the two. Four 1 MiB buffers cycle
/// through a bounded channel; a write error stops the reader at its next
/// hand-off and is what the call returns
/// (research/RAR-PERF-AUDIT-2026-09-02.md, round 8).
fn piped_copy<W: std::io::Write + Send>(
        ...
        let read_result: std::io::Result<()> = (|| {
            loop {
                let Ok(mut buf) = empty_rx.recv() else {
                    // The writer stopped early; its error is the answer.
                    return Ok(());
                };
                let mut len = 0;
                while len < BUF {
                    let n = rd.read(&mut buf[len..])?;
                    if n == 0 {
                        break;
                    }
                    len += n;
                ...
                if full_tx.send((buf, len)).is_err() {
                    return Ok(());
                }
            }
        })();
        drop(full_tx);
        let written = writer.join().expect("7z writer thread panicked")?;
        read_result?;
        Ok(written)
    })
}
