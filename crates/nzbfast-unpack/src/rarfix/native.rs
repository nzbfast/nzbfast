...
fn unpack_pass(
    ...
    // The budget and the preallocation ceiling disagree about a volume
    // nothing could measure, so they are taken TOGETHER and not derived
    // from each other - see `extract_budget_and_prealloc_cap`, which
    // carries the incident that put this here.
    let (bomb_budget, free_for_output) =
        crate::rarfix::extract_budget_and_prealloc_cap(crate::diskfree::free_bytes(dir));
    ...
    let open = move |meta: &rars::ExtractedEntryMeta| {
        ...
        let created = std::fs::File::create(target)?;
        // Reservation bounded by the free space this pass may use; the
        // engine writes 1 MiB chunks, so the BufWriter is pass-through.
        let _ = nzbkit::disk::preallocate_output(&created, meta.unpacked_size, free_for_output);
        let file = std::io::BufWriter::new(created);
