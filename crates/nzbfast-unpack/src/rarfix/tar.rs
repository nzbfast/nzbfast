...
pub(crate) fn is_tar_container(path: &std::path::Path) -> bool {
    ...
    //
    // The block comes from the head memo, which caches exactly
    // `nzbkit::tar::BLOCK` bytes (that is what set `HEAD_LEN`): the
    // ladder asks this of every file in the directory from three sites
    // per level, and the `stat` this used to do on its own is now the
    // memo's whole cost on the repeats.
    let Some(h) = nzbkit::headpeek::head(path) else {
        return false;
    };
    if h.file_len < (nzbkit::tar::BLOCK * 3) as u64 {
        return false;
    }
    nzbkit::tar::looks_like_tar(h.bytes())
}
...
pub fn first_tar_container(dir: &std::path::Path) -> Option<PathBuf> {
...
pub fn extract_tar(dir: &std::path::Path, jobs: &[PathBuf]) -> Vec<PathBuf> {
