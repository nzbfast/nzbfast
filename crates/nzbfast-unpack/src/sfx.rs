...
pub fn sfx_disk_shape(path: &std::path::Path) -> Option<DiskArchive> {
    match sfx_kind(path)? {
...
pub fn collect_sfx_archives(dir: &std::path::Path) -> Result<Vec<PathBuf>> {
    let mut out = Vec::new();
...
/// stub itself).
pub fn extract_sfx(dir: &std::path::Path, archives: &[PathBuf], password: Option<&str>) -> bool {
    let options = nzbkit::mem::rar_read_options(password.map(str::as_bytes));
    // One parse session for the loop. Each SFX archive here is its own
    // set with its own salt, so across archives the shared RAR 5 key
    // cache buys nothing - but the carve fallback below re-opens the SAME
    // archive, and through `read_path_with_options` (a fresh cache per
    // call) an encrypted SFX whose direct read got as far as its salt
    // paid the 2^15-round PBKDF2 a second time on the carved copy. The
    // session serves the repeat from the cache
...
mod sfx_tests {
    ...
    fn a_dump_carrying_an_archive_is_not_a_self_extractor() {
        ...
        let sevenz = std::fs::read(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nzbkit-base/tests/fixtures/sevenz/store-single.7z"
        ))
        .unwrap();
        let zip = std::fs::read(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nzbkit-base/tests/fixtures/zip/store_deflate.zip"
        ))
        .unwrap();
