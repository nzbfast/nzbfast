...
pub const TAIL_NESTED_ENTRY_DEPTH: usize = 1;
...
pub(crate) fn extract_one_level_at(
    ...
    if !obf.is_empty() {
        ...
        } else if r.ok() || depth > 0 {
            claim(NestOutcome::Produced, &mut out);
        }
        // …and when this arm delivered NOTHING, refused a fragment, and
        // the directory holds nothing else either, it claims NOTHING and
        // lets the level fall to the "looks like an archive but no
        // extractor claimed it" backstop at the bottom of this function.
        //
        // That is the line `ObfReport::ok` has drawn since 22 Aug 2026 -
        // "a stray beside a set that extracted" is forgiven, "this
        // directory was NOTHING BUT fragments" is not - and until now
        // only the two payload-path callers (`repair::reextract_dir`,
        // `rarfix`) read it. The offline `nzbfast extract` reads this
        // verdict instead, so it exited 0 on an undeliverable set:
        // measured 3 Sep 2026 by the `obfuscated_missing_first`
        // robustness fixture and, independently, by SABnzbd's own
        // `obfuscated_single_rar_set_missing_first_rar`
        // (research/RAR-PERF-AUDIT-2026-09-02.md round 43).
        //
        // DEPTH is what separates the two, and it is the same line
        // `ObfReport::ok`'s own doc draws. Depth 0 is the offline
        // `nzbfast extract <dir>`: the user pointed at a directory and
        // asked for it to be unpacked, so the volumes on disk ARE the
        // job's payload, exactly as they are for `reextract_dir` and
        // `rarfix`, and "not one usable set could be formed from them"
        // has to stay a failure. Depth >= 1 is a nested pass inside a job
        // whose payload may already have been published by an EARLIER
        // pass - the 22 Aug field case, where the fragment is a leftover
        // beside a delivered movie and no arm at this level claims
        // anything at all. Forgiving on `r.ok()` alone drops that case
        // through the backstop, which reads the fragment's `Rar!` head
        // and fails a job that is complete: measured, it fails
        // `a_stray_mid_set_volume_alone_beside_the_payload_does_not_fail_the_job`.
        //
        // A directory-content test was tried first and REJECTED: asking
        // "is there a file here that is not a refused fragment" forgives
        // on any sidecar - a leftover `.nfo`, a `.par2`, the harness's
        // own log - which is not evidence that a payload was delivered.
    }
...
/// Does the file start with the `PAR2\0PKT` packet magic?
///
/// Defined in [`crate::archname`] since the crate-split prep and
/// re-exported here, because the callers reach it under both names.
pub(crate) use crate::archname::file_starts_with_par2_magic;
...
pub fn dir_has_par2(dir: &std::path::Path) -> Result<bool> {
...
// The password ladder lives in `rarfix::passwords` since the
// crate-split prep: every probe in it opens a container through the
// extractor (`open_sevenz`, `sevenz_set_is_encrypted`), and the
// extractor's own arms then asked this module for the candidates -
// which is the one thing that made `unpack` and `rarfix` need each
// other. Re-exported here, so every `crate::unpack::` and bare-name
// caller is unchanged.
pub use crate::rarfix::passwords::*;
...
/// The name grammar the RAR extract paths share: `.rar`/`.rNN` by name,
/// or a rollover / numeric extension carrying the Rar! magic.
///
/// Defined in [`crate::archname`] since the crate-split prep and
/// re-exported here, because the callers reach it under both names.
pub use crate::archname::looks_like_named_rar;
...
/// the worse failure.
pub fn nested_archive_beside_leftovers(
    dir: &std::path::Path,
...
pub fn is_extractable_archive(path: &std::path::Path) -> bool {
...
/// The per-file runtime state and the name question hanging off it.
///
/// Both live in [`crate::fileslot`] since the crate-split prep and are
/// re-exported here, so `crate::unpack::FileSlot`,
/// `crate::unpack::slot_name::*` and the bare names reached through
/// `super::*` all still resolve.
pub use crate::fileslot::{FileSlot, slot_name};
pub use published_names::*;
pub use slot_name::NAME_UNDECIDED;

/// Does the file start with the 7-Zip signature (`7z\xBC\xAF\x27\x1C`)?
///
/// Defined in [`crate::archname`] since the crate-split prep and
/// re-exported here, because the callers reach it under both names.
pub(crate) use crate::archname::rar_recovery_volume_magic;
pub(crate) use crate::archname::sevenz_magic;

/// The bounded PAR2 packet-byte scan of a directory, and the cap it
/// takes off the process budget.
///
/// Defined in [`crate::par2scan`] since the crate-split prep and
/// re-exported here, because the callers reach them under both names.
pub(crate) use crate::par2scan::{collect_par2_bytes, par2_scan_cap};

/// What a `verify_dir` pass concluded, with the two answers that used to
/// share `false` told apart.
///
/// The bool this replaces meant "everything checked out" OR "nothing was
/// checked at all" - no PAR2 in the directory, every PAR2 file over the
/// scan cap, or a parse that failed - and a caller could not tell which,
/// so both call sites discarded it and `nzbfast verify` exited 0 over
/// PAR2-detected damage (TODO 310). The manifest arm of the same command
/// has always had a bool that means one thing and exits 1 on it; these
/// three variants are what let the two arms agree.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DirVerify {
    /// Every file in every adopted set checked out, blocks and MD5.
    Clean,
    /// At least one file is damaged, missing or unreadable. The reason
    /// per file is already on the log at `warn` level.
    Damaged,
    /// No verdict was reached: nothing to verify, or nothing readable to
    /// verify it with. NOT a pass, and NOT damage.
    NothingToVerify,
}

impl DirVerify {
    /// True only for [`DirVerify::Clean`] - i.e. only when this pass
    /// actually PROVED the files on disk. `NothingToVerify` answers false
    /// here for the same reason it is not `Clean`: nothing was read.
    pub fn proved_clean(self) -> bool {
        matches!(self, DirVerify::Clean)
    }
}

pub fn verify_dir(dir: &std::path::Path) -> Result<DirVerify> {
    use nzbkit::par2::verify_file_path_tiered;

    // Resolved ONCE for the whole directory, not per member. The value
    // behind it is a live daemon setting, and a flip landing between two
    // members of one set would have them answer under different rules -
    // the thing `nzbkit::par2::set_fast_check` exists to prevent.
    ...
    // Hash independent members in parallel, biggest first so a large member
    // cannot become a tail after all the short ones finish. This used to walk
    // the set serially: a normal 21-volume release left 31 cores idle while
    // doing two MD5 chains over one volume at a time.
    let jobs: Vec<_> = sets
        .iter()
    ...
    if jobs.is_empty() {
        warn!(
            target: "par2",
            "the PAR2 set(s) in {} name no files - nothing verified",
            dir.display()
        );
        return Ok(DirVerify::NothingToVerify);
    }
    ...
    std::thread::scope(|scope| {
            ...
            .map(|_| {
                scope.spawn(|| {
                    ...
                    loop {
                        ...
                        // One pass for exact-size and non-regular inputs, so a
                        // large damaged member never incurs a second complete
                        // read. Known regular-file size mismatches take the
                        // positioned diagnostic shortcut instead.
                        let result =
                            verify_file_path_tiered(path, file, set.block_size, inner, fast_check);
                        out.push((ji, result));
                    }
                    out
                })
        ...
        worker_results = handles
            .into_iter()
            .map(|handle| handle.join().expect("PAR2 verify worker panicked"))
            .collect();
    });
    let mut results: Vec<Option<std::io::Result<nzbkit::par2::FileVerify>>> =
        (0..jobs.len()).map(|_| None).collect();
    for (ji, result) in worker_results.into_iter().flatten() {
        results[ji] = Some(result);
    }

    let mut all_ok = true;
    for ((_, f, _), result) in jobs.iter().zip(results) {
        match result.expect("PAR2 verify worker filled every result") {
            Ok(v) => {
...
mod par2_scan_cap_tests {
    ...
    /// A directory with no PAR2 in it is `NothingToVerify`, and that is
    /// the whole point of the enum (TODO 310): it used to be `false`,
    /// which the caller could not tell apart from damage, so the CLI
    /// discarded the answer and exited 0 over both.
    #[test]
    fn an_empty_directory_reaches_no_verdict_rather_than_a_clean_one() {
        let dir = temp_dir("noverdict");
        std::fs::write(dir.join("payload.bin"), vec![7u8; 4096]).unwrap();
        assert_eq!(verify_dir(&dir).unwrap(), DirVerify::NothingToVerify);
        assert!(
            !verify_dir(&dir).unwrap().proved_clean(),
            "nothing read proves nothing"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }

    /// Sniffable bytes that are not a parseable set are the same answer
    /// for the same reason: the pass could not read what it needed, so it
    /// has no verdict to give - not a conviction, and not a pass either.
    #[test]
    fn an_unparseable_set_reaches_no_verdict_either() {
        let dir = temp_dir("unparseable");
        write_sniffable(&dir, "junk.par2", 2048);
        assert_eq!(verify_dir(&dir).unwrap(), DirVerify::NothingToVerify);
        let _ = std::fs::remove_dir_all(&dir);
    }
