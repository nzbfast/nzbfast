...
pub(crate) fn run_external_par2(
    ...
    let status = parked.is_ok().then(|| {
        nzbfast_core::sandbox::command(par2_bin, &par2_policy(out_dir, extra_args))
            .arg("repair")
            .arg("-q")
            .arg(par2_arg)
            .args(extra_args)
...
/// What the external par2 may touch: `out_dir` (it repairs in there),
/// plus READ access to every directory an extra argument points into.
///
/// Derived from the arguments rather than restated from `donor_dirs`,
/// which is deliberate: whatever we hand the child, the child may read,
/// and a donor path added to the invocation tomorrow is covered without
/// a second edit here. Nothing outside `out_dir` becomes writable -
/// donors are sources, and par2 writes only its repaired targets and
/// its `<name>.1` backups, all of them in the working directory.
fn par2_policy(out_dir: &Path, extra_args: &[std::path::PathBuf]) -> nzbfast_core::sandbox::Policy {
    let mut policy = nzbfast_core::sandbox::Policy::tool("par2", out_dir);
    for arg in extra_args {
        let Some(parent) = arg.parent() else { continue };
        // The `./name` form every same-directory argument takes: cwd is
        // already `out_dir`, which is writable above.
        if parent.as_os_str().is_empty() || parent == Path::new(".") {
            continue;
        }
        policy.allow_read(parent);
    }
    policy
}
...
///
/// THE `.par2` SKIP STAYS, and it was decided rather than inherited
/// (claim `donor-parity-catalog-harvest`, 1 Sep 2026). The native
/// engine learned that day to harvest a donor's recovery volumes as
/// parity when they carry the repair's own
/// set id (`nzbkit::par2repair::catalog::harvest_donor_recovery`), and
/// the obvious symmetry - lift the filter here too - does not work and
/// would only mislead. par2cmdline takes extra arguments as candidate
/// DATA FILES to match against the recovery set; it loads parity only
/// from the set named on its command line and the volumes it finds
/// beside that name in its cwd, which is `out_dir`. Handing it a donor
/// volume therefore buys no recovery block - it buys a whole-file scan
/// of a par2 file against every member descriptor, which cannot match.
/// Getting donor parity into this engine would mean COPYING those
/// volumes into `out_dir` under the set's own volume naming, which is
/// a write into the job directory on a guess and a separate decision
/// from this one.
