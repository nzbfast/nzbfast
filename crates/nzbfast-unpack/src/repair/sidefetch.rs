...
///
/// Defined in [`crate::streamhub`] since the crate-split prep, beside
/// the `tail_cancel` map that owns one per job, and re-exported here so
/// `super::*` importers and `crate::repair::SideCancel` are unchanged.
pub use crate::streamhub::SideCancel;

/// `.volNNN+MM.par2` / `.volNNN-MMM.par2` → declared recovery-slice count.
pub fn vol_count_from_name(name: &str) -> Option<usize> {
...
pub fn volume_prealloc_cap(nzb: &Nzb) -> u64 {
...
fn strip_side_pool_seams(pc: &mut nzbkit::pool::PoolConfig) {
    ...
    // `wire_charge` deliberately STAYS cloned, and it is the same
    // question answered the other way (TODO 313 item 1). A side-fetched
    // BODY is wire-side memory on the same process budget as the
    // download beside it, so it belongs in the ledger the cap is read
    // from; and unlike the channel gauge, its release is pool-internal
    // - every exit from a worker's deque releases, the consumer is
    // never asked - so this pool cannot leak the ledger the way a
    // cloned `channel_gauge` leaks that one. What it inherits with it
    // is the download's `inflight_cap`, whose only effect is to hold a
    // side pipeline at one request in flight per connection while the
    // wire is already full.
...
pub async fn fetch_volume_articles(
    servers: &[(ServerConfig, nzbkit::pool::PoolConfig)],
...
pub async fn fetch_volume_articles_with(
    servers: &[(ServerConfig, nzbkit::pool::PoolConfig)],
    ids: Vec<nzbkit::pool::ArticleReq>,
    id_to_file: std::collections::HashMap<Arc<str>, usize>,
    out_dir: &Path,
...
impl VolumeFailures {
    ...
    /// unattributable one.
    pub fn for_file(&self, fi: usize) -> u32 {
        self.per_file
            .get(&fi)
            .copied()
            .unwrap_or(0)
