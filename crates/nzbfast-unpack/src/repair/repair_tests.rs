...
async fn mapped_verdict_with_reports(
    ...
    try_mapped_repair(
        ...
        ex,
        reports,
        // No verifier behind these rigs and one set in every case - see
        // the parameter's own note.
        None,
        missing,
        &mut Vec::new(),
        &mut Vec::new(),
...
fn an_obfuscated_7z_is_found_under_any_extension() {
    ...
    std::fs::write(&path, &container).unwrap();
    assert!(
        crate::archname::sevenz_archive_part(&path),
        "magic outranks the extension"
    );
...
fn an_armed_disk_ladder_publishes_its_volume_count_and_its_bytes() {
    ...
    let arm = crate::unpackprog::arm(Some(&hub.unpack), "nzo-205", 2);
    let p = hub
        .unpack
        .lock_ok()
        .get("nzo-205")
...
fn a_memberless_recovery_volume_does_not_forgive_a_lone_fragment() {
    ...
    let obf = crate::unpack::collect_obfuscated_rar_volumes(&dir).unwrap();
    // Since 3 Sep 2026 the RAR5 `.rev` marker is refused by the
    // COLLECTOR (`rar_recovery_volume_magic`), so only the fragment is a
    // candidate; this asserted `2` while the pass was the thing that had
    // to know a memberless set is not a delivery. The guarantee below is
    // unchanged and is now reached by the stronger route - a recovery
    // volume that never enters the pass cannot forgive anything, and
    // cannot be swept as a spent input either.
    assert_eq!(obf.len(), 1, "only the fragment is a candidate volume");
    assert!(
        !obf.iter().any(|p| p.ends_with("0f1e2d3c4b5a69788796a5b4")),
        "the .rev-shaped recovery volume was collected as a payload volume"
    );
...
/// A `.rev` beside a NAMED set that extracted perfectly must not fail
/// the job - and must not even be looked at as a volume.
///
/// The regression this pins: a RAR5 `.rev` starts `Rar!\x1aRev`, which
/// passes the `Rar!` magic test the obfuscated collector uses, so the
/// `.rev` files were collected as candidate volumes; the parser then
/// refused every one of them with `UnsupportedSignature`, `parsed` came
/// out empty, and the arm reported `failed`. `nzbfast extract` exited 1
/// on a set whose every expected byte was on disk - and in the rebuild
/// case, after the same `.rev` had just been used to reconstruct the
/// missing volume. Measured 3 Sep 2026 by the `rar5_rev_present` and
/// `rar5_rev_rebuild` robustness fixtures, which had been red since
/// before the 2-3 Sep optimisation campaign began
/// (research/RAR-PERF-AUDIT-2026-09-02.md round 43).
#[test]
fn a_rev_beside_a_named_set_does_not_fail_the_job() {
    let total: Vec<u8> = (0..300_000u32)
        .map(|i| (i as u8).wrapping_mul(17).wrapping_add(7))
        .collect();
    let vols = reex_vols(&total);
    let dir = reex_dir("named-set-plus-rev");
    std::fs::write(dir.join("set.part1.rar"), &vols[0]).unwrap();
    std::fs::write(dir.join("set.part2.rar"), &vols[1]).unwrap();
    std::fs::write(dir.join("set.part1.rev"), rev_shaped_file()).unwrap();

    // Not a candidate volume in the first place.
    let obf = crate::unpack::collect_obfuscated_rar_volumes(&dir).unwrap();
    assert!(
        obf.is_empty(),
        "the .rev was collected as an obfuscated volume: {obf:?}"
    );
    assert!(
        obf_extract_at(&dir, 0),
        "a .rev beside a set that extracted failed the job"
    );
    assert_eq!(std::fs::read(dir.join("film.mkv")).unwrap(), total);
    std::fs::remove_dir_all(&dir).unwrap();
}

/// A directory whose obfuscated content is NOTHING BUT headless
/// fragments must FAIL at depth 0 - the offline `nzbfast extract` - and
/// not report the job done.
///
/// `ObfReport::ok` has said so since 22 Aug 2026, but only the two
/// payload-path callers read it, so the CLI exited 0: nothing can
/// produce a byte from these volumes and nothing did, and a script or an
/// *arr reading that exit code is told the payload arrived. Caught 3 Sep
/// 2026 by the `obfuscated_missing_first` robustness fixture and,
/// independently, by SABnzbd's own
/// `obfuscated_single_rar_set_missing_first_rar`. The sibling above is
/// the case this must not swallow: a fragment beside DELIVERED content
/// is still forgiven.
#[test]
fn a_directory_of_nothing_but_fragments_fails_the_offline_extract() {
    let dir = reex_dir("frag-only-cli");
    let stray = mid_set_fragment();
    for name in ["gone00", "gone01", "gone02"] {
        std::fs::write(dir.join(name), &stray).unwrap();
    }
    assert!(
        !obf_extract_at(&dir, 0),
        "an undeliverable set of headless fragments reported success"
    );
    for name in ["gone00", "gone01", "gone02"] {
        assert_eq!(
            std::fs::read(dir.join(name)).unwrap(),
            stray,
            "{name} was altered or removed"
        );
    }
    std::fs::remove_dir_all(&dir).unwrap();
}

/// A `.rev` recovery volume rides along with the obfuscated set. It was
/// collected by the same `Rar!` magic test until 3 Sep 2026 and parsed
/// as a memberless set of its own; `rar_recovery_volume_magic` now keeps
/// the RAR5 spelling out of the collector entirely. Either way the real
/// set is swept and the recovery volume is not, which is what this pins.
///
