...
pub fn collect_obfuscated_rar_volumes(dir: &std::path::Path) -> Result<Vec<PathBuf>> {
    let mut out = Vec::new();
    for e in std::fs::read_dir(dir)?.flatten() {
        ...
        if e.file_type().is_ok_and(|t| t.is_file())
            ...
            && rar_magic(&path)
            // A RAR5 `.rev` is `Rar!` magic and is NOT a volume: see
            // `rar_recovery_volume_magic`. Excluded here rather than
            // tolerated in the parse loop below, because the loop cannot
            // tell "a recovery volume, which nothing should extract"
            // from "a volume this build cannot read", and the two owe
            // opposite verdicts.
            && !rar_recovery_volume_magic(&path)
        {
            out.push(path);
        }
    }
