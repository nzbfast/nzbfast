...
pub const MAX_PAR2_CAPTURE: usize = 256 << 20;
...
pub struct SniffCtl {
    ...
    pub state: std::sync::Mutex<SniffState>,
...
pub struct SniffState {
    ...
    pub cancelled_ids: std::collections::HashMap<usize, (Vec<std::sync::Arc<str>>, u64)>,
    ...
    pub reconciled: std::collections::HashSet<usize>,
}

impl SniffCtl {
    pub fn any_sniffed(&self) -> bool {
    ...
    pub fn deferred_slots(&self) -> Vec<usize> {
    ...
    pub fn deferred_files(&self) -> Vec<usize> {
        self.deferred_slots()
            .into_iter()
    ...
    pub fn promote_pending_head16(&self, extractor: &nzbkit::extract::Extractor) {
    ...
    pub fn matched_deferred(&self, set: &nzbkit::par2::Par2Set) -> Vec<(usize, u64)> {
        let st = self.state.lock_ok();
        st.sniffed
            .iter()
            .filter(|&&s| st.bootstrap != Some(s))
    ...
    pub fn mark_reconciled(&self, sidx: usize) {
    ...
    pub fn note_completed(&self, sidx: usize) -> bool {
        let mut st = self.state.lock_ok();
...
pub type IdSlots = std::collections::HashMap<std::sync::Arc<str>, (u32, u64)>;
...
/// What [`elect_bootstrap`] decided for a newly sniffed volume.
pub(crate) enum Elected {
    /// Nothing held the bootstrap: this volume takes it, and the caller
    /// owes the activation counter one completion.
    First,
    /// This volume takes the bootstrap FROM the named slot, which must
    /// now be deferred. The completion obligation moves with the
    /// election, so the counter is left alone.
    Demoted(usize),
    /// Somebody else holds the bootstrap: defer this volume.
    Defer,
}

/// The bootstrap election, over the sniff state alone.
///
/// One question, asked once per sniffed volume: does this volume become
/// the one we download in full and activate the set from?
///
/// # The rule, and what it deliberately does NOT read
///
/// The bootstrap goes to the SMALLEST sniffed volume, because the
/// cheapest complete definition of a recovery set is its index file and
/// hash-named posts arrive in arbitrary size order - the biggest volume
/// can be half the set. The election stays open until [`SniffState::locked`],
/// which is set by [`SniffCtl::note_completed`] the moment the
/// incumbent's last article lands, because that same call is what fires
/// `maybe_activate_par2`: from then on the set is live or one function
/// call away from it, bound slots carry indexes into its tables, and
/// there is nothing here that could unwind that.
///
/// It used to read the incumbent's `remaining` as well - a demote was
/// refused unless the incumbent was still INCOMPLETE. That clause is
/// gone, and its removal is the whole of this function's behaviour
/// change. It was a second, earlier spelling of the same commitment:
/// `remaining` reaching zero and `locked` being set happen back to back
/// on ONE thread (`Par2Race::slot_drained`), so the only demote it could
/// ever refuse was one the commitment rule itself would have allowed, in
/// a window a few instructions wide. Reading a per-slot fetch counter
/// here also said the wrong thing about the rule out loud: completion
/// ORDER is not what decides this, and a completed 0.2 MB index volume
/// is not a commitment worth defending against a smaller one. The
/// election now cannot consult a slot at all - there is no `slots`
/// argument to reach it through.
///
/// # The limit, which the removal above does not touch
///
/// This election cannot be made independent of arrival order, and no
/// predicate over the incumbent can make it so: the run commits to the
/// first volume it sniffs before it has seen the second, so on a post
/// whose volumes are one article each - every par2 index file under an
/// ordinary article size - the winner is simply whichever article
/// DECODED first. Two volumes only get to compete when their articles
/// are decoded concurrently, which is a property of the box, not of the
/// post. Where two DIFFERENT recovery sets are sniffed, that decides
/// which SET the job activates, and a foreign leftover set can take it
/// from the download's own. Order-independence there needs the job to
/// stop electing a single set - `LiveVerifier::activate` already merges
/// (W4-15) and the named-par2 path already adopts every set - which is a
/// product change and not this one - measured 3 Sep 2026, including
...
pub(crate) fn elect_bootstrap(
    st: &mut SniffState,
    sidx: usize,
    smaller_than_incumbent: impl FnOnce(usize) -> bool,
) -> Elected {
    match st.bootstrap {
        None => {
        ...
        Some(b) if !st.locked && smaller_than_incumbent(b) => {
            st.bootstrap = Some(sidx);
            Elected::Demoted(b)
        }
        Some(_) => Elected::Defer,
    }
}

/// The election, pinned where it can be pinned: on any box, with no
/// pool, no queue and no arrival order at all.
///
/// It does NOT reproduce the CI failure these came out of - that one
/// wants a 4-vCPU runner and is a property of decode concurrency, not of
/// this function (the handoff named at [`elect_bootstrap`] has the
/// measurements). What these hold is the half that is decidable: WHICH
/// inputs the election is allowed to have. Three assertions here would
/// have passed against the old predicate too; the fourth is the one that
/// changed, and the reason the old spelling cannot be reintroduced by
/// accident is stronger than any of them - `elect_bootstrap` takes no
/// slot, so there is no `remaining` in scope to read.
#[cfg(test)]
mod election_tests {
    use super::{Elected, SniffState, elect_bootstrap};

    /// A state in which `b` already holds the bootstrap and nothing has
    /// been spent on it - `locked` false, whatever `b`'s own articles
    /// have done, because the election cannot see them.
    fn held_by(b: usize) -> SniffState {
        SniffState {
            bootstrap: Some(b),
            ..Default::default()
        }
    ...
    /// Nothing holds it yet, so the first volume sniffed takes it - and
    /// says so, because the caller has an activation counter to bump
    /// exactly once.
    #[test]
    fn the_first_sniffed_volume_takes_the_bootstrap() {
        let mut st = SniffState::default();
        assert!(matches!(
            elect_bootstrap(&mut st, 7, |_| false),
            Elected::First
        ));
        assert_eq!(st.bootstrap, Some(7));
    }

    /// The rule the election exists for: the smallest volume wins,
    /// whichever order they were sniffed in.
    #[test]
    fn a_smaller_volume_takes_it_and_names_who_must_be_deferred() {
        let mut st = held_by(3);
        assert!(matches!(
            elect_bootstrap(&mut st, 5, |b| b == 3),
            Elected::Demoted(3)
        ));
        assert_eq!(st.bootstrap, Some(5));
    }

    /// ...and a bigger one does not, so a volume that is half the set
    /// cannot displace the index file that arrived first.
    #[test]
    fn a_bigger_volume_never_takes_it() {
        let mut st = held_by(3);
        assert!(matches!(
            elect_bootstrap(&mut st, 5, |_| false),
            Elected::Defer
        ));
        assert_eq!(st.bootstrap, Some(3));
    }

    /// THE ONE THAT CHANGED. The incumbent has finished arriving - every
    /// article of it is on disk - and a strictly smaller volume is
    /// sniffed. The old arm read the incumbent's `remaining` and refused
    /// this, so the run kept a completed volume it had already been told
    /// was the more expensive of the two; on a post whose index volumes
    /// are one article each, that refusal was the entire election.
    /// Completion is not an input here: only `locked` closes it.
    #[test]
    fn a_completed_incumbent_no_longer_defends_against_a_smaller_volume() {
        let mut st = held_by(3);
        // `locked` is false: `note_completed` has not run for slot 3, so
        // nothing has been spent on it yet however many of its articles
        // have landed.
        assert!(!st.locked);
        assert!(matches!(
            elect_bootstrap(&mut st, 5, |b| b == 3),
    ...
    /// must not have moved: once the election is LOCKED the set is live
    /// or one call away from it, and even a strictly smaller volume is
    /// simply deferred.
    #[test]
    fn a_locked_election_defends_even_against_a_smaller_volume() {
        let mut st = SniffState {
            bootstrap: Some(3),
            locked: true,
            ..Default::default()
        };
...
pub fn reclassify_sniffed_par2(
    ctl: &SniffCtl,
    slots: &[Arc<FileSlot>],
    sidx: usize,
    head: &[u8],
    ...
    let (is_bootstrap, demoted) = {
        ...
        if ctl.allow_bootstrap {
            match elect_bootstrap(&mut st, sidx, |b| fbytes(sidx) < fbytes(b)) {
                Elected::First => {
                ...
                Elected::Demoted(b) => {
                    // The completion obligation moved with the election
                    // (net zero on the counter), and the old partial
                    // capture is dropped so recovery_blocks_seen counts
                    // exactly the volume the repair planner is told is
                    // already on hand.
...
/// both ways.
pub fn backfill_pre_activation(
    verifier: &nzbkit::live::LiveVerifier,
    extractor: &nzbkit::extract::Extractor,
    n_slots: usize,
    par2_slots: &[bool],
...
pub fn nzb_age_days(date: i64) -> u32 {
    if date <= 0 {
        return 0;
    }
...
/// `nzb_age_days` on the dates that are not dates.
///
/// It sat in `diag/main_tests.rs` as a coda to the gone-age threshold
/// row, reached through main.rs's `use unpack::*` glob. The crate-split
/// step 2 cut put `diag` in `nzbfast-core`, a layer below this one, so
/// the coda moved to the function it is actually about rather than
/// being dropped. The threshold row itself stayed where it was - it is
/// `diag`'s own sentence.
#[cfg(test)]
mod nzb_age_days_tests {
    /// An NZB with no usable date reads as age 0 - unknown is not old,
    /// so it keeps the retry rather than being written off.
    #[test]
    fn an_undatable_post_is_fresh_and_never_written_off() {
