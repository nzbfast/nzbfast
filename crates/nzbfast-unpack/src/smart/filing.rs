...
pub fn tv_organize(
    ...
    let moved = done.len();
    // The job is filed and about to be reported so. Make the names
    // durable BEFORE the private folder goes, or there is no `out_dir`
    // left to open - see [`filed`]. This is the one door here whose
    // destination is the user's library and whose parents may have just
    // been created, so it is the one with a walk rather than a single
    // fsync.
    filed(dest_parent, &dest, out_dir);
...
pub fn tv_rename(dir: &Path, stem: &str, suffix: &str, titles: &EpisodeTitles) -> usize {
    ...
    // Flat: every rename here was within `dir`, which this door never
    // creates, so one fsync covers the names that arrived, the names that
    // went, and any placeholder a failed rename removed. Only when
    // something actually landed - see [`filed`].
    if renamed > 0 {
        filed(dir, dir, dir);
    }
    renamed
...
fn names_the_release(name: &str) -> bool {
    let p = nzbkit::release::parse_release(name);
...
pub fn rename_nameless_video(out_dir: &Path, base: &str) -> bool {
    ...
    // Reaching here means the video rename succeeded, so something landed
    // whatever the sidecars did. Flat in `out_dir`, so one fsync - see
    // [`filed`].
    filed(out_dir, out_dir, out_dir);
    true
}
...
pub fn rename_movie(parent: &Path, out_dir: &Path, base: &str) -> Option<PathBuf> {
    ...
    if let Some((video, ext, sidecars)) = &one {
        let video = *video;
        // Whether this arm actually placed anything, which decides whether
        // it owes [`filed`] a flush. Tracked rather than assumed: this arm
        // runs whenever the job has exactly one feature video, and a job
        // whose video is already correctly named with no sidecars to move
        // changes no directory entry at all.
        let mut moved = false;
        ...
        if target != *video {
            match std::fs::OpenOptions::new()
                ...
                .open(&target)
            {
                Ok(_) => match std::fs::rename(video, &target) {
                    Ok(()) => moved = true,
                    Err(e) => {
        ...
        for (f, rest) in sidecars {
            ...
            if subtarget != *f
                && std::fs::OpenOptions::new()
                    ...
                    .is_ok()
            {
                if std::fs::rename(f, &subtarget).is_err() {
        ...
        // Flat in `out_dir`, and BEFORE the folder rename below: after
        // that call `out_dir` no longer names this directory, and on the
        // Windows entry-by-entry fallback it does not exist at all. See
        // [`filed`]. The folder rename's own durability is `rename_dir`'s
        // to owe, in nzbname.rs.
        if moved {
            filed(out_dir, out_dir, out_dir);
        }
    }
...
/// Make a filing pass DURABLE: flush the directory entries it created and
/// removed, so a power cut after the job reported success cannot leave the
/// payload on disk with its user-visible name gone.
///
/// The sibling of `unpack/published_names.rs`'s `landed` (X5-22), at this
/// file's door. Every writer upstream calls `sync_data`, so the BYTES of a
/// filed episode survive a cut. Their NAMES did not: all five doors here
/// placed files at their final, user-visible names with a plain
/// `fs::rename` and synced no directory on any of them, so an externally
/// visible rc=0 could be followed by a cut that leaves the payload under
/// its posted hash - or, for `tv_organize`, reachable from neither the
/// season folder nor the private one.
///
/// THIS IS PER PASS, NOT PER RENAME, and that is the difference from the
/// sibling. `publish` places ONE file per call and funnels three exits;
/// these doors place a whole job - a season pack is one `dest` and one
/// `out_dir` however many episodes go through it - so the funnel sits at
/// the exit that REPORTS the success, where one flush covers every rename
/// the pass made. It also covers the placeholder removals a partly failed
/// pass left behind, which is the same directory and the same fsync.
///
/// FOUR CALLERS, ONE PER SUCCESS EXIT: `tv_organize` (before it drops the
/// emptied private folder, or there is no `out_dir` left to open),
/// `tv_rename` when it renamed anything, `rename_nameless_video` on its
/// `true`, and `rename_movie` before it hands the folder to
/// `nzbname::rename_dir` - after that call `out_dir` no longer names this
/// directory, and on the Windows fallback path it does not exist at all.
///
/// NOT CALLED ON THE ROLLBACK ARM, deliberately. `tv_organize`'s undo
/// exists to leave the job exactly as it was and reports None, so nothing
/// outside ever saw a success to be made durable; the worst a lost undo
/// costs is the payload sitting in the season folder under a canonical
/// name that no history row claims, which is recoverable and is not the
/// class this exists for.
///
/// WHAT THIS DOES NOT DO. It is the CODE half: `sync_dir` returns nothing,
/// the guarantee's oracle is a journalled block image replaying only
/// acknowledged writes, and no box on this fleet can run one - so nothing
/// here proves durability, only that the syncs are issued on every exit
/// that reports success. `sync_dir` is best-effort by contract (SMB/CIFS
/// and some FUSE mounts refuse to open a directory for fsync, and Windows
/// has no directory handle to sync through at all), so a filing pass is
/// never FAILED over a volume that cannot do this - a job whose bytes are
/// all on disk must not be failed for a barrier the filesystem does not
/// offer.
fn filed(root: &Path, dest: &Path, src: &Path) {
    for d in filed_dirs(root, dest, src) {
        #[cfg(test)]
...
#[cfg(test)]
thread_local! {
    /// What [`filed`] flushed, so the funnel is falsifiable and not merely
    /// asserted in a doc comment.
    ///
    /// `sync_dir` returns nothing and no box on this fleet can run the
    /// crash harness, so a door with its `filed` call deleted places its
    /// files exactly as well as one that keeps it and every behavioural
    /// test in `sweep_rename_tests.rs` stays green. This is what tells the
    /// two apart.
    ///
    /// A THREAD-LOCAL and never a `static`: `cargo test` runs these on
    /// several threads of ONE process, so a process-global vector would
    /// have each test reading its neighbours' filing (the class
    /// `tools/test-global-gate.py` exists for).
    static FLUSHED: std::cell::RefCell<Vec<PathBuf>> =
        const { std::cell::RefCell::new(Vec::new()) };
}

/// The directories flushed since the last call, and clear.
#[cfg(test)]
pub(super) fn take_flushed() -> Vec<PathBuf> {
    FLUSHED.with_borrow_mut(std::mem::take)
}

/// Which directories a filing pass changed, in the order they must be
/// flushed. Split out of [`filed`] because it is the half that carries the
/// decisions and the half a crash harness could not check anyway - so it
/// is pinned directly instead. `pub(super)` only so the pin can reach it.
///
/// THE ORDER IS THE DESTINATION FIRST, and the two orders are not equally
/// good because a cut between two of these syncs is a real state. With the
/// destination flushed first what survives is the new name plus, at worst,
/// the old one still beside it - a duplicate entry, which costs a stray
/// file. The other order can leave the old entry gone with the new one not
/// yet on disk, which is the payload reachable under NO name.
///
/// THE ANCESTOR WALK covers a rename into a directory that was just
/// created, which here is exactly `tv_organize`: `create_dir_all(dest)`
/// makes `Show/` and `Season NN/` on the first episode of a show, and each
/// of those directories' own entry in ITS parent is a separate metadata
/// change. Journalling filesystems order the mkdir into the transaction an
/// fsync commits, but that is ext4/XFS/APFS behaviour rather than a POSIX
/// guarantee, so the chain is walked instead of leaned on.
///
/// THE ROOT IS NOT `out_dir` HERE, which is the one place this cannot copy
/// the sibling. `published_names.rs` stops at the job's output directory
/// because `rename_out_under` binds its destination there. The four flat
/// doors in this file rename WITHIN one directory that they never create,
/// so their walk is one entry and the root is that directory itself -
/// there is no fresh parent to cover. `tv_organize` moves into the user's
/// library, so its root is `dest_parent`, the library path the caller
/// supplies and this function never creates below: `create_dir_all` makes
/// the chain from there down, and that is precisely the chain the walk
/// covers. Above `dest_parent` is the user's own ground, and a walk with
/// no stopping rule fsyncs every directory up to `/`.
///
/// The `!starts_with` arm is the same guard the sibling carries for the
/// same reason: a `dest` that is not under `root` cannot arrive from
/// `tv_path`, and without the arm it would walk to the filesystem root.
///
/// THE SOURCE DIRECTORY GOES LAST and only when the walk did not already
/// reach it - which for the four flat doors it always does, since one
/// fsync covers the entry that arrived and the entry that went. A rename
/// touches both directories, and the entry that went is exactly as
/// undurable as the one that arrived, so `tv_organize`'s private folder is
/// named even though every name it loses is one the user never sees.
pub(super) fn filed_dirs(root: &Path, dest: &Path, src: &Path) -> Vec<PathBuf> {
    let mut dirs: Vec<PathBuf> = Vec::new();
    let mut cur = Some(dest);
    // No dedupe inside the loop and no empty-path test: a parent chain is
    // strictly ascending so it cannot revisit a directory, and
    // `Path::new("").parent()` is None so the walk terminates on its own.
    // The sibling deleted both guards after a mutation of each survived
    // every test - an unreachable guard makes the one beside it
    // unfalsifiable - and neither is reachable here either. `src` below is
    // the only place a repeat is possible, so it is the only place a check
    // lives.
    ...
    if !dirs.iter().any(|s| s.as_path() == src) {
