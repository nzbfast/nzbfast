...
/// How many times [`rename_dir`] re-walks the ladder when the name it
/// picked was taken in the gap before the rename.
///
/// One step is enough for a single racing job, and the loop exists for
/// the case where several finish together; past a handful the answer is
/// that the folder keeps its own name, which costs nothing but a name.
const LADDER_ATTEMPTS: usize = 4;

/// The first name under `parent` that no entry holds: `base`, then
/// `base.2`, `base.3`, and so on.
///
/// A LOOK, not a claim, and the block below is why - plus what that
/// argument does not cover, which is the caller's fallback.
fn free_name(parent: &Path, base: &str) -> PathBuf {
    ...
    //
    // NOTHING IS DESTROYED by getting this wrong AT THE RENAME, unlike
    // the four guards in `filing.rs` that share the fingerprint, and the
    // reason is the SOURCE TYPE: `out_dir` is a directory, and MEASURED
    // on APFS 31 Aug 2026 `rename(2)` answers ENOTDIR for a directory
    // onto a symlink of either kind. What happened instead is that the
    // rename failed, its entry-by-entry fallback failed EEXIST at
    // `create_dir_all` (a
    ...
    //
    // THAT ARGUMENT COVERED THE RENAME AND STOPPED THERE, which is the
    // hole the 1 Sep 2026 residue sweep found: `rename(2)` refusing is
    // only the FIRST attempt, and what follows it is `move_dir_contents`,
    // where `create_dir_all` answered Ok for an entry that is already a
    // DIRECTORY - the one type the ladder is likeliest to be raced by,
    // since a directory is what every other job's folder is. Two jobs
    // that both looked, both saw `base` free and both went for it merged
    // the loser's entries onto the winner's with a plain per-entry
    // rename, which REPLACES a same-named file: the winner's payload
    // gone, no error, both jobs logging a successful rename. The
    // fallback takes `create_dir` now, which refuses every existing
    // entry including a directory, and `rename_dir` steps the ladder on
    // that refusal rather than merging - so the loser lands on `base.2`,
    // which is what the ladder was always for.
...
/// than written twice.
///
/// The ladder is walked once per ATTEMPT, and there are
/// [`LADDER_ATTEMPTS`] of them, because a name it hands back can be
/// taken in the gap before the rename - see `move_dir_contents`, which
/// refuses to merge into an entry it did not create and reports
/// `AlreadyExists` so this loop can step past the winner.
pub(super) fn rename_dir(parent: &Path, out_dir: &Path, base: &str) -> Option<PathBuf> {
    if parent.join(base) == out_dir {
        return None;
    }
    let mut raced = None;
    for _ in 0..LADDER_ATTEMPTS {
        let target = free_name(parent, base);
        match std::fs::rename(out_dir, &target) {
            ...
            Err(e) => match move_dir_contents(out_dir, &target) {
                ...
                // THE NAME WAS TAKEN IN THE GAP, by the only thing that
                // can take it: another job finishing onto the same base.
                // `move_dir_contents` refuses rather than merging (see
                // its `create_dir`), so nothing has moved and stepping
                // the ladder is free - this job takes `base.2` on the
                // next pass, which is the outcome the ladder exists to
                // produce. Distinguishable from every other failure by
                // kind, so a genuine EEXIST from a deeper entry does not
                // spin here: `create_dir` is the FIRST thing the
                // fallback does, and nothing has been renamed when it
                // answers.
    ...
    // Out of attempts: several jobs are finishing onto this base at once
    // and every rung this walk picked was gone by the time it was used.
    // The folder keeps its own name, which is the same outcome as any
    // other reported failure here and costs only a name.
    warn!(
        target: "smart",
        "rename dir {}: gave up after {LADDER_ATTEMPTS} attempts, last {}",
        out_dir.display(),
        raced.map_or_else(|| "-".into(), |t: PathBuf| t.display().to_string())
    );
    None
}
...
///
/// `create_dir` AND NOT `create_dir_all`, which is the whole guard: the
/// per-entry rename below REPLACES a same-named file, so merging into a
/// directory somebody else owns destroys their payload silently, and
/// `create_dir_all` answers Ok for exactly that case. Refusing with
/// `AlreadyExists` before anything moves is what lets the caller step
/// its ladder. It also cannot create a missing parent, which is right:
/// `to` is a sibling of `from`, so the parent is there by construction,
/// and a call that would have to build one is a caller bug rather than
/// a directory to conjure.
