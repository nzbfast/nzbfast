...
// -----------------------------------------------------------------
// Filing durability: the directory entry that names the payload.
// -----------------------------------------------------------------

/// The DECISION half of filing durability: which directories a pass
/// changed, and in what order they must be flushed.
///
/// The guarantee itself is not testable here - its oracle is a journalled
/// block image replaying only acknowledged writes, and no box on this
/// fleet has one - so what is pinned is the part a reviewer gets wrong
/// instead: the destination first (a cut after it leaves at worst a
/// duplicate name, where the other order can leave the payload reachable
/// under none), the ancestor chain up to and INCLUDING the root, and no
/// second fsync of a directory the walk already reached.
#[test]
fn a_filing_pass_flushes_the_destination_first_and_stops_at_its_root() {
    let dir = std::path::Path::new("/jobs/out");
    let lib = std::path::Path::new("/lib/tv");

    // The four flat doors: one directory holds the name that arrived and
    // the name that went, and nothing in the chain was created, so it is
    // ONE fsync and no walk.
    assert_eq!(
        filing::filed_dirs(dir, dir, dir),
        vec![dir.to_path_buf()],
        "a flat rename is ONE directory, not three"
    );

    // `tv_organize`: `create_dir_all` may have just made both `Show/` and
    // `Season 01/`, so each owes its own entry in its parent a flush, and
    // the private folder the episode left owes one too.
    assert_eq!(
        filing::filed_dirs(lib, &lib.join("Show/Season 01"), dir),
        vec![
            lib.join("Show/Season 01"),
            lib.join("Show"),
            lib.to_path_buf(),
            dir.to_path_buf(),
        ],
        "the destination leads, the chain stops at the library root, and \
         the source folder is named last"
    );

    // The source joins the list only when the walk did not already reach
    // it. Every flat door above is that case; this is the shape where the
    // check has something to do on a walked chain.
    assert_eq!(
        filing::filed_dirs(lib, &lib.join("Show/Season 01"), &lib.join("Show")),
        vec![
            lib.join("Show/Season 01"),
            lib.join("Show"),
            lib.to_path_buf(),
        ],
        "no directory is fsynced twice"
    );

    // A destination outside the root cannot arrive from `tv_path`, and the
    // guard is here because the alternative to stopping is walking to `/`
    // and fsyncing every directory on the way.
    assert_eq!(
        filing::filed_dirs(lib, std::path::Path::new("/elsewhere/a"), dir),
        vec![std::path::PathBuf::from("/elsewhere/a"), dir.to_path_buf()],
        "the walk must stop rather than climb out of the library"
    );
}

/// The FUNNEL: every exit in filing.rs that reports a placed file must
/// flush the directories it changed, and the exits that placed nothing
/// must not.
///
/// It is the flush that is asserted and not merely that the rename still
/// works. `sync_dir` returns nothing, so a door with its `filed` call
/// deleted places its files exactly as well as one that keeps it and every
/// other test in this file stays green - `take_flushed` is what tells the
/// two apart.
#[test]
fn every_exit_that_places_a_file_flushes_the_directory_it_changed() {
    let _steady = trash_globals_steady();
    let root = scratch("filedurable");

    // `tv_organize`, the one door whose destination is the user's library
    // and whose parents it may have just created.
    let stem = "My.Show.S01E02.1080p.WEB.x264-TEST";
    let out = root.join("job");
    std::fs::create_dir_all(&out).unwrap();
    std::fs::write(out.join("blob.mkv"), b"v").unwrap();
    let lib = root.join("tv");
    let _ = filing::take_flushed();
    let dest = tv_organize(&lib, stem, &out, " [1080p]", &EpisodeTitles::default()).unwrap();
    assert_eq!(
        filing::take_flushed(),
        vec![dest.clone(), lib.join("My Show"), lib.clone(), out.clone()],
        "the season folder, the show folder it was created in, the library \
         root, then the private folder the episode left"
    );

    // Filing NOTHING is not filing: the pass placed no entry, so there is
    // nothing it made that a cut could lose. `create_dir_all` ran and its
    // directories are not flushed - an empty season folder is not a loss.
    let empty = root.join("emptyjob");
    std::fs::create_dir_all(&empty).unwrap();
    let _ = filing::take_flushed();
    assert!(
        tv_organize(
            &lib,
            "Other.Show.S01E01.1080p-X",
            &empty,
            "",
            &EpisodeTitles::default()
        )
    ...
    assert!(
        filing::take_flushed().is_empty(),
        "a pass that placed nothing owes no flush"
    );

    // `tv_rename`, flat in one directory it never created.
    let ren = root.join("ren");
    std::fs::create_dir_all(&ren).unwrap();
    std::fs::write(ren.join("blob.mkv"), b"v").unwrap();
    let _ = filing::take_flushed();
    assert_eq!(
        tv_rename(&ren, stem, " [1080p]", &EpisodeTitles::default()),
        1
    );
    assert_eq!(filing::take_flushed(), vec![ren.clone()]);
    // Idempotent: the second pass renames nothing, so it flushes nothing.
    assert_eq!(
        tv_rename(&ren, stem, " [1080p]", &EpisodeTitles::default()),
        0
    );
    assert!(filing::take_flushed().is_empty());

    // `rename_nameless_video`, video plus its sidecar, both flat.
    let deob = root.join("deob");
    std::fs::create_dir_all(&deob).unwrap();
    std::fs::write(deob.join("n1iY94U6fTpMVY9GPD.mkv"), b"v").unwrap();
    std::fs::write(deob.join("n1iY94U6fTpMVY9GPD.en.srt"), b"s").unwrap();
    let _ = filing::take_flushed();
    assert!(rename_nameless_video(
        &deob,
        "Example.Movie.2024.1080p.WEB.x264-GRP"
    ));
    assert_eq!(filing::take_flushed(), vec![deob.clone()]);

    // `rename_movie`: the file half is flat in `out_dir`, and the flush
    // must be issued BEFORE the folder rename - after it, `out_dir` no
    // longer names this directory.
    let mov = root.join("mov");
    std::fs::create_dir_all(&mov).unwrap();
    std::fs::write(mov.join("hash.mkv"), b"v").unwrap();
    std::fs::write(mov.join("hash.en.srt"), b"s").unwrap();
    let _ = filing::take_flushed();
    let moved = rename_movie(&root, &mov, "Example Movie (2024)").unwrap();
    assert_eq!(
        filing::take_flushed(),
        vec![mov.clone()],
        "the flush names the directory as it was, not the folder's new name"
    );
    assert!(moved.join("Example Movie (2024).mkv").is_file());

    // The video alone, with no sidecar to place. Every case above moves
    // both, so either half of `rename_movie`'s bookkeeping could be lost
    // without the flush going with it - these two cases are where each
    // half is on its own.
    let solo = root.join("solo");
    std::fs::create_dir_all(&solo).unwrap();
    std::fs::write(solo.join("hash.mkv"), b"v").unwrap();
    let _ = filing::take_flushed();
    assert!(rename_movie(&root, &solo, "Solo Movie (2024)").is_some());
    assert_eq!(filing::take_flushed(), vec![solo.clone()]);

    // The sidecar alone: the video's canonical name is held by a
    // directory, so its claim declines silently and only the sidecar
    // moves. Reachable in ordinary life - the name the movie wants is
    // taken by something the job did not put there.
    let side = root.join("side");
    std::fs::create_dir_all(&side).unwrap();
    std::fs::write(side.join("hash.mkv"), b"v").unwrap();
    std::fs::write(side.join("hash.en.srt"), b"s").unwrap();
    std::fs::create_dir(side.join("Taken Name.mkv")).unwrap();
    let _ = filing::take_flushed();
    let sided = rename_movie(&root, &side, "Taken Name").unwrap();
    assert!(sided.join("hash.mkv").is_file(), "the video kept its name");
    assert!(sided.join("Taken Name.en.srt").is_file());
    assert_eq!(
        filing::take_flushed(),
        vec![side.clone()],
        "a pass that placed only a sidecar still placed something"
    );

    // The same door over a job that is already correctly named: this arm
    // still runs, and it changes no directory entry, so it owes nothing.
    let noop = root.join("noop");
    std::fs::create_dir_all(&noop).unwrap();
    std::fs::write(noop.join("Already Named.mkv"), b"v").unwrap();
    let _ = filing::take_flushed();
    assert!(rename_movie(&root, &noop, "Already Named").is_some());
    assert!(
        filing::take_flushed().is_empty(),
        "the one-video arm runs for every movie job; only one that placed \
         something owes a flush"
    );
