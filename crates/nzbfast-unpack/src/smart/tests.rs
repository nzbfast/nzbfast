...
fn delete_filed_episode_spares_the_upgrade_that_replaced_it() {
    ...
    let style = nzbkit::release::NameStyle {
        resolution: true,
        video_codec: true,
        audio_codec: false,
        source: true,
    ...
    let suffix =
        |stem: &str| nzbkit::release::quality_suffix(&nzbkit::release::parse_release(stem), &style);
...
fn encrypted_rar_scan() {
    use crate::unlockpw::encrypted_rar;
...
fn a_locked_zip_is_detected_and_the_password_unpacks_it() {
    ...
    for (tag, enc) in [
        ...
        let found = crate::unlockpw::encrypted_archive(&d)
            .unwrap_or_else(|| panic!("{tag}: lock not detected"));
        ...
        assert!(
            crate::unlockpw::encrypted_archive(&d).is_none(),
...
/// [`payload_release_stem`] answers off the FEATURE and holds it to the
/// project's single dark-verdict test, and [`proved_release_stem`]
/// additionally wants a recovery set on disk to have declared that
/// feature's own path. Wave-7 rows W7-05 and W7-06 respectively, and
/// the asymmetry between the two bars is the design - see their notes.
#[test]
fn the_payload_names_the_job_only_when_it_reads_as_a_name() {
    let d = scratch("payloadstem");
    let mk = |n: &str, len: u64| {
        std::fs::File::create(d.join(n))
            .unwrap()
            .set_len(len)
            .unwrap();
    };
    // A hash-named feature is the whole population W7-05 is about, and
    // it may not classify a job as itself.
    mk("7f3a1c0b9e2d4a68.mkv", 4_000_000);
    assert_eq!(payload_release_stem(&d), None);
    assert_eq!(proved_release_stem(&d), None);

    // Settle names it. The stem is the answer, extension off.
    std::fs::rename(
        d.join("7f3a1c0b9e2d4a68.mkv"),
        d.join("Example Movie 2024 1080p BluRay-GRP.mkv"),
    )
    .unwrap();
    assert_eq!(
        payload_release_stem(&d).as_deref(),
        Some("Example Movie 2024 1080p BluRay-GRP")
    );
    // ...and it is still not PROVED: nothing on disk declares it, so
    // whichever tier produced that name may have been the weakest one.
    assert_eq!(proved_release_stem(&d), None);

    // The set's own FileDesc is what makes the difference, and it is
    // readable here because the sweep has not run yet.
    std::fs::write(
        d.join("release.par2"),
        par2_index(
            0x53,
            &[("Example Movie 2024 1080p BluRay-GRP.mkv", 4_000_000)],
        ),
    )
    .unwrap();
    assert_eq!(
        proved_release_stem(&d).as_deref(),
        Some("Example Movie 2024 1080p BluRay-GRP")
    );

    // A set that declares something ELSE proves nothing about the
    // feature - the claim is per path, not per directory.
    std::fs::write(
        d.join("release.par2"),
        par2_index(0x54, &[("Some Other Release 2020.mkv", 4_000_000)]),
    )
    .unwrap();
    assert_eq!(proved_release_stem(&d), None);
}
