//! This crate's scratch guard, reachable from its own unit tests.
//!
...
//! imported by a lib any other way. One include per crate, from here, so
//! the whole tree holds the SAME type and the sweep's `Once` fires once
//! per process rather than once per including module.
//!
