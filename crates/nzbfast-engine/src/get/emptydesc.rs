    ...
    for f in adopt {
        if let Some((sidx, path)) = empty_slots.next() {
            ...
            if super::settle::filedesc_name_is_better(&slots[sidx], &f.name) {
                ...
                // X6-05: the rename is not the answer to "did the
                // descriptor's OWN path get a file", and marking it
                // landed on the strength of the rename alone was X6-03's
                // hole reached through the ACCEPTED arm. `claim` can push
                // the publish onto a `{slot:03}-` form (a fold twin of a
                // name another slot already holds, a W4-17 file/directory
                // topology clash), and a disambiguated path is the one
                // thing this tier must never produce -
                // `materialize_empty_filedesc`'s own doc says so. The
                // publish itself can also fail outright, which leaves the
                // slot's file under its posted name. So ask the disk the
                // same question the other two arms ask, and let it decide
                // whether the descriptor is satisfied.
                //
                // This cannot cost the ordinary case a landing: the
                // rename has just put the slot's empty file at the
                // descriptor's own path, so the probe's `CreateNew` hits
                // `AlreadyExists` over a 0-byte regular file and answers
                // true for one extra `lstat`. It answers false only where
                // the path holds a non-empty file, a directory, or is
                // unreachable through a file ancestor - the states in
                // which the member really is undelivered and belongs on
                // the missing list for `report_unsatisfied_zero_length`.
...
/// is satisfied.
///
/// Three callers: past the last paired empty slot, X6-03's refused-gate
/// arm, where a slot keeps its own better name and the descriptor's OWN
/// path still needs a file under it, and (X6-05) the ACCEPTED-gate arm
/// after its publish, which may have been disambiguated onto a
/// `{slot:03}-` path or have failed outright. All three are the same
/// question - is there a real empty file at the descriptor's exact path
/// - so one function answers it once.
///
...
fn whole_file_md5(path: &Path) -> Option<[u8; 16]> {
    ...
    let mut h = nzbkit::md5fast::Md5::new();
