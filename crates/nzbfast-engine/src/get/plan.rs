...
pub(super) fn build_intake(
    ...
    // TIMED, because until 3 Sep 2026 neither half of a resume was
    // (handoff item 9, claim `startup-and-resume`). A resume has exactly
    // two costs before the first byte moves - PARSING the journal, and
    // REPLAYING it - and they answer different questions: the parse
    // scales with the RECORD COUNT and the replay with the BYTES, so a
    // slow resume that reported one number could not be attributed to
    // either. Both are `Instant`s around calls that already existed;
    ...
    if !restored.ids.is_empty() {
        let moved: u64 = restored
            ...
            .sum();
        // The rate is over `moved`, the bytes the replay ADMITTED, and it
        // is a rate for both routes even though only the materialising
        // one writes: the mapped route still READS every admitted byte
        // once, to check it against the X5-02 crc32 the record carries
        // (`article_authentic`). So the number to compare a resume
        // against is a disk READ rate, not a copy rate.
        info!(
            target: "resume",
            "restored {} article(s) ({:.1} MB) from previous run's output files - \
             journal parse {:.0} ms, replay {:.0} ms ({:.0} MB/s){}",
            restored.ids.len(),
            moved as f64 / 1e6,
            open_ms,
            restore_ms,
            if restore_ms > 0.0 { moved as f64 / 1e3 / restore_ms } else { 0.0 },
            if resume_map { "" } else { ", materialising volumes" }
        );
...
fn resume_map_admitted(
    ...
    // (`tasks/stall.rs`) are the two ends of the same fact - one
...
/// `requeue_cost` (`tasks/stall.rs`), which predicts this gate's
...
/// real ledger.
pub fn resume_map_admits(restored_bytes: u64, widest_slot: u64, cap: u64, seatable: u64) -> bool {
...
pub(super) fn clamp_concurrency(
    ...
    let decoders = {
        ...
        // The PROBE and not the class: the clamp is scoped to a class
        // read off the filesystem's own device, so a btrfs or ZFS volume
        // resolved through the mount table does not inherit it. See
        // `disk::decoders_for_storage`.
        let storage = nzbkit::disk::probe_storage(out_dir);
