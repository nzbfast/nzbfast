...
pub struct JobSpec<'a> {
    ...
    pub donor_nzbs: Vec<PathBuf>,
    pub progress: Option<Arc<AtomicU64>>,
    pub hub: Option<Arc<StreamHub>>,
    ...
    pub net_done: Option<tokio::sync::oneshot::Sender<std::time::Instant>>,
    pub budget: nzbkit::mem::MemBudget,
}
...
mod tests {
    ...
    ///
    /// IT SCANS THE WORKSPACE AND NOT THIS CRATE, since the crate-split
    /// step 4 cut moved `get` into `nzbfast-engine`. `JobSpec` is
    /// DECLARED here and CONSTRUCTED entirely above: all three sites are
    /// in the bin (`run_cmds.rs`, `sidecar.rs`,
    /// `tasks/worker.rs`), so a scan of this crate's own `src`
    /// reaches ZERO and the floor below is what said so - loudly, which
    /// is what it is for. The roster is spelled out rather than
    /// discovered, the way `tests/adoptguard/mod.rs` spells its own:
    /// a crate silently dropped from a walk reports a population that
    /// has moved as one that is clean.
    #[test]
    fn every_daemon_job_spec_leaves_the_journal_to_its_caller() {
        ...
        let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("crates/nzbfast-engine has a parent")
            .to_path_buf();
        // `nzbfast-daemon` JOINED THE LIST at the serve split's lane 2
        // and `nzbfast-tasks` at lane 3, each time because the cut moved
        // a construction site out of `nzbfast/src/serve/` into a crate of
        // its own. The floor below is what said so BOTH times - a roster
        // keyed on paths fails LOUDLY at a cut, dropping from five sites
        // to two at lane 2 and from three to two at lane 3.
        ...
        while let Some(d) = stack.pop() {
                ...
                .flatten()
            {
                ...
                for line in src.lines() {
                    ...
                    // A path component and not a prefix: `rel` opens
                    // with the CRATE (`nzbfast/src/serve/...`), so a
                    // prefix test would read every daemon site as a CLI
                    // one and demand `Run` from all three. EITHER
                    // component names the daemon since lane 2 of the
                    // serve split: the layer is `nzbfast-daemon` now,
                    // `tasks` is `nzbfast-tasks` since lane 3, and what
                    // stayed under `serve/` is the wiring - all three
                    // equally inside the daemon, and owing `Caller` for
                    // the same reason.
                    let daemon = rel.components().any(|c| {
                        c.as_os_str() == "serve"
                            || c.as_os_str() == "nzbfast-daemon"
                            || c.as_os_str() == "nzbfast-tasks"
                    });
