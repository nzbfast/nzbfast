...
impl Rig {
    ...
    /// The same call, keeping the `Unlooked` tally the slot list is
    /// resolved alongside.
    fn resolve(
        &self,
        si: usize,
...
/// `FillReport::unlooked_*` counts the MAPPED/CHASED skip and nothing
/// else, which is the whole of what makes the number mean something.
///
/// Its purpose is to size the population a live in-stream patch could
/// serve (`research/DUPEFILL-LIVE-PATCH-SIZING-2026-09-01.md` section
/// 7). `wanted_files` refuses a report for five OTHER reasons, and
/// every one of them is a refusal about this post's shape that no
/// change to this pass would lift - so folding them in would make the
/// figure bigger and stop it answering the question it was added for.
/// This rig cannot produce a mapped slot (its `Extractor` is built
/// disabled, which is what makes the path resolution testable), so what
/// it pins is the control arm: the other four refusals reachable here
/// all count ZERO. The non-zero case is pinned where a real RAR volume
/// exists - see the note above this test's module in `dupefill.rs`.
///
/// A widening of the tally would turn every assertion below red, which
/// is the point: this is a decision about what the number MEANS, and it
/// is not one to change by accident while making a log line richer.
#[test]
fn the_unexamined_tally_counts_only_the_mapped_skip() {
    let r = Rig::new("unlooked", &["shared.bin", "shared.bin"], &["shared.bin"]);
    let len = r.payload_len();

    // The ordinary case: a resolvable report, nothing skipped.
    let ok = vec![(0, report("shared.bin", vec![1], len))];
    let (w, u) = r.resolve(0, &ok);
    assert_eq!(w.len(), 1, "premise: this report resolves");
    assert_eq!((u.slots, u.blocks), (0, 0));

    // A report the SET GUARD holds out. Refused, and not counted: set 1
    // declining a slot it never claimed says nothing about damage a
    // different piece of work could reach.
    let (w, u) = r.resolve(1, &ok);
    assert!(w.is_empty(), "premise: the set guard refuses this");
    assert_eq!((u.slots, u.blocks), (0, 0));

    // A report with no damage at all never reaches any refusal.
    let clean = vec![(0, report("shared.bin", vec![], len))];
    let (w, u) = r.resolve(0, &clean);
    assert!(w.is_empty());
    assert_eq!((u.slots, u.blocks), (0, 0));

    // A report whose `par2_name` names no member of this set.
    let mut unnamed = report("shared.bin", vec![1], len);
    unnamed.par2_name = Some("not-in-this-set.bin".to_string());
    let (w, u) = r.resolve(0, &[(0, unnamed)]);
    assert!(w.is_empty(), "premise: the name resolves to no member");
    assert_eq!((u.slots, u.blocks), (0, 0));
}
