...
/// The payload files the volume-payload rescue PUBLISHED that this set's
/// own whole-file MD5 cannot prove byte-exact, for the caller to hold
/// once that set's repair has come up short and no sibling set proved
/// its names.
///
/// # The state, and why nothing else can see it
///
/// `repair/volpayload.rs` buys a recovery volume that might BE a payload
/// file the set declares and is missing, proves it by exact length plus
/// md5-16k, and RENAMES it to the FileDesc's name. That file then has no
/// slot (`build_fetch_plan` skipped the volume it was posted as, before
/// a slot existed) and came out of no archive - so both arms of
/// `get/tail/disposition.rs`'s failing-job quarantine are blind to it,
/// exactly as they are blind to the candidates that rescue DECLINED.
/// Those are the `rescue_left` population and the rescue reports them
/// itself; a PUBLISHED file cannot join them there, because the repair
/// that runs next may still heal it. Until this function it was left in
/// a failed job's output directory under a real payload name, decided by
/// nothing having looked.
///
/// # The ruling, and the two measurements behind it
///
/// It is NOT "quarantine it", and it is not "leave it" either. Both were
/// measured on 31 Aug 2026 against a two-file post whose first member is
/// posted under a recovery volume's name and whose second is wholly lost
/// (`a_rescued_payload_survives_a_failed_job` and
/// `a_holed_rescued_payload_does_not_survive_a_failed_job`):
///
/// * THE HONEST TWIN IS SPARED. Post the same surviving file under its
///   own name so it gets a slot, fail the job the same way, and
///   `held_downloaded_files` leaves it in place - a stably plain file
///   the job can prove whole IS its own deliverable, which is TODO 159
///   item 1's ruling (a user getting two of three files beats none).
///   The two output trees are identical but for the journal. So an
///   unconditional quarantine here would withhold a file only because
///   of WHICH DOOR its bytes came through, which is an accident and not
///   a rule.
/// * A HOLED CANDIDATE PASSES THE PUBLICATION PROOF. md5-16k plus a
///   length is an IDENTITY test; refuse one middle article of the
///   candidate and 150,000 bytes land with 40,452 of them zero, the
///   first 16 KiB intact, and the rescue names it payload. That is
///   exactly the false artifact `quarantine_partials` exists to prevent
///   ("a holed .nfo looks exactly like a real .nfo"), so an
///   unconditional spare here would ship one.
///
/// So the predicate is the one all three of the quarantine's arms
/// already use - provably whole stays delivered - written in the only
/// vocabulary a slotless file has: the recovery set's own whole-file
/// MD5, which is a STRONGER proof than either indexed arm's (an arrival
/// census, or a source-volume map). It is not a second discrimination
/// beside them; it is the same one, reachable.
///
/// # Why here, and why the answer goes back into `rescue_left`
///
/// Here, because this is the one frame that holds all three of the
/// answer's parts - the set (so `desc.md5` is in hand), `out_dir`, and
/// the verdict that this set's repair came up short - and it is reached
/// only through the `failed` walk below, which has already excused a set
/// whose names a sibling proved. `finish_job` has the first of those and
/// not the other two.
///
/// AFTER the repair, because the question is about the state the repair
/// LEAVES: par2 heals a holed rescued file in place when it has the
/// parity, and a file asked about at publication time would be judged on
/// bytes the repair was about to correct. That is the whole reason this
/// cannot simply be pushed onto `rescue_left` where the declined
/// candidates are.
///
/// Into `rescue_left` rather than renamed here, so there is ONE
/// quarantine door and one line about it: `quarantine_failed_payload`
/// renames the whole population through
/// `nzbkit::journal::quarantine_paths`, which is what keeps the retry
/// round trip intact (`unquarantine_partials` restores the base name at
/// the head of the next attempt, before `restore` reads it).
///
/// # The population is DISK STATE, not this attempt's publications
///
/// F5 (1 Sep 2026). The first cut walked only the names the rescue
/// published ON THIS CALL, and that made the whole ruling fire exactly
/// once - on the attempt that happened to run the rescue. The retry
/// closes the loop the wrong way round: `get/plan.rs` restores every
/// `*.nzbfast-partial` unconditionally at the head of the next attempt
/// (`unquarantine_partials`, before `Journal::open`), so the holed
/// payload is back under its real name; `volpayload.rs`'s
/// `absent_files` then sees that name OCCUPIED, the rescue does not
/// re-arm, `published` is empty, and this function held nothing. The
/// same 150,000-byte file with a 40 KB zero hole shipped under a real
/// payload name in a failed job's directory on every attempt after the
/// first, which is the exact harm the ruling was landed to prevent (an
/// *arr imports on name and size). So the question asked here is now
/// "what is ON DISK under one of this set's names that nothing else
/// judges", which is idempotent across attempts.
///
/// Two arms, and the second is the one that survives a retry:
///
/// * the names this call PUBLISHED, kept because they are slotless BY
///   CONSTRUCTION rather than by the path comparison below;
/// * every FileDesc of this set that is PRESENT in `out_dir` and that NO
///   SLOT owns, whoever put it there. A slot-owned file is excluded
///   because taking it here would overturn TODO 159 item 1's ruling that
///   a stably plain file the job can prove whole is its own deliverable
///   - the control arm
///   `the_honest_twin_of_a_rescued_payload_is_spared_the_same_way`.
///
///   "OWNED" IS `extractor.slot_path(i)`, and so is
///   `held_downloaded_files`' own population, which is why this
///   exclusion cannot be read as "the other judge has it". Both
///   `filter_map` that same `Option`, so a slot with NO WRITER THIS RUN
///   - a fully-resumed member, whose bytes were all on disk before the
///   attempt began - answers `None` in both and is in neither list. Such
///   a file is therefore judged HERE and nowhere else, on this set's own
///   whole-file MD5, which is the stricter of the two judgements and the
///   safe direction: a resumed member that proves byte-exact is spared
///   exactly as a rescued one is, and one that does not is renamed aside
///   rather than shipped under a payload name in a failed job's
///   directory.
///
/// The MD5 predicate is untouched: a slotless present member the set's
/// own whole-file MD5 proves byte-exact is still spared, so
/// `a_rescued_payload_survives_a_failed_job` reads the same.
///
/// THE ONE TRADE, stated rather than left to be discovered: a file a
/// user (or a donor copy) dropped into `out_dir` under one of this set's
/// names, not byte-exact, is now renamed aside on a job that was failing
/// anyway, where before it was left bare. That is the conservative
/// direction and it matches `unquarantine_partials`' own occupancy rule,
/// and the bytes are kept either way - but it is a behaviour change.
///
/// STATED LIMIT, and it is about the OTHER population. A candidate the
/// content proof DECLINED sits at the name its own yEnc header declared.
/// Where that is one of this set's FileDescs - an obfuscated post whose
/// article names the real file - the walk below covers it on the retry
/// like any other present slotless member. Where it is not, the restored
/// leftover is invisible here, and `sidefetch.rs`'s
/// `destination_takes_a_volume` then refuses to rewrite it, so it is
/// absent from the paths `left_behind` is built from and stays bare.
/// That one wears a volume-shaped name rather than a payload one, so
/// nothing imports it on name and size; closing it means reporting a
/// REFUSED destination without over-crediting the written paths every
/// other caller counts slices from, which is a different site and a
/// separate decision.
fn unproven_rescues(
    ...
    // The files a slot already speaks for, by their CURRENT on-disk
    // location (`slot_path` tracks a verified-name publish), so the
    // exclusion holds for an obfuscated post too.
    let owned: Vec<PathBuf> = (0..slots.len())
        .filter_map(|i| extractor.slot_path(i))
        .collect();
    let mut held: Vec<PathBuf> = Vec::new();
    for desc in &set.files {
        let p = nzbkit::disk::join_out_name(out_dir, &nzbkit::disk::sanitize_out_name(&desc.name));
        // A published name is one of this set's own FileDescs by
        // construction; a miss means the rescue and the set have parted
        // company, and holding on no evidence is not this function's
        // call to make. So the first arm is expressed as a predicate
        // over the set's own files rather than as a second walk.
        let was_published = published.iter().any(|n| n == &desc.name);
        // A REGULAR FILE and nothing else, tested WITHOUT following a
        // link. A plain `exists()` here renamed aside the DIRECTORY a
        // user had left at a member's name, which
        // `a_landing_that_cannot_finish_leaves_no_partial_file_and_no_staging_copy`
        // plants deliberately as a blocker and requires to survive as a
        // directory - and neither a directory nor a symlink is a thing
        // this rescue could have published, so widening the population
        // to disk state must not widen it to those.
        let plain_file = std::fs::symlink_metadata(&p).is_ok_and(|m| m.is_file());
        if !was_published && (!plain_file || owned.iter().any(|o| o == &p)) {
            continue;
        }
        // Read in one pass rather than slurped: this is a payload
        // file and the whole point is that we do not know it is
        // small. An unreadable or vanished file is NOT proved, so it
        // is held - the conservative direction, and
        // `quarantine_paths` skips a path that is not there anyway.
        let whole = nzbkit::par2::verify_file_md5_path(&p, desc).unwrap_or(false);
        if !whole && !held.contains(&p) {
            held.push(p);
        }
    }
    held
...
async fn disk_repair_declined_sets(
    ...
    // Paired with the published-rescue names each failing set left in
    // `out_dir`, because `unproven_rescues` below cannot be asked until
    // every set has had its turn - a sibling set may yet prove this
    // one's names.
    let mut failed: Vec<(usize, Vec<String>)> = Vec::new();
    for (i, plan) in declined.iter().enumerate() {
        ...
        // Per set, and never shared: a name is only judged against the
        // set whose FileDesc it came from.
        let mut published_rescues: Vec<String> = Vec::new();
        // Filled by `fetch_and_repair` off `RepairReport::per_file`
        // (declared name, landed relative name), one entry per FileDesc
        // this set's own collision took off its declared name -
        // deduplicated across however many native passes ran. Rendered
        // ONCE here rather than by the engine itself, which reports the
        // same fact per PASS (`nzbkit::par2repair::dupclaim`'s stated
        // limit) - the job log is the one place that can see every pass
        // this call made and say it a single time.
        ...
        let one = fetch_and_repair(
            ...
            &plan.damaged,
            rescue_left,
            &mut published_rescues,
            &mut mismatches,
        )
        .await?;
        for (declared, landed) in &mismatches {
            warn!(
                target: "repair",
                "{declared:?} could not be honoured on disk for set {} - it landed as \
                 {landed:?} instead",
                plan.index,
            );
        }
        ...
        if one {
            ...
            failed.push((i, published_rescues));
        }
    }
    for (i, published) in &failed {
        ...
        // This set's names really are unrepaired, so the rescued
        // payload files are judged and the ones this set's own MD5
        // cannot prove join the declined candidates in `rescue_left`.
        // Judged off DISK STATE and not off `published` alone, or the
        // ruling would fire only on the attempt that ran the rescue and
        // the retry would put the same holed file back under its real
        // name: see [`unproven_rescues`].
        rescue_left.extend(unproven_rescues(
            declined[*i].set.as_ref(),
            out_dir,
            published,
            slots,
            extractor,
    ...
    Ok(failed
        .iter()
        .all(|(i, _)| already_proved(declined[*i].set.as_ref(), proven)))
}
