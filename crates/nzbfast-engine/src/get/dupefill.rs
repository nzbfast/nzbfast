...
//!   `tasks/worker.rs::predecessor_posting`.
...
pub(super) struct FillReport {
    ...
    /// Slots this pass DECLINED TO LOOK AT because they were mapped or
    /// chased when it ran, and the bad blocks they were carrying.
    ///
    /// The one quantity in this report that is not about what the pass
    /// DID. It exists because nothing in the tree could say whether the
    /// pass examined any of a job's damage at all: every counter above
    /// is zero both when the pass looked and found nothing and when it
    /// never looked, and those are opposite facts about a job. On a
    /// RAR payload - most real releases - the first entry point skips
    /// every damaged slot, so a field install saw the same silence as
    /// a job with no damage.
    ///
    /// **Scoped to the mapped/chased skip alone, deliberately.**
    /// [`wanted_files`] refuses a report for five other reasons (no
    /// PAR2 name, no set member, a member with no blocks, no file on
    /// disk, two slots claiming one member) and none of them is counted
    /// here. They are refusals about THIS post's shape that no change
    /// to this pass would lift; the mapped/chased skip is the one that
    /// names a population a different piece of work could serve, which
    /// is the question these fields were added to answer. Widening them
    /// to "everything wanted_files declined" would make the number
    /// bigger and stop it meaning that.
    ///
    /// Normally contributed by the FIRST entry point only: the second
    /// runs after the materialize loop has demoted every mapped and
    /// chased slot of a declined set, so it finds nothing to skip. A
    /// non-zero count from that pass means a `materialize` call failed,
    /// which is warned about at its own site.
    ///
    /// Added 1 Sep 2026 -
    /// `research/DUPEFILL-LIVE-PATCH-SIZING-2026-09-01.md` section 7,
    /// which prices the feature this measures the population for and
    /// declines to build it until these numbers exist.
    pub(super) unlooked_slots: usize,
    /// Bad blocks carried by [`FillReport::unlooked_slots`] - the size
    /// of that population in the unit the rest of this report counts.
    pub(super) unlooked_blocks: usize,
}
...
impl FillReport {
    ...
    pub(super) fn absorb(&mut self, other: FillReport) {
        ...
        self.unlooked_slots += other.unlooked_slots;
        self.unlooked_blocks += other.unlooked_blocks;
    }
...
/// What [`wanted_files`] walked past, for [`FillReport::unlooked_slots`].
///
/// A separate type rather than a second return tuple element because it
/// has two fields that only mean anything together: a slot count with no
/// block count says nothing about how much damage went unexamined, and a
/// block count with no slot count cannot be read against a job's shape.
#[derive(Default)]
struct Unlooked {
    slots: usize,
    blocks: usize,
...
pub(super) async fn fill_from_duplicate_postings(
    ...
    let (wanted, unlooked) = wanted_files(
        target, set_index, verifier, reports, extractor, slots, out_dir,
    );
    let mut out = fill_wanted(
        servers, target, &wanted, donor_nzbs, donor_dirs, cancel, budget,
    )
    .await;
    // Folded in AFTER the pass, not passed into it: `fill_wanted` takes
    // nothing but a set, some `Wanted` and some donor NZBs, which is
    // what keeps the whole wire-and-proof half testable without an
    // `Extractor` (see [`Wanted`]). This figure is a fact about the
    // RESOLVER, so it joins the report at the one point that has both.
    out.unlooked_slots = unlooked.slots;
    out.unlooked_blocks = unlooked.blocks;
    out
}
...
pub(super) async fn fill_wanted(
    ...
    for w in &wanted {
        ...
        if landed.len() == w.bad.len() && !landed.is_empty() {
            ...
            match nzbkit::par2::verify_file_md5_path(&w.path, &target.files[w.file]) {
                Ok(true) => out.proven.push(w.sidx),
                Ok(_) => warn!(
                    target: "repair",
...
fn wanted_files(
    ...
    let mut out = Vec::new();
    let mut unlooked = Unlooked::default();
    for (sidx, r) in reports {
        ...
        if extractor.is_mapped(*sidx) || extractor.is_chased(*sidx) {
            // Counted on the way past, which is the only place that can:
            // one frame out this slot is indistinguishable from one that
            // had no damage. See [`FillReport::unlooked_slots`] for what
            // the number is for and why it counts THIS arm and not the
            // five refusals around it.
            unlooked.slots += 1;
            unlooked.blocks += r.bad_blocks.len();
            continue;
        }
