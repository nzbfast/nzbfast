...
///
/// `rescue_left` CARRIES TWO POPULATIONS, and the second does not come
/// from the rescue at all. The candidates above are the ones its content
/// proof DECLINED. The ones it ACCEPTED were renamed to a FileDesc name
/// and are invisible here for the identical reason, but they cannot be
/// reported at rescue time: the repair that runs next may heal them in
/// place, so their disposition is only decidable once it has failed.
/// `get/settle/repair.rs`'s `unproven_rescues` asks that question where
/// the recovery set is in scope and appends the answer to this same
/// vector - a published file the set's OWN whole-file MD5 cannot prove
/// byte-exact, which md5-16k plus a length cannot rule out. So do not
/// read this arm as "whatever the rescue failed to prove", and read that
/// doc before adding a fourth population here.
///
