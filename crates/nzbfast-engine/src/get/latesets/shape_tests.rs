...
//! * the one call site in `crates/nzbfast-engine/src/get/settle.rs` is still at
...
///
/// [`super::Outstanding`] carries five fields and none of them is that
/// refusal string, so the three green tiers below cannot see one: they
/// answer "was the download SHORT, and did a vouched set account for
/// the shortfall", which is a different question from "did this post
/// contradict itself" (M4-14a) or "did the re-extraction after a
/// successful PAR2 repair fail". Settle's own assignment from
/// `late_good` was unconditional until this pin, so either refusal
/// could be flipped away by a chain or residual heal that had nothing
/// to say about it - and `get::tail::finish_job` returns Completed in
/// its `if all_good` arm some eighty lines ABOVE the
/// `if let Some(why) = reextract_failed` arm that would have failed the
/// job, so the refusal went out with the journal at rc=0.
///
/// STRUCTURAL, and for the reason the module header gives about its
/// neighbours: the shape needs a vouched non-activated set that
/// accounts for the WHOLE shortfall AND an independent refusal live at
/// the same instant, which no fixture in the suite builds today
/// (`e2e_lateset::chainset` builds the first half, `e2e_integrity`'s
/// `a_conflicting_ypart_overlap_resolves_whole_and_stays_in_range` the
/// second, and that one settles through `settle::noset`, which never
/// calls this pass at all). A pin that reads the source holds the
/// invariant in the meantime, and holds it against the NEXT route into
/// the same hole rather than only against the two that are known.
///
/// FIX A HIT by keeping the reconciliation, never by deleting it: any
/// spelling that consults `reextract_failed` where the late verdict is
/// taken satisfies this. If a later change moves the reconciliation
/// somewhere else entirely, repoint this scan at where it went.
#[test]
fn the_late_set_verdict_cannot_outvote_a_recorded_refusal() {
    let settle = code_only(SETTLE).expect("settle.rs lexes");
    let call = settle
        .find("apply_nonactivated_disk_sets(")
        .expect("settle.rs calls the late-set pass");
    let end = settle[call..]
        .find("sweep_spent_sources(&spent)")
        .map(|d| call + d)
        .expect("settle.rs sweeps the deferred spent sources after the pass");
    let region = &settle[call..end];
    assert!(
        region.contains("late_good"),
        "the late-set pass's verdict must be taken between the call and the \
         deferred sweep - if `late_good` moved, this scan is reading the \
         wrong region and is no longer checking anything"
    );
    assert!(
        region.contains("reextract_failed"),
        "the late-set verdict must be reconciled with `reextract_failed` \
         where it is taken - an unconditional `all_good = late_good` lets a \
         set that only ever spoke about a SHORT download overturn a refusal \
         it cannot see, and `finish_job` then reports Completed and deletes \
         the journal before it ever reaches the refusal arm"
    );
}
