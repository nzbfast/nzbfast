...
/// The one-shot data backfills and the one table rebuild, out of this
/// file for the size gate's 4,000-line ceiling. DDL stays here; anything
/// that MOVES ROWS lives there.
mod backfill;
/// The FTS5 virtual tables and their triggers - an optional subsystem,
/// same ceiling.
mod fts;

use backfill::{
    predb_pt_backfill, quality_backfill_slice, rebuild_marks_if_needed, retroactive_backfills,
};
use fts::{ensure_fts, ensure_people};
...
fn additive_columns(db: &Connection) {
    ...
    for ddl in [
        ...
        // (review sweep 7, H2). The worst of it was a silent permanent
...
fn derived_indexes_and_triggers(db: &Connection) {
    ...
    // without the other and it silently stops being used (review sweep
    // 4, L3 mechanism B).
    let _ = db.execute(
