...
// this file was 3,448 lines of the 4,000-line production ceiling - the
// narrowest in the repo - and 1,115 of those were these three inline
// `#[cfg(test)]` modules, which the gate charges against the PRODUCTION
// ceiling because only a `#[cfg(test)] mod foo;` TARGET is scored at the
// 12,000-line test one. They were SIBLINGS at the top level, so each one
// takes its own file: lifting the contiguous block as a unit would have
// nested them under a new `tests` module and silently renamed every test
// path to `ingest::tests::multi_server_indexing::*` and friends.
// `cfg(test)` is redundant on a module that only test code reaches, but
// it is what size-gate.py's CFG_TEST_MOD resolver keys on to score the
// child as test code rather than gate it at the production fn ceiling;
// all of them resolve to `ingest/<name>.rs` because this module is
...
impl Index {
    ...
    /// 2. **Every `n` distinct across the filenames** - OR the
    ///    positions form a perfect `k`-fold cover of `1..=m`, which is
    ///    `k` whole sessions stacked in one window (see below). A
    ///    session position is unique within ONE session, so two files
    ///    claiming position 1 are otherwise not a session.
    ...
    ///    nothing.
    ///
    /// **The k-fold arm, and why it does not widen the false-positive
    /// risk.** A standing handle that posts two twelve-track albums into
    /// one OVER window inserts `n = 1` twice, so plain distinctness
    /// vetoed the `(poster, 12)` key and BOTH albums kept
    /// `total_parts = 12` with one segment apiece - `RelAgg::complete`
    /// could never go true for either, and D3 refuses a later batch that
    /// disagrees about the total, so nothing could fix the rows
    /// afterwards. That is the standing-handle shape `albumfold` was
    /// written for. The alternative reading - a repeated position from
    /// leading PART counters - needs every filename to still be unique
    /// (arm 1 is unchanged), and a part counter over one file repeats
    /// its filename by construction, so reaching a two-fold cover of
    /// `1..=m` that way would take `2m` unrelated files from one poster
    /// each contributing exactly one article to this window. The
    /// requirement is exact: every position from 1 to `m`, each seen the
    /// same number of times. A ragged overlap is still refused.
    ///
    ...
    fn session_totals_that_count_files(
        ...
        seen.into_iter()
            .filter(|((_, m), by_name)| {
                ...
                // Arm 2: one position per file. Every set is a
                // singleton by arm 1, so this counts how many files
                // claim each `n` - one apiece is a single session, and
                // the same count k >= 2 over every position of `1..=m`
                // is k sessions stacked.
                let mut counts: HashMap<u32, usize> = HashMap::new();
                for n in by_name.values().flatten() {
                    *counts.entry(*n).or_default() += 1;
                }
                counts.values().all(|&c| c == 1)
                    || (counts.len() == *m as usize
                        && counts.keys().all(|&n| n >= 1 && n <= *m)
                        && counts.values().all(|&c| c == by_name.len() / counts.len()))
            })
            .map(|(k, _)| k)
            .collect()
    }
