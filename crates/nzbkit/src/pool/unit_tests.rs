...
// `gates::CONN_DARK`'s pins - the alive-is-not-connected deadlock, 30 Aug
// 2026. Out here for the same size gate, and out of gates.rs itself for a
// reason of its own that the file's own header gives. Same `#[cfg(test)]`
// for the same resolver reason as the three above.
#[cfg(test)]
mod conn_dark_tests;

// The single-fault provider rigs - a ghost cap, a total outage, three
// mute shapes, a desync, a heal, a takedown, an auth blip - out under
// the same gate on 31 Aug 2026, when this file sat 41 lines from its
// ceiling. They share none of this module's helpers, so the move was
// verbatim. Same `#[cfg(test)]` for the same resolver reason as the
// four above.
#[cfg(test)]
mod provider_fault_rigs;
...
fn required_mask_counts_only_live_lower_levels() {
    ...
    assert_eq!(sh.required_mask_at(0, 0), 0, "level 0 answers to nobody");
}
...
fn shard_plans_are_born_before_any_thread() {
    ...
    assert_eq!(shared.required_mask_at(1, 0), server_bit(0));
