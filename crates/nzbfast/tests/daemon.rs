...
mod daemon_health;
// The 30-day per-provider quality ledger, end to end (sibling dir,
// size gate).
mod daemon_provquality;
...
// The two client facades - the SAB-compatible API and the NZBGet
// JSON-RPC one - as a client sees them (sibling dir, size gate).
mod daemon_facade;
...
/// what the cost came to. When TODO 306 added the two faster twins, the
/// arm this test was written for stopped being the one that fires here
/// and nothing said so; deleting that arm outright left this test
/// passing in 5.6 s, unchanged.
///
/// So the second assertion names the EARLY twin by the clause only it
/// composes. That is what actually runs at these thresholds and it is
/// not a threshold artefact: the early arm is evaluated BEFORE the
/// window bookkeeping and takes its verdict off the run rather than off
/// a rolling window, so it needs neither the 2 s warmup nor a full
/// window and reaches the verdict in about two watchdog ticks. Its
/// windowed elder cannot be reached on this fleet at all, at any
/// threshold, and now has a row of its own that can:
/// `e2e_qprog::an_unprobed_server_leaves_the_windowed_arm_to_speak`.
///
/// Both assertions are kept. The shared phrase is the 14 Aug claim and
/// stands on its own; the clause is which mechanism delivers it today.
/// If a change moves this row onto another arm, that is a fact worth
/// being told about rather than absorbed in silence - move the second
/// assertion and say in the message why the arm changed.
#[tokio::test(flavor = "multi_thread")]
async fn gone_post_defers_so_the_queue_moves_on() {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        assert!(
            q.contains("answered so far came back missing"),
            "the EARLY post-is-gone arm is the one that speaks for a head \
             that has landed NOTHING on a fleet where every server has \
             itself refused - it reads the run, so it needs neither the \
             warmup nor a full window. Its two siblings compose different \
             clauses (`carries what is left of this post` for the \
             flatline twin, `came back missing and not a byte arrived` \
             for the windowed elder), so a defer here that names one of \
             those has moved this row onto an arm it was not written \
             for: {q}"
        );
...
async fn clear_queue_empties_every_row_and_counts_them() {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        // ...and it says WHY. The dashboard's bulk Remove reports in red
        // off this key: without it the handler ran on into its success
        // branch and toasted "Removed 0 from the queue." in green over a
        // write that never happened. Asserted on the arm rather than on
        // the page because three controls read it - the bulk bar, the
        // row's own X, and the clear-everything button, which reaches it
        // through `mustOk` and had nothing to print but "rejected".
        let e = v["error"].as_str().unwrap_or_default();
        assert!(
            e.contains("nothing in the queue matched"),
            "a refused queue delete must say why: {r}"
        );
        // The page shows this through `tErr`, which keys on the WIRE
        // TEXT: `err.<the whole sentence>` in web/i18n/en.reference.json
        // and all 27 catalogues. Change the words here and 27 locales
        // silently fall back to English, so the sentence is pinned.
        assert_eq!(
            e,
            "nothing in the queue matched that - it may have just finished, \
             or been removed already",
            "the wire text is an i18n key - move web/i18n with it: {r}"
        );
    })
    .await
    .unwrap();
