...
//! not two). `crates/nzbkit/tests/scratch/mod.rs` and
//! `crates/nzbtray/tests/scratch/mod.rs` are copies; keep the three in
//! step. The nzbtray one has no integration target above it and reaches
//! this shape only through `src/testscratch.rs`'s `#[path]`, which is
//! the point: one file in three places beats three designs.
//!
