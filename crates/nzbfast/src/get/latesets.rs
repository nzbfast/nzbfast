...
pub(super) fn apply_nonactivated_disk_sets(
    ...
    // message's contract per `failkind::RECOVERY_SHORTFALL_CLAUSE`. Kept
    // in step with `good`: cleared wherever the residual/chain
    // reconciliation below turns `good` back to true, because a
    // shortfall this pass went on to heal is no longer why anything
    // failed. `blocks_over_set`'s `multi=true` is always right here - a
    // vouched set requires a NAMING active set, so the post carries at
    // least two recovery sets whenever this fires.
    ...
    // for the same slot.
    let assigned = keep_uniquely_assignable_residuals(&residual, &short, &slot_bytes, slots);
    if !good
...
/// `crates/nzbkit/src/par2repair/unit_tests.rs` pins that off a
...
/// is complete on disk.
///
/// Conservative in exactly the four ways
/// [`residual_accounts_for_the_shortfall`] is, and it is the SAME
/// question with a stronger warrant: `derrs_net` must be zero, the
/// census must not have counted something these slots cannot see, and
/// EVERY outstanding slot must be accounted for - by a residual the
/// X5-24 gate already KEPT, or by a chain rebuild [`assign_by_length`]
/// finds uniquely assignable to it in both directions.
///
...
fn chain_accounts_for_the_shortfall(
    ...
    if derrs_net > 0 || short.is_empty() || incomplete > census_len || chained.is_empty() {
        return false;
    }
...
#[cfg(test)]
mod tests {
    use super::{Chained, assign_by_length, chain_accounts_for_the_shortfall, decide, fits};
    ...
    fn the_chain_tier_refuses_every_way_it_is_meant_to() {
        ...
        let ok = |c: &[Chained], sh: &[usize], inc, census, derrs| {
            chain_accounts_for_the_shortfall(&none, c, sh, &bytes, &slots, inc, census, derrs)
        };
