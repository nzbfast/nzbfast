...
//! (`serve/postproc.rs` - before finalize the names are pre-rename and
...
fn declared_in(dir: &Path, out: &mut HashSet<PathBuf>) {
    let (bytes, skipped) = match crate::unpack::collect_par2_bytes(dir, SET_SCAN_CAP) {
        Ok(v) => v,
        Err(_) => return,
    };
    if bytes.is_empty() {
...
mod tests {
    ...
    /// id per member and a FileDesc for each. No IFSC and no recovery
    /// slices - `Par2Set::parse` leaves `blocks` empty for a file with
    /// no IFSC packet, and nothing here asks about blocks. Same shape
    /// as `identity.rs`'s fixture builder, kept local because that one
    /// is `#[cfg(feature = "indexer")]` and this must hold on the slim
    /// build too.
    fn par2_index(set: u8, members: &[(&str, u64)]) -> Vec<u8> {
