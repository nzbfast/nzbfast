...
//! `get/vrig.rs` installs its extractor on the hub, `serve/api/queue.rs`
...
//! (`serve/tasks/runner.rs`) - which is BEFORE this ladder runs, since
...
pub(crate) struct UnpackProgress {
...
pub(crate) struct UnpackArm {
    prev: Option<Arc<UnpackProgress>>,
    hub: Option<Arc<crate::streamhub::StreamHub>>,
    owner: String,
}

/// Arm progress reporting for the ladder about to run on this thread.
///
/// `volumes` is what the caller counted on disk. No hub means nobody is
...
pub(crate) fn arm(
    ...
    h.unpack.lock_ok().insert(owner.to_string(), p.clone());
    UnpackArm {
...
impl Drop for UnpackArm {
    fn drop(&mut self) {
        if let Some(h) = &self.hub {
            h.unpack.lock_ok().remove(&self.owner);
        }
...
pub(crate) fn watch(written: &Arc<AtomicU64>, archives: &[rars::Archive], resumed: u64) {
    begin_set();
...
pub(crate) fn attempt(written: &Arc<AtomicU64>, total: u64, resumed: u64) {
    with_live(|p| {
...
pub(crate) fn raise_total(total: u64) {
    with_live(|p| {
        let mut cur = p.cur.lock_ok();
...
pub(crate) fn unpacked_total(archives: &[rars::Archive]) -> u64 {
    archives
        .iter()
...
mod tests {
    ...
    fn armed(volumes: u64) -> (Arc<crate::streamhub::StreamHub>, UnpackArm) {
        ...
        let arm = arm(&Some(hub.clone()), "nzo-1", volumes);
        (hub, arm)
    }
    ...
    fn the_hub_entry_appears_with_the_arm_and_leaves_with_it() {
        let hub = Arc::new(crate::streamhub::StreamHub::default());
        {
            let _a = arm(&Some(hub.clone()), "nzo-1", 130);
