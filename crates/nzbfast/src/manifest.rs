...
//!
//! What it is NOT: a proof about extracted archive output. For an
//! archive post the PAR2 set covers the volumes, and the extractor does
//! not retain per-file checksums of what it produced, so those files
//! appear here as presence-only entries - name and length, honestly
//! marked unverifiable. A later stage may hash them during the finalize
//! move; this one records only what the download already proved.
//!
...
pub(crate) const MANIFEST_NAME: &str = ".nzbfast.manifest";
...
pub(crate) enum Role {
    ...
    Source,
    /// On disk at write time with no checksum to hold it to (extracted
    /// output). Verify checks existence and length only.
    Presence,
}
...
pub(crate) struct VerifyReport {
    pub(crate) files: Vec<(String, FileStatus)>,
...
impl Manifest {
    ...
    /// to say about it.
    ///
    /// Everything else on disk the set never covered becomes a presence
    /// entry: name and length, honestly marked unverifiable.
    pub(crate) fn write_reconciled(&mut self, dir: &Path) -> std::io::Result<()> {
    ...
    pub(crate) fn load(dir: &Path) -> std::io::Result<Manifest> {
    ...
    pub(crate) fn verify(&self, dir: &Path) -> std::io::Result<VerifyReport> {
        ...
        for e in &self.files {
            ...
            let status = match std::fs::metadata(&path) {
                ...
                Ok(_) if e.md5.is_none() => FileStatus::PresentUnverified,
...
fn check_entry(e: &Entry, block_size: u64, path: &Path) -> std::io::Result<FileStatus> {
    ...
    let md5_hex: String = hex(&whole.finalize().into());
    let md5_ok = e.md5.as_deref() == Some(md5_hex.as_str());
    if md5_ok && bad.is_empty() {
        Ok(FileStatus::Ok)
    } else {
...
pub(crate) fn verify_cli(dir: &Path) -> std::io::Result<bool> {
    ...
    for (name, status) in &report.files {
        match status {
            ...
            FileStatus::PresentUnverified => {
                info!(
                    target: "par2",
                    "• {name} - present, size ok, no checksum recorded (extracted output)"
                );
            }
...
mod tests {
    ...
    /// normal), while the extracted output the set never covered is
    /// recorded present - honestly unverifiable.
    #[test]
    fn consumed_volumes_are_sources_and_extracted_output_is_presence() {
        let dir = temp_dir("archive");
        let vol = body(8192, 4);
        // The volume is NOT on disk (the spent-volume sweep took it);
        // the extracted movie is.
        std::fs::write(dir.join("Movie.mkv"), body(50000, 5)).unwrap();
        ...
        let mkv = back.files.iter().find(|e| e.name == "Movie.mkv").unwrap();
        assert_eq!(mkv.role, Role::Presence);
        let report = back.verify(&dir).unwrap();
        assert!(report.all_ok(), "{report:?}");
        assert!(
            report
        ...
        assert!(
            report
                .files
                .iter()
                .any(|(n, s)| n == "Movie.mkv" && *s == FileStatus::PresentUnverified)
        );
        // But a payload file that goes missing IS damage.
