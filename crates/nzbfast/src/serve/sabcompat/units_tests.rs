...
fn the_wire_priority_stays_inside_sabs_five_words() {
    ...
    for p in [
        ...
        0,
        -1,
        crate::serve::job::DUPE_PRIORITY,
        crate::serve::job::SAB_DEFAULT_PRIORITY,
        -2,
        -4,
        3,
        7,
    ...
    // `slot_json`.
    j.priority = crate::serve::job::DUPE_PRIORITY;
    assert_eq!(sab_priority_name(&j), "Normal");
    assert_eq!(crate::serve::job::priority_name(j.priority), "Duplicate");
}
...
fn sab_age_speaks_sabs_own_day_hour_minute_tokens() {
    ...
    assert_eq!(sab_age(0), "-");
    assert_eq!(sab_age(-1), "-");
    let now = crate::serve::epoch_secs() as i64;
