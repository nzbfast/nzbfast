...
fn row(d: &Arc<Daemon>, id: &str) -> Value {
    let v = super::sabcompat::queue_json(d, &std::collections::HashMap::new());
    v["queue"]["slots"]
        .as_array()
        .expect("slots array")
        .iter()
...
fn only_the_unpacking_row_carries_its_ladder_counters() {
    with_daemon("unpackprog", |d| {
        ...
        let hub = d.hub.clone();
        let arm = crate::unpackprog::arm(&Some(hub.clone()), "nzo-a", 130);
        let a = row(d, "nzo-a");
...
fn a_real_disk_unpack_puts_a_byte_lane_on_the_row() {
    with_daemon("unpackprog-live", |d| {
        ...
        let arm = crate::unpackprog::arm(&Some(d.hub.clone()), "nzo-live", 2);
