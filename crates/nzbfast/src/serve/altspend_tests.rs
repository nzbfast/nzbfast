...
fn job(id: &str, name: &str, extra: serde_json::Value) -> Arc<Mutex<Job>> {
...
fn ctx_for(d: &Daemon, id: &str, name: &str) -> super::AltCtx {
    d.alt_ctx(
        id,
        name,
        crate::serve::giveup::target_keys(&crate::wall::parse_release(name)),
    )
}
...
fn the_breakers_own_failed_stems_still_count_as_copies() {
    ...
    flat_rate_config(&d);
    d.alt.max_copies.store(2, Ordering::Relaxed);
    let keys = crate::serve::giveup::target_keys(&crate::wall::parse_release(ORIG));
    {
