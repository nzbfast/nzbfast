...
impl ScriptStatus {
    ...
    /// FAILURE.
    pub(super) fn fold(self, next: ScriptStatus) -> ScriptStatus {
        match next {
...
impl NzbCommands {
    pub(super) fn parse(lines: &[String]) -> Self {
...
pub(super) fn script_chain(raw: &str) -> Vec<PathBuf> {
    raw.split([',', ';'])
        .map(str::trim)
...
impl Daemon {
    ...
    pub(super) fn resolve_scripts(&self, job: &Arc<Mutex<Job>>) -> Vec<PathBuf> {
        let (over, cat) = {
            let g = job.lock_ok();
