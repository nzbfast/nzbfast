...
impl Stored {
    ...
    pub(super) fn record(&mut self, f: &JobFacts, now: i64) {
        let key = day_key(now);
...
pub(super) fn report(s: &Stored, now: i64) -> Report {
...
impl ProvQuality {
    ...
    pub(super) fn record(&self, f: &JobFacts, now: i64) {
    ...
    pub(super) fn report(&self, now: i64) -> Report {
        report(&self.stored.lock_ok(), now)
    }

    /// The report as the API ships it.
    pub(super) fn report_json(&self, now: i64) -> serde_json::Value {
