...
//! respelled `pub(in crate::serve)`. Two items changed visibility here
...
/// half owns it.
pub(super) fn restore_job_settings(
    daemon: &Arc<Daemon>,
...
pub(super) fn restore_ui_and_index_settings(
    daemon: &Arc<Daemon>,
    saved: &serde_json::Map<String, Value>,
) {
...
#[cfg(feature = "indexer")]
pub(super) fn seed_index_keep_kinds(settings_path: &Path) -> Mutex<Vec<String>> {
    Mutex::new(
        load_settings(settings_path)
...
#[cfg(feature = "indexer")]
pub(super) fn seed_index_evict_scope(settings_path: &Path) -> Mutex<String> {
    Mutex::new(
        load_settings(settings_path)
...
#[cfg(feature = "indexer")]
pub(super) fn seed_index_evict_headroom(settings_path: &Path) -> AtomicU64 {
    AtomicU64::new(
        load_settings(settings_path)
...
pub(super) fn seed_watchlist(settings_path: &Path) -> Mutex<Vec<crate::watchlist::WatchItem>> {
...
/// the watcher.
pub(super) fn seed_watch_state(spool: &Path) -> Mutex<crate::watchlist::WatchState> {
