...
pub(in crate::serve) enum FinishAction {
    #[default]
    None,
    Script,
    Sleep,
...
impl FinishAction {
    pub(in crate::serve) fn parse(s: &str) -> Option<Self> {
        Some(match s.trim() {
            "" | "none" | "off" => Self::None,
            "script" => Self::Script,
    ...
    pub(in crate::serve) fn as_str(self) -> &'static str {
        match self {
            Self::None => "none",
            Self::Script => "script",
            Self::Sleep => "sleep",
...
pub(in crate::serve) const DEFAULT_DELAY_SECS: u64 = 60;
...
impl FinishState {
    pub(in crate::serve) fn action(&self) -> FinishAction {
        *self.action.lock_ok()
    }

    pub(in crate::serve) fn set_action(&self, a: FinishAction) {
    ...
    pub(in crate::serve) fn set_script(&self, p: Option<PathBuf>) {
        *self.script.lock_ok() = p;
    }

    pub(in crate::serve) fn script(&self) -> Option<PathBuf> {
        self.script.lock_ok().clone()
    }

    pub(in crate::serve) fn set_delay_secs(&self, n: u64) -> u64 {
        let n = n.min(MAX_DELAY_SECS);
        self.delay_secs.store(n, Ordering::Relaxed);
        n
    }

    pub(in crate::serve) fn delay_secs(&self) -> u64 {
    ...
    /// on the lane.
    pub(in crate::serve) fn note_drained(&self) {
...
pub(in crate::serve) fn drain_blocker(d: &Daemon) -> Option<&'static str> {
...
pub(in crate::serve) fn spawn_finish_watcher(daemon: &Arc<Daemon>) {
    let dw = Arc::downgrade(daemon);
    let stop = crate::serve::RunStop::current();
    crate::serve::spawn_aux("queue-finish", move || {
...
fn tick(d: &Arc<Daemon>) -> std::time::Duration {
...
pub(in crate::serve) fn cancel(d: &Arc<Daemon>) -> Value {
...
fn run_finish_script(d: &Arc<Daemon>) {
    ...
    match crate::serve::script::run_capped_capture(cmd, secs) {
...
fn run_power_action(d: &Arc<Daemon>, action: FinishAction) {
    ...
    for (prog, args) in power_commands(action) {
        ...
        match crate::serve::script::run_capped_capture(cmd, 60) {
...
pub(in crate::serve) fn payload(d: &Daemon) -> Value {
