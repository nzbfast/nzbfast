...
//! the wire.
//!
//! The mover is strictly whole-job (`serve/mover.rs`), so a 40 GB
...
impl Daemon {
    ...
    pub(super) fn early_publish_dest(
        ...
        if self.rename_media_only.load(Ordering::Relaxed)
            || self.rename_junk.load(Ordering::Relaxed)
        {
            return None;
        }
    ...
    pub(super) fn early_take(&self, g: &mut Job) -> Vec<PathBuf> {
        if g.early_published.is_empty() {
            return Vec::new();
        }
...
/// held up for.
pub(super) fn early_unlink(paths: &[PathBuf]) {
    if paths.is_empty() {
        return;
    }
    for p in paths {
