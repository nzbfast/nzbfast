...
pub(crate) struct ResumeArm(Vec<Entry>);
...
impl ResumeArm {
    ...
    pub(crate) fn new(outputs: &[nzbkit::extract::ResumeOutput]) -> ResumeArm {
    ...
    pub(crate) fn stood_down(outputs: &[nzbkit::extract::ResumeOutput]) -> ResumeArm {
...
pub(crate) fn plan(dir: &Path) -> std::collections::HashMap<String, (PathBuf, u64, u32)> {
    ARMED.with(|a| {
        a.borrow()
            ...
            .filter(|e| {
                crate::rarfix::sanitized_entry_path(dir, &e.member).as_ref() == Some(&e.path)
            })
...
pub(crate) fn clear_unresumed<I: Iterator<Item = String>>(
    ...
    let doomed: Vec<PathBuf> = members
        ...
        .filter_map(|m| crate::rarfix::sanitized_entry_path(dir, &m))
        .collect();
    if doomed.is_empty() {
        return;
    }
