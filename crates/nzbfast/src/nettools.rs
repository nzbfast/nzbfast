...
pub(crate) async fn sysbench_cmd(config: &Path, group: &str) -> Result<()> {
    ...
    // groupscan.rs): only when the network row IS the limit,
...
pub(crate) fn stream_cmd(
    ...
    if !no_open && !m3u.is_empty() {
        match player_argv(&m3u) {
            ...
            // #[cfg(target_os)] inside `player_argv` - open, explorer or
            // xdg-open - and the only varying part is the m3u URL, which
            // `player_argv` already refuses when it carries a credential.
            // The child is the user's own media player: confining its reads
            // or its network would break the playback this command exists to
            // start.
