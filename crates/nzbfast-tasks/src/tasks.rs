...
mod worker;
use runner::*;
pub use worker::spawn_download_worker;
mod stall;
mod tuner;
mod watchfolder;
...
pub use stall::*;
pub use tuner::*;
// `pub(super)` since the watch-fail vocabulary moved down to
// `serve::watchfail` (2 Sep 2026): nothing `pub(crate)` is left in this
// module for a wider re-export to carry, and rustc says so.
pub use watchfolder::*;
...
/// never touches the index write connection (see crates/nzbfast-daemon/src/searchlog.rs),
...
#[cfg(feature = "indexer")]
pub fn spawn_search_log_writer(daemon: &Arc<Daemon>) {
    let d = daemon.clone();
...
pub fn spawn_hunt_worker(daemon: &Arc<Daemon>) {
    let d = daemon.clone();
    tokio::spawn(async move {
        loop {
            d.hunt.wake.notified().await;
            let d2 = d.clone();
            let _ = tokio::task::spawn_blocking(move || crate::hunt::hunt_tick(&d2)).await;
        }
    });
}
...
pub fn spawn_usage_flush(daemon: &Arc<Daemon>) {
    let d = daemon.clone();
    tokio::spawn(async move {
        loop {
            tokio::time::sleep(std::time::Duration::from_secs(30)).await;
...
/// pending.
pub fn spawn_queue_saver(daemon: &Arc<Daemon>) {
    let d = daemon.clone();
...
pub fn spawn_memory_trim(daemon: &Arc<Daemon>) {
    let d = daemon.clone();
    tokio::spawn(async move {
...
pub fn spawn_auto_speed(daemon: &Arc<Daemon>, config: &std::path::Path) {
    let d = daemon.clone();
...
#[cfg(feature = "indexer")]
pub fn spawn_group_catalog(daemon: &Arc<Daemon>, config: &std::path::Path) {
    let d = daemon.clone();
    let config = config.to_path_buf();
    tokio::spawn(async move {
        let d2 = d.clone();
...
pub fn spawn_index_scan(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            // index-deepen-lap-timing (1 Sep 2026): the lap's own
            // stage breakdown. The group pass has been timed per group
            // for a while (`done:` in `index_scan_into`, and now its
            // per-leg line), but nothing said what share of a LAP each
            // stage is - so "raising `index_deepen` lengthens the lap"
            // could not be priced, and neither could the far larger
            // fact that most of a lap is the interval sleep. Every
            // stage below is timed; the line is emitted once the lap
            // completes.
            let lap_t0 = Instant::now();
            // Spotnet first - see `spot_pass` for why it leads.
            let t_spots = Instant::now();
            if scan_spots {
                spot_pass(&daemon2, &config, &db).await;
            }
            let t_spots = t_spots.elapsed();
            let t_reclass = Instant::now();
            reclassify_pending_rows(&daemon2, &db, &cats).await;
            let t_reclass = t_reclass.elapsed();
            ...
            // Wall clock of the whole fan-out, which is NOT the sum of
            // the per-group lines: `par` groups run at once, so this is
            // the schedule's makespan and the quantity a lap actually
            // pays.
            let t_groups = t_groups.elapsed();
            ...
            let t_gapfill = t_gapfill.elapsed();
            if waiting() {
                drop(index_pass);
                continue;
            }
            ...
            info!(
                ...
                "lap work {:.1?}: spots {:.1?}, reclassify {:.1?}, {} group(s) {:.1?} \
                 (par {par}), titles {:.1?}, gapfill {:.1?}, maintenance {:.1?}, \
                 evict {:.1?}, stats {:.1?} - next pass in {}s",
                lap_t0.elapsed(),
                t_spots,
                t_reclass,
                groups.len(),
...
pub fn spawn_watchlist_watcher(daemon: &Arc<Daemon>) {
...
pub fn spawn_library_recheck(daemon: &Arc<Daemon>, config: &std::path::Path) {
    let d = daemon.clone();
    let config = config.to_path_buf();
    tokio::spawn(async move {
        loop {
...
#[cfg(feature = "indexer")]
pub fn spawn_enrichment_workers(daemon: &Arc<Daemon>) {
...
///
/// A check that is REFUSED (bad signature, or a serial that fails the
/// anti-rollback ratchet) leaves whatever banner is already up alone,
/// exactly as an unreachable channel does - a hostile channel must not
/// be able to take down news of a release that is genuinely out.
///
...
pub fn spawn_update_checker(daemon: &Arc<Daemon>) {
        ...
        loop {
            ...
            if d.update_checks.load(Ordering::Relaxed) {
                match check_update(&d) {
                    ...
                    Ok(None) => {}
                    // No log line here on purpose: `check_update` logs
                    // its own failures, because it is the only place
                    // that can tell an unreachable channel (routine,
                    // info) from a refused manifest (a security event,
                    // warn). Logging the returned string here could
                    // only ever pick one level for both.
                    Err(_) => {}
                }
            }
            drop(d);
...
pub fn spawn_scheduled_bench(daemon: &Arc<Daemon>, config: &std::path::Path) {
