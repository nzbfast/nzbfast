...
//! Daemon-backed functions in tasks.rs. StallTracker basics live
...
fn watch_fail_id_is_an_opaque_16_hex_handle_of_the_full_path() {
    let a = crate::watchfail::watch_fail_id(std::path::Path::new("/a/same.nzb"));
    assert_eq!(a.len(), 16);
    assert!(
        a.chars()
    ...
    assert_eq!(
        a,
        crate::watchfail::watch_fail_id(std::path::Path::new("/a/same.nzb"))
    );
    // The FULL path is the identity: same basename, different dir.
    let b = crate::watchfail::watch_fail_id(std::path::Path::new("/b/same.nzb"));
    assert_ne!(a, b);
...
fn watch_fail_kind_matches_exactly_except_the_kept_prefix() {
    ...
    assert_eq!(crate::watchfail::watch_fail_kind(&wrapped), "rejected");
    assert_eq!(crate::watchfail::watch_fail_kind(""), "rejected");
    let kept = format!("{}: Permission denied (os error 13)", watchfail::KEPT);
    assert_eq!(crate::watchfail::watch_fail_kind(&kept), "kept");
}
...
fn media_claim_name_prefers_a_non_empty_identity() {
    ...
    assert_eq!(crate::mediadisk::media_claim_name(&j), "posted.name");
    let j = mkjob("posted.name", "Canonical.Name-GRP");
    assert_eq!(crate::mediadisk::media_claim_name(&j), "Canonical.Name-GRP");
    // Whitespace is non-empty: no trimming happens here.
    let j = mkjob("posted.name", "   ");
    assert_eq!(crate::mediadisk::media_claim_name(&j), "   ");
}
...
fn tune_hint_scores_providers_against_the_local_link_not_the_line() {
    use crate::locallink::{LinkKind, LocalLink};
...
fn a_sticky_cap_gauge_banks_one_episode_once() {
    use crate::tasks::stall::fold_caps_for_test as fold;
    let now_ms_for_test = || {
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
