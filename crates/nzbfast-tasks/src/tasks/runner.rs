...
pub(super) struct JobTail {
    ...
    pub(super) prov_facts: Vec<crate::provquality::HostFacts>,
        ...
        && led.spent() >= quota
    {
        ...
        if fresh {
            ...
            *guard_reason = Some("quota".into());
            ...
            crate::hooks::notify_event(d, "quota", &msg);
...
pub(super) fn reset_hub_for_job(
    ...
    // is now carried down to `PoolConfig::line_anchor_measured`, and
    // since 2 Sep 2026 a rule rests on it: TODO 275 item 7 lets a
    // MEASURED anchor's fleet walk past `LINE_CAP_MAX_FLEET` to
    // `pool::linecap::supply_ceiling`, bounded by the account's own
    // grant, and `conntune::line_cap_headroom_fleet` spawns the slots
    // for that raise to land in. So this store decides a ceiling now
    // rather than only captioning a log line.
...
pub(super) fn settle_job_tail(
    ...
    if let Some(l) = &detached.pool_live {
        ...
        for s in &l.servers {
            ...
            prov_facts.push(crate::provquality::HostFacts {
                host: s.host.clone(),
                tried,
                missing,
...
// on a child of `runner.rs` resolves against tasks/runner/
