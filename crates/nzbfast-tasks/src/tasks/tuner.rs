...
pub fn spawn_live_tuner(daemon: &Arc<Daemon>, config: &std::path::Path) {
    let d = daemon.clone();
...
/// Is the LINK idle - not merely the job queue?
///
/// An index scan or deepening pass pulls headers over the SAME provider
/// the ladder measures (field case: a 90k headers/s deepening pull
/// mid-probe reads every rung flat and fakes a knee at a fraction of the
/// true one), so `scan_active` counts as busy alongside the queue.
///
/// One copy, asked in THREE places that must agree: the gate that starts
/// a probe, the callback that stops one mid-climb, and the guard that
/// decides whether the result may be recorded. Two of those were written
/// out separately and the third did not exist.
fn link_idle(d: &Daemon) -> bool {
    !d.scan_active.load(Ordering::Relaxed)
...
/// The lane context `spawn_auto_connections` clones once at spawn and
/// every rung then reads: the daemon, the config it was loaded from,
/// that file's path, and the runtime handle the blocking probe drives
/// through.
///
/// A bundle for the same reason `get`'s `DecodeCtx` and `LossLedgers`
/// are: [`record_ladder_result`] needs eight values across the seam and
/// `clippy::too_many_arguments` is fatal here.
struct LadderLane<'a> {
    d: &'a Arc<Daemon>,
    cfg: &'a nzbkit::config::Config,
    cfg_path: &'a std::path::Path,
    rt: &'a tokio::runtime::Handle,
}

/// What one completed connection ladder does to the tune: charge the
/// bytes if the link stopped being idle under it, re-measure where the
/// rungs disagree, apply the plausibility guards, persist the knee and
/// say what was decided.
///
/// Moved out of [`spawn_auto_connections`] for the 500-line function
/// ceiling. Every guard, every log line and the order they run in are
/// the original lines. Three mechanical changes and no others: the four
/// lane values are reached through `lane`, `ids` arrives as a slice
/// (`to_vec` where the arm wrote `clone`, the same allocation), and the
/// arm's four `continue`s are `return`s - the arm was the LAST statement
/// of the `loop` body, so both spellings do the identical thing.
fn record_ladder_result(
    lane: &LadderLane<'_>,
    srv: &nzbkit::config::ServerConfig,
    ids: &[String],
    now: u64,
    steps: Vec<nzbkit::sysbench::LadderStep>,
) {
    // Asked BEFORE the re-measure, which is up to two
    // more minutes of the user's account corroborating
    // rungs the guard further down is going to discard
    // anyway - and two more minutes of this probe
    // competing with the job that just started. Billed
    ...
    if !link_idle(lane.d) {
        ...
        info!(
            ...
            "{}: a download or index scan started while the ladder ran - \
             stopped it and discarded the measurement rather than \
             recording a knee taken against a busy link",
            srv.host
        );
        return;
    }
    ...
    // The rungs the re-measure below completes, counted
    // OUT HERE because the 120 s timeout drops that
    // future and its own `out` vec with it, so on that
    // arm the bytes a finished rung really moved were
    // unrecoverable (F21, 1 Sep 2026 - found on the
    // hand-triggered ladder in api/servers.rs, fixed on
    // both).
    let remeasure_owed = std::sync::atomic::AtomicU64::new(0);
    let steps = match contested {
        ...
        Some((rungs, peak)) => {
            ...
            let again = lane.rt.block_on(async {
                tokio::time::timeout(
                    std::time::Duration::from_secs(120),
                    nzbkit::sysbench::remeasure(srv, ids.to_vec(), &rungs, peak, 5, |st| {
                        remeasure_owed.fetch_add(st.bytes, Ordering::Relaxed);
                    }),
                )
                .await
            });
            match again {
                ...
                // would charge the re-measure twice, so
                // the counter is zeroed here.
                Ok(Ok(extra)) => {
                    remeasure_owed.store(0, Ordering::Relaxed);
                    crate::conntune::merge_samples(&steps, &extra)
                }
                // A failed re-measure leaves the ladder as
                // it was: still jagged, so still suspect.
                // Its completed rungs are still owed.
                _ => {
                    warn!(
                        target: "tune",
    ...
    // The climb re-asks per rung and the check above
    // covers the gap before it, but the RE-MEASURE runs
    // for up to two minutes with no callback to carry
    // one - so this is the window still open, and it is
    // the last moment before `record`. A measurement
    // taken against a busy link reads every rung flat,
    // which is the faked-low-knee shape the
    // corroboration rule exists for arriving from our
    // own side; an unrecorded provider is simply
    // re-probed, where a wrongly-recorded one is
    // applied.
    if !link_idle(lane.d) {
        info!(
            target: "tune",
    ...
    let limit = crate::conntune::effective_limit(
        lane.d.connections.load(Ordering::Relaxed),
        srv.connections,
    );
    ...
    // influence the flag.
    let fresh = crate::conntune::load(lane.cfg_path);
    if fresh
        .get(&srv.host)
    ...
    let suspect = crate::conntune::is_suspect(best, limit, knee.jagged, fresh.get(&srv.host));
    if knee.jagged {
        warn!(
            target: "tune",
    ...
    mirror_shaped(lane.d, lane.cfg_path, &srv.host);
    if suspect {
        info!(
            target: "tune",
    ...
    update_tune_hint(
        lane.d,
        &lane.cfg.servers,
        &crate::conntune::load(lane.cfg_path),
    );
}
...
pub fn spawn_auto_connections(daemon: &Arc<Daemon>, config: &std::path::Path) {
    let d = daemon.clone();
    ...
    crate::spawn_aux("auto-conn", move || {
        ...
        loop {
            ...
            let Some(ids) = rt.block_on(crate::probeids::real_ladder_ids(&d, &srv)) else {
                info!(
                    target: "tune",
            ...
            let res = rt.block_on(async {
                tokio::time::timeout(
                    ...
                    nzbkit::sysbench::conn_ladder(&srv, ids.clone(), cap, ceiling, 5, {
                        ...
                        move |phase, at, steps| {
                            *dd.ladder_live.lock_ok() = Some(crate::daemon::LadderLive {
                                host: host.clone(),
                                phase: phase.into(),
                                at,
                                steps: steps.to_vec(),
                            ...
                            // Re-ask before each rung's fleet goes on
                            // the wire. The gate at the top of the loop
                            // is check-once and the climb runs for four
                            // minutes past it, so a job the scheduler
                            // picked up meanwhile is measured by every
                            // rung after it - and the reading was
                            // discarded below either way. Stopping is
                            // what is new: four more minutes of the
                            // user's account, against the very download
                            // this ladder is now competing with, bought
                            // nothing.
                            if !link_idle(&dd) {
                                return false;
                            }
            ...
            match res {
                Ok(Ok(steps)) if !steps.is_empty() => record_ladder_result(
                    &LadderLane {
                        d: &d,
                        cfg: &cfg,
                        cfg_path: &cfg_path,
...
#[cfg(test)]
#[path = "tuner_ladder_tests.rs"]
mod tuner_ladder_tests;
