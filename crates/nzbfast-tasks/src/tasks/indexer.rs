...
/// The confirm lane's pick side - the seed queue, the correlation picks
/// it stands in for, and the durable retry catalogue. Out of this file
/// for the size gate's 4,000-line ceiling.
#[cfg(feature = "indexer")]
mod confirm_queue;
#[cfg(feature = "indexer")]
use confirm_queue::{
    CONFIRM_RECENT_KEY, confirm_catalog_fence, confirm_inbox_room_or_log, flush_confirm_retry,
    pick_kind, retire_confirm_pick, retry_confirm_pick, seed_next,
};
// The queue's own cases live in this file's `mod tests` and reach past
// what the lane itself calls.
#[cfg(all(test, feature = "indexer"))]
use confirm_queue::{
    ConfirmRetry, SEED_QUEUE_KEY, SEED_RECENT_KEY, confirm_retry_path, save_confirm_retry,
    seed_pop, seed_pop_at, seed_retry_at,
};
...
// The lap's gate hold and the handback between its stages. A file of
// its own because it is the one piece of the lap that is about the
// GATE rather than about the index.
mod pass_gate;
pub(crate) use pass_gate::PassGate;
