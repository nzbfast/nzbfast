...
//! module are spelled `crate::stream::` rather than `super::`.
//! The FINAL on-disk pass and the claim name it judges against are
//! `crate::mediadisk` since 2 Sep 2026: `histmigrate` and
//! `stream`'s tests call them, and neither belongs to this lane.
...
/// [`crate::stream::payload_in_flight`] documents: the
...
///
/// `pub(crate)` for its test, which lives with the rest of
...
pub fn miss_is_in_flight(d: &Daemon, job: &Arc<Mutex<Job>>, id: &str) -> bool {
    if crate::stream::payload_in_flight(d, job) {
        info!(
            target: "media",
...
pub fn spawn_media_prober(daemon: &Arc<Daemon>) {
    let d = daemon.clone();
...
fn probe_live_facts(d: &Daemon, id: &str) -> Option<nzbkit::mediaprobe::MediaFacts> {
    ...
    let (file, w, mut r, _lease) = crate::stream::open_live_probe(d, id)?;
