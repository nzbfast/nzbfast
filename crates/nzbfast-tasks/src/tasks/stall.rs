...
#[cfg(test)]
pub(crate) fn fold_caps_for_test(d: &Arc<Daemon>) -> Vec<(String, usize)> {
    fold_caps(d)
}
...
/// double-count.
pub(crate) fn fold_and_bank_caps(d: &Arc<Daemon>) {
...
pub fn spawn_slow_job_watchdog(
    daemon: &Arc<Daemon>,
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            crate::insurance::insurance_yields_to_arrivals(&d);
...
mod requeue_cost_tests {
    ...
    fn the_refetch_arm_reads_lag_and_small_change_as_no_cost() {
        ...
        *d.active_dl.lock_ok() = Some(id.into());
        ...
        let d2 = crate::testutil::test_daemon(&bare);
