...
#[cfg(feature = "indexer")]
pub(crate) fn wall_enricher(d: Arc<Daemon>, stop: crate::RunStop) {
    use nzbkit::index::Lane;
    for lane in [Lane::Movies, Lane::MusicBooks] {
        let d2 = d.clone();
        crate::spawn_aux("wall-enrich", move || wall_enrich_lane(d2, lane, stop));
    }
...
#[cfg(feature = "indexer")]
pub(crate) fn lane_kind(kind: &str) -> crate::wall::Kind {
    match kind {
...
#[cfg(feature = "indexer")]
pub(crate) fn wall_enrich_lane(d: Arc<Daemon>, lane: nzbkit::index::Lane, stop: crate::RunStop) {
...
#[cfg(feature = "indexer")]
pub(crate) const PERSON_ART_CAP_BYTES: u64 = 192 * 1024 * 1024;
...
#[cfg(feature = "indexer")]
pub(crate) fn person_photo_fetcher(d: Arc<Daemon>, stop: crate::RunStop) {
    let art = d.spool.join("art");
    let _ = std::fs::create_dir_all(&art);
        ...
        for (id, url) in batch {
            ...
            match crate::wall::fetch_image(&url) {
                Some(bytes) => {
                    ...
                    let _ = crate::maint::publish_art(&art, &name, &bytes);
                    got_any = true;
                }
                None => {
                    failed.insert(id);
...
#[cfg(feature = "indexer")]
pub(crate) fn prune_person_art(dir: &std::path::Path, cap: u64) {
...
#[cfg(feature = "indexer")]
pub(crate) fn imdb_ratings_refresher(d: Arc<Daemon>, stop: crate::RunStop) {
    loop {
        if stop.stopping() {
            return;
        }
...
#[cfg(feature = "indexer")]
pub fn spawn_predb_feed(daemon: &Arc<Daemon>) {
    {
            ...
            loop {
                ...
                let reason = nzbkit::predb::run_once(
                    ...
                    |m| {
                        ...
                        let mut pend = heard.predb.pending.lock_ok();
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let batch: Vec<nzbkit::predb::PreLine> =
                    std::mem::take(&mut *daemon2.predb.pending.lock_ok());
                if !batch.is_empty() {
                    ...
                    match stored {
                        ...
                        None => {
                            ...
                            let mut pend = daemon2.predb.pending.lock_ok();
                ...
                if !daemon2.predb.enabled.load(Ordering::Relaxed) {
                    continue;
                }
                ...
                if daemon2.predb.corr_enabled.load(Ordering::Relaxed) {
                    ...
                    let auto = daemon2.predb.corr_auto.load(Ordering::Relaxed);
                    if daemon2.indexing_pause_reason().is_none()
                ...
                if now - last_prune >= 3_600 && daemon2.index_maintenance_ok() {
                    last_prune = now;
                    let keep_rows = daemon2.predb.max_rows.load(Ordering::Relaxed);
...
mod headshot_art_tests {
    ...
    fn a_failed_headshot_publish_leaves_no_file_to_be_skipped() {
        ...
        assert!(crate::maint::publish_art(&dir, &name, b"a-whole-portrait"));
        ...
        if sealed {
            assert!(!crate::maint::publish_art(&dir, &name, b"XX"));
            assert!(
                !live.is_file(),
        ...
        assert!(crate::maint::is_art_staging_name(&staged), "{staged}");
