...
//! and the quarantine directory unusable files are moved into.
//!
//! The six states a file can end up in when it does NOT become a job -
//! their strings, their classifier and the row handle the dashboard
//! names them by - are `crate::watchfail`, one layer down,
//! because the SAB facade publishes all three and this lane only writes
//! them.
//!
...
pub fn spawn_watch_folder(daemon: &Arc<Daemon>) {
    let d = daemon.clone();
