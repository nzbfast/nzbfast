    ...
    tokio::spawn(async move {
        ...
        loop {
            let Some(mut run) = cur.take() else {
                ...
                if let Start::Started(r) = start_next(&mut st, false, Carry::Alone, None).await {
                    cur = Some(r);
                }
                continue;
            };
            ...
            // TODO 313 item 10: a hand-over that the SPILL governor
            // asked for, rather than one queue-dry asked for, sizes its
            // successor by absorption - what that job can actually
            // hold, bounded by the slice the head just walked down to.
            let spill = spill_size(&st.d);
            let next = loop {
                match start_next(&mut st, true, Carry::Into(&mut run), spill).await {
            ...
            // A SPILL is the one shape where the runner does not now
            // stand and wait for N: the head is lending its sockets
            // precisely because it cannot use them, and the win in the
            // measurement comes from the lane taking the NEXT queued
            // job as it finishes rather than one job overlapping one
            // head. `spill_episode` owns both runs until the head
            // drains, and hands back whatever is still live.
            match next {
                Some(lane) if st.d.spill.is_live() => {
                    cur = spill_episode(&mut st, run, lane).await;
                }
                next => {
                    // Whether or not a successor started, N drains now
                    // and is settled and submitted BEFORE the runner
                    // looks at anything of N+1's: that is the
                    // history-order guarantee.
...
/// The sockets the next spilled lane may build on, or `None` when no
/// spill episode is live and this is an ordinary queue-dry hand-over.
///
/// ABSORPTION, not a share (TODO 313 item 10): the job about to start
/// is given what it can hold - `min(its remaining articles, what is
/// left of the slice)` - and the residue stays with the head rather
/// than being spent on a job that cannot use it. A small job is
/// article-bound, and that is the whole reason this mechanism is worth
/// anything: twelve articles cannot hold thirty-two sockets however
/// many they are offered.
///
/// The pick has not happened yet, so this asks the queue for the job
/// that pick will most likely take. Getting it wrong costs a socket or
/// two either way and never correctness - the lane is bounded by the
/// slice, by its own fleet build and by the account lease whatever this
/// returns.
fn spill_size(d: &Arc<Daemon>) -> Option<usize> {
    let slice = d.spill.lent();
    if slice == 0 {
        return None;
    }
    ...
    let next_bytes = d
        ...
        .find_map(|j| {
            ...
            .then_some(g.total_bytes.saturating_sub(g.downloaded_bytes))
        })
        .unwrap_or(0);
    Some(crate::spill::absorb(
        crate::spill::articles_for(next_bytes),
        slice,
    ))
}

/// A live spill episode: the struggling HEAD stays in the drain slot
/// and lanes cycle through the active slot behind it, each taking the
/// next queued job as the last one finishes, until the head drains or
/// the governor takes the sockets back.
///
/// **This is where item 10's win actually is**, and it is the reason
/// the feature is "spill to the QUEUE" rather than "spill to the next
/// job": in the measured shape a head with dead air had EIGHT small
/// jobs behind it, and clearing one of them buys a few percent where
/// clearing all eight buys 29-45% of total wall. One lane that chains
/// down the queue clears them all.
///
/// **Two phases at once and no more** (`spill::MAX_DOWNLOAD_PHASES`):
/// the head plus one lane. That is this daemon's structural bound - two
/// wire slots, `wire_counters` reading exactly those two under one
/// lock, one watchdog owner and one drainer - and the measurement says
/// four lanes buy nothing over two anyway.
///
/// **The history-order guarantee is deliberately broken here, for
/// spilled lanes only.** The ordinary path settles and submits the
/// draining job before it looks at anything of its successor's; a
/// spilled lane by construction finishes FIRST, which is the point of
/// the feature ("the smalls finish before the head"). The head still
/// drains, settles and submits through the same `finish` on the way
/// out, and the post-processing lane already runs tails concurrently
/// and files whichever finishes first (`postproc_jobs`), so nothing
/// downstream of here learns a new shape.
async fn spill_episode(st: &mut Runner, mut head: Running, lane: Running) -> Option<Running> {
    let mut lane = Some(lane);
    while let Some(mut l) = lane.take() {
        let head_first = tokio::select! {
            at = &mut head.net_rx => {
                head.net_at = Some(at.unwrap_or_else(|_| Instant::now()));
                true
            }
            at = &mut l.net_rx => {
                l.net_at = Some(at.unwrap_or_else(|_| Instant::now()));
                false
            }
        };
        if head_first {
            // The head is done with the wire. It settles first, and the
            // lane becomes the ordinary running job - so a lane that is
            // still downloading keeps its pipeline and is handed back to
            // the loop untouched.
            lane = Some(l);
            break;
        }
        finish(st, l).await;
        // Take the next queued job onto the same lent sockets, while
        // the head is still going. A governor that has reclaimed in the
        // meantime ends the chain here.
        lane = match st.d.spill.is_live() && crate::spill::phase_available(&st.d) {
            false => None,
            true => {
                let size = spill_size(&st.d);
                loop {
                    match start_next(st, true, Carry::KeepDrain, size).await {
                        Start::Started(r) => break Some(r),
                        Start::Ended => continue,
                        Start::Nothing => break None,
                    }
                }
            }
        };
    }
    // The episode is over however it ended: the head has the wire back,
    // so shut the gate and walk its targets up before it drains.
    crate::spill::reclaim(&st.d, "the spill is over");
    if head.net_at.is_none() {
        let at = (&mut head.net_rx).await.unwrap_or_else(|_| Instant::now());
        head.net_at = Some(at);
    }
    finish(st, head).await;
    lane
}
    ...
    j.relocating > 0
        ...
        || (insurance
            && (!j.insurance
                ...
                || crate::insurance::insure_refusal(j).is_some()))
}

/// What this start does with the job that is already on the wire.
enum Carry<'a> {
    /// Nothing is: the ordinary pick with the runner idle.
    Alone,
    /// This run's drain overlaps the start - push it into the drain
    /// slot as the new job claims the hub.
    Into(&'a mut Running),
    /// Leave the drain slot exactly as it is (TODO 313 item 10). A lane
    /// spilled behind a struggling head starts while that head is
    /// ALREADY in the slot and still downloading, so this start must
    /// not touch it: clearing it would silence the head's queue row,
    /// take its bytes out of `wire_counters` and hand the slow-job
    /// watchdog a job it can no longer see.
    KeepDrain,
}

impl Carry<'_> {
    /// Does this start overlap a job that is still on the wire? True
    /// for both overlapping shapes, which is what the pre-pipeline arms
    /// mean by it.
    fn overlapping(&self) -> bool {
        !matches!(self, Carry::Alone)
    }
}
...
/// `carry`: what to do with the run already on the wire - see
/// [`Carry`]. For `Into`, its pool-facing figures are detached off the
/// hub right before the new job claims it, and not a moment earlier - a
/// start that finds nothing leaves the carried run owning the hub for
/// its whole drain.
/// `spill`: this start is a lane of a live spill episode, sized at that
/// many sockets by absorption.
async fn start_next(st: &mut Runner, quick: bool, carry: Carry<'_>, spill: Option<usize>) -> Start {
    let d = st.d.clone();
    ...
    let keep_drain = matches!(carry, Carry::KeepDrain);
    let drain = match carry {
        Carry::Into(c) => Some(c),
        _ => None,
    }
    .map(|c| {
    ...
    let progress = {
        ...
        // A spilled lane leaves the slot alone: the head in it is still
        // downloading, and this start is not its hand-over.
        if !keep_drain {
    ...
    *d.hub.handoff.lock_ok() = d.hub.conn_budget.get().map(|_| handoff.clone());
    // TODO 313: and this run's side of a queue spill, in the same place
    // and read by the same builder. A HEAD carries a seat whenever the
    // switch is on, whether or not an episode is live - its pools are
    // built long before a spill starts and cannot be handed a gate
    // afterwards. A LANE carries its absorption size with it.
    //
    // ITEM 12: the role decided here is also what freezes the
    // governor's denominator, so the two can never disagree about which
    // job is the head. `note_pool_build` runs unconditionally because
    // the switch can be flipped on mid-run and the read side already
    // short-circuits on it.
    let spill_role = match spill {
        Some(_) => nzbkit::pool::handoff::SpillRole::Lane,
        None => nzbkit::pool::handoff::SpillRole::Head,
    };
    crate::spill::note_pool_build(&d, spill_role);
    *d.hub.spill.lock_ok() = crate::spill::enabled(&d).then(|| nzbkit::pool::handoff::SpillSeat {
        gate: d.spill.gate(),
        role: spill_role,
        sockets: spill.unwrap_or(0),
    });
    ...
    //
    // §310 stage 2 adds the second population, and it is a DIFFERENT
    // question reaching the same field. A heal job carries the library
    // folder it was started to repair (`Job::heal_dir`) as a path rather
    // than as a row, because the settle manifest is built to outlive the
    // history record - see that field. Both are stated the same way to
    // the run below: a directory whose files the adoption scan may read.
    let donor_dirs: Vec<std::path::PathBuf> = {
        let (alt_from, heal_dir) = {
            let g = job.lock_ok();
            (g.alt_from.clone(), g.heal_dir.clone())
        };
        let predecessor = (!alt_from.is_empty())
            .then(|| {
            ...
            .flatten();
        // The heal folder FIRST: on the one job that can carry both, it
        // is the directory the user asked to repair and the one holding
        // the bytes worth adopting.
        [heal_dir, predecessor.unwrap_or_default()]
            .into_iter()
    ...
    for p in &donor_dirs {
        info!(
            target: "repair",
            "{nzo_id}: files already on disk at {} are available to this \
             run as donors",
            p.display()
        );
    }
...
async fn finish(st: &mut Runner, run: Running) {
    ...
    if handed_over {
        ...
        *d.started_at.lock_ok() = None;
        // TODO 313: is anything ELSE still on the wire? A spilled lane
        // ends here like any other job, and while it does the head that
        // lent it those sockets is still downloading in the drain slot.
        // Saying "the line is idle" then is simply false - it closes
        // the chart's shading over a job that is still running and
        // starts the idle clock on a line that is busy. Off a spill
        // this is always true and every line below is reached exactly
        // as it was.
        let line_now_idle = d.drain_dl.lock_ok().is_none();
