...
//! the `crates/nzbkit` and `crates/nzbtray` copies.
//!
...
//! `#[path]`, exactly as the other three crates do, so the four files
//! stay one file in four places rather than becoming four designs. This
//! copy arrived with the `nzbfast-core` cut (crate-split step 2): the
//! `streamhub` volume-ladder tests build a real `Extractor` over a temp
//! directory, and they moved down here with the module.
//!
