...
/// How many catalogues this install could consult, and how many
/// actually answered.
///
/// `configured` is the half that matters and the reason this type
/// exists. [`Verdict::Named`] needs TWO catalogues to agree, so an
/// install with one configured cannot reach it - ever, for any file.
/// Without this, such an install gets [`Verdict::Uncorroborated`] and a
/// note reading "nothing confirms it" on every single job, which reads
/// as a near miss when it is a structural ceiling, and says nothing
/// about the one thing that would change it.
///
/// Measured 1 Sep 2026: no `tmdb_key` was set in settings.json, in
/// config.local.json or in the daemon's environment, so this install
/// had exactly one catalogue and the lane could never rename anything.
/// Nothing reported that. See
/// `research/IDENTIFY-IS-OFF-2026-09-01.md`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Catalogues {
    /// How many sources this install is able to ask at all. Wikidata is
    /// keyless so this is never zero; a second needs configuring.
    pub configured: usize,
    /// How many of them actually returned an answer this time. Lower
    /// than `configured` means one was unreachable, which is a
    /// transient the user can retry rather than a setting to change.
    pub answered: usize,
}

impl Catalogues {
    /// Can this install reach [`Verdict::Named`] at all? False means no
    /// file will ever be renamed by this lane until a second catalogue
    /// is configured, whatever the bytes say.
    pub fn can_corroborate(&self) -> bool {
        self.configured >= 2
    }
}
...
pub struct Outcome {
    ...
    /// install that its outcome is a ceiling rather than a near miss.
...
impl Outcome {
    ...
    pub fn log_line(&self) -> String {
        let head = match &self.verdict {
            ...
            // Two very different situations wore one sentence until
            // 1 Sep 2026, and the difference is the only actionable
            // thing this lane ever says. With one catalogue configured
            // the verdict is a CEILING - no file will ever be renamed
            // here, however good the match - and the old wording
            // ("nothing confirms it") reads as a near miss on every
            // job, forever, while naming no remedy. With two, it is an
            // ordinary transient worth retrying.
            Verdict::Uncorroborated { title, year } if !self.catalogues.can_corroborate() => {
                format!(
                    "possibly {title} ({year}) - the only film one catalogue lists at this \
                     runtime. Renaming needs TWO catalogues to agree and only one is \
                     configured, so this can never be confirmed as things stand; add a \
                     second film catalogue in Settings to enable it"
                )
            }
            Verdict::Uncorroborated { title, year } => format!(
                "possibly {title} ({year}) - the only film one catalogue lists at this \
                 runtime, but the second catalogue could not be reached, so the name was \
                 left alone"
            ),
...
/// that a daemon started with NZBFAST_NO_ENRICH does not reach a
/// catalogue, and `netfetch::EnrichResolver` holds that under all three
/// fetchers below. A refused resolve is an unanswered source, so the
/// verdict is `NoAnswer` and the note the user reads says the
/// catalogues did not answer, which is what happened.
pub fn identify(
    ...
    // Counted from the SETTINGS, not from what came back, and counted
    // here rather than after the calls: a file with no usable runtime
    // returns before any source is consulted, and its note should still
    // be able to say whether this install could have corroborated
    // anything.
    let configured = 1 + usize::from(tmdb_key.is_some_and(|k| !k.is_empty()));
    ...
    Outcome {
        ...
        catalogues: Catalogues {
            configured,
            answered: results.iter().filter(|r| r.answered).count(),
        },
    }
}
...
mod tests {
    ...
    /// An Outcome for the log/accept tests. `configured` is the knob
    /// under test in `a_single_catalogue_install_is_told_why`; two is
    /// the ordinary case everywhere else.
    fn outcome(verdict: Verdict, facts: &str, configured: usize) -> Outcome {
        Outcome {
            verdict,
            facts: facts.into(),
            shortlist: vec![],
    ...
    fn a_lone_catalogue_hit_on_the_wrong_film_does_not_rename() {
        ...
        assert_eq!(outcome(v, "", 2).accepted_name(), None);
    }
    ...
    #[test]
    fn a_single_catalogue_install_is_told_the_ceiling_not_a_near_miss() {
        // The defect this fixes: with one catalogue configured,
        // `Named` is unreachable for EVERY file, and the old note said
        // "nothing confirms it" - which reads as bad luck on this one
        // job rather than a setting that ends the matter. Measured on
        // the live install 1 Sep 2026: no tmdb_key anywhere, so this
        // was the outcome of every identify attempt it ever made.
        let v = Verdict::Uncorroborated {
            title: "Supergirl".into(),
            year: 2026,
        };
        let lone = outcome(v.clone(), "108 min", 1);
        assert!(!lone.catalogues.can_corroborate());
        let line = lone.log_line();
        assert!(
            line.contains("only one is configured"),
            "the note must name the CAUSE, not just the outcome: {line}"
        );
        assert!(line.contains("add a second film catalogue"), "{line}");
        // ...and it still declines, and still says what it thinks.
        assert_eq!(lone.accepted_name(), None);
        assert!(line.contains("Supergirl"), "{line}");

        // With two configured the same verdict is a TRANSIENT - one
        // source was unreachable - so it must NOT tell the user to go
        // and configure something they already have.
        let both = outcome(v, "108 min", 2);
        let line2 = both.log_line();
        assert!(both.catalogues.can_corroborate());
        assert!(line2.contains("could not be reached"), "{line2}");
        assert!(!line2.contains("add a second film catalogue"), "{line2}");
    }

    #[test]
    fn one_configured_catalogue_can_never_reach_named() {
        // The guard behind the note. `decide` is the gate, and with a
        // single answered source every arm it can reach declines - so
        // the ceiling the note describes is a property of the code and
        // not a claim in a doc comment that could rot.
        for window in [
            vec![cand("wikidata", "Supergirl", 2026, 108)],
            vec![
                cand("wikidata", "Supergirl", 2026, 108),
                cand("wikidata", "Holland", 2025, 110),
            ],
            ...
            let v = decide(108, &[answered(window)]);
            assert!(
                !matches!(v, Verdict::Named { .. }),
                "one catalogue must never reach Named, got {v:?}"
            );
        }
    }
    ...
    fn a_declining_verdict_never_yields_a_name() {
        ...
        for verdict in [
            ...
            let o = outcome(verdict, "108 min", 2);
