...
//!
//! At the crate root and not under `serve/` since TODO 276 item 3: none
//! of it needs a daemon, `nzbfast sysbench` on the CLI asks for exactly
//! this through `crate::nettools`, and answering that from inside
//! `serve` was one of the three single references holding a
//! 133,000-line dependency cycle together. The daemon's five-minute
//! poll, which does own a `Daemon`, stayed in `crates/nzbfast-daemon/src/locallink.rs`.
//!
...
impl LocalLink {
    ...
    pub fn verdict(&self, line_bps: u64) -> String {
    ...
    pub fn measured_note(&self, measured_bps: u64) -> String {
        let Some(ceiling) = self.ceiling_bps() else {
            return String::new();
        };
...
impl TxMedian {
    ...
    pub fn steady(&mut self, link: &mut LocalLink) {
...
/// not be read.
pub fn probe(target: &str) -> Option<LocalLink> {
...
fn run(cmd: &str, args: &[&str]) -> Option<String> {
    // sandbox-seam-gate: every caller passes a string LITERAL naming a
    // platform network tool - route, ifconfig, networksetup, system_profiler,
    // ip, iw, netsh - with literal arguments beside it. No part of the
    // program or the argv comes from an NZB, a config value or a request, so
    // there is nothing here for a policy to confine; the child's only effect
    // is the text this function parses.
...
pub fn probe_local_link(host: &str, port: u16) -> Option<LocalLink> {
    if disabled() {
        return None;
    }
...
pub fn target_ip(host: &str, port: u16) -> Option<String> {
