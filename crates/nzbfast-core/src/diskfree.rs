...
//! quota ledger BUILT on this stayed in `crates/nzbfast-daemon/src/disk.rs`, which is
...
pub fn disk_stat(path: &std::path::Path) -> Option<(u64, u64)> {
...
pub fn disk_stat(_path: &std::path::Path) -> Option<(u64, u64)> {
    None
}
...
pub fn disk_stat_walk(path: &std::path::Path) -> Option<(u64, u64)> {
    let mut p = path;
    loop {
