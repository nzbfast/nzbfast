...
/// The two `repair::RepairShortfall::Blocks` clauses these rows
/// classify, as the strings the producer emits.
///
/// They are LITERALS here because `repair` is a layer above this crate
/// since the crate-split step 2 cut, so nothing in `nzbfast-core` can
/// call `.clause()`. Both are held to the producer by
/// `repair::shortfall::failkind_clause_pin` in `nzbfast`, which asserts
/// `.clause()` returns exactly these for exactly these inputs - so the
/// cross-layer coupling is still a compile-and-run assertion, just made
/// from the side that can see both halves. Do NOT reword either string
/// on its own: change the producer, watch that pin go red, and move
/// both together.
const SHORTFALL_9_OF_8: &str =
    "9 recovery block(s) needed but the recovery set that covers this damage carries only 8";
const SHORTFALL_20_OF_8: &str = "20 recovery block(s) needed but the recovery set that covers \
     this damage carries only 8 (recovery set abababab)";
...
mod producers {
    ...
    fn both_appended_recovery_clauses_also_open_the_parked_offer() {
        ...
        //
        // SPELLED OUT rather than built from `repair::RepairShortfall`,
        // which is where it was until the crate-split step 2 cut put
        // `repair` a layer above this crate. The coupling is not lost:
        // `repair::shortfall::failkind_clause_pin` asserts this exact
        // string is what `.clause()` emits for these inputs, so a
        // reworded clause reddens there and this row keeps testing what
        // it always did - that `failkind` classifies the words the
        // producer actually writes.
        let shortfall = SHORTFALL_9_OF_8;
    ...
    fn both_repair_verdicts_are_unrepairable_and_offer_a_search() {
        ...
        // The multi-set spelling on purpose: the classification is
        // about the ARITHMETIC, and a set tag riding on the end of the
        // clause must not move it (31 Aug 2026). Spelled out for the
        // reason `SHORTFALL_9_OF_8` gives, and pinned to `.clause()` by
        // the same test.
        let blocks =
            format!("verification failed and PAR2 repair could not complete: {SHORTFALL_20_OF_8}");
