...
fn a_post_still_propagating_is_not_a_dead_one() {
    ...
    // The undatable-post coda that used to close this row now lives at
    // the function it is about, `unpack::instream::nzb_age_days_tests`:
    // that function is a layer above this crate since the crate-split
    // step 2 cut, and it was only ever reachable from here through
    // main.rs's `use unpack::*` glob.
}
