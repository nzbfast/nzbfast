...
impl EatMode {
    pub fn parse(s: &str) -> Option<EatMode> {
...
/// same place.
pub const MARGIN: u64 = 512 * 1024 * 1024;
...
pub fn decide(mode: EatMode, verified: bool, consented: bool, forecast: Forecast) -> EatVerdict {
    if mode == EatMode::Off {
...
/// archive shape.
pub fn forecast(dir: &Path, volume_bytes: u64, encrypted: bool) -> Forecast {
    Forecast {
...
pub struct EatArm(bool);

impl EatArm {
    pub fn new(on: bool) -> EatArm {
...
pub fn volume_bytes(paths: &[std::path::PathBuf]) -> u64 {
    paths
        .iter()
