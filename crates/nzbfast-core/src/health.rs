...
pub const MAX_PROBES: u32 = 2;
/// How long a queued job must sit before the second probe.
pub const RECHECK_AFTER_SECS: i64 = 3600;
...
pub use nzbkit::preflight::Avail;
...
pub fn promote_band(h: Option<&PostHealth>) -> u32 {
...
impl PostHealth {
    ...
    pub fn no_server_can_supply(&self) -> bool {
        self.bucket == Bucket::Red
            && !self.waived
            && self.servers > 0
            && self.answered == self.servers
...
pub fn score(answers: &[ServerAnswer], age_days: u32, at: i64, probes: u32) -> Option<PostHealth> {
    let Tally {
        sampled,
        present,
        absent,
...
pub fn sample_size(total_bytes: u64) -> usize {
...
pub fn failure_clause(h: &PostHealth) -> Option<String> {
...
pub fn giveup_reason(h: &PostHealth) -> String {
    format!(
...
pub fn health_json(h: &PostHealth) -> serde_json::Value {
    serde_json::json!({
...
pub fn health_from_json(v: &serde_json::Value) -> Option<PostHealth> {
    let u = |k: &str| v.get(k).and_then(serde_json::Value::as_u64).unwrap_or(0) as u32;
    Some(PostHealth {
        bucket: Bucket::from_str(v.get("bucket").and_then(serde_json::Value::as_str)?)?,
        reason: v
