...
impl<T> WorkStealingQueue<T> {
    ...
    pub(crate) fn close(&self) {
        // nzbfast: the store happens under the SAME mutex `steal()` holds
        // across its `closed` check and its `condvar.wait()`. Upstream stored
        // and notified while holding nothing, which left the two unordered: a
        // worker could read `closed` as false, this whole function could then
        // run against no registered waiter, and the worker would park with
        // nobody alive to wake it again. `Lzma2ReaderMt::drop` detaches its
        // workers rather than joining them, so that lost wakeup was a thread -
        // and its decode buffer - leaked for the life of the process, once per
        // reader that hit the window. It reproduces on the first round of
        // `a_close_racing_a_parking_worker_is_never_lost`.
        //
        // Taking the lock only orders the store; the notify is deliberately
        // outside it so a woken worker does not immediately block on us.
        {
...
mod tests {
    ...
    /// nzbfast: `close()` must not be able to land in the window where a
    /// worker has already read `closed` as false under the queue lock and
    /// has not yet parked on the condvar.
    ///
    /// `steal()` checks `closed` and calls `condvar.wait()` while HOLDING
    /// the queue mutex. `close()` stored `closed` and called `notify_all()`
    /// while holding NOTHING, so the two were unordered: a worker could read
    /// false, close() could run to completion against no waiter, and the
    /// worker then parked with nobody left alive to wake it. Its thread is
    /// detached by `Lzma2ReaderMt::drop`, so it never exits - which is a
    /// leaked thread and a leaked decode buffer for every reader that hit
    /// the window.
    #[test]
    fn a_close_racing_a_parking_worker_is_never_lost() {
        ...
        for round in 0..2000 {
            ...
            match rx.recv_timeout(Duration::from_secs(5)) {
                Ok(None) => {}
                Ok(Some(item)) => panic!("round {round}: stole {item} from an empty queue"),
                Err(_) => panic!(
                    "round {round}: the worker never woke - close() was lost against a \
                     worker that was about to park, so that thread parks forever"
                ),
            }
            handle.join().unwrap();
        }
