...
mod tests {
    ...
    #[test]
    // nzbfast: upstream's Cargo.toml excludes `/tests/data` from the
    // published crate, so the `wget-*` binary this reads was never in the
    // copy we vendored and the test cannot run here. Kept, not deleted, so
    // the next upstream bump still shows what upstream covers. The BCJ
    // round trip IS covered from our side by the `mx9_code_bcj` fixture in
    // `decoder::differential`, which decodes a real 7-Zip BCJ+LZMA2 stream.
    #[ignore = "tests/data/ is excluded from the published crate, so it was not vendored"]
