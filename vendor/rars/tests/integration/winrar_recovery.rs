...
//!
//!   cargo test -p rars --features parallel --test integration \
//!       -- --ignored --nocapture
...
/// The WRITER direction, which every other leg in this file is the
/// mirror of: an embedded RAR 3.x recovery record built HERE, damaged,
/// and repaired by RARLab's `rar`.
///
/// A record no other reader accepts is not a recovery record, and a
/// round trip through our own repair path cannot tell the difference -
/// a shared misreading of the tag or of the parity stride would pass
/// it. `rar 7.23` can no longer create a RAR4 archive (`-ma4` is gone)
/// but still repairs one, so it is the only independent oracle this
/// writer has. It has to name the damaged sector it found and hand back
/// bytes identical to what we wrote.
#[test]
#[ignore = "needs the proprietary `rar` binary"]
fn rar4_recovery_record_we_wrote_is_repaired_by_winrar() {
    if !have_rar() {
        eprintln!("SKIP: `rar` not on PATH - this test needs RARLab's binary");
        return;
    }
    let dir = workdir("rar4-rr-writer");
    let payload = dir.join("payload.bin");
    write_payload(&payload, 200_000);
    let data = fs::read(&payload).expect("read payload");

    let mut features = rars::FeatureSet::store_only();
    features.recovery_record = true;
    let pristine = rars::rar15_40::write_stored_archive(
        &[rars::rar15_40::StoredEntry {
            name: b"protected.bin",
            data: &data,
            file_time: 0,
            file_attr: 0x20,
            host_os: 3,
        ...
        rars::rar15_40::WriterOptions::new(rars::ArchiveVersion::Rar40, features)
            .with_recovery_percent(5),
    )
    .expect("write a RAR4 archive with a recovery record");
    let original = dir.join("orig.rar");
    fs::write(&original, &pristine).expect("write pristine");

    // `rar` has to see the record before it can use it. It says so on
    // the listing, and a silent "no record" here would leave the repair
    // below passing for the wrong reason on an undamaged file.
    ...
    assert!(
        listing.contains("recovery record"),
        "rar does not see a recovery record: {listing}"
    );

    // One sector inside the member's data, clobbered whole.
    let mut damaged_bytes = pristine.clone();
    damaged_bytes[100 * 512..101 * 512].fill(0xa5);
    let damaged = dir.join("dmg.rar");
    fs::write(&damaged, &damaged_bytes).expect("write damaged");
    ...
    let output = Command::new("rar")
        ...
        .output()
        .expect("run rar r");
    assert!(output.status.success(), "rar r failed");
    let said = String::from_utf8_lossy(&output.stdout).to_lowercase();
    assert!(
        said.contains("data recovery record found"),
        "rar did not find our record: {said}"
    );
    assert!(
        said.contains("sector 100") && said.contains("recovered"),
        "rar did not locate the damage from our tags: {said}"
    );
    let fixed = dir.join("fixed.dmg.rar");
    assert_eq!(
        fs::read(&fixed).expect("read rar's repair"),
        pristine,
        "rar's repair of our record is not byte-identical to what we wrote"
    );

    // ...and our own repair path agrees with it, so the two readers are
    // not merely both wrong in the same direction.
    assert_eq!(
        rars::ArchiveReader::read(&damaged_bytes)
            .expect("parse damaged")
            .as_rar15_40()
            .expect("a RAR4 archive")
            .repair_protect_head()
            .expect("our repair"),
        pristine
    );
