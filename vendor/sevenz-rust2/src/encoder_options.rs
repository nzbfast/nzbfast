...
impl AesEncoderOptions {
    ...
    /// The two draws go through [`crate::write_entropy`] rather than
    /// straight to `getrandom`, so a caller holding an
    /// [`crate::EntropyScope`] gets a reproducible archive. Unscoped -
    /// which is every caller that says nothing - this is the same OS
    /// draw it always was. nzbfast local change; see
    /// `README-nzbfast.md`.
    pub fn new(password: Password) -> Self {
        let mut iv = [0; 16];
        crate::write_entropy::fill(&mut iv, "IV");

        let mut salt = [0; 16];
        crate::write_entropy::fill(&mut salt, "salt");
