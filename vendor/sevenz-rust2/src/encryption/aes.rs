...
pub(crate) struct Aes256Sha256Decoder<R> {
    /// nzbfast: the input buffer, allocated on first use (see get_more_data).
    ibuffer: Vec<u8>,
    cipher: Cipher,
    input: R,
    done: bool,
...
impl<R: Read> Aes256Sha256Decoder<R> {
    ...
    fn get_more_data(&mut self) -> std::io::Result<usize> {
        if self.done {
            ...
            self.ostart = 0;
            self.obuffer.clear();
            // nzbfast: a persistent 1 MiB input buffer in place of a
            // 512-byte stack array - one read and one cipher call per
            // MiB instead of 2,048 of each.
            if self.ibuffer.is_empty() {
                self.ibuffer.resize(1 << 20, 0);
            }
            let readin = self.input.read(&mut self.ibuffer)?;
            if readin == 0 {
                self.done = true;
                ...
                let n = self
                    .cipher
                    .update(&mut self.ibuffer[..readin], &mut self.obuffer)?;
                self.ofinish = n;
                Ok(n)
            }
        }
