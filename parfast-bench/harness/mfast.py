...
RIG = os.path.join(HOME, "pubrun", "fast")
LOCK = os.path.join(HOME, "pubrun", "fast.log")
SLICE = 768000
...
ARMS = [
    ("parfast", "parfast-joint", [], {"NZBFAST_FORNEY_JOINT": "0"}),
...
def main():
    ...
    lock.take()
    try:
        print("MFAST-START %s" % utcnow(), flush=True)
        box_facts()
        ...
        if len(set(gold.values())) != len(members):
            print("MFAST-FAIL payload members not distinct", flush=True)
            sys.exit(9)
        ...
        if len(probe) != len(members):
            print("MFAST-FAIL payload is near-copies", flush=True)
            sys.exit(9)
        for nm in members:
        ...
        if r["rc"] != 0 or not parfiles:
            print("MFAST-FAIL create rc=%d" % r["rc"], flush=True)
            sys.exit(9)
        ...
        def run_rung(label, binname, extra, env, rung, rep, seed, tag):
            ...
            print("LEG round=fast3 rep=%d m=%d tool=%s argv='%s' wall=%s cpu=%s "
            ...
            if g2 != len(members):
                ...
                print("RESET rep=%d m=%d tool=%s slice_restore_left=%d "
                      ...
                      flush=True)
                if g3 != len(members):
                    print("MFAST-FAIL work dir unrecoverable", flush=True)
                    sys.exit(9)
            for nm in parfiles:
                subprocess.check_call(["cp", os.path.join(pristine, nm),
                                       os.path.join(work, nm)])
        ...
        for rep in REPS:
            for label, binname, extra, env in ARMS:
                ...
                print("VERIFY round=fast3 rep=%d tool=%s argv='%s' wall=%s cpu=%s "
            ...
            print("MFAST-REP-END rep=%d %s" % (rep, utcnow()), flush=True)
        print("MFAST-END %s" % utcnow(), flush=True)
    finally:
        lock.release()
