param(
  ...
  [int]$IdleLockMin = 30,         # the rig lock must have been unheld this long
...
#   1. no rig lock HELD for IdleLockMin minutes running - a box that other rounds
...
#
# SO IT ASKS THE READ-ONLY QUESTION, Test-RigLockHeld, AND NEVER Test-Path.
# Until 16 Sep 2026 this loop read the lock file's mere EXISTENCE as a hold,
# which is the defect that cost apple-m3-ultra eight hours that day
# (an internal note) - an orphaned lock
# naming a dead pid would have parked this waiter on `lock-present` until
# -MaxWaitH ran out, 72 hours by default, for a box that was free the whole
# time. Liveness comes from the HOLDER, never from the clock, and plib.ps1's
# Get-RigLockHolder is the one place that rule lives. A waiter must NOT call
# Take-RigLock to find out: that takes the box this script exists not to take.
...
while ((Get-Date) -lt $deadline) {
  if (Test-RigLockHeld) { $lockFreeSince = $null; $quiet = 0; "WAIT lock-held ts=$((Get-Date).ToUniversalTime().ToString('HH:mm:ss'))Z"; Start-Sleep 60; continue }
