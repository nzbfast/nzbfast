...
RARGV = {"parfast": ["r", "-q", "-t32", "-T16", "f.par2"],
         "par2turbo": ["r", "-q", "-t32", "-T16", "f.par2"]}
VARGV = {"parfast": ["v", "-q", "-t32", "-T16", "f.par2"],
         "par2turbo": ["v", "-q", "-t32", "-T16", "f.par2"]}
...
def main():
    ...
    try:
        ...
        r = run_leg(os.path.join(BIN, "parfast"),
                    ["c", "-q", "-t32", "-T16", "-s%d" % SLICE, "-c%d" % RBLK,
                     "f.par2"] + members,
        ...
        def run_rung(tool, rung, rep, seed, tag, argv):
            ...
            print("LEG round=applad rep=%d m=%d tool=%s argv='%s' wall=%s cpu=%s "
        ...
        for rep in REPS:
            for tool in TOOLS:
                ...
                print("VERIFY round=applad rep=%d tool=%s argv='%s' wall=%s cpu=%s "
